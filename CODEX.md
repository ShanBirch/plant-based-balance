...
- The API-based `Balance Client DM Manager + Lead Reconciler` is a separate 24-hour worker that wakes every 15 minutes. Balance's webhook/draft/scheduled-worker pipeline owns normal unlinked-lead replies. The manager remains responsible for linked-client draft/support work, checkout readback, hard-hold routing, and reconciliation of already scheduled or sent lead work; it must never independently draft, react to, or send a normal unlinked-lead reply. It stops claiming new work after minute seven, reconciles and releases its own lock by minute nine, and uses a 15-minute crash-recovery lease. Its local file lock is never shared with the browser dispatcher.
...
- The Balance AI coach pipeline owns every normal unlinked-lead reply: Instagram webhook or Graph reconciliation, `ig-instant-draft`, then the scheduled reply worker with canonical no-double-send readback. The API manager must not create a parallel lead reply. Shannon should not receive lead handoffs for ordinary judgment calls; explicit safety, authenticity, transport, or unrecoverable-media holds still route to Needs You. Linked IG/FB clients remain approval-only.
...
- Do not default to a polished single `Haha` opener. Laugh only when the moment is genuinely funny, use rough phone-native forms such as `hahahah`, `hahahahah`, or `ahahaha`, and vary the opening; many notes should open directly on the exact thing the lead said instead.
