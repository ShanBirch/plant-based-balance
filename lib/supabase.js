...
var PBB_OFFLINE_CACHE_PREFIX = 'pbb_offline_workouts_v1:';

function pbbOfflineCacheKey(namespace, parts) {
  return PBB_OFFLINE_CACHE_PREFIX + namespace + ':' + parts.map(part => encodeURIComponent(String(part ?? ''))).join(':');
}

function pbbReadOfflineCache(namespace, parts, fallback = null) {
  try {
    const raw = localStorage.getItem(pbbOfflineCacheKey(namespace, parts));
...
function pbbHasOfflineCache(namespace, parts) {
  try {
    return localStorage.getItem(pbbOfflineCacheKey(namespace, parts)) !== null;
  } catch (_) {
    return false;
  }
}

function pbbWriteOfflineCache(namespace, parts, value) {
  try {
    localStorage.setItem(pbbOfflineCacheKey(namespace, parts), JSON.stringify({
      value,
      cachedAt: new Date().toISOString()
    }));
  } catch (_) {
    // Cache storage can be full or disabled. The live DB result should still win.
  }
}

async function pbbCachedRead(namespace, parts, fetcher, fallback = null) {
  try {
    const value = await fetcher();
    pbbWriteOfflineCache(namespace, parts, value);
    return value;
  } catch (error) {
    if (pbbHasOfflineCache(namespace, parts)) {
      const cached = pbbReadOfflineCache(namespace, parts, fallback);
      console.warn(`Offline cache hit for ${namespace}`, error);
      return cached;
    }
    throw error;
  }
...
function isBalanceAdminEmail(email) {
  return String(email || '').trim().toLowerCase() === BALANCE_ADMIN_EMAIL;
}
...
window.BALANCE_ADMIN_EMAIL = BALANCE_ADMIN_EMAIL;
window.isBalanceAdminEmail = isBalanceAdminEmail;
...
const db = {
  ...
  users: {
    ...
    async get(userId) {
      return pbbCachedRead('user_profile', [userId], async () => {
        const { data, error } = await supabase
          .from('users')
          .select('*')
          .eq('id', userId)
  ...
  pushSubscriptions: {
    ...
    async isAdmin() {
      const { data: { user } } = await supabase.auth.getUser();
      return isBalanceAdminEmail(user?.email);
    },
  ...
  workouts: {
    async createHistory(userId, workoutData) {
      const insertData = {
        ...
        template_name: workoutData.workoutName || workoutData.templateName || workoutData.template_name || null
      };
    ...
    async getHistory(userId, limit = 500) {
      return pbbCachedRead('workout_history', [userId, limit], async () => {
        const { data, error } = await supabase
          .from('workouts')
          .select('*')
          .eq('user_id', userId)
    ...
    /**
     * Get every saved set from the most recent session for each requested exercise.
     * This avoids the global workout-history limit cutting an older exercise or
     * only part of its previous session out of the prefill cache.
     */
    async getLatestHistoryForExercises(userId, exerciseNames) {
      ...
      const cacheNames = [...names].sort();
      return pbbCachedRead('latest_workout_history_exercises', [userId, cacheNames.join('|')], async () => {
        const loadLatestSession = async (exerciseName) => {
          ...
          const latestDate = latestRows && latestRows[0] && latestRows[0].workout_date;
        ...
        // A workout can contain several exercises, so one transient mobile
        // request failure must not discard every other exercise's history.
        // Retry each exercise once and keep any cached rows for only the
        // exercises that still fail.
        const results = await Promise.allSettled(names.map(async exerciseName => {
          let lastError = null;
          for (let attempt = 0; attempt < 2; attempt += 1) {
            try {
              return await loadLatestSession(exerciseName);
            } catch (error) {
              lastError = error;
            }
          }
        ...
        results.forEach((result, index) => {
          if (result.status === 'fulfilled') {
            ...
            failedNames.add(names[index].toLocaleLowerCase());
            firstFailure = firstFailure || result.reason;
          }
        });

        if (failedNames.size > 0) {
          const cachedRows = pbbReadOfflineCache(
            'latest_workout_history_exercises',
            [userId, cacheNames.join('|')],
            []
          );
          const failedCachedRows = Array.isArray(cachedRows)
            ? cachedRows.filter(row => failedNames.has(
                String(row?.exercise_name || row?.exercise || '').trim().toLocaleLowerCase()
              ))
            : [];
          histories.push(failedCachedRows);
          console.warn('Some exercise history requests failed; retained available history', {
            failedExercises: [...failedNames]
          });
        }

        const combined = histories.flat();
        if (combined.length === 0 && firstFailure) throw firstFailure;
    ...
    async getCustomWorkouts(userId) {
      return pbbCachedRead('custom_workouts', [userId], async () => {
        const { data, error } = await supabase
          .from('workouts')
          .select('*')
          .eq('user_id', userId)
    ...
    async getCurrentWorkout(userId) {
      return pbbCachedRead('current_workout', [userId], async () => {
        const { data, error } = await supabase
          .from('workouts')
          .select('*')
          .eq('user_id', userId)
  ...
  workoutCustomizations: {
    ...
    async get(userId, workoutKey) {
      return pbbCachedRead('workout_customization', [userId, workoutKey], async () => {
        const { data, error } = await supabase
    ...
    async getAllForUser(userId) {
      return pbbCachedRead('workout_customizations_all', [userId], async () => {
        const { data, error } = await supabase
          .from('workout_customizations')
          .select('*')
          .eq('user_id', userId);
  ...
  customPrograms: {
     ...
     */
    async getAll(userId) {
      return pbbCachedRead('custom_programs_all', [userId], async () => {
        const { data, error } = await supabase
          .from('custom_workout_programs')
          .select('*')
          .eq('user_id', userId)
    ...
    async getActive(userId) {
      return pbbCachedRead('custom_program_active', [userId], async () => {
        const { data, error } = await supabase
          .from('custom_workout_programs')
          .select('*')
          .eq('user_id', userId)
     ...
     */
    async get(programId) {
      return pbbCachedRead('custom_program', [programId], async () => {
        const { data, error } = await supabase
          .from('custom_workout_programs')
          .select('*')
          .eq('id', programId)
  ...
  workoutReplacements: {
    ...
    async getActive(userId) {
      ...
      return pbbCachedRead('workout_replacements_active', [userId, today], async () => {
        const { data, error } = await supabase
          .from('workout_replacements')
          .select('*')
          .eq('user_id', userId)
    ...
    async getForDay(userId, dayOfWeek) {
      ...
      return pbbCachedRead('workout_replacement_day', [userId, dayOfWeek, today], async () => {
        const { data, error } = await supabase
          .from('workout_replacements')
          .select('*')
          .eq('user_id', userId)
     ...
     */
    async getAll(userId) {
      return pbbCachedRead('workout_replacements_all', [userId], async () => {
        const { data, error } = await supabase
          .from('workout_replacements')
          .select('*')
          .eq('user_id', userId)
  ...
  quizResults: {
    ...
    async getLatest(userId) {
      return pbbCachedRead('quiz_latest', [userId], async () => {
        const { data, error } = await supabase
          .from('quiz_results')
          .select('*')
          .eq('user_id', userId)
    ...
    async upsert(userId, mealTimes) {
      const { data, error } = await supabase
        ...
        .upsert({
          ...
          // Keep meal times for the on-time XP calculation, but the reminder
          // notification feature has been retired from Settings.
  ...
  admin: {
    ...
    async getUserWeekData(userId, days = 7) {
      ...
      const [
        ...
        safeQuery(() => supabase.from('nudges').select('sender_id, receiver_id, message, created_at').or(`sender_id.eq.${userId},receiver_id.eq.${userId}`).gte('created_at', sinceISO).order('created_at', { ascending: true })),
  ...
  stories: {
    ...
    async create(userId, storyData) {
      ...
      if (storyData.course_action_id) {
        insertData.course_action_id = storyData.course_action_id;
      }

      if (storyData.expires_at) {
        insertData.expires_at = storyData.expires_at;
      }
      ...
      if (window.socialJourney && typeof window.socialJourney.refresh === 'function') {
        setTimeout(() => window.socialJourney.refresh(), 0);
      }
      return data;
    },
     ...
     */
    async getNetworkStories(userId, options = {}) {
      ...
      if (Number.isFinite(limit) && limit > 0) {
        query = query.range(offset, offset + Math.floor(limit) - 1);
      }
    ...
    /**
     * Get one active feed story with the same shape the feed renderer expects.
     */
    async getFeedStory(storyId, viewerId = null) {
      const sanitizeFeedMediaValue = (value) => {
      ...
      const { data, error } = await supabase
        ...
        .select(`
          ...
          created_at,
          expires_at,
          user:users!user_id(name, email, profile_photo, is_test_account)
        `)
        .eq('id', storyId)
        .gt('expires_at', new Date().toISOString())
        .maybeSingle();
      ...
      if (error) throw error;
      if (!data || data.user?.is_test_account) return null;
      ...
      return {
        ...
        media_url: sanitizeFeedMediaValue(data.media_url),
        thumbnail_url: sanitizeFeedMediaValue(data.thumbnail_url),
    ...
    async markAsViewed(storyId, viewerId) {
      if (!storyId || !viewerId) return null;
     ...
     */
    async update(storyId, updates, userId = null) {
      const nextUpdates = {
        ...(updates || {}),
        updated_at: new Date().toISOString()
      };
     ...
     */
    async addComment(storyId, userId, commentText, mentionedUserIds = []) {
      const mentionIds = Array.isArray(mentionedUserIds)
        ? [...new Set(mentionedUserIds.map(id => String(id || '').trim()).filter(Boolean))]
        : [];

      if (mentionIds.length > 0) {
        try {
          const { data, error } = await supabase
            .rpc('add_feed_comment_with_mentions', {
              p_story_id: storyId,
          ...
          const isMissingMentionRpc = ['PGRST202', '42883', '42P01'].includes(error && error.code)
            || /add_feed_comment_with_mentions|feed_comment_mentions|function.*not.*found/i.test(message);
          if (!isMissingMentionRpc) throw error;
          console.warn('Feed comment mention RPC is not available yet; posting plain comment:', error);
        }
      }
    ...
    /**
     * Toggle the current user's like on a feed comment
     */
    async toggleCommentLike(commentId) {
      const { data, error } = await supabase
        .rpc('toggle_feed_comment_like', { p_comment_id: commentId });
   ...
   */
  friends: {
    _friendsCache: new Map(),
    _friendsCacheTtlMs: 30000,

    _cloneFriends(rows) {
      return Array.isArray(rows) ? rows.map(row => ({ ...row })) : [];
    },

    _getCachedFriends(userId) {
      const cached = this._friendsCache.get(userId);
      if (!cached) return null;
      if (Date.now() - cached.cachedAt > this._friendsCacheTtlMs) {
        this._friendsCache.delete(userId);
        return null;
      }
      return this._cloneFriends(cached.rows);
    },

    _setCachedFriends(userId, rows) {
      const cleanRows = this._cloneFriends(rows);
      this._friendsCache.set(userId, {
        cachedAt: Date.now(),
        rows: cleanRows
      });
      return this._cloneFriends(cleanRows);
    },
    ...
    async getFriends(userId) {
      const cached = this._getCachedFriends(userId);
      ...
      if (error) throw error;
      return this._setCachedFriends(userId, data || []);
    },
    ...
    async getFriendsWithFallback(userId) {
      ...
      try {
        ...
        if (!res1.error) return this._setCachedFriends(userId, res1.data || []);
      ...
      try {
        ...
        if (!res2.error) return this._setCachedFriends(userId, res2.data || []);
      ...
      return this._setCachedFriends(userId, friends);
    },
  ...
  points: {
    ...
    /**
     * Award points for meal, workout, progress photo, or other point events
     * Calls the edge function which handles anti-cheat and streak logic
     */
    async awardPoints(userId, type, referenceId, options = {}) {
      let safeReferenceId = referenceId;
      const needsUuidReference = type === 'workout' || type === 'personal_best' || type === 'social_share';
      const uuidPattern = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
      if (needsUuidReference && !uuidPattern.test(String(safeReferenceId || ''))) {
        if (typeof crypto !== 'undefined' && typeof crypto.randomUUID === 'function') {
          safeReferenceId = crypto.randomUUID();
        } else if (typeof crypto !== 'undefined' && typeof crypto.getRandomValues === 'function') {
          safeReferenceId = '10000000-1000-4000-8000-100000000000'.replace(/[018]/g, c =>
            (Number(c) ^ crypto.getRandomValues(new Uint8Array(1))[0] & 15 >> Number(c) / 4).toString(16)
          );
        } else {
          safeReferenceId = `${Date.now()}00000000000000000000000000000000`
            .slice(0, 32)
      ...
      const response = await fetch('/api/award-points', {
        ...
        body: JSON.stringify({
          ...
          mealTime: options.mealTime || null,
          shareKind: options.shareKind || null,
          shareDestination: options.shareDestination || null,
  ...
  personalBests: {
    ...
    async getAll(userId, limit = 10) {
      return pbbCachedRead('personal_bests_all', [userId, limit], async () => {
        const { data, error } = await supabase
          .from('personal_bests')
          .select('*')
          .eq('user_id', userId)
    ...
    async getForExercise(userId, exerciseName) {
      return pbbCachedRead('personal_best_exercise', [userId, exerciseName], async () => {
        const { data, error } = await supabase
          .from('personal_bests')
          .select('*')
          .eq('user_id', userId)
    ...
    async getForExercises(userId, exerciseNames) {
      ...
      const cacheNames = [...new Set(exerciseNames)].sort();
      return pbbCachedRead('personal_bests_exercises', [userId, cacheNames.join('|')], async () => {
        const { data, error } = await supabase
          .from('personal_bests')
          .select('*')
          .eq('user_id', userId)
      ...
      for (const [exerciseName, current] of Object.entries(exerciseMap)) {
        ...
        if (isNewWeightPB || isNewRepsPB) {
          ...
          if (isNewWeightPB && current.bestWeight > 0) {
            let weightHistoryRow = null;
            try {
              const { data: insertedWeightHistory, error: weightHistErr } = await supabase
                .from('pb_history')
                .insert({
                  user_id: userId,
                  exercise_name: exerciseName,
                ...
                .select('id, achieved_at, workout_date')
                .single();
              if (weightHistErr) console.warn('Failed to insert weight PB history:', weightHistErr);
              else weightHistoryRow = insertedWeightHistory;
            } catch (histErr) {
              console.warn('Error inserting weight PB history:', histErr);
            }

            newPBs.push({
              historyId: weightHistoryRow?.id || null,
              pbHistoryId: weightHistoryRow?.id || null,
              ...
              improvement: previousWeight ? current.bestWeight - previousWeight : null,
              achievedAt: weightHistoryRow?.achieved_at || null,
              workoutDate: weightHistoryRow?.workout_date || today
            });
          }

          if (isNewRepsPB && current.bestReps > 0) {
            let repsHistoryRow = null;
            try {
              const { data: insertedRepsHistory, error: repsHistErr } = await supabase
                .from('pb_history')
                .insert({
                  user_id: userId,
                  exercise_name: exerciseName,
              ...
              else repsHistoryRow = insertedRepsHistory;
            } catch (histErr) {
            ...
            if (!isNewWeightPB || current.bestReps !== current.bestWeightReps) {
              newPBs.push({
                historyId: repsHistoryRow?.id || null,
                pbHistoryId: repsHistoryRow?.id || null,
                exercise: exerciseName,
                type: 'reps',
                ...
                improvement: previousReps ? current.bestReps - previousReps : null,
                achievedAt: repsHistoryRow?.achieved_at || null,
                workoutDate: repsHistoryRow?.workout_date || today
              });
            }
          }
        }
  ...
  weighIns: {
    /**
     * Save a weigh-in for a specific date.
     * If a row already exists for that day, it is replaced.
     */
    async save(userId, weighInDate, weightKg, notes = undefined, bodyFatPct = undefined) {
      const roundedWeightKg = Number(parseFloat(weightKg).toFixed(1));

      if (!Number.isFinite(roundedWeightKg) || roundedWeightKg <= 0) {
        throw new Error('A valid weight is required');
      }

      const { data: existingRow, error: existingError } = await supabase
        .from('daily_weigh_ins')
        .select('notes,body_fat_pct')
        .eq('user_id', userId)
        .eq('weigh_in_date', weighInDate)
        .maybeSingle();
      ...
      const record = {
        ...
        notes: notes !== undefined && notes !== null ? notes : (existingRow?.notes ?? null)
      };

      const resolvedBodyFatPct = bodyFatPct !== undefined && bodyFatPct !== null
        ? parseFloat(bodyFatPct)
        : existingRow?.body_fat_pct;
      if (Number.isFinite(resolvedBodyFatPct) && resolvedBodyFatPct > 0) {
        record.body_fat_pct = parseFloat(Number(resolvedBodyFatPct).toFixed(1));
      }
      ...
      await this.refreshUserWeight(userId);
      return data;
    },
    ...
    async log(userId, weightKg, notes = null, bodyFatPct = null) {
      return await this.save(userId, getLocalDateString(), weightKg, notes, bodyFatPct);
    },

    /**
     * Refresh the user's current weight from the latest weigh-in.
     */
    async refreshUserWeight(userId) {
      try {
        const latest = await this.getLatest(userId);
        const latestWeightKg = latest ? Number(latest.weight_kg) : null;

        const { error: profileError } = await supabase
          .from('users')
          .update({ weight: latestWeightKg })
          .eq('id', userId);

        if (profileError) {
          console.warn('Could not sync weigh-in weight to profile:', profileError);
        }

        if (typeof window !== 'undefined') {
          if (window.currentUser && typeof window.currentUser === 'object') {
            window.currentUser.weight = latestWeightKg;
          }
          if (window.userProfile && typeof window.userProfile === 'object') {
            window.userProfile.weight = latestWeightKg;
          }
          ...
          ['localStorage', 'sessionStorage'].forEach((storageName) => {
            try {
              ...
              if (!storage) return;
              const profile = JSON.parse(storage.getItem('userProfile') || '{}');
              profile.weight = latestWeightKg;
              storage.setItem('userProfile', JSON.stringify(profile));
            } catch (storageError) {
              console.warn(`Could not update ${storageName} profile weight:`, storageError);
            }
          });

          if (typeof document !== 'undefined') {
            const weightDisplay = document.getElementById('profile-weight-display');
            if (weightDisplay) {
              if (latestWeightKg === null || !Number.isFinite(latestWeightKg)) {
                weightDisplay.textContent = '--';
              } else {
                const preferLbs = localStorage.getItem('weightUnitPreference') === 'lbs';
                weightDisplay.textContent = preferLbs
        ...
        console.warn('Weigh-in saved but profile weight sync failed:', syncError);
        return null;
      }
    },

    /**
     * Delete a weigh-in and refresh the profile weight.
     */
  ...
  activityLogs: {
    ...
    async create(userId, activityData) {
      const { data, error } = await supabase
        ...
        .insert([{
          ...
          external_activity_id: activityData.external_activity_id || null,
          source_metadata: activityData.source_metadata || {},
          imported_at: activityData.imported_at || null
        }])
        .select()
        .single();
    ...
    async getRecentImported(userId, source, limit = 3) {
      const { data, error } = await supabase
        .from('activity_logs')
        .select('*')
        .eq('user_id', userId)
        .eq('source', source)
        .eq('shared_to_feed', false)
    ...
    /**
     * Get recent imports across device sources. The caller must see the newest
     * handled row too, otherwise an older unhandled walk can resurface after a
     * newer one is shared or dismissed.
     */
    async getRecentImportedFromSources(userId, sources, limit = 25) {
      const allowedSources = Array.isArray(sources) ? sources.filter(Boolean) : [];
  ...
  /**
   * Exercise notes table operations (per-exercise notes for users & coach)
   */
  exerciseNotes: {
    /**
     * Load notes for a list of exercises (own notes + shared/coach notes)
     * @param {string} userId - User's UUID
     * @param {string[]} exerciseNames - Array of exercise name strings
     * @returns {{ userNotes: Object, coachNotes: Object }} - Maps of exercise_name → note_text
     */
    async loadForExercises(userId, exerciseNames) {
      if (!exerciseNames || exerciseNames.length === 0) return { userNotes: {}, coachNotes: {} };

      const { data, error } = await supabase
        .from('exercise_notes')
        .select('user_id, exercise_name, note_text, is_shared')
        .in('exercise_name', exerciseNames)
      ...
      (data || []).forEach(row => {
        if (row.user_id === userId) {
          userNotes[row.exercise_name] = row.note_text;
        }
        if (row.is_shared && row.user_id !== userId) {
          coachNotes[row.exercise_name] = row.note_text;
        }
      });

      return { userNotes, coachNotes };
    },

    /**
     * Save or update a note for an exercise
     * @param {string} userId - User's UUID
     * @param {string} exerciseName - Exercise name
     * @param {string} noteText - The note content
     * @param {boolean} isShared - Whether note is visible to all users
     */
    async save(userId, exerciseName, noteText, isShared = false) {
      const { data, error } = await supabase
        .from('exercise_notes')
        .upsert([{
          user_id: userId,
     ...
     * @param {string} exerciseName - Exercise name
     */
    async remove(userId, exerciseName) {
      const { error } = await supabase
        .from('exercise_notes')
        .delete()
        .eq('user_id', userId)
...
const storage = {
  ...
  async uploadExerciseVideo(userId, file, exerciseId, options = {}) {
    if (!userId) throw new Error('Missing user ID');
    if (!file) throw new Error('Missing video file');

    const logUploadDiagnostic = (event, data = {}) => {
      ...
      logger(event, {
        ...
        clientFileName: file.name || 'exercise-video.mp4',
    ...
    const rawExt = file.name && file.name.includes('.')
      ? file.name.split('.').pop()
      : '';
    const detectedMimeType = file.type || 'video/mp4';
    const mimeExt = detectedMimeType.includes('webm')
      ? 'webm'
      : detectedMimeType.includes('quicktime')
        ? 'mov'
        : detectedMimeType.includes('3gpp')
          ? '3gp'
          : 'mp4';
    const fileExt = (rawExt || mimeExt).toLowerCase().replace(/[^a-z0-9]/g, '') || mimeExt;
    const safeExerciseId = String(exerciseId || Date.now()).replace(/[^a-zA-Z0-9_-]/g, '');
    const mimeType = file.type || `video/${fileExt === 'mov' ? 'quicktime' : fileExt}`;
    ...
    if (!token) {
      try {
        ...
        token = result?.data?.session?.access_token || result?.session?.access_token || '';
      } catch (_) {}
    }
    if (!token) throw new Error('Please log in before uploading.');

    logUploadDiagnostic('custom_exercise_upload_request_start', {
      hasAuthToken: true
    });

    let startResponse;
    try {
      startResponse = await fetch('/api/create-exercise-video-upload', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          fileName: file.name || ('exercise-video.' + fileExt),
          contentType: mimeType,
          size: Number(file.size || 0)
        })
      });
    } catch (error) {
      logUploadDiagnostic('custom_exercise_direct_upload_setup_error', {
        errorName: error?.name || 'Error',
        errorMessage: error?.message || String(error || 'upload setup request failed')
      });
      throw error;
    }

    if (!startResponse.ok) {
      let errorMessage = 'Could not start the exercise video upload.';
      try {
        const errorData = await startResponse.json();
        errorMessage = errorData.error || errorMessage;
      } catch (_) {}
      logUploadDiagnostic('custom_exercise_direct_upload_setup_rejected', {
    ...
    if (!startData?.uploadUrl || !startData?.authorizationToken || !startData?.fileName || !startData?.publicUrl) {
      throw new Error('Could not prepare the exercise video upload.');
    }

    logUploadDiagnostic('custom_exercise_direct_upload_signed', {
      responseOk: true
    });
    ...
    const directUploadHeaders = {
      ...
      'Content-Type': mimeType || startData.contentType || 'video/mp4',
      // Avoid reading a multi-gigabyte clip into phone memory just to hash it.
      // B2 calculates and returns its checksum after receiving the direct upload.
      'X-Bz-Content-Sha1': 'do_not_verify'
    };
    const uploadViaXhr = typeof window.uploadB2FileViaXhr === 'function'
      ? window.uploadB2FileViaXhr
      : null;
    const onProgress = typeof options.onProgress === 'function' ? options.onProgress : null;
    if (onProgress) onProgress(0);
    const prefersXhr = typeof window.shouldUseXhrForDirectFeedUpload === 'function'
      && window.shouldUseXhrForDirectFeedUpload();
    try {
      if (prefersXhr && uploadViaXhr) {
        uploadResponse = await uploadViaXhr(startData.uploadUrl, directUploadHeaders, file, { onProgress });
      } else {
        try {
          ...
          if (!uploadViaXhr) throw fetchError;
          uploadResponse = await uploadViaXhr(startData.uploadUrl, directUploadHeaders, file, { onProgress });
        }
      }
    } catch (error) {
      logUploadDiagnostic('custom_exercise_direct_upload_network_error', {
        errorName: error?.name || 'Error',
        errorMessage: error?.message || String(error || 'direct B2 upload failed')
      });
      throw new Error('Could not connect to video storage. Check reception and retry.');
    }

    if (!uploadResponse.ok) {
      const errorMessage = await uploadResponse.text().catch(() => '');
      logUploadDiagnostic('custom_exercise_direct_upload_rejected', {
        httpStatus: uploadResponse.status,
        responseOk: false,
        errorMessage: errorMessage || 'B2 rejected direct upload'
      });
      throw new Error('Video storage rejected the upload. Please retry.');
    }

    const uploadData = await uploadResponse.json().catch(() => ({}));
    if (onProgress) onProgress(100);
    logUploadDiagnostic('custom_exercise_direct_upload_success', {
      httpStatus: uploadResponse.status,
      responseOk: true,
    ...
    return {
      publicUrl: startData.publicUrl,
      storagePath: startData.fileName,
      upload: uploadData,
      b2FileId: uploadData.fileId || '',
      contentType: mimeType || startData.contentType,
  ...
  async uploadFormCheckVideo(userId, file, requestId) {
    if (!userId) throw new Error('Missing user ID');
    if (!file) throw new Error('Missing video file');
    ...
    const rawExt = file.name && file.name.includes('.')
      ? file.name.split('.').pop()
      : '';
    const mimeExt = file.type === 'video/webm' ? 'webm' : 'mp4';
    const fileExt = (rawExt || mimeExt).toLowerCase().replace(/[^a-z0-9]/g, '') || mimeExt;
    const safeRequestId = String(requestId || Date.now()).replace(/[^a-zA-Z0-9_-]/g, '');
    const fileName = `${userId}/form-checks/${safeRequestId}.${fileExt}`;
    ...
    if (!urlData) throw new Error('Failed to get public URL');
    return { publicUrl: urlData.publicUrl, storagePath: fileName, upload: data };
  }
};
...
db.customExercises = {
  ...
  async create(userId, exerciseData) {
    const { data, error } = await supabase
      ...
      .insert([{
        ...
        is_public: !!exerciseData.isPublic
      }])
      .select()
      .single();
  ...
  /**
   * Get only custom exercises created by the current user
   */
  ...
  async delete(userId, exerciseId) {
    ...
    if (exercise && exercise.storage_path && !String(exercise.storage_path).startsWith('exercises/')) {
      try {
...
// ── Guest Mode: intercept DB calls to return safe demo data ───────
// When window.guestMode is true, all dbHelper read calls return empty/demo
// data and all write calls become no-ops. This prevents Supabase auth
// errors and lets the dashboard render in preview mode.
(function setupGuestModeInterceptor() {
  // Demo data returned by key read methods
  const guestDemoData = {
    'users.get': () => window.userProfile,
    'userFacts.get': () => window.userFacts,
    'users.updateLastLogin': () => null,
    'users.update': () => null,
    'users.upsert': () => null,
    'activity.log': () => null,
    'stories.getNetworkStories': () => [],
    'stories.getNetworkFeed': () => [],
    'conversations.getHistory': () => [],
  };

  // Safe default returns by type
  function safeDefault(methodName) {
    // Write operations → no-op
    if (/^(create|update|upsert|delete|remove|insert|log|track|submit|award|credit|debit|toggle|set|mark|dismiss|save|record|send|add|join|settle|grant)/.test(methodName)) {
      return () => null;
    }
    // getAll / list / load / getHistory → empty array
    if (/^(getAll|getHistory|getRecent|getBy|getFor|list|load|search|fetch|count|check)/.test(methodName)) {
      return () => [];
    }
    // get (singular) → null
    ...
    // Any remaining method that sounds like it returns a collection → empty array
    if (/History|All|List|Recent|Many|Feed|Messages|Logs|Posts/.test(methodName)) {
      return () => [];
    }
    // Fallback
    return () => null;
  ...
  function wrapNamespace(nsName, ns) {
    ...
    for (const key of Object.keys(ns)) {
      if (typeof ns[key] === 'function') {
        const fullKey = nsName + '.' + key;
        wrapped[key] = async function(...args) {
          if (!window.guestMode) return ns[key].apply(ns, args);
          // Check for specific demo data first
          if (guestDemoData[fullKey]) return guestDemoData[fullKey](...args);
          // Otherwise return safe default
          return safeDefault(key)();
        };
      } else if (typeof ns[key] === 'object' && ns[key] !== null) {
        // Nested namespace (e.g. db.pbbPoints.xxx)
        wrapped[key] = wrapNamespace(nsName + '.' + key, ns[key]);
      } else {
        wrapped[key] = ns[key];
      }
    }
  ...
  // Wrap all db namespaces
  const wrappedDb = {};
  for (const nsName of Object.keys(db)) {
    if (typeof db[nsName] === 'object' && db[nsName] !== null) {
      wrappedDb[nsName] = wrapNamespace(nsName, db[nsName]);
    } else if (typeof db[nsName] === 'function') {
      const origFn = db[nsName];
      wrappedDb[nsName] = async function(...args) {
        if (!window.guestMode) return origFn.apply(db, args);
        return null;
      };
    } else {
      wrappedDb[nsName] = db[nsName];
    }
  }

  window.dbHelpers = wrappedDb;
  window.db = wrappedDb;

  // Also wrap supabaseClient.rpc() calls for guest mode
  const origRpc = supabase.rpc.bind(supabase);
  supabase.rpc = function(fnName, params, options) {
    if (window.guestMode) {
    ...
    return origRpc(fnName, params, options);
  };

  // Wrap supabase.from() for guest mode — intercept select/insert/update/delete chains
  ...
  const noopChain = {
    ...
    range: () => noopChain,
    single: () => noopChain,
    maybeSingle: () => noopChain,
    csv: () => noopChain,
    contains: () => noopChain,
    overlaps: () => noopChain,
    textSearch: () => noopChain,
    ilike: () => noopChain,
    like: () => noopChain,
    then: (resolve) => resolve({ data: [], error: null, count: 0 }),
    catch: () => ({ data: [], error: null, count: 0 }),
    data: [],
    error: null,
    count: 0,
  };
  supabase.from = function(table) {
    if (window.guestMode) return noopChain;
