...
- `live.cjs`: transport restricted to Gold Coast AI Solutions → Shan n Sunny. Requires exact identity, a matching session and original-flow opt-out. Manual sessions must be unexpired; explicitly selected automatic mode remains active until switched off.
- `automatic.cjs`: runs from the existing background inbound worker when the exact test thread has `learn_ai_experiment.mode=automatic`. Uses an atomic delivery lease, one generation claim per inbound, canonical messages and confirmed receipts. `BALANCE` starts a new test episode. No local process or bearer-token renewal is needed for automatic replies.
...
For Shannon's explicitly requested self-test switch, use a persistent `automatic` session instead. Keep the old scheduled tester paused so it cannot send into Shannon's conversation. To switch back, remove the selected `learn_ai_experiment` flag and restore the original opt-out state, then resume the old tester only when wanted. The initial matched comparison and later self-test switch are separate actions; the comparison itself left the old schedule unchanged.
