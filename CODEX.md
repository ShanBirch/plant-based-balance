...
- A first inbound carrying a verified Meta `ADS` referral is the paid-ad fast lane: persist its ad/referral attribution on the thread, run the normal media/context/safety/draft review, and schedule a clean passing draft for four minutes later. Never send it instantly. The paid-ad lane is message-scoped, so a historic ad origin alone never grants a later message fast-lane delivery. Separately, any current unlinked-lead inbound that clearly discusses exercise or training may use the same four-minute lane after a clean pass, allowing rapid fitness conversations to continue. Any reviewer warning or hard hold falls back to the DM manager; linked clients always remain draft-only.
