...
export const POINTS_CONFIG = {
  ...
  POINTS_PER_FEED_COMMENT: 2,   // 2 points for commenting on another user's Feed post
  POINTS_PER_MEAL_FEED_SHARE: 1, // 1 point for sharing a logged meal to Feed
  POINTS_PER_PROGRESS_PHOTO: 10, // 10 points for a weekly progress photo
  POINTS_PER_PROGRESS_PHOTO_SHARE: 10, // 10 extra points for sharing weekly progress photos to Feed
  ...
  DAILY_QUIZ_BONUS: 5,          // 5 XP bonus for completing the daily quiz with 100%
...
// ============================================================
// LEVELING SYSTEM
// 99 levels total, takes ~10 months of daily use to max
// ============================================================

export const LEVEL_CONFIG = {
  MAX_LEVEL: 99,
  // XP curve: points = 3 * (level - 1) + 0.3 * (level - 1)^2.1
  // Early levels need multiple XP hits, while each 10-level band costs more than the last.
  // - Level 10: ~57 points
  // - Level 20: ~202 points
  // - Level 30: ~440 points
  // - Level 50: ~1209 points
  // - Level 75: ~2748 points
  // - Level 99: ~4851 points
  BASE_XP_PER_LEVEL: 3,
  CURVE_MULTIPLIER: 0.3,
  CURVE_EXPONENT: 2.1,
};

/**
 * Calculate the lifetime points required to reach a given level
 * Uses formula: points = 3 * (level - 1) + 0.3 * (level - 1)^2.1 (level 1 = 0 points)
 */
export const getPointsForLevel = (level) => {
  if (level <= 1) return 0;
  const levelIndex = level - 1;
  return Math.floor(
    LEVEL_CONFIG.BASE_XP_PER_LEVEL * levelIndex
    + LEVEL_CONFIG.CURVE_MULTIPLIER * Math.pow(levelIndex, LEVEL_CONFIG.CURVE_EXPONENT)
  );
};
