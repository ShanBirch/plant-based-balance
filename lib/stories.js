...
// Keep browser-prepared videos below Netlify's request body ceiling.
const FEED_VIDEO_UPLOAD_TARGET_BYTES = 6 * 1024 * 1024;
...
async function prepareUploadableVideo(file, onProgress, options = {}) {
    ...
    const finalMinBitrate = isSmallUploadTarget ? 700000 : 1600000;
...
async function uploadStoryMediaToBackblaze(file, options = {}) {
    ...
    if (file.size > FEED_MEDIA_UPLOAD_REQUEST_SAFE_BYTES) {
        ...
        throw new Error(type.startsWith('video/')
            ? 'That video is still too large. Trim it to just the set and try again.'
            : 'That upload is too large. Please choose a smaller file.');
    }
