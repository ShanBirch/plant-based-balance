...
export default async () => {
  ...
  await fetch(`${process.env.URL||'https://plantbased-balance.org'}/.netlify/functions/lcp-dm-background`,{method:'POST',headers:{'Content-Type':'application/json','x-hub-signature-256':'sha256='+crypto.createHmac('sha256',rows[0].value).update(body).digest('hex')},body,signal:AbortSignal.timeout(8000)});
};
