-- ============================================================
-- FIX: Friends showing as "Unknown" when users.name is NULL
-- Adds COALESCE fallback to use email prefix when name is missing
-- ============================================================

-- Fix get_friends function - use email prefix as fallback for NULL names
...
BEGIN
  ...
  SELECT * FROM (
    ...
    SELECT
      u.id as friend_id,
      COALESCE(u.name, split_part(u.email, '@', 1)) as friend_name,
...
-- Fix get_friends_with_status function - use email prefix as fallback for NULL names
...
-- Fix get_friend_activity_feed - use email prefix as fallback for NULL names
...
-- Also fix pending friend requests to show email prefix when name is NULL
...
BEGIN
  ...
  SELECT
    f.id as request_id,
    u.id as sender_id,
    COALESCE(u.name, split_part(u.email, '@', 1)) as sender_name,
