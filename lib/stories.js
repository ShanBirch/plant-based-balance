...
window.loadPhotoFeed = async function(targetGridId = 'photo-feed-grid', emptyStateId = 'photo-feed-empty') {
    ...
    try {
        ...
        grid.innerHTML = sortedStories.map(story => {
            ...
            const thumbnailSrc = story.thumbnail_url || story.media_url;
            const initial = story.user_name ? story.user_name.charAt(0).toUpperCase() : '?';
            const timeAgo = getTimeAgo(new Date(story.created_at));
            ...
            if (totalReactions > 0) {
                ...
                reactionBadge = `<div style="position:absolute; bottom:28px; right:4px; background:rgba(0,0,0,0.6); color:white; font-size:0.65rem; padding:2px 5px; border-radius:8px; display:flex; align-items:center; gap:2px;">${topReactions}<span>${totalReactions}</span></div>`;
            }

            // Video play icon overlay
            const videoIcon = isVideo ? `<div style="position:absolute; top:50%; left:50%; transform:translate(-50%,-50%); width:30px; height:30px; background:rgba(0,0,0,0.5); border-radius:50%; display:flex; align-items:center; justify-content:center;"><svg viewBox="0 0 24 24" style="width:16px; height:16px; fill:white;"><path d="M8 5v14l11-7z"/></svg></div>` : '';
            ...
            const avatarHtml = story.profile_photo
                ...
                : `<span style="font-size:0.55rem; color:white; font-weight:700;">${initial}</span>`;

            return `
                <div class="feed-grid-item" style="position:relative; aspect-ratio:1; overflow:hidden; cursor:pointer; background:#f1f5f9;" onclick="openFeedPostViewer('${storyId}')">
                    <img src="${thumbnailSrc}" style="width:100%; height:100%; object-fit:cover;" loading="lazy" onerror="this.style.display='none'">
                    ${videoIcon}
                    ${reactionBadge}
                    <!-- User avatar overlay -->
                    <div style="position:absolute; bottom:4px; left:4px; display:flex; align-items:center; gap:4px;">
                        <div style="width:20px; height:20px; border-radius:50%; background:var(--primary); display:flex; align-items:center; justify-content:center; overflow:hidden; border:1.5px solid white; flex-shrink:0;">
                            ${avatarHtml}
                        </div>
                        <span style="font-size:0.6rem; color:white; text-shadow:0 1px 3px rgba(0,0,0,0.8); font-weight:600; max-width:60px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">${story.user_name || ''}</span>
                    </div>
                </div>
            `;
        }).join('');
...
window.openFeedPostViewer = async function(storyId) {
    ...
    try {
        ...
        const mediaHtml = isVideo
            ? `<video src="${story.media_url}" autoplay playsinline controls style="width:100%; max-height:70vh; object-fit:contain; background:black; border-radius:0;"></video>`
        ...
        // Mark as viewed
        if (!story.has_viewed && story.user_id !== window.currentUser.id) {
            dbHelpers.stories.markAsViewed(sid, window.currentUser.id);
        }
...
window.toggleFeedReaction = async function(storyId, reactionKey, buttonEl) {
    ...
    try {
        ...
        // Refresh the viewer to show updated reactions
