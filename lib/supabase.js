...
const PBB_SUPABASE_READ_CIRCUIT_KEY = 'pbb_supabase_read_circuit_v1';
const PBB_SUPABASE_READ_FAILURE_LIMIT = 2;
const PBB_SUPABASE_READ_COOLDOWN_MS = 60 * 1000;
const PBB_SUPABASE_READ_TIMEOUT_MS = 8000;
const PBB_SUPABASE_FAILURE_RESET_MS = 2 * 60 * 1000;

function isBalanceAdminEmail(email) {
  return String(email || '').trim().toLowerCase() === BALANCE_ADMIN_EMAIL;
}

function pbbReadSupabaseCircuit() {
  try {
    return JSON.parse(localStorage.getItem(PBB_SUPABASE_READ_CIRCUIT_KEY) || '{}') || {};
  } catch (_) {
    return {};
  }
}

function pbbWriteSupabaseCircuit(value) {
  try {
    localStorage.setItem(PBB_SUPABASE_READ_CIRCUIT_KEY, JSON.stringify(value));
  } catch (_) {
    // Ignore storage failures; the network request can still proceed.
  }
}

function pbbClearSupabaseCircuit() {
  try {
    localStorage.removeItem(PBB_SUPABASE_READ_CIRCUIT_KEY);
  } catch (_) {
    // Ignore storage failures.
  }
}

function pbbSupabaseCircuitOpen() {
  const circuit = pbbReadSupabaseCircuit();
  return Number(circuit.openUntil || 0) > Date.now();
}

function pbbRecordSupabaseReadFailure() {
  const now = Date.now();
  const current = pbbReadSupabaseCircuit();
  const recentFailure = now - Number(current.lastFailureAt || 0) < PBB_SUPABASE_FAILURE_RESET_MS;
  const failures = (recentFailure ? Number(current.failures || 0) : 0) + 1;
  pbbWriteSupabaseCircuit({
    failures,
    lastFailureAt: now,
    openUntil: failures >= PBB_SUPABASE_READ_FAILURE_LIMIT
      ? now + PBB_SUPABASE_READ_COOLDOWN_MS
      : Number(current.openUntil || 0)
  });
}

function pbbIsSupabaseReadRequest(url, method) {
  if (!url || !url.startsWith(SUPABASE_URL)) return false;
  const verb = String(method || 'GET').toUpperCase();
  if (verb === 'GET' || verb === 'HEAD' || verb === 'OPTIONS') return true;
  return verb === 'POST' && /\/rest\/v1\/rpc\/get_[^/?#]*/.test(url);
}

function pbbSupabaseCircuitResponse() {
  return new Response(JSON.stringify({
    code: 'PBB_SUPABASE_READ_PAUSED',
    message: 'Balance is reconnecting. Please try again shortly.'
  }), {
    status: 503,
    statusText: 'Balance reconnecting',
    headers: { 'Content-Type': 'application/json' }
  });
}

const pbbNativeFetch = window.fetch.bind(window);

async function pbbSupabaseFetch(input, init = {}) {
  const url = typeof input === 'string' ? input : String(input?.url || '');
  const method = init?.method || (typeof input !== 'string' && input?.method) || 'GET';
  const isReadRequest = pbbIsSupabaseReadRequest(url, method);

  if (isReadRequest && pbbSupabaseCircuitOpen()) {
    return pbbSupabaseCircuitResponse();
  }

  let controller = null;
  let timeout = null;
  let fetchInit = init;
  if (isReadRequest && !init?.signal) {
    controller = new AbortController();
    fetchInit = { ...init, signal: controller.signal };
    timeout = setTimeout(() => controller.abort(), PBB_SUPABASE_READ_TIMEOUT_MS);
  }

  try {
    const response = await pbbNativeFetch(input, fetchInit);
    if (isReadRequest) {
      if ([503, 504, 522, 523, 524].includes(response.status)) {
        pbbRecordSupabaseReadFailure();
      } else if (response.ok) {
        pbbClearSupabaseCircuit();
      }
    }
    return response;
  } catch (error) {
    if (isReadRequest) pbbRecordSupabaseReadFailure();
    throw error;
  } finally {
...
const storage = {
  ...
  async uploadExerciseVideo(userId, file, exerciseId) {
    ...
    const mimeExt = mimeType.includes('webm')
      ? 'webm'
    ...
    const fileName = `${userId}/exercises/${safeExerciseId}.${fileExt}`;
