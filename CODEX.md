...
4. When earned, offer the six-week Balance Learn course as one AUD $149 payment for the full six weeks and complete the sale through DMs.
...
1. Explain the fit as one AUD $149 payment for the full six weeks inside the DM. Be clear that it is a six-week course with one weekly check-in and plan review, six weeks of app/community access, and no automatic renewal. Ongoing access after week six is separate.
...
- Paid Meta leads knowingly entered a commercial conversation, so answer offer, price, inclusions, fit and starting questions directly from the first reply and preserve that contract throughout the thread. Organic conversations begin from the exact human context, use statement-led elicitation, and bridge only from the lead's own goal, blocker, help signal or explicit request. Do not import the paid quick replies or fixed two-question progression into a normal DM. When useful, connect one relevant Learn-series idea to the person's own pattern; give the full six-week curriculum only when they ask what they will learn.
...
- Guided-tour audience (Shannon, 18 September 2026): new accounts from every entry route, including organic Instagram conversation -> website -> app, receive the first guided tour. Do not require paid Meta attribution or a QA allowlist. The first-run cohort starts 18 September Brisbane and includes the account that exposed the missed tour. Account-scoped completion is persisted in auth user metadata and local storage; completed and established members remain exempt from unsolicited replay. Active Meta previews and dedicated QA keep their existing paths. Initialize the unified Home daily plan when setup completes, including when setup finishes after app startup. Keep eligibility in `lib/onboarding-progress.js`; preserve payment, checkout and learning requirements.
...
- Older Meta prompt questions that may still appear in conversation history include:
  - "What's included in the six-week Balance Learn program?"
  - "How does the weekly check-in work?"
  - "Do I need to already be plant-based?"
- Older ads may still send "Do you offer personalized coaching plans?". Answer yes by explaining the Balance Learn six-week curriculum and weekly review, then ask about the lead's goal. Do not mention Online Coaching in that paid-ad branch.
