    ...
    try {
        ...
        grid.innerHTML = sortedStories.map(story => {
            ...
            const quickReactions = FEED_REACTIONS.map(r => {
                ...
                return `<button onclick="event.stopPropagation(); toggleFeedReaction('${storyId}', '${r.key}', this)" style="background:none; border:none; padding:4px; cursor:pointer; font-size:1.2rem; opacity:${isActive ? '1' : '0.5'}; transform:${isActive ? 'scale(1.15)' : 'scale(1)'}; transition:all 0.2s;">${r.emoji}</button>`;
            }).join('');

            return `
                <div class="feed-post-card" style="background:white; border-bottom:8px solid #f1f5f9;">
                    <!-- Post header -->
                    ...
                    <!-- Reaction buttons -->
                    <div style="display:flex; align-items:center; padding:6px 10px; gap:2px;">
                        ${quickReactions}
                    </div>
                    <!-- Reaction summary -->
                    ${reactionSummary}
