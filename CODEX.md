...
- The single scheduled Instagram Chrome operator is the 24-hour browser shift dispatcher. Its watchdog wakes every 15 minutes, but only the renewable global lease owner works, for at most one 30-minute shift. A wake that finds a healthy active lease immediately exits without opening Chrome, closing the active run, or altering its ledger. Each owned shift works one persistent lane task to its real boundary instead of restarting the full checklist or rolling through all eight lanes. Resume the newest unfinished lane first; only after it is genuinely completed does the next watchdog advance to the next durable rotation slot. It actively works for 25 minutes, reserves five minutes for reconciliation and clean Chrome release, and claims one person at a time. Once the 20-interaction shift limit is reached, it keeps doing read-only inspection, reconciliation, queue preparation, and cursor repair until minute 25. Its replaced dedicated Instagram Chrome jobs remain paused so browser ownership never overlaps.
