    ...
    if (error) {
      ...
      try {
        const { data: verify } = await supabase
          ...
          .maybeSingle();
        if (verify && verify.endpoint && verify.endpoint.startsWith('native://')) {
          console.log('[NativePush] Token verified in database:', verify.endpoint.substring(0, 30) + '...');
          window._pushTokenStatus = 'verified';
        } else {
          console.warn('[NativePush] Token saved but not found on verify. Row:', verify);
