...
test('returning phones receive the picker and retirement updates', () => {
  ...
  assert.match(dashboard, /dashboard-script-11-calorie_tracker_functions\.js\?v=36-meal-reminders-retired/);
  assert.match(dashboard, /native-push\.js\?v=41-meal-reminders-retired/);
  assert.match(read('sw.js'), /pbb-app-v426-meal-reminders-retired/);
});
