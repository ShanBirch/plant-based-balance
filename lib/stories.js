...
const FEED_UPLOAD_DIAGNOSTIC_MAX_QUEUE = 40;
...
function shouldCaptureFeedUploadDiagnostic(source, options = {}) {
    ...
    return raw.includes('feed_workout_share') || raw.includes('workout_share') || raw.includes('share_set');
}
...
async function uploadStoryMediaToBackblaze(file, options = {}) {
    ...
    if (file.size > FEED_MEDIA_UPLOAD_REQUEST_SAFE_BYTES) {
...
async function uploadStoryMediaMultipartToBackblaze(file, options = {}) {
    ...
    try {
        ...
        for (let partIndex = 0; partIndex < partCount; partIndex += 1) {
            ...
            const part = file.slice(startByte, endByte, 'application/octet-stream');
            const partSha1 = await getFeedBlobSha1(part);
            let uploaded = false;
...
async function refreshFeedAfterTahliaApproval({ preserveScroll = false } = {}) {
    const previousScrollY = preserveScroll ? window.scrollY : 0;
    tahliaFeedApprovalState.loaded = false;
    await loadTahliaFeedApprovals({ force: true });
...
window.approveTahliaFeedItem = async function(alertId, actionId, buttonEl) {
    ...
    try {
        ...
        await refreshFeedAfterTahliaApproval();
    } catch (error) {
...
function updateTahliaFeedEditCounter() {
    const input = document.getElementById('tahlia-feed-edit-input');
    const counter = document.getElementById('tahlia-feed-edit-counter');
    if (input && counter) counter.textContent = `${input.value.length}/500`;
}

function ensureTahliaFeedEditModal() {
    let modal = document.getElementById('tahlia-feed-edit-overlay');
    if (modal) return modal;
    modal = document.createElement('div');
    modal.id = 'tahlia-feed-edit-overlay';
    modal.className = 'tahlia-feed-edit-overlay';
    modal.setAttribute('aria-hidden', 'true');
    modal.innerHTML = `
        <div class="tahlia-feed-edit-dialog" role="dialog" aria-modal="true" aria-labelledby="tahlia-feed-edit-title">
            <div class="tahlia-feed-edit-header">
                <div>
                    <p class="tahlia-feed-edit-kicker">Private Feed review</p>
                    <h2 id="tahlia-feed-edit-title">Edit Tahlia draft</h2>
                </div>
                <button type="button" class="tahlia-feed-edit-close" aria-label="Close editor">&times;</button>
            </div>
            <div class="tahlia-feed-edit-body">
                <label for="tahlia-feed-edit-input">Post or comment wording</label>
                <textarea id="tahlia-feed-edit-input" maxlength="500" rows="5" autocomplete="off"></textarea>
                <div class="tahlia-feed-edit-meta">
                    <span>Saved changes help future Tahlia drafts match your style.</span>
                    <span id="tahlia-feed-edit-counter">0/500</span>
                </div>
            </div>
            <div class="tahlia-feed-edit-footer">
                <button type="button" class="tahlia-feed-edit-cancel">Cancel</button>
                <button type="button" class="tahlia-feed-edit-save">Save edit</button>
            </div>
        </div>`;
    document.body.appendChild(modal);
    modal.addEventListener('click', event => {
        if (event.target === modal) window.closeTahliaFeedEditModal();
    });
    modal.querySelector('.tahlia-feed-edit-close').addEventListener('click', window.closeTahliaFeedEditModal);
    modal.querySelector('.tahlia-feed-edit-cancel').addEventListener('click', window.closeTahliaFeedEditModal);
    modal.querySelector('.tahlia-feed-edit-save').addEventListener('click', window.saveTahliaFeedEdit);
    modal.querySelector('#tahlia-feed-edit-input').addEventListener('input', updateTahliaFeedEditCounter);
    return modal;
}

window.closeTahliaFeedEditModal = function() {
    const modal = document.getElementById('tahlia-feed-edit-overlay');
    if (!modal) return;
    modal.classList.remove('is-open');
    modal.setAttribute('aria-hidden', 'true');
    document.body.classList.remove('tahlia-feed-edit-open');
    const returnFocus = tahliaFeedEditState.returnFocus;
    tahliaFeedEditState.alertId = '';
    tahliaFeedEditState.actionId = '';
    tahliaFeedEditState.originalText = '';
    tahliaFeedEditState.returnFocus = null;
    if (returnFocus?.isConnected) returnFocus.focus();
};

window.openTahliaFeedEditModal = function(alertId, actionId, buttonEl) {
    if (!isShannonFeedReviewer()) return;
    ...
    const modal = ensureTahliaFeedEditModal();
    const input = modal.querySelector('#tahlia-feed-edit-input');
    const saveButton = modal.querySelector('.tahlia-feed-edit-save');
    tahliaFeedEditState.alertId = String(alertId || '');
    tahliaFeedEditState.actionId = String(actionId || item.approval_action_id || '');
    tahliaFeedEditState.originalText = text;
    tahliaFeedEditState.returnFocus = buttonEl || document.activeElement;
    input.value = text;
    ...
    document.body.classList.add('tahlia-feed-edit-open');
    updateTahliaFeedEditCounter();
    requestAnimationFrame(() => {
        input.focus();
...
window.saveTahliaFeedEdit = async function() {
    if (!isShannonFeedReviewer() || !tahliaFeedEditState.alertId || !tahliaFeedEditState.actionId) return;
    const modal = document.getElementById('tahlia-feed-edit-overlay');
    const input = modal?.querySelector('#tahlia-feed-edit-input');
    const saveButton = modal?.querySelector('.tahlia-feed-edit-save');
    ...
    if (editedText === tahliaFeedEditState.originalText) {
        window.closeTahliaFeedEditModal();
        return;
    }
    input.disabled = true;
    ...
    try {
        ...
        const response = await fetch('/.netlify/functions/perform-coach-action', {
            ...
            body: JSON.stringify({
                alertId: tahliaFeedEditState.alertId,
                actionId: tahliaFeedEditState.actionId,
                editedText,
                originalText: tahliaFeedEditState.originalText,
                source: 'balance_feed_tahlia_approval',
                saveOnly: true
            })
        });
        ...
        await refreshFeedAfterTahliaApproval({ preserveScroll: true });
    } catch (error) {
...
document.addEventListener('keydown', event => {
    if (event.key === 'Escape' && document.getElementById('tahlia-feed-edit-overlay')?.classList.contains('is-open')) {
        window.closeTahliaFeedEditModal();
    }
});
...
function renderTahliaFeedApprovalPanel(item, compact = false) {
    ...
    return `<div class="tahlia-feed-approval-panel${compact ? ' tahlia-feed-approval-panel-compact' : ''}" data-tahlia-approval-alert-id="${escapeHtml(item.approval_alert_id || '')}" onclick="event.stopPropagation();">
        ...
        <div class="tahlia-feed-approval-actions">
            ...
            <button type="button" class="tahlia-feed-approval-edit" onclick="event.stopPropagation(); openTahliaFeedEditModal('${alertId}', '${actionId}', this)">Edit</button>
    ...
    try {
        ...
        const renderedFeedHtml = sortedStories.map(story => {
            ...
            const commentPlaceholder = isPendingTahliaApproval
                ...
                : (isOwnStory ? 'Add a comment or @tag...' : 'Add a comment or @tag (+2 XP)');
            ...
            const mediaTapAttrs = isPendingTahliaApproval
                ? 'aria-label="Pending Tahlia Feed preview"'
            ...
            const captionHtml = cardCaptionText
                ? `<div style="padding:4px 14px 8px; font-size:0.85rem; color:var(--text-main); line-height:1.4;"><span style="font-weight:600;">${safeUserName}</span> ${formatFeedText(cardCaptionText)}</div>`
...
window.openFeedPostViewer = async function(storyId, options = {}) {
    ...
    try {
        ...
        if (isCardType) {
            ...
            if (cardData && cardPhotoSrc && (isMealCard || isMealPayload)) {
...
function renderFeedCommentRow(comment, storyId, context = 'inline') {
    ...
    if (comment.pending_tahlia_approval && isShannonFeedReviewer()) {
        return `<div ${rowId ? `id="${escapeHtml(rowId)}"` : ''} class="feed-comment-row feed-comment-row-pending-tahlia" data-feed-comment-id="${escapeHtml(commentId)}" data-feed-story-id="${escapeHtml(storyId)}">
            <div class="tahlia-feed-pending-comment-copy">
