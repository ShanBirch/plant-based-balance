  ...
  async function open(week) {
    try {
      ...
      el.querySelector('[data-plan]')?.addEventListener('submit',async e=>{
        ...
        try { await savePlan(week,e.target.elements.reflection.value); await open(week); } catch(error){el.querySelector('[data-error]').textContent=error.message;button.disabled=false;}
      });
