...
window.renderFitnessDiaryCard = function(cardData) {
    ...
    const note = trimText(cardData.note, 96);
    const hasReflection = !!(highlight || struggle || note);
    ...
    const reflectionHtml = hasReflection
        ? [
            ...
            note ? { label: 'Extra note', value: note } : null
            ...
            <div style="background:rgba(255,255,255,0.12);border:1px solid rgba(255,255,255,0.14);border-radius:14px;padding:12px 13px;">
                ...
                <div style="font-size:0.9rem;color:#ffffff;font-weight:780;line-height:1.34;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;">${escapeHtml(row.value)}</div>
            </div>
        `).join('')
    ...
    return `
        <div style="width:100%;aspect-ratio:4/5;background:linear-gradient(160deg,#10291f 0%,#0f766e 46%,#2563eb 100%);display:flex;flex-direction:column;padding:28px 24px;box-sizing:border-box;position:relative;overflow:hidden;color:#fff;text-shadow:0 2px 8px rgba(0,0,0,0.32);">
            <div style="z-index:1;flex:1;display:flex;flex-direction:column;">
