...
- Common paid-Meta funnel turns use one guided progression contract: clicked prompt -> plant-based connection and, when not already supplied, one brief reason/values question -> goal -> matching transformation proof when genuinely relevant -> blocker -> tailored text offer plus the evergreen Balance app proof video -> app-setup acceptance -> signed interactive app-preview handoff -> explicit checkout handoff. The funnel controls the objective, verified facts and next decision, but the AI writes every ordinary response fresh from the complete newest inbound turn. It must first answer or reflect one exact detail from the person's words, then make the smallest useful next move. Deterministic copy is reserved for exact checkout/app destinations, safety gates and a guaranteed stage-correct fallback when a normal paid-Meta draft cannot clear a non-blocking wording review; proof media may be deterministic but must not replace the tailored reply. A non-blocking style, generic-wording or question-fatigue warning may improve or replace a reply but must never leave a normal understood paid-Meta inbound unanswered. Select transformation proof from the person's stated outcome and known blocker, never force the same client story into every thread, and send no transformation when the match is weak or the context is pregnancy, postpartum, injury, pain, rehabilitation, an eating disorder or self-harm. Proof wording and its native attachment are atomic: the final reviewed wording must explicitly introduce the person or media before it sends, and any repair that removes that introduction must also remove the attachment. Every non-terminal progression reply asks exactly one purposeful next question and never repeats an answered question. Any meaningful goal blocker can earn the tailored path; it is not limited to work or children. Food uncertainty such as `I don't know what to eat`, `dunno what to eat`, or an obvious typo of that phrase is a concrete nutrition blocker, not a reason for another food-intake interview. Once goal and blocker are known, stop interviewing: use concise text to recognise the blocker, explain why a rigid plan fails around their life, and show how Shannon would set Balance up for them. Name the six-week Foundations course, their workout program, meal plan, and one weekly training/food check-in and adjustment. State one AUD $149 payment for the full six weeks with no subscription or auto-renewal, introduce the current offer-agnostic app proof video, then offer the interactive setup with one consent question. After acceptance, explain that the signed preview takes them through their profile, starting workout program and meal plan before opening the full app so they can look around before paying and decide. Paid Meta replies are text-only apart from approved native proof media; never proactively generate or send a synthetic Shannon voice note in this funnel. Send the app-preview URL only after the lead says they are keen; its paywall uses the same fixed six-week offer, not App + Community. For unlinked leads, the only content-based safety stopper is explicit suicide or self-harm language. Pregnancy, postpartum, injury, pain, hospital, medical, eating/body-image, and difficult-emotion language stays in the careful non-diagnostic AI-coach lane. Direct checkout handoffs, real opt-outs, authenticity questions, and genuine media/context uncertainty remain separate exceptions. `I stop and start again` is a blocker, not an opt-out.
- Plant-based-control refinement (2026-08-15): on a fresh Founders Pass opener, give a small direct answer and ask whether they are currently plant-based or vegan, or looking to go plant-based or vegan, before asking fitness goals unless they already supplied that context. If they confirm vegan, plant-based or vegetarian, ask the missing duration and reason before goals; when both are missing, one natural compound question is allowed, otherwise ask only the missing fact. Transformation proof must match both outcome and reliable person context, never a handle guess. The current approved transformation photos feature women, so use them for a reliably matched woman and skip them for a known man or unresolved gender until approved male transformation proof exists. Native transformation-photo and app-video deliveries are not question exceptions: the same atomic turn must end with the next purposeful question. The app video normally ends by asking whether they want a free personalised look at their own workout and meal plan before paying. Only signed app-preview and checkout URL turns contain no question and pause.
- Broad-pain refinement (2026-08-28): freeze `broad_pain` only from verified paid-Meta attribution and never change it from later message wording. Do not introduce or ask about plant-based or vegan status, duration or reasons. Use no more than two discovery questions across the complete episode, skipping supplied facts: first the desired six-week change, then the real-life blocker or support need. Once both are known, stop discovery, explain the neutral six-week Balance setup and exact current terms, then offer the free personalised app preview before payment. Send the signed preview immediately when requested or accepted without reconfirming. A generic `I'm ready` stays on the promised preview path; only an explicit request to join, pay, sign up or receive the checkout link may bypass it. Keep the plant-based-control and organic contracts unchanged.
