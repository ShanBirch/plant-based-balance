...
export const POINTS_CONFIG = {
  ...
  // === DAILY CALORIE GOAL BONUS ===
  CALORIE_GOAL_BONUS: 1,        // 1 bonus point for hitting daily calorie goal
  CALORIE_GOAL_TOLERANCE: 0.10, // 10% tolerance (within 90-110% of goal counts)
...
/**
 * Check if total calories are within tolerance of calorie goal
 * Returns true if within 10% (either direction) of the goal
 */
export const isWithinCalorieGoal = (totalCalories, calorieGoal) => {
  if (!calorieGoal || calorieGoal <= 0 || !totalCalories || totalCalories <= 0) {
    return false;
  }
