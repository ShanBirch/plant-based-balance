...
async function saveNativePushToken(token, _retryCount) {
  ...
  try {
    ...
    if (error) {
      ...
      // Remove ALL other push subscriptions for this user —
      // old native tokens are stale (FCM generates new tokens on reinstall),
      // and web push from a WebView can't deliver when the app is closed.
      // Only keep the current native:// FCM subscription.
      try {
        const currentEndpoint = `native://${token}`;
        const { data: allSubs } = await supabase
          .from('push_subscriptions')
          .select('id, endpoint')
          .eq('user_id', window.currentUser.id);
        if (allSubs && allSubs.length > 0) {
          var staleSubs = allSubs.filter(function(s) { return s.endpoint !== currentEndpoint; });
          if (staleSubs.length > 0) {
            var staleIds = staleSubs.map(function(s) { return s.id; });
            await supabase.from('push_subscriptions').delete().in('id', staleIds);
            console.log('[NativePush] Cleaned up', staleSubs.length, 'stale subscription(s) (old native + web)');
          }
        }
      } catch (cleanupErr) {
