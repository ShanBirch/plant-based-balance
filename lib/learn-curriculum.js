(function(root, factory) {
  ...
  function version(row) {
    ...
    if (['eight_v1','bridge_eight_v1','legacy_six'].includes(saved)) return saved;
    // Existing records retain their sequence even if an older app drops the new setting.
    if (row?.created_at && Date.parse(row.created_at) < Date.parse('2026-09-13T21:05:49Z')) return Number(row.current_week)>6 ? 'legacy_six' : 'bridge_eight_v1';
    return 'eight_v1';
  }
  function weeks(value='eight_v1') {
    const v = typeof value==='string' ? value : version(value);
    return (v==='legacy_six'?original:v==='bridge_eight_v1'?bridge:fresh).map((w,i)=>({...w,number:i+1,lessonIds:[...w.lessonIds]}));
  }
