...
(function() {
    ...
    // =============================================================================
    // HEALTH IQ PROGRESSION SYSTEM
    // =============================================================================

    const HEALTH_IQ_LEVELS = [
        { level: 1,  title: 'Curious',     lessonsRequired: 0,   xpBonus: 0,  icon: '🌱' },
        { level: 2,  title: 'Informed',    lessonsRequired: 5,   xpBonus: 5,  icon: '📖' },
        { level: 3,  title: 'Aware',       lessonsRequired: 15,  xpBonus: 10, icon: '💡' },
        { level: 4,  title: 'Savvy',       lessonsRequired: 30,  xpBonus: 15, icon: '🔬' },
        { level: 5,  title: 'Sharp',       lessonsRequired: 50,  xpBonus: 20, icon: '⚡' },
        { level: 6,  title: 'Expert',      lessonsRequired: 75,  xpBonus: 25, icon: '🎯' },
        { level: 7,  title: 'Scholar',     lessonsRequired: 100, xpBonus: 30, icon: '🏆' },
        { level: 8,  title: 'Genius',      lessonsRequired: 125, xpBonus: 40, icon: '🧬' },
        { level: 9,  title: 'Master',      lessonsRequired: 145, xpBonus: 50, icon: '👑' },
        { level: 10, title: 'Enlightened', lessonsRequired: 160, xpBonus: 100, icon: '✨' },
    ];

    function getHealthIQLevel(lessonsCompleted) {
        let current = HEALTH_IQ_LEVELS[0];
        for (const level of HEALTH_IQ_LEVELS) {
            if (lessonsCompleted >= level.lessonsRequired) {
                current = level;
            } else {
                break;
            }
    ...
    function getNextHealthIQLevel(lessonsCompleted) {
        for (const level of HEALTH_IQ_LEVELS) {
            if (lessonsCompleted < level.lessonsRequired) {
                return level;
            }
    ...
    function getHealthIQProgress(lessonsCompleted) {
        const current = getHealthIQLevel(lessonsCompleted);
        const next = getNextHealthIQLevel(lessonsCompleted);
        if (!next) {
            return { current, next: null, percent: 100, lessonsToNext: 0 };
        }
        const lessonsInRange = next.lessonsRequired - current.lessonsRequired;
        const lessonsProgress = lessonsCompleted - current.lessonsRequired;
        const percent = Math.round((lessonsProgress / lessonsInRange) * 100);
        return { current, next, percent, lessonsToNext: next.lessonsRequired - lessonsCompleted };
    }

    function checkHealthIQLevelUp(oldLessonsCount, newLessonsCount) {
        const oldLevel = getHealthIQLevel(oldLessonsCount);
        const newLevel = getHealthIQLevel(newLessonsCount);
        if (newLevel.level > oldLevel.level) {
            return { leveled: true, oldLevel, newLevel };
        }
        return { leveled: false, oldLevel, newLevel };
    }
        ...
        'body-4': [
            {
                ...
                games: [
                    ...
                    { type: GAME_TYPES.ORDER_SEQUENCE, question: "Order this posture reset, from the moment you notice you've been still to settling into a fresh position:", items: ["Notice you've been in the same position for a while", "Pause what you're doing", "Stand up and take a few steps to reset your body", "Stretch in the opposite direction to your previous position", "Settle back in — but in a different position than before"]},
        ...
        'fuel-4': [
            ...
            },
            {
                ...
                games: [
                    ...
                    { type: GAME_TYPES.ORDER_SEQUENCE, question: "Rank from BEST to WORST pre-workout snack choice 30 minutes before training (fastest to digest = best):", items: ["Banana — quick-digesting, pure carbs, easy on the stomach", "Toast with jam — simple carbs, digests fast", "Apple with light peanut butter — good but the fat slows digestion slightly", "Large burrito with beans — too heavy, too much fat/fiber, will sit in your stomach"], acceptableOrders: [[0,1,2,3],[1,0,2,3]] },
        ...
        'longevity-1': [
  {
    ...
    content: {
      intro: `Aging isn't just "getting older." It's a collection of specific biological processes happening at the cellular level. Scientists have identified 9 "Hallmarks of Aging"—the root drivers of decline.

Think of your body like a complex machine. Over time, parts wear out, circuits glitch, and systems fail to coordinate. The Hallmarks explain exactly HOW that machine breaks down.

These 9 hallmarks fall into three categories:
• Primary Hallmarks: the initial damage (DNA, telomeres, epigenetics)
• Antagonistic Hallmarks: the body's response that can backfire (nutrient sensing, mitochondria, senescence)
• Integrative Hallmarks: the final systemic breakdown (stem cells, cell communication)

Understanding them moves us from "fighting aging as a mystery" toward "managing aging as biology."

In the next lessons, we'll break down each one: what's happening in your cells right now, and how your daily choices affect the outcome.`,
      keyPoint: "The 9 Hallmarks of Aging are specific, identifiable biological drivers — not mysteries. Understanding them puts you in control.",
    },
    games: [
      {
        type: GAME_TYPES.TAP_ALL,
        question: "What are the three categories of aging hallmarks?",
        options: [
          { text: "Primary (initial damage)", correct: true },
          { text: "Antagonistic (body's response)", correct: true },
          { text: "Integrative (final breakdown)", correct: true },
          { text: "Superficial (visible wrinkles)", correct: false },
        ],
      },
      {
        type: GAME_TYPES.SWIPE_TRUE_FALSE,
        question: "Aging is a single mysterious process that just 'happens' to everyone equally.",
        answer: false,
        explanation: "Modern science identifies 9 distinct biological hallmarks that drive aging — and many are influenced by lifestyle.",
      },
      {
        type: GAME_TYPES.MATCH_PAIRS,
        pairs: [
          { left: "Primary Hallmarks", right: "Root-cause cellular damage" },
          { left: "Antagonistic Hallmarks", right: "Body's responses that can backfire" },
          { left: "Integrative Hallmarks", right: "Final systemic decline" },
          { left: "9 Hallmarks total", right: "Identified drivers of aging" },
        ],
      },
      {
        type: GAME_TYPES.FILL_BLANK,
        sentence: "Scientists have identified ___ Hallmarks of Aging that drive cellular decline.",
        options: ['9', '3', '12', '5'],
        answer: '9',
      },
      {
        type: GAME_TYPES.ORDER_SEQUENCE,
        question: "Order from cause to effect: how do the hallmark categories relate?",
        items: [
          "Primary Hallmarks create initial damage (DNA, epigenetics, telomeres)",
          "Antagonistic Hallmarks represent the body's responses (which can themselves cause harm)",
          "Integrative Hallmarks are the final result: stem cell loss and failed communication",
        ],
      },
    ],
  },
  {
    ...
    content: {
      intro: `Two of the most fundamental Hallmarks deal with your genetic blueprint. Think of them as hardware failure vs. software corruption.

GENOMIC INSTABILITY is the "hardware" damage. Every day your DNA is bombarded by UV radiation, toxins, and metabolic waste — creating thousands of tiny lesions. Your cells repair most of them, but some escape. Over decades, this uncorrected damage accumulates, and cells start to malfunction.

EPIGENETIC ALTERATIONS are the "software" glitch. Your DNA sequence (the hardware) mostly stays the same, but the "bookmarks" that tell cells which genes to read — called epigenetic tags — slowly drift out of place. Cells start misreading their instructions, forgetting what they are or how to behave.

This epigenetic drift is what scientists measure when they calculate your biological age. It's the most universal marker of aging we have.

The good news: You can protect both. By reducing DNA-damaging habits (smoking, excessive UV, toxins) and supporting repair through quality sleep and nutrient-dense food, you slow both processes significantly.`,
      keyPoint: "Genomic instability is physical DNA damage (hardware), while epigenetic alterations are gene expression errors (software). Both are modifiable.",
    },
    games: [
      {
        type: GAME_TYPES.MATCH_PAIRS,
        pairs: [
          { left: "Genomic Instability", right: "Physical DNA damage (hardware)" },
          { left: "Epigenetic Alterations", right: "Gene expression drift (software)" },
          { left: "Epigenetic tags", right: "Bookmarks that tell genes what to do" },
          { left: "Biological age", right: "Measured by epigenetic drift" },
        ],
      },
      {
        type: GAME_TYPES.SWIPE_TRUE_FALSE,
        question: "Your DNA sequence itself changes significantly as you age.",
        answer: false,
        explanation: "The sequence (hardware) stays mostly intact. What changes is how it's READ — the epigenetic software. That's the aging driver.",
      },
      {
        type: GAME_TYPES.TAP_ALL,
        question: "What causes DNA hardware damage?",
        options: [
          { text: "UV radiation from the sun", correct: true },
          { text: "Environmental toxins", correct: true },
          { text: "Metabolic waste products", correct: true },
          { text: "Quality sleep", correct: false },
        ],
      },
      {
        type: GAME_TYPES.FILL_BLANK,
        sentence: "Epigenetic _______ represent 'software drift' — cells misreading their own instructions.",
        options: ['alterations', 'replications', 'mutations', 'deletions'],
        answer: 'alterations',
      },
      {
        type: GAME_TYPES.ORDER_SEQUENCE,
        question: "How DNA damage leads to aging:",
        items: [
          "UV, toxins, or metabolic waste strikes a DNA strand",
          "A lesion (hardware damage) is created",
          "Repair enzymes fix most of it — but some escapes",
          "Uncorrected damage accumulates over decades",
          "Cells malfunction, and tissues decline",
        ],
      },
    ],
  },
  {
    ...
    content: {
      intro: `Your chromosomes have protective caps called telomeres — think of them like the plastic aglets on the ends of shoelaces that stop the fabric from fraying.

Every time a cell divides, the telomeres shorten slightly. Eventually, they get too short for safe division. At that point, the cell either dies cleanly (apoptosis) or becomes a dysfunctional "zombie" (senescent) cell. This makes telomere length a literal countdown timer for cell life.

By age 60, most people's telomeres are significantly shorter than at age 20. But — and this is powerful — lifestyle dramatically affects the speed of shortening.

Chronic stress, poor diet, poor sleep, and sedentary behavior all ACCELERATE telomere attrition. Meanwhile, vigorous exercise, meditation, plant-based diets, and stress management have been shown to SLOW — and even partially REVERSE — telomere loss.

The enzyme telomerase can actively rebuild telomeres. Lifestyle factors that reduce cellular stress help keep it active.`,
      keyPoint: "Telomeres are your cellular clock. Their shortening rate is significantly modifiable through lifestyle choices.",
    },
    games: [
      {
        type: GAME_TYPES.MATCH_PAIRS,
        pairs: [
          { left: "Telomeres", right: "Protective chromosome end-caps" },
          { left: "Telomerase", right: "Enzyme that rebuilds telomeres" },
          { left: "Short telomeres", right: "Cell can no longer safely divide" },
          { left: "Chronic stress", right: "Accelerates telomere shortening" },
        ],
      },
      {
        type: GAME_TYPES.SWIPE_TRUE_FALSE,
        question: "Telomere shortening rate is entirely determined by genetics — lifestyle doesn't matter.",
        answer: false,
        explanation: "Studies consistently show lifestyle factors (exercise, stress, sleep, diet) significantly alter how fast telomeres shorten.",
      },
      {
        type: GAME_TYPES.TAP_ALL,
        question: "What accelerates telomere shortening?",
        options: [
          { text: "Chronic psychological stress", correct: true },
          { text: "Vigorous regular exercise", correct: false },
          { text: "Smoking", correct: true },
          { text: "Sedentary lifestyle", correct: true },
          { text: "Poor sleep quality", correct: true },
        ],
      },
      {
        type: GAME_TYPES.FILL_BLANK,
        sentence: "Telomeres are like the _______ on shoelaces — they protect the chromosome from fraying.",
        options: ['aglets', 'knots', 'strings', 'fabric'],
        answer: 'aglets',
      },
      {
        type: GAME_TYPES.SCENARIO_STORY,
        scenario: "Two 50-year-old identical twins are tested. One has been chronically stressed and sedentary. The other exercises regularly and meditates. Their telomere lengths differ significantly.",
        ...
        explanation: "Even with identical DNA, lifestyle choices create measurable biological age differences.",
      },
    ],
  },
  {
    ...
    content: {
      intro: `Two Hallmarks deal with how cells manage their supply chain: Loss of Proteostasis and Deregulated Nutrient Sensing.

LOSS OF PROTEOSTASIS is when your protein quality-control system breaks down. Think of each cell as a factory. Healthy factories detect and recycle defective products instantly. Aging factories let misfolded, dysfunctional proteins pile up. This toxic buildup is literally what you see in Alzheimer's disease (amyloid plaques) and Parkinson's disease — the garbage that couldn't be cleared.

DEREGULATED NUTRIENT SENSING is when your cells lose their fuel gauges. Key pathways — like mTOR, AMPK, and insulin signaling — are designed to sense food availability and switch between "growth mode" and "repair mode." In modern life, constant eating keeps the body stuck in growth mode, which suppresses the repair and cleaning processes (autophagy) that prevent aging.

Our cells evolved for a world of occasional feast and frequent famine. Constant abundance confuses the sensors and accelerates decline.

Balancing periods of eating (growth) with periods of fasting or low intake (repair) keeps both the protein factory clean and the fuel gauges accurate.`,
      keyPoint: "Proteostasis maintains protein quality; nutrient sensing switches between growth and repair. Both decline when we eat constantly.",
    },
    games: [
      {
        type: GAME_TYPES.MATCH_PAIRS,
        pairs: [
          { left: "Proteostasis", right: "Protein quality control system" },
          { left: "mTOR pathway", right: "Senses nutrients, promotes growth" },
          { left: "Autophagy", right: "Cellular cleanup — suppressed by eating" },
          { left: "Protein misfolding buildup", right: "Linked to Alzheimer's and Parkinson's" },
        ],
      },
      {
        type: GAME_TYPES.SWIPE_TRUE_FALSE,
        question: "Constantly eating throughout the day can suppress cellular repair processes.",
        answer: true,
        explanation: "Nutrient sensors like mTOR stay 'on' when food is always present, blocking the autophagy (cleanup) that prevents aging.",
      },
      {
        type: GAME_TYPES.TAP_ALL,
        question: "What happens during loss of proteostasis?",
        options: [
          { text: "Proteins misfold and aren't cleared", correct: true },
          { text: "Toxic protein buildup accumulates", correct: true },
          { text: "Cell internal environment becomes cleaner", correct: false },
          { text: "Linked to neurodegenerative diseases", correct: true },
        ],
      },
      {
        type: GAME_TYPES.FILL_BLANK,
        sentence: "Autophagy — the cellular cleanup process — is _______ when nutrient sensing pathways are constantly activated.",
        options: ['suppressed', 'enhanced', 'unaffected', 'accelerated'],
        answer: 'suppressed',
      },
      {
        type: GAME_TYPES.ORDER_SEQUENCE,
        question: "How constant eating affects cell health:",
        items: [
          "Food enters the bloodstream — blood sugar and amino acids rise",
          "Nutrient sensors (mTOR, insulin) activate and signal 'growth mode'",
          "Cell focuses on building and storing",
          "Repair and cleanup processes (autophagy) are suppressed",
          "Damaged proteins and organelles accumulate over time",
        ],
      },
    ],
  },
  {
    ...
    content: {
      intro: `Every cell in your body contains hundreds to thousands of mitochondria — the power plants that produce ATP, the universal energy currency of life. Mitochondrial Dysfunction is what happens when these engines start to fail.

When mitochondria are healthy, they produce clean energy with minimal waste. As they age or get damaged, they start leaking "reactive oxygen species" (ROS, or free radicals) — like a car engine with a broken exhaust pipe. The engine still runs, but the leaked exhaust slowly destroys everything around it.

This oxidative damage cascades into accelerated aging across all tissues. It's also why mitochondrial health is so central to energy levels and metabolism.

The repair process for mitochondria is called Mitophagy, and like all cellular cleanup, it only activates during periods of metabolic stress — intense exercise, fasting, or cold exposure.

You can literally grow new, healthier mitochondria through lifestyle. This process, called Mitochondrial Biogenesis, is one of the most powerful anti-aging adaptations the body can make.`,
      keyPoint: "Mitochondria power your cells. When they fail, energy drops and oxidative damage accelerates aging. Exercise creates new ones.",
    },
    games: [
      {
        type: GAME_TYPES.MATCH_PAIRS,
        pairs: [
          { left: "Mitochondria", right: "Cellular power plants" },
          { left: "ATP", right: "The cell's energy currency" },
          { left: "Free Radicals (ROS)", right: "Damaging 'exhaust' from broken mitochondria" },
          { left: "Mitochondrial Biogenesis", right: "Growing new mitochondria via exercise" },
        ],
      },
      {
        type: GAME_TYPES.SWIPE_TRUE_FALSE,
        question: "Exercise can stimulate the creation of new, healthier mitochondria.",
        answer: true,
        explanation: "Mitochondrial biogenesis — triggered by exercise and fasting — is one of the most powerful anti-aging adaptations available.",
      },
      {
        type: GAME_TYPES.TAP_ALL,
        question: "What damages mitochondria and accelerates dysfunction?",
        options: [
          { text: "Chronic inflammation", correct: true },
          { text: "Sedentary behavior", correct: true },
          { text: "Blood sugar dysregulation", correct: true },
          { text: "High-intensity exercise", correct: false },
          { text: "Poor sleep", correct: true },
        ],
      },
      {
        type: GAME_TYPES.FILL_BLANK,
        sentence: "Mitochondria produce _______, the universal energy molecule used by all cells.",
        options: ['ATP', 'DNA', 'glucose', 'cortisol'],
        answer: 'ATP',
      },
      {
        type: GAME_TYPES.ORDER_SEQUENCE,
        question: "How exercise improves your energy at the cellular level:",
        items: [
          "Exercise creates massive demand for energy — more than mitochondria can supply",
          "This energy 'crisis' triggers cellular stress signals",
          "Stress signals activate genes for building new mitochondria",
          "New, efficient mitochondria are created (biogenesis)",
          "Overall cellular energy production and efficiency improves",
        ],
      },
    ],
  },
  {
    ...
    content: {
      intro: `Two hallmarks are about what happens when the body's "social order" breaks down: Cellular Senescence and Altered Intercellular Communication.

CELLULAR SENESCENCE happens when a cell is too damaged to work properly but refuses to die. Instead of self-destructing cleanly (apoptosis), it turns into a "zombie cell" — no longer doing its job, but still alive and actively secreting a toxic cocktail of inflammatory signals called SASP (Senescence-Associated Secretory Phenotype). This poisons neighboring healthy cells, accelerating their decline.

ALTERED INTERCELLULAR COMMUNICATION is what happens when the "noise" from these zombie cells and from chronic inflammation overwhelms the body's normal signaling. It's like a city where every alarm is going off simultaneously — the fire department can't tell where the real fire is, and nothing gets coordinated properly.

This "inflammaging" — chronic, age-driven, low-grade inflammation — is linked to virtually every major age-related disease.

Reducing zombie cells (through exercise, fasting, and plant compounds like quercetin) and keeping inflammation low (diet, sleep, stress management) are central to longevity.`,
      keyPoint: "Zombie cells (senescence) spread toxic signals, while chronic inflammation corrupts cell-to-cell communication — both are modifiable.",
    },
    games: [
      {
        type: GAME_TYPES.MATCH_PAIRS,
        pairs: [
          { left: "Senescent cell", right: "Zombie — alive but toxic" },
          { left: "SASP", right: "Toxic signal cocktail from zombie cells" },
          { left: "Inflammaging", right: "Chronic age-driven inflammation" },
          { left: "Apoptosis", right: "Clean, programmed cell death" },
        ],
      },
      {
        type: GAME_TYPES.SWIPE_TRUE_FALSE,
        question: "Senescent 'zombie' cells are harmless — they just stop working.",
        answer: false,
        explanation: "Zombie cells actively secrete SASP — harmful signals that damage neighboring healthy cells and accelerate aging in surrounding tissue.",
      },
      {
        type: GAME_TYPES.TAP_ALL,
        question: "What helps clear senescent zombie cells?",
        options: [
      ...
      {
        type: GAME_TYPES.FILL_BLANK,
        sentence: "SASP is the _______ cocktail secreted by zombie cells that damages nearby tissue.",
        options: ['toxic signal', 'nutritional', 'repair', 'energy'],
        answer: 'toxic signal',
      },
      {
        type: GAME_TYPES.ORDER_SEQUENCE,
        question: "How a zombie cell spreads aging to neighbors:",
        items: [
          "A cell sustains severe damage (UV, toxins, metabolic stress)",
          "Instead of dying cleanly, it enters senescence (zombie state)",
          "It begins secreting SASP — a toxic inflammatory cocktail",
          "Nearby healthy cells are exposed to constant SASP signals",
          "Neighbors experience accelerated damage and may themselves become senescent",
        ],
      },
    ],
  },
  {
    ...
    content: {
      intro: `Stem Cell Exhaustion is the final integrative hallmark — the point where the body's ability to replace damaged cells runs out.

Stem cells are your body's "blank templates." They can differentiate into whatever cell type is needed — skin, muscle, bone, blood — to repair a wound or replace worn-out tissue. This is why a 10-year-old heals a scrape in three days, but a 70-year-old might take three weeks.

As we age, our reserve of active stem cells depletes, and the remaining ones lose their "potency" — the ability to transform and divide effectively. When repair can't keep pace with damage, that's when aging accelerates visibly.

Stem cells are highly sensitive to their environment. Chronic inflammation, blood sugar dysregulation, and oxidative stress all damage the stem cell "niche" — the environment they live in — reducing their effectiveness.

While we can't simply "add" more stem cells yet, we can protect the ones we have. The same foundational habits — movement, plants, quality sleep, stress management — maintain the stem cell environment and slow their exhaustion.`,
      keyPoint: "Stem cell exhaustion is when the body can no longer repair itself fast enough. Protecting their environment through lifestyle slows this process.",
    },
    games: [
      {
        type: GAME_TYPES.MATCH_PAIRS,
        pairs: [
          { left: "Stem cells", right: "Blank template repair cells" },
          { left: "Stem cell exhaustion", right: "Repair capacity running out" },
          { left: "Stem cell niche", right: "The environment stem cells live in" },
          { left: "Potency", right: "A stem cell's ability to transform" },
        ],
      },
      {
        type: GAME_TYPES.SWIPE_TRUE_FALSE,
        question: "Aging is essentially about damage accumulating faster than repair can keep up.",
        answer: true,
        explanation: "When stem cell exhaustion tips the balance — damage wins over repair — that's when the visible signs of aging accelerate.",
      },
      {
        type: GAME_TYPES.TAP_ALL,
        question: "What damages the stem cell environment?",
        options: [
          { text: "Chronic inflammation", correct: true },
          { text: "Blood sugar dysregulation", correct: true },
          { text: "Oxidative stress", correct: true },
          { text: "Regular physical activity", correct: false },
          { text: "Chronic stress", correct: true },
        ],
      },
      {
        type: GAME_TYPES.FILL_BLANK,
        sentence: "Stem cells can transform into _______ cell types needed—skin, muscle, blood, and more.",
        options: ['many different', 'only one', 'no', 'temporary'],
        answer: 'many different',
      },
      {
        type: GAME_TYPES.ORDER_SEQUENCE,
        question: "How healing capacity changes with age:",
        items: [
          "Damage occurs in a tissue (injury or normal wear)",
          "Distress signals call for stem cell backup",
          "Stem cells migrate to the area and transform into needed cell types",
          "Over decades, the stem cell reserve and potency decline",
          "Healing slows down and tissue quality gradually deteriorates",
        ],
      },
    ],
  },
  {
    ...
    content: {
      intro: `There are two ways to measure age. CHRONOLOGICAL AGE is the number of birthdays you've had — a measure of time, completely fixed. BIOLOGICAL AGE is a measure of how your cells are actually functioning — and it's highly malleable.

Two people can both be 55 years old chronologically, but one can have the biology of a 40-year-old and the other the biology of a 70-year-old. That 30-year gap is entirely created by lifestyle differences.

Scientists measure biological age using "Epigenetic Clocks" — analyzing the specific DNA methylation patterns that shift predictably with aging. Other markers include telomere length, inflammatory markers, cardiovascular fitness (VO2 max), and physical performance tests like grip strength.

The most powerful finding in modern longevity science: biological age is REVERSIBLE. Comprehensive lifestyle change — consistent exercise, whole-food diet, quality sleep, stress management, and social connection — has been shown to measurably reduce biological age.

This entire module exists because your choices today literally change how old your cells are tomorrow.`,
      keyPoint: "Chronological age is fixed, but biological age is modifiable. The entire goal of this module is helping you lower yours.",
    },
    games: [
      {
        type: GAME_TYPES.MATCH_PAIRS,
        pairs: [
          { left: "Chronological age", right: "Fixed — number of birthdays" },
          { left: "Biological age", right: "Modifiable — how cells actually function" },
          { left: "Epigenetic clock", right: "DNA methylation-based age measurement" },
          { left: "VO2 max", right: "Cardiovascular fitness marker of bio-age" },
        ],
      },
      {
        type: GAME_TYPES.SWIPE_TRUE_FALSE,
        question: "Biological age can be significantly younger or older than chronological age.",
        answer: true,
        explanation: "Lifestyle choices compound over decades, creating gaps of 15-30 years between chronological and biological age in extreme cases.",
      },
      {
        type: GAME_TYPES.FILL_BLANK,
        sentence: "_______ age predicts disease risk and health outcomes far better than birthday age.",
        options: ['Biological', 'Mental', 'Calendar', 'Social'],
        answer: 'Biological',
      },
      {
        type: GAME_TYPES.TAP_ALL,
        question: "What factors are used to measure biological age?",
        options: [
          { text: "Epigenetic clock (DNA methylation)", correct: true },
          { text: "Telomere length", correct: true },
          { text: "Inflammatory marker levels", correct: true },
          { text: "Birth certificate", correct: false },
          { text: "Physical performance (grip, VO2 max)", correct: true },
        ],
      },
      {
        type: GAME_TYPES.SCENARIO_STORY,
        scenario: "Two 55-year-olds are tested by an epigenetic clock. Person A exercises daily, eats whole foods, sleeps 8 hours, and has strong relationships. Person B is sedentary, eats processed food, sleeps 5 hours. Their biological ages differ by 18 years.",
        question: "What created this 18-year difference?",
        options: [
          { text: "Genetics — they must have different DNA", correct: false },
          { text: "Cumulative lifestyle differences over decades", correct: true },
          { text: "The test must be inaccurate", correct: false },
        ],
        explanation: "Lifestyle choices compound. Each healthy habit prevents a small amount of biological aging daily — and over years, that adds up to decades of difference.",
      },
      {
        type: GAME_TYPES.ORDER_SEQUENCE,
        question: "How daily choices drive biological age outcomes:",
        items: [
          "Daily lifestyle choices (movement, diet, sleep, stress) create cellular conditions",
          "These conditions directly affect cellular repair, inflammation, and metabolism",
          "Over time, epigenetic patterns shift to reflect those conditions",
          "Biological age diverges from chronological age — younger or older",
          "Biological age becomes the primary predictor of future health outcomes",
        ],
      },
    ],
  },
...
These "soft" factors have hard biological effects.`,
    ...
    games: [
      ...
      {
        type: GAME_TYPES.ORDER_SEQUENCE,
        question: "Order these from the smallest circle outward: How connection and purpose build, level by level:",
        items: [
          "One-on-one — deepen a few close relationships",
          "A small community — find a group around shared values or interests",
          "Contribution — do something useful within that community",
          "Purpose — serve a cause larger than any single group",
        ],
        correctOrder: [0, 1, 2, 3],
      },
      {
...
Practical recommendations: Use mostly moderate reps (6-12) for compounds. Include some heavier work (4-6 reps) for strength. Include some higher reps (12-20+) for isolation and variety. Take most sets within 1-3 reps of failure.`,
    ...
    games: [
      ...
      },
      {
        ...
        correctOrder: [0, 1, 2, 3, 4],
        acceptableOrders: [[0,1,2,3,4],[1,0,2,3,4],[0,2,1,3,4],[1,2,0,3,4],[2,0,1,3,4],[2,1,0,3,4]],
      },
      {
        type: GAME_TYPES.SWIPE_TRUE_FALSE,
...
There's no perfect program—only the one you'll actually do consistently with effort. Start where you are, apply the principles, and trust the process.`,
    ...
    /**
     * A unit is "fully completed" when every lesson inside it is in
     * lessons_completed. This derives completion from the lesson state
     * directly so the UI doesn't get stuck if units_completed drifts out
     * of sync (older progress, partial DB sync, completion flows that
     * didn't push into units_completed, etc).
     *
     * Units that contain no lessons (content coming soon) are treated as
     * "not completed" so they never auto-unlock the next unit — but they
     * also can't block progression, so callers that use this for locking
     * should fall back to the previous populated unit.
     */
    function isUnitFullyCompleted(unitId, progress) {
    ...
    function getRandomAvailableLesson(progress) {
        ...
        for (const module of modules) {
            ...
            for (let ui = 0; ui < units.length; ui++) {
                const unit = units[ui];
                // Unit unlock: first unit is always accessible, others need
                // every lesson in the previous populated unit to be completed.
                // Derive from lessons_completed so units_completed drift can't
                // leave a partially-played unit looking locked.
                let unitLocked = false;
                for (let pi = ui - 1; pi >= 0; pi--) {
                    const prev = units[pi];
                    if (getLessonsForUnit(prev.id).length === 0) continue;
                    if (!isUnitFullyCompleted(prev.id, progress)) unitLocked = true;
                    break;
                }
    ...
    async function ensureLearningProgressLoaded() {
        ...
        try {
            if (window.supabaseClient && window.currentUser) {
                ...
                // Restore daily quiz completion from server (survives app reinstall)
                const today = getLocalDateString();
                if (localStorage.getItem('dailyQuizCompletedDate') !== today) {
                    try {
                        const { data: milestone } = await window.supabaseClient
                            .from('learning_milestones')
                            .select('id')
                            .eq('user_id', window.currentUser.id)
                            .eq('milestone_type', 'daily_quiz')
                            .eq('milestone_id', today)
                            .maybeSingle();
                        if (milestone) {
                            localStorage.setItem('dailyQuizCompletedDate', today);
                        }
                    } catch (e) {
                        // Ignore — quiz card will just show if check fails
                    }
                }
            } else if (localProgress) {
                learningState.userProgress = localProgress;
    ...
    async function initLearning() {
        ...
        // Don't overwrite the view if a quiz battle is in progress
        if (learningState.currentView === 'quizBattle') {
            return;
        }
        renderLearningHome();
    }
    ...
    function renderLearningHome() {
        ...
        const iqProgress = getHealthIQProgress(completedLessons);
        const iqLevel = iqProgress.current;
        const iqNext = iqProgress.next;
        ...
        container.innerHTML = `
            ...
            <div id="xp-info-banner" style="margin: 15px; padding: 16px 20px; background: linear-gradient(135deg, #059669 0%, #10b981 100%); border-radius: 16px; display: flex; align-items: center; gap: 15px; position: relative;">
                <button onclick="document.getElementById('xp-info-banner').style.display='none'; localStorage.setItem('xpBannerDismissed','true');" style="position: absolute; top: 4px; right: 4px; background: rgba(255,255,255,0.2); border: none; color: white; width: 44px; height: 44px; border-radius: 50%; cursor: pointer; display: flex; align-items: center; justify-content: center; font-size: 18px; line-height: 1; padding: 0; z-index: 1;">✕</button>
            ...
            <!-- Health IQ Card -->
            <div class="health-iq-card" style="margin: 0 15px 20px;">
                <div class="health-iq-header">
                    <div class="health-iq-level-badge">
                        <span class="health-iq-icon">${iqLevel.icon}</span>
                        <span class="health-iq-level-num">${iqLevel.level}</span>
                    </div>
                    <div class="health-iq-info">
                        <div class="health-iq-title">Health IQ</div>
                        <div class="health-iq-rank">${iqLevel.title}</div>
                    </div>
                    <div class="health-iq-lessons-count">
                        <div class="health-iq-count-num">${completedLessons}</div>
                        <div class="health-iq-count-label">lessons</div>
                    </div>
                </div>
                ${iqNext ? `
                <div class="health-iq-progress-section">
                    <div class="health-iq-progress-bar">
                        <div class="health-iq-progress-fill" style="width: ${iqProgress.percent}%;"></div>
                    </div>
                    <div class="health-iq-progress-label">
                        <span>${iqProgress.lessonsToNext} lesson${iqProgress.lessonsToNext !== 1 ? 's' : ''} to ${iqNext.icon} ${iqNext.title}</span>
                        <span>+${iqNext.xpBonus} XP</span>
                    </div>
                </div>
                ` : `
                <div class="health-iq-max-level">
                    Max Level Reached! You've mastered it all.
                </div>
                `}
            </div>
    ...
    function renderModuleCard(module, progress) {
        const units = getUnitsForModule(module.id);
        // Derive from lessons_completed so the count never lags behind the
        // actual progress (see isUnitFullyCompleted).
        const completedUnits = units.filter(u => isUnitFullyCompleted(u.id, progress)).length;
    ...
    function renderUnitCard(unit, index, progress, module) {
        ...
        // Derive completion + lock from lessons_completed directly so progress
        // never looks stuck when units_completed drifts (e.g. older saves).
        const isUnitComplete = hasLessons && completedLessons === lessons.length;
        // Find the nearest populated previous unit and require it to be fully
        // completed. Empty "coming soon" units are skipped so they can't block.
        const siblingUnits = getUnitsForModule(unit.moduleId);
        let prevUnit = null;
        for (let pi = index - 1; pi >= 0; pi--) {
            if (getLessonsForUnit(siblingUnits[pi].id).length > 0) {
                prevUnit = siblingUnits[pi];
                break;
            }
        }
        const isLocked = !!prevUnit && !isUnitFullyCompleted(prevUnit.id, progress);
    ...
    async function awardQuizChallengePoint() {
        try {
            if (!window.currentUser?.id || !window.supabaseClient) return;

            // Find active Health IQ quiz challenges this user is participating in
            const { data: participations, error } = await window.supabaseClient
                .from('challenge_participants')
                .select('challenge_id, challenge_points, challenges!inner(challenge_type, status)')
                .eq('user_id', window.currentUser.id)
                .eq('status', 'accepted')
            ...
            // Increment challenge_points by 1 for each active quiz challenge
            for (const p of participations) {
                await window.supabaseClient
                    .from('challenge_participants')
                    .update({ challenge_points: (p.challenge_points || 0) + 1 })
                    .eq('challenge_id', p.challenge_id)
                    .eq('user_id', window.currentUser.id);
            }

            // Refresh challenge cards on home screen
            if (typeof loadHomeChallenges === 'function') loadHomeChallenges();
        } catch (err) {
            console.log('Quiz challenge point award skipped:', err.message);
        }
    }
    ...
    async function completeLesson() {
        ...
        // Capture lesson count BEFORE completion for Health IQ level-up check
        const lessonsBeforeCompletion = progress.lessons_completed.length;
        ...
        try {
            if (window.supabaseClient && window.currentUser) {
                ...
                if (error) {
                    ...
                    // Update learning streak from DB result
                    if (data.current_streak !== undefined && learningState.userProgress) {
                        learningState.userProgress.current_streak = data.current_streak;
                    }
                    ...
                    // Award 1 point to any active Health IQ quiz challenges for 100% score
                    // (counts both new lessons and replays so users can actively compete)
                    if (data.score_percentage === 100) {
                        awardQuizChallengePoint().catch(() => {});
                    }
                }
            }
        } catch (err) {
        ...
        // Applies 2x multiplier if user is in any active challenge (non-additive: always 2x max)
        let dailyQuizBonus = 0;
        if (learningState.isDailyQuiz && scorePercent === 100 && !isDailyQuizCompletedToday()) {
            const _quizXpMultiplier = typeof window.getXPMultiplier === 'function' ? await window.getXPMultiplier() : 1;
            dailyQuizBonus = 5 * _quizXpMultiplier;
            markDailyQuizCompleted();

            // Persist daily quiz completion to server (survives app reinstall)
            try {
                if (window.supabaseClient && window.currentUser) {
                    await window.supabaseClient
                        .from('learning_milestones')
                        .upsert({
                            ...
                            milestone_type: 'daily_quiz',
                            milestone_id: getLocalDateString(),
                            xp_awarded: dailyQuizBonus
                        }, { onConflict: 'user_id,milestone_type,milestone_id' });
                }
            } catch (e) {
                console.log('Daily quiz milestone save skipped:', e.message);
            }
        ...
        // ===== HEALTH IQ LEVEL-UP CHECK =====
        const lessonsAfterCompletion = progress.lessons_completed.length;
        const healthIQLevelUp = checkHealthIQLevelUp(lessonsBeforeCompletion, lessonsAfterCompletion);
        let healthIQBonusXP = 0;
        if (healthIQLevelUp.leveled) {
            const _hiqXpMultiplier = typeof window.getXPMultiplier === 'function' ? await window.getXPMultiplier() : 1;
            healthIQBonusXP = healthIQLevelUp.newLevel.xpBonus * _hiqXpMultiplier;
            // Award Health IQ bonus XP to database
            try {
                if (window.supabaseClient && window.currentUser && healthIQBonusXP > 0) {
                    const { data: currentPoints } = await window.supabaseClient
                        .from('user_points')
                        .select('lifetime_points')
                        .eq('user_id', window.currentUser.id)
                    ...
                    if (currentPoints) {
                        await window.supabaseClient
                            .from('user_points')
                            .update({ lifetime_points: (currentPoints.lifetime_points || 0) + healthIQBonusXP })
                            .eq('user_id', window.currentUser.id);
                    }

                    // Save Health IQ milestone
                    await window.supabaseClient
                        .from('learning_milestones')
                        .upsert({
                            user_id: window.currentUser.id,
                            milestone_type: 'health_iq_level',
                            milestone_id: String(healthIQLevelUp.newLevel.level),
                            xp_awarded: healthIQBonusXP
                        }, { onConflict: 'user_id,milestone_type,milestone_id' });
                ...
                console.log('Health IQ bonus XP award skipped:', hiqErr);
            }
        }

        // Use DB result if RPC succeeded (source of truth), otherwise fall back to local calculation
        const finalXpEarned = (rpcSucceeded ? dbXpEarned : xpEarned) + dailyQuizBonus + healthIQBonusXP;
        const result = {
            xp_earned: finalXpEarned,
            is_new_lesson: dbIsNewLesson,
            needs_perfect_score: dbNeedsPerfectScore,
            daily_quiz_bonus: dailyQuizBonus,
            was_daily_quiz: learningState.isDailyQuiz,
            health_iq_level_up: healthIQLevelUp.leveled ? healthIQLevelUp : null,
            health_iq_bonus_xp: healthIQBonusXP,
            current_streak: learningState.userProgress?.current_streak || 0,
        };
    ...
    function renderLessonComplete(result, lesson, module) {
        ...
        // Health IQ progress for lesson complete screen
        const completedLessons = learningState.userProgress?.lessons_completed?.length || 0;
        const iqProgress = getHealthIQProgress(completedLessons);
        const iqLevel = iqProgress.current;
        const iqNext = iqProgress.next;

        // Health IQ level-up celebration HTML
        let healthIQLevelUpHtml = '';
        if (result?.health_iq_level_up) {
            const newLvl = result.health_iq_level_up.newLevel;
            const oldLvl = result.health_iq_level_up.oldLevel;
            healthIQLevelUpHtml = `
                <div class="health-iq-levelup-banner" style="animation: healthIQSlideIn 0.6s ease-out 0.8s both;">
                    <div class="health-iq-levelup-glow"></div>
                    <div class="health-iq-levelup-content">
                        <div class="health-iq-levelup-icon">${newLvl.icon}</div>
                        <div class="health-iq-levelup-label">HEALTH IQ LEVEL UP!</div>
                        <div class="health-iq-levelup-transition">
                            <span class="health-iq-old-rank">${oldLvl.title}</span>
                            <span class="health-iq-arrow">&#8594;</span>
                            <span class="health-iq-new-rank">${newLvl.title}</span>
                        </div>
                        <div class="health-iq-levelup-bonus">+${result.health_iq_bonus_xp} XP Bonus</div>
                    </div>
                </div>
            `;
            // Show the full-screen celebration overlay after a short delay
            setTimeout(() => showHealthIQCelebration(newLvl, oldLvl, result.health_iq_bonus_xp), 400);
        }

        // Mini Health IQ progress bar for lesson complete
        let healthIQMiniHtml = '';
        if (!result?.health_iq_level_up && iqNext && scorePercent === 100) {
            healthIQMiniHtml = `
                <div style="margin-top: 16px; padding-top: 16px; border-top: 1px solid #e5e7eb;">
                    <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 6px;">
                        <span style="font-size: 0.8rem; font-weight: 600; color: #6b7280;">${iqLevel.icon} Health IQ: ${iqLevel.title}</span>
                        <span style="font-size: 0.75rem; color: #9ca3af;">${iqProgress.lessonsToNext} to ${iqNext.title}</span>
                    </div>
                    <div style="height: 6px; background: #e5e7eb; border-radius: 3px; overflow: hidden;">
                        <div style="height: 100%; width: ${iqProgress.percent}%; background: linear-gradient(90deg, #6366f1, #8b5cf6); border-radius: 3px; transition: width 0.5s ease;"></div>
                    </div>
                </div>
            `;
        }
        ...
        container.innerHTML = `
            <div style="min-height: 100vh; background: ${module.color}; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 20px; text-align: center;">
                ...
                ${result?.current_streak > 0 ? `
                    <div style="margin-top: 20px; display: flex; align-items: center; justify-content: center; gap: 10px; padding: 12px 24px; background: rgba(255,255,255,0.2); border-radius: 30px;">
                        <span style="font-size: 1.5rem;">🔥</span>
                        <span style="color: white; font-weight: 700; font-size: 1.05rem;">${result.current_streak} Day Learning Streak!</span>
                    </div>
                ` : ''}
    ...
    function showHealthIQCelebration(newLevel, oldLevel, bonusXP) {
        // Remove any existing celebration
        const existing = document.getElementById('health-iq-celebration');
        if (existing) existing.remove();

        const overlay = document.createElement('div');
        overlay.id = 'health-iq-celebration';
        overlay.className = 'health-iq-celebration-overlay';
        overlay.innerHTML = `
            <div class="health-iq-celebration-backdrop"></div>
            <div class="health-iq-celebration-particles" id="hiq-particles"></div>
            <div class="health-iq-celebration-card">
                <div class="health-iq-celebration-rings">
                    <div class="health-iq-ring"></div>
                    <div class="health-iq-ring"></div>
                    <div class="health-iq-ring"></div>
                </div>
                <div class="health-iq-celebration-badge">
                    <span>${newLevel.icon}</span>
                </div>
                <div class="health-iq-celebration-title">Health IQ Level Up!</div>
                <div class="health-iq-celebration-level">Level ${newLevel.level}</div>
                <div class="health-iq-celebration-rank-transition">
                    <span class="hiq-old">${oldLevel.title}</span>
                    <span class="hiq-arrow">&#8594;</span>
                    <span class="hiq-new">${newLevel.title}</span>
                </div>
                <div class="health-iq-celebration-xp">+${bonusXP} XP Bonus!</div>
                <button class="health-iq-celebration-btn" onclick="document.getElementById('health-iq-celebration').classList.add('dismissing'); setTimeout(() => document.getElementById('health-iq-celebration')?.remove(), 400);">
                    Continue
                </button>
            </div>
        `;
        ...
        const particleContainer = document.getElementById('hiq-particles');
        if (particleContainer) {
            const colors = ['#FFD700', '#8B5CF6', '#EC4899', '#10B981', '#F59E0B', '#6366F1'];
            for (let i = 0; i < 40; i++) {
                const particle = document.createElement('div');
                particle.className = 'hiq-particle';
    ...
    // Expose internal state for the home-page inline lesson card (read + minimal mutate)
    window._learningState = learningState;
    ...
    // Expose Health IQ helpers for dashboard/profile
    window._getHealthIQLevel = function() {
        const completed = learningState.userProgress?.lessons_completed?.length || 0;
        return getHealthIQLevel(completed);
    };
    window._getHealthIQProgress = function() {
        const completed = learningState.userProgress?.lessons_completed?.length || 0;
        return getHealthIQProgress(completed);
    };
    window.HEALTH_IQ_LEVELS = HEALTH_IQ_LEVELS;
    ...
    const QUIZ_BATTLE_TIME_LIMITS = {
        [GAME_TYPES.SWIPE_TRUE_FALSE]: 12,
        [GAME_TYPES.FILL_BLANK]: 15,
        [GAME_TYPES.TAP_ALL]: 18,
        [GAME_TYPES.SCENARIO_STORY]: 22,
        [GAME_TYPES.MATCH_PAIRS]: 26,
        [GAME_TYPES.ORDER_SEQUENCE]: 26,
    };
    ...
    function generateQuizBattleQuestions(battleId) {
        console.log('[QuizBattle] Generating questions for seed:', battleId);
    ...
    window.showQuizBattleInviteModal = async function() {
        console.log('[QuizBattle] showQuizBattleInviteModal called');
                ...
                <div class="battle-invite-body">
                    ...
                    <div style="padding: 10px 0 14px;">
                        ...
                        <div style="display: flex; gap: 6px; flex-wrap: wrap;">
                            ...
                            <button onclick="window._setQuizBattleBet(1000, this)" class="qb-bet-btn" style="flex: 1; min-width: 50px; padding: 8px; background: rgba(255,255,255,0.08); border: 2px solid rgba(255,255,255,0.15); border-radius: 8px; color: white; font-weight: 700; font-size: 0.8rem; cursor: pointer;">🪙 1K</button>
                            <button onclick="window._setQuizBattleBet(2000, this)" class="qb-bet-btn" style="flex: 1; min-width: 50px; padding: 8px; background: rgba(255,255,255,0.08); border: 2px solid rgba(255,255,255,0.15); border-radius: 8px; color: white; font-weight: 700; font-size: 0.8rem; cursor: pointer;">🪙 2K</button>
                            <button onclick="window._setQuizBattleBet(2500, this)" class="qb-bet-btn" style="flex: 1; min-width: 50px; padding: 8px; background: rgba(255,255,255,0.08); border: 2px solid rgba(255,255,255,0.15); border-radius: 8px; color: white; font-weight: 700; font-size: 0.8rem; cursor: pointer;">🪙 2.5K</button>
                            <button onclick="window._setQuizBattleBet(5000, this)" class="qb-bet-btn" style="flex: 1; min-width: 50px; padding: 8px; background: rgba(255,255,255,0.08); border: 2px solid rgba(255,255,255,0.15); border-radius: 8px; color: white; font-weight: 700; font-size: 0.8rem; cursor: pointer;">🪙 5K</button>
                        </div>
                    ...
                    <div style="padding: 8px 0 4px; font-size: 0.7rem; color: rgba(255,255,255,0.4); text-transform: uppercase; letter-spacing: 1px;">Play vs Bot</div>
                    <div style="display: flex; gap: 8px; margin-bottom: 16px;">
                        <div onclick="window._startQuizBattleVsAI('easy')" style="flex:1;padding:12px 8px;border-radius:12px;text-align:center;cursor:pointer;background:rgba(16,185,129,0.1);border:2px solid rgba(16,185,129,0.2);transition:all 0.2s;">
                            <div style="font-size:1.3rem;margin-bottom:4px;">🟢</div>
                            <div style="font-size:0.75rem;font-weight:700;color:white;">Easy</div>
                            <div style="font-size:0.6rem;color:rgba(255,255,255,0.5);margin-top:2px;">~40% accuracy</div>
                        </div>
                        <div onclick="window._startQuizBattleVsAI('medium')" style="flex:1;padding:12px 8px;border-radius:12px;text-align:center;cursor:pointer;background:rgba(245,158,11,0.1);border:2px solid rgba(245,158,11,0.2);transition:all 0.2s;">
                            <div style="font-size:1.3rem;margin-bottom:4px;">🟡</div>
                            <div style="font-size:0.75rem;font-weight:700;color:white;">Medium</div>
                            <div style="font-size:0.6rem;color:rgba(255,255,255,0.5);margin-top:2px;">~60% accuracy</div>
                        </div>
                        <div onclick="window._startQuizBattleVsAI('hard')" style="flex:1;padding:12px 8px;border-radius:12px;text-align:center;cursor:pointer;background:rgba(239,68,68,0.1);border:2px solid rgba(239,68,68,0.2);transition:all 0.2s;">
                            <div style="font-size:1.3rem;margin-bottom:4px;">🔴</div>
                            <div style="font-size:0.75rem;font-weight:700;color:white;">Hard</div>
                            <div style="font-size:0.6rem;color:rgba(255,255,255,0.5);margin-top:2px;">~80% accuracy</div>
                        </div>
                    </div>

                    <div style="padding: 8px 0 4px; font-size: 0.7rem; color: rgba(255,255,255,0.4); text-transform: uppercase; letter-spacing: 1px;">Or challenge a friend</div>
                    ${friendsHtml}
                </div>
    ...
    // ---- START QUIZ BATTLE VS AI ----

    window._startQuizBattleVsAI = function(difficulty) {
        window._closeQuizBattleInvite();

        // Mark view as quizBattle BEFORE switchAppTab so initLearning won't overwrite it
        learningState.currentView = 'quizBattle';

        // Switch to learning tab
        if (typeof switchAppTab === 'function') switchAppTab('learning');

        const AI = window.GameAI;
        if (!AI) { console.error('GameAI not loaded'); return; }

        const config = AI.QuizBattle.getConfig(difficulty);
        const diffLabels = { easy: '🟢 Easy Bot', medium: '🟡 Medium Bot', hard: '🔴 Hard Bot' };
        const botName = diffLabels[difficulty] || '🤖 Bot';
        const battleId = 'ai-' + Date.now() + '-' + Math.random().toString(36).slice(2);

        // Generate AI answers upfront
        const aiAnswers = AI.QuizBattle.simulateAnswers(QUIZ_BATTLE_QUESTIONS, difficulty);

        // Start the battle in local/AI mode
        startQuizBattleVsAI(battleId, botName, difficulty, aiAnswers);
    };

    function startQuizBattleVsAI(battleId, botName, difficulty, aiAnswers) {
        // Reset state
        quizBattleState = {
            battleId,
            channel: null,
        ...
        // After countdown (3.5s), start simulating AI answers
        setTimeout(() => {
            simulateAIQuizAnswers();
        }, 3500);
    }

    function simulateAIQuizAnswers() {
        if (!quizBattleState || !quizBattleState.isAIBattle) return;

        const answers = quizBattleState.aiAnswers;
        let cumulativeDelay = 0;

        for (let i = 0; i < answers.length; i++) {
            cumulativeDelay += answers[i].delayMs;
            const timer = setTimeout(() => {
                if (!quizBattleState || !quizBattleState.isAIBattle) return;
                if (answers[i].correct) {
                    quizBattleState.opponentScore++;
                    updateQuizBattleScoreboard();
                }
                quizBattleState.aiCurrentQuestion = i + 1;

                // If this is the last question, mark AI as finished
                if (i === answers.length - 1) {
                    quizBattleState.opponentFinished = true;
                    // If player is also done, show results
                    if (quizBattleState.currentIndex >= QUIZ_BATTLE_QUESTIONS) {
                        showQuizBattleAIResults();
                    }
                }
            }, cumulativeDelay);
            quizBattleState.aiTimers.push(timer);
        }
    }

    function showQuizBattleAIResults() {
        // Clean up AI timers
        if (quizBattleState.aiTimers) {
            quizBattleState.aiTimers.forEach(t => clearTimeout(t));
            quizBattleState.aiTimers = [];
        }
        ...
        // Calculate AI total time
        let aiTotalTime = 0;
        if (quizBattleState.aiAnswers) {
            quizBattleState.aiAnswers.forEach(a => { aiTotalTime += a.delayMs; });
        }
        const aiTimeStr = (aiTotalTime / 1000).toFixed(1) + 's';
            ...
            <div style="min-height: 80vh; display: flex; flex-direction: column; align-items: center; justify-content: center; background: linear-gradient(135deg, #1a1a2e 0%, #16213e 50%, #0f3460 100%); padding: 30px; text-align: center;">
                ...
                <div style="display: flex; gap: 16px; margin-bottom: 24px; width: 100%; max-width: 320px;">
                    ...
                    <div style="flex: 1; background: ${!iWon && !isDraw ? 'rgba(16,185,129,0.15)' : 'rgba(255,255,255,0.06)'}; border: 2px solid ${!iWon && !isDraw ? 'rgba(16,185,129,0.3)' : 'rgba(255,255,255,0.1)'}; border-radius: 16px; padding: 16px;">
                        ...
                        <div style="font-size: 0.7rem; color: rgba(255,255,255,0.4);">${aiTimeStr}</div>
                    </div>
                </div>
    ...
    window._sendQuizBattleInvite = async function(friendId, friendName) {
        console.log('[QuizBattle] _sendQuizBattleInvite to:', friendId, friendName);
        const betAmount = window._currentQuizBattleBet || 0;
        console.log('[QuizBattle] Bet amount:', betAmount);
        ...
        try {
            if (window.supabaseClient && window.currentUser) {
                console.log('[QuizBattle] Creating battle in DB...');
                ...
                console.log('[QuizBattle] create_quiz_battle RPC response:', { data, error });
                if (error) throw error;
                battleId = data;
            }
        } catch (e) {
        ...
        try {
            if (window.supabaseClient && window.currentUser) {
                await window.supabaseClient.from('nudges').insert({
                    ...
                    message: betAmount > 0
                        ? '⚡ QUIZ BATTLE! 🪙 ' + betAmount + ' coin prize pool! Tap to accept!'
                        : '⚡ QUIZ BATTLE! Tap to accept and play!',
                    coin_bet: betAmount,
                    nudge_type: 'quiz_battle_invite',
            ...
            if (typeof sendDMNotification === 'function') {
                const quizMsg = betAmount > 0
                    ? '⚡ QUIZ BATTLE! 🪙 ' + betAmount + ' coin prize pool! Tap to accept!'
                    : '⚡ QUIZ BATTLE! Tap to accept and play!';
                sendDMNotification(friendId, quizMsg);
            }
        } catch (e) {
    ...
    window.acceptQuizBattle = async function(battleId, fromName, betAmount) {
        console.log('[QuizBattle] acceptQuizBattle called:', { battleId, fromName, betAmount });
        ...
        try {
            if (window.supabaseClient && window.currentUser) {
                ...
                console.log('[QuizBattle] join_quiz_battle RPC response:', { data, error });
                if (error) throw error;
                if (data && data.error) {
    ...
    function startQuizBattle(battleId, opponentName, coinBet, isChallenger) {
        console.log('[QuizBattle] startQuizBattle:', { battleId, opponentName, coinBet, isChallenger });
        // Reset state
        quizBattleState = {
            battleId,
            channel: null,
        ...
        container.innerHTML = `
            <div style="min-height: 80vh; display: flex; flex-direction: column; align-items: center; justify-content: center; background: linear-gradient(135deg, #1a1a2e 0%, #16213e 50%, #0f3460 100%); padding: 30px;">
                <div style="text-align: center; color: white;">
                    ...
                    ${quizBattleState.coinBet > 0 ? `<div style="font-size: 0.9rem; color: #fbbf24; margin-bottom: 20px;">🪙 ${quizBattleState.coinBet} prize pool</div>` : ''}
    ...
    function renderQuizBattleQuestion() {
        ...
        console.log('[QuizBattle] Rendering question', idx + 1, 'of', QUIZ_BATTLE_QUESTIONS);
        
        const container = document.getElementById('learning-content');
        if (!container) return;

        // Clear any leftover timer from a previous question
        ...
        const timeLimit = QUIZ_BATTLE_TIME_LIMITS[game.type] || 15;
        ...
        const myInterval = setInterval(() => {
            // If this interval was orphaned (overwritten by a newer question's timer), clean up
            if (quizBattleState.timerInterval !== myInterval) {
        ...
        quizBattleState.timerInterval = myInterval;
    ...
    function handleQuizBattleAnswer(isCorrect) {
        console.log('[QuizBattle] handleQuizBattleAnswer:', isCorrect, 'Current index:', quizBattleState.currentIndex);
        if (quizBattleState.answered) return;
    ...
    async function finishQuizBattle() {
        console.log('[QuizBattle] finishQuizBattle called. Final Score:', quizBattleState.score);
        // AI BATTLE - show results directly, no DB
        if (quizBattleState.isAIBattle) {
            if (quizBattleState.opponentFinished) {
                showQuizBattleAIResults();
            } else {
                // Wait for AI to finish
                showQuizBattleAIWaiting();
            }
            return;
        }
        ...
        try {
            if (window.supabaseClient && window.currentUser) {
                ...
                console.log('[QuizBattle] submit_quiz_battle_result RPC response:', { data, error });
    ...
    function showQuizBattleAIWaiting() {
        const container = document.getElementById('learning-content');
        if (!container) return;
        ...
        const waitInterval = setInterval(() => {
            if (!quizBattleState || !quizBattleState.isAIBattle) {
    ...
    window.LearningMascot = {
        ...
        show() {
            ...
            if (mascot) {
                // Lazy-load the mascot 3D model on first show to avoid an always-active
                // WebGL context that competes with the main avatar on iOS Safari.
                // On iOS, skip mascot 3D model to save a WebGL context slot.
                // The mascot container still shows; it just won't have 3D.
                if (!window._pbbIsIOSSafari) {
                    const mv = document.getElementById('mascot-model');
