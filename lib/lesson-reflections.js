(function () {
    ...
    window.BalanceLessonReflections = {
        load, has: id => {
            if (loadedUser !== window.currentUser?.id) return false;
            const actions=window.BalanceLearnActionReview;
            const action=Array.from({length:window.socialJourney?.getLearnWeekCount?.() || 6},(_,i)=>window.BalanceLearnWeeklyActions?.experiment(i+1)).find(a=>a?.lessonId===id && a.week <= (actions?.state?.current_week || 0));
            if(action){const row=actions.record(action.week);return !!row?.reflection_text || actions.complete(row);}
        ...
        async open(context) {
            ...
            try {
                const actions = window.BalanceLearnActionReview;
                if (actions) await actions.load();
                const action = Array.from({length:window.socialJourney?.getLearnWeekCount?.() || 6},(_,i)=>window.BalanceLearnWeeklyActions?.experiment(i+1)).find(a=>a?.lessonId===context.lessonId && a.week <= (actions?.state?.current_week || 0));
                const actionRecord = action ? actions.record(action.week) : null;
                ...
                if (data && (!action || actionRecord?.reflection_text || actions.complete(actionRecord))) { if(loadedUser !== userId) {completed = new Set(); loadedUser = userId;} completed.add(context.lessonId); return; }
                ...
                await new Promise(resolve => {
                    ...
                    #lesson-reflection-popup{color-scheme:light;--lr-focus:#866019}
                    html[data-pbb-theme="dark"] #lesson-reflection-popup,html.pbb-theme-dark #lesson-reflection-popup,body[data-pbb-theme="dark"] #lesson-reflection-popup,body.dark-mode #lesson-reflection-popup{color-scheme:dark;--lr-focus:#e9c875}
                    #lesson-reflection-popup *{box-sizing:border-box;color:var(--lr-ink)!important;-webkit-text-fill-color:var(--lr-ink)!important;font-family:Inter,system-ui,sans-serif!important;text-shadow:none}
                    #lesson-reflection-popup h2,#lesson-reflection-popup label,#lesson-reflection-popup textarea{color:var(--lr-ink)!important;-webkit-text-fill-color:var(--lr-ink)!important}
                    ...
                    #lesson-reflection-popup .lr-kicker{font-size:12px;letter-spacing:.08em;color:var(--lr-muted)!important;-webkit-text-fill-color:var(--lr-muted)!important}
                    #lesson-reflection-popup h2{font:700 28px/1.2 Inter,system-ui,sans-serif;margin:12px 0}
                    #lesson-reflection-popup .lr-lesson{font-size:14px;line-height:1.5;color:var(--lr-muted)!important;-webkit-text-fill-color:var(--lr-muted)!important;overflow-wrap:anywhere;margin:0 0 20px}
                    #lesson-reflection-popup label{display:block;font-size:15px;line-height:1.5;margin-bottom:10px}
                    #lesson-reflection-popup textarea{display:block;width:100%;min-height:120px;max-height:220px;resize:vertical;border:1px solid var(--lr-line);border-radius:12px;padding:12px;font:400 16px/1.5 Inter,system-ui,sans-serif;background:var(--lr-paper)!important}
                    ...
                    #lesson-reflection-popup .lr-help{font-size:14px;line-height:1.5;color:var(--lr-muted)!important;-webkit-text-fill-color:var(--lr-muted)!important;margin:10px 0}
                    #lesson-reflection-popup .lr-error{font-size:14px;color:var(--lr-error)!important;-webkit-text-fill-color:var(--lr-error)!important;line-height:1.4}
                    ...
                    #lesson-reflection-popup :focus-visible{outline:3px solid var(--lr-focus);outline-offset:3px}
                    ...
                    if (action) { overlay.querySelector('label').textContent = action.reflection; overlay.querySelector('.lr-help').textContent = 'Save your reflection here. Report what happened in your weekly check-in. Your coach confirms the course action separately.'; }
                    ...
                    form.onsubmit = async event => {
                        ...
                        try {
                            ...
                            if (action) await actions.savePlan(action.week,reflection);
