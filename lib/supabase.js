...
const db = {
  ...
  workouts: {
    ...
    async updateHistorySet(setId, workoutData) {
      const updateData = {
        exercise_name: workoutData.exercise,
        set_number: workoutData.set,
        time_duration: workoutData.time,
      ...
      if (workoutData.isDropSet) {
        updateData.is_drop_set = true;
        updateData.drop_set_weights = workoutData.dropSetWeights || null;
        updateData.drop_set_reps = workoutData.dropSetReps || null;
      } else {
        updateData.is_drop_set = false;
        updateData.drop_set_weights = null;
        updateData.drop_set_reps = null;
      }
      ...
      // If error is about missing drop set columns, retry without them
      if (error && error.message && error.message.includes('drop_set')) {
        delete updateData.is_drop_set;
        delete updateData.drop_set_weights;
        delete updateData.drop_set_reps;
    ...
    async deleteHistorySets(setIds) {
      if (!setIds || setIds.length === 0) return true;
