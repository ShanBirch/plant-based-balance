-- Make the transformation cohort a six-week challenge.
-- Regular friend challenges and plant_based_30 cohorts keep their existing copy/duration.
...
SET
    ...
    duration_days = 42,
    end_date = CASE
        WHEN status = 'active' AND start_date IS NOT NULL THEN start_date + 42
        WHEN status = 'pending' THEN CURRENT_DATE + 42
        ELSE end_date
    END,
    updated_at = NOW()
WHERE is_system_cohort = TRUE
  AND cohort_type = 'transform_30'
...
BEGIN
    v_duration_days := CASE
        WHEN p_cohort_type = 'transform_30' THEN 42
        ELSE 30
    END;

    v_challenge_name := CASE p_cohort_type
        WHEN 'plant_based_30' THEN '30 Day Challenge'
        WHEN 'transform_30' THEN '6-Week Transformation Challenge'
    ...
    IF v_existing_challenge_id IS NOT NULL THEN
        IF p_cohort_type = 'transform_30' THEN
            UPDATE public.challenges
            SET
                name = v_challenge_name,
                duration_days = v_duration_days,
                end_date = CASE
                    WHEN status = 'active' AND start_date IS NOT NULL THEN start_date + v_duration_days
                    WHEN status = 'pending' THEN CURRENT_DATE + v_duration_days
                    ELSE end_date
                END,
    ...
    ELSIF p_cohort_type = 'transform_30' THEN
        UPDATE public.challenges
        SET
            name = v_challenge_name,
            duration_days = v_duration_days,
...
CREATE OR REPLACE FUNCTION public.accept_cohort_invitation(
    p_user_id UUID,
    p_challenge_id UUID
)
RETURNS JSONB
...
BEGIN
    SELECT
        ...
        COALESCE(c.duration_days, CASE WHEN c.cohort_type = 'transform_30' THEN 42 ELSE 30 END),
        c.cohort_type
      INTO v_status, v_deadline, v_min_participants, v_duration_days, v_cohort_type
    ...
    IF v_cohort_type = 'transform_30' THEN
        v_duration_days := 42;
    END IF;
    ...
    IF v_min_participants IS NOT NULL AND v_accepted_count >= v_min_participants THEN
        ...
        SET status = 'active',
            name = CASE
                WHEN v_cohort_type = 'transform_30' THEN '6-Week Transformation Challenge'
                ELSE name
            END,
            start_date = CURRENT_DATE,
            end_date = CURRENT_DATE + v_duration_days,
...
GRANT EXECUTE ON FUNCTION public.accept_cohort_invitation(UUID, UUID) TO authenticated;
...
BEGIN
    ...
    IF NOT FOUND THEN
        ...
        SELECT
            NULL::UUID,
            CASE
                WHEN p_cohort_type = 'transform_30' THEN '6-Week Transformation Challenge'
                WHEN p_cohort_type = 'plant_based_30' THEN '30 Day Challenge'
                ELSE 'Challenge'
            END::TEXT,
            'pending'::TEXT,
            0::INT,
