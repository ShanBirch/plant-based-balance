...
function getRandomMessage(mealType, phase) {
    const mealMessages = FITGOTCHI_MESSAGES[mealType] || FITGOTCHI_MESSAGES.breakfast;
...
export default async function(req) {
    ...
    // Send both advance (30 min before) and on-time reminders
    const phases = [
        { name: 'advance', rpcFunction: 'get_users_needing_meal_advance_reminders', logSuffix: '_advance' },
        { name: 'ontime', rpcFunction: 'get_users_needing_meal_reminders', logSuffix: '' },
    ];
    let totalSent = 0;
    let totalFailed = 0;
        ...
        for (const mealType of mealTypes) {
            try {
                const usersResponse = await fetch(
                    `${SUPABASE_URL}/rest/v1/rpc/${phase.rpcFunction}`,
                    {
                        method: 'POST',
                        headers: {
                ...
                if (!usersResponse.ok) {
                    ...
                    // If the advance function doesn't exist yet, skip silently
                    if (phase.name === 'advance' && (errText.includes('42883') || errText.includes('does not exist'))) {
                        console.log(`[MealReminders] Advance reminder function not deployed yet, skipping ${mealType}`);
                        continue;
                    }
                    console.error(`Error fetching ${phase.name} ${mealType} users:`, errText);
                    continue;
                }

                const users = await usersResponse.json();
                if (users.length > 0) {
                    console.log(`Found ${users.length} users needing ${phase.name} ${mealType} reminders`);
                }
                    ...
                    try {
                        ...
                        if (isNative) {
                            ...
                            await webpush.sendNotification(pushSubscription, JSON.stringify({
                                ...
                                vibrate: [200, 100, 200],
                                tag: `meal-reminder-${phase.name}-${mealType}`,
                        ...
                        if (didSend) {
                            ...
                            // Log with phase suffix to allow both advance and ontime reminders per meal
                            await fetch(`${SUPABASE_URL}/rest/v1/meal_reminder_log`, {
                                ...
                                body: JSON.stringify({
                                    ...
                                    meal_type: mealType + phase.logSuffix,
                                    reminder_date: reminderDate
                                })
                            });
                            totalSent++;
                            console.log(`Sent ${phase.name} ${mealType} reminder to ${user.user_id.substring(0, 8)}... via ${isNative ? 'FCM' : 'WebPush'}`);
                        } else {
                            totalFailed++;
                        }
                    } catch (error) {
                        console.error(`Failed ${phase.name} ${mealType} reminder for ${user.user_id}:`, error.message);
                ...
                console.error(`Error processing ${phase.name} ${mealType}:`, err.message);
            }
        }
    }
