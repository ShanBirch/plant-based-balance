...
async function context(actor,user=actor,enrollmentId=null) {
  ...
  return {available:true,enrollment,records,enrollments,current_week:actions.effectiveWeek(rows[0]),can_review:actor!==user,curriculum_version:actions.curriculum.version(rows[0]),total_weeks:actions.curriculum.total(actions.curriculum.version(rows[0]))};
}
...
function assertMemberContext(ctx,input) {
  ...
  if(!actions.experiment(week,ctx.curriculum_version) || week>ctx.current_week)throw error('That action week is not available yet.',409);
  ...
  return {week,existing,definition:actions.experiment(week,ctx.curriculum_version)};
}
