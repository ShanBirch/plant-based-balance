...
test('DM video and checkout change at the same Brisbane launch-offer boundary',()=>{
    ...
    assert.match(launchVideo,/balance-learn-dm-149-v18-sharp\.mp4$/);
    assert.match(standardVideo,/balance-learn-dm-450-v18-sharp\.mp4$/);
});
...
test('queued v16 videos upgrade to the community spacing revision', () => { for (const price of [149,450]) { const old = 'https://plantbased-balance.org/assets/balance-learn-dm-' + price + '-v16-checkin.mp4'; assert.equal(resolveAttachment(old,Date.parse('2026-09-18')),launchVideo); assert.equal(resolveAttachment(old,Date.parse(LEARN_INTRO_END)),standardVideo); assert.equal(stripPaidMetaProofMediaUrls('Video: ' + old),'Video:'); } });

test('queued v17 attachments upgrade to sharp 1080p delivery while preserving introduction guards', () => {
 const {maySendDraftVideoAttachment}=require('../netlify/functions/_lib/paid-meta-proof-media.js');
 for(const price of [149,450]) {
  const old=`https://plantbased-balance.org/assets/balance-learn-dm-${price}-v17-community.mp4`;
  assert.equal(resolveAttachment(old,Date.parse('2026-09-19T12:00:00+10:00')),launchVideo);
  assert.equal(resolveAttachment(old,Date.parse(LEARN_INTRO_END)),standardVideo);
  assert.equal(stripPaidMetaProofMediaUrls(`Here is the course video: ${old}`),'Here is the course video:');
  assert.equal(maySendDraftVideoAttachment({videoUrl:old,replyText:'Here is the course video'}),true);
  assert.equal(maySendDraftVideoAttachment({videoUrl:old,replyText:'How are you?'}),false);
 }
});
