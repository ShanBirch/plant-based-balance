...
const ads = [
  {
    ...
    primaryText: "If your routine only works when life is quiet, it is not the right routine yet. Balance helps you build training around the week you actually have, with clear steps and Shannon in your corner. The six-week Founders Pass is AU$89.99 once and does not auto-renew. Message 'BALANCE' and I will show you what is included.",
  ...
  },
  {
    ...
    primaryText: "Work, kids, shifts and low-energy days are not failures. They are part of the plan. Balance Foundations gives you a realistic six-week training rhythm plus one weekly check-in and plan review with Shannon for AU$89.99 once. Message 'BALANCE' to take a look.",
...
async function render(ad, width, height) {
  ...
  const svg = Buffer.from(`<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">
    ...
    <text x="${margin}" y="${story ? 1840 : 1300}" fill="${C.deepGold}" font-family="Arial, Helvetica, sans-serif" font-size="${story ? 31 : 25}" font-weight="900">FOUNDATIONS  ·  AU$89.99 ONCE</text>
