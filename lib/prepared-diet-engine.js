...
(function(root,factory){
  ...
  function selectTemplate(input){
    ...
    const menus=DAYS.map((_,day)=>Object.fromEntries(Object.keys(TIMES).map((slot,index)=>{
      const options=pool(s,slot,day),preferred=input?.preferred_meals?.[day+':'+slot];
      const familiar=options.find(recipe=>recipe.name===preferred||recipe.id===preferred);
      return [slot,(familiar||options[(Math.floor(day/2)+index)%options.length]).id];
    })));
    return {id:'prepared-v3-'+s.key,...s,menus};
  }
  ...
  function buildPlan(targets,input){
    ...
    return {plan_name:'Your '+label(t.style)+' Balance Week',plan_description:nutritionGuidance(t,[...notices]),diet_type:t.style,dietary_requirements:[t.style,...t.restrictions],template_id:t.id,library_version:3,total_meals:35,current_week:1,
      ...
      weeks:[{week_number:1,theme:'Your food, your preferences',theme_description:'Seven days with portions calculated from your selected ingredients.',days}]};
  }
  return {VERSION:3,STYLES,RESTRICTIONS,RECIPES,FOODS,normalizeSelection,selectTemplate,expandTemplate,nutrition,compatible,scaleDay,buildPlan,nutritionGuidance,label};
});
