-- Keep explicitly enabled paid-Meta live-worker threads out of the browser
-- rescue lane when the browser action is still unclaimed and no outbound was
-- delivered. Claimed browser work remains untouched.
...
BEGIN
    ...
    SET owner = 'codex_live_worker',
        ...
        reason = COALESCE(q.reason, '{}'::jsonb) || jsonb_build_object(
            ...
            'routed_at', NOW(),
            'recovered_no_send_action', q.owner <> 'dm_manager'
        ),
    ...
    WHERE q.thread_id = p_thread_id
      ...
      AND (
          ...
          OR (
              q.owner = 'browser_dispatcher'
              AND q.status IN ('ready', 'waiting')
              AND COALESCE(q.reason->>'failed_delivery_rescue', 'false') = 'true'
              AND COALESCE(q.reason->>'codex_live_chat_required', 'false') = 'true'
          )
      )
      AND NOT EXISTS (
          SELECT 1
