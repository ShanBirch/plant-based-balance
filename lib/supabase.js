...
const db = {
  ...
  admin: {
    ...
    async getCalorieTrackingStats() {
      ...
      const { count: recentMealLogs } = await supabase
        .from('meal_logs')
        .select('*', { count: 'exact', head: true })
        .gte('created_at', sevenDaysAgo.toISOString());

      // Unique users logging meals (last 30 days)
      const { data: mealUsers } = await supabase
        .from('meal_logs')
        .select('user_id')
        .gte('created_at', thirtyDaysAgo.toISOString());
      const uniqueMealLoggers = new Set((mealUsers || []).map(m => m.user_id)).size;
      ...
      (mealTypes || []).forEach(m => {
        ...
        mealTypeBreakdown[t] = (mealTypeBreakdown[t] || 0) + 1;
      });

      // Daily nutrition entries (people completing end-of-day logging)
      const { count: dailyNutritionEntries } = await supabase
        .from('daily_nutrition')
        .select('*', { count: 'exact', head: true })
        .gte('created_at', thirtyDaysAgo.toISOString());

      const { data: dailyNutritionUsers } = await supabase
        .from('daily_nutrition')
        .select('user_id')
        .gte('created_at', thirtyDaysAgo.toISOString());
      const uniqueDailyLoggers = new Set((dailyNutritionUsers || []).map(d => d.user_id)).size;

      return {
        totalMealLogs: totalMealLogs || 0,
        recentMealLogs: recentMealLogs || 0,
        uniqueMealLoggers,
        mealTypeBreakdown,
        dailyNutritionEntries: dailyNutritionEntries || 0,
        uniqueDailyLoggers
      };
    },
    ...
    async getWorkoutStats() {
      ...
      // Unique users working out (30d)
      const { data: workoutUsers } = await supabase
        .from('workouts')
        .select('user_id')
        .eq('workout_type', 'history')
        .gte('created_at', thirtyDaysAgo.toISOString());
      const uniqueWorkoutUsers = new Set((workoutUsers || []).map(w => w.user_id)).size;

      // Custom workout programs created
      const { count: customPrograms } = await supabase
        .from('custom_workout_programs')
        .select('*', { count: 'exact', head: true });
      ...
      // Personal bests set (30d)
      const { count: recentPBs } = await supabase
        .from('personal_bests')
        .select('*', { count: 'exact', head: true })
      ...
      return {
        ...
        uniqueWorkoutUsers,
        customPrograms: customPrograms || 0,
        activePrograms: activePrograms || 0,
        recentPBs: recentPBs || 0
      };
    },
    ...
    async getBattleStats() {
      ...
      // Tamagotchi battles
      const { count: totalTamagotchiBattles } = await supabase
        .from('tamagotchi_battles')
        .select('*', { count: 'exact', head: true });

      const { count: recentTamagotchiBattles } = await supabase
        .from('tamagotchi_battles')
        .select('*', { count: 'exact', head: true })
        .gte('created_at', thirtyDaysAgo.toISOString());

      const { count: completedTamagotchiBattles } = await supabase
        .from('tamagotchi_battles')
        .select('*', { count: 'exact', head: true })
        .eq('status', 'completed');

      // Unique tamagotchi battlers (30d)
      const { data: tamBattlers } = await supabase
        .from('tamagotchi_battles')
        .select('challenger_id, opponent_id')
        .gte('created_at', thirtyDaysAgo.toISOString());
      const uniqueTamBattlers = new Set();
      (tamBattlers || []).forEach(b => {
        uniqueTamBattlers.add(b.challenger_id);
        uniqueTamBattlers.add(b.opponent_id);
      });

      // Quiz battles
      const { count: totalQuizBattles } = await supabase
        .from('quiz_battles')
        .select('*', { count: 'exact', head: true });

      const { count: recentQuizBattles } = await supabase
        .from('quiz_battles')
        .select('*', { count: 'exact', head: true })
        .gte('created_at', thirtyDaysAgo.toISOString());

      const { count: completedQuizBattles } = await supabase
        .from('quiz_battles')
        .select('*', { count: 'exact', head: true })
        .eq('status', 'completed');

      // Unique quiz battlers (30d)
      const { data: quizBattlers } = await supabase
        .from('quiz_battles')
        .select('challenger_id, opponent_id')
        .gte('created_at', thirtyDaysAgo.toISOString());
      const uniqueQuizBattlers = new Set();
      (quizBattlers || []).forEach(b => {
        uniqueQuizBattlers.add(b.challenger_id);
        uniqueQuizBattlers.add(b.opponent_id);
      });

      // Workout duels
      const { count: totalWorkoutDuels } = await supabase
        .from('workout_duels')
        .select('*', { count: 'exact', head: true });

      const { count: recentWorkoutDuels } = await supabase
        .from('workout_duels')
        .select('*', { count: 'exact', head: true })
        .gte('created_at', thirtyDaysAgo.toISOString());

      const { count: completedWorkoutDuels } = await supabase
        .from('workout_duels')
        .select('*', { count: 'exact', head: true })
        .eq('status', 'completed');
      ...
      return {
        tamagotchi: {
          total: totalTamagotchiBattles || 0,
          recent: recentTamagotchiBattles || 0,
          completed: completedTamagotchiBattles || 0,
          uniquePlayers: uniqueTamBattlers.size
        },
        quiz: {
          total: totalQuizBattles || 0,
          recent: recentQuizBattles || 0,
          completed: completedQuizBattles || 0,
          uniquePlayers: uniqueQuizBattlers.size
        },
        workoutDuels: {
          total: totalWorkoutDuels || 0,
          recent: recentWorkoutDuels || 0,
          completed: completedWorkoutDuels || 0
        }
      };
    },
    ...
    async getChallengeStats() {
      ...
      // Unique users in challenges
      const { data: challUsers } = await supabase
        .from('challenge_participants')
        .select('user_id')
        .eq('status', 'accepted');
      const uniqueChallengers = new Set((challUsers || []).map(c => c.user_id)).size;

      // Recent challenges with details
      ...
      // Get participant counts per recent challenge
      ...
      for (const ch of (recentChallenges || [])) {
        const { count: pCount } = await supabase
          ...
          .eq('status', 'accepted');
        challengeDetails.push({ ...ch, participantCount: pCount || 0 });
      }
    ...
    async getSocialStats() {
      ...
      (storyTypes || []).forEach(s => {
        ...
        mediaBreakdown[t] = (mediaBreakdown[t] || 0) + 1;
      });

      // Unique story posters (30d)
      const { data: storyPosters } = await supabase
        .from('stories')
        .select('user_id')
        .gte('created_at', thirtyDaysAgo.toISOString());
      const uniquePosters = new Set((storyPosters || []).map(s => s.user_id)).size;
      ...
      // Story views (30d)
      const { count: totalStoryViews } = await supabase
        .from('story_views')
        .select('*', { count: 'exact', head: true })
      ...
      return {
        ...
        recentStories: recentStories || 0,
        mediaBreakdown,
        uniquePosters,
        totalReactions: totalReactions || 0,
        totalComments: totalComments || 0,
        totalStoryViews: totalStoryViews || 0,
    ...
    async getChatStats() {
      ...
      // Group chat messages (30d)
      const { count: groupChatMessages30d } = await supabase
        .from('group_chat_messages')
        .select('*', { count: 'exact', head: true })
        .gte('created_at', thirtyDaysAgo.toISOString());

      // Group chat messages (7d)
      const { count: groupChatMessages7d } = await supabase
        .from('group_chat_messages')
        .select('*', { count: 'exact', head: true })
        .gte('created_at', sevenDaysAgo.toISOString());

      // Unique group chatters (30d)
      const { data: groupChatters } = await supabase
        .from('group_chat_messages')
        .select('user_id')
        .gte('created_at', thirtyDaysAgo.toISOString());
      const uniqueGroupChatters = new Set((groupChatters || []).map(g => g.user_id)).size;

      // Private messages (30d)
      const { count: privateMessages30d } = await supabase
        .from('private_messages')
        .select('*', { count: 'exact', head: true })
        .gte('created_at', thirtyDaysAgo.toISOString());

      // Unique DM users (30d)
      const { data: dmSenders } = await supabase
        .from('private_messages')
        .select('sender_id')
        .gte('created_at', thirtyDaysAgo.toISOString());
      const uniqueDmUsers = new Set((dmSenders || []).map(d => d.sender_id)).size;

      // Community chat (Shannon) messages (30d)
      const { count: communityMessages30d } = await supabase
        .from('conversations')
        .select('*', { count: 'exact', head: true })
        .eq('role', 'user')
        .gte('created_at', thirtyDaysAgo.toISOString());

      // Win shares in group chat (30d)
      const { count: winShares } = await supabase
        .from('group_chat_messages')
        .select('*', { count: 'exact', head: true })
        .eq('is_win_share', true)
        .gte('created_at', thirtyDaysAgo.toISOString());

      return {
        groupChat: {
          messages30d: groupChatMessages30d || 0,
          messages7d: groupChatMessages7d || 0,
          uniqueChatters: uniqueGroupChatters,
          winShares: winShares || 0
        },
        privateMessages: {
          messages30d: privateMessages30d || 0,
          uniqueUsers: uniqueDmUsers
        },
        communityChat: {
          messages30d: communityMessages30d || 0
        }
      };
    },
    ...
    async getLearningStats() {
      ...
      // Active learners (lesson in last 30d)
      const { data: activeLearners } = await supabase
        .from('user_learning_progress')
        .select('user_id, total_lessons_completed, current_learning_streak, longest_learning_streak')
      ...
      return {
        ...
        recentLessons: recentLessons || 0,
        totalMilestones: totalMilestones || 0,
        learnerDetails: activeLearners || []
      };
    },
    ...
    async getCheckinStats() {
      ...
      // Unique check-in users (30d)
      const { data: checkinUsers } = await supabase
        .from('daily_checkins')
        .select('user_id')
        .gte('created_at', thirtyDaysAgo.toISOString());
      const uniqueCheckinUsers = new Set((checkinUsers || []).map(c => c.user_id)).size;

      // Weigh-ins (30d)
      const { count: recentWeighIns } = await supabase
        .from('daily_weigh_ins')
        .select('*', { count: 'exact', head: true })
        .gte('created_at', thirtyDaysAgo.toISOString());

      // Reflections (30d)
      const { count: recentReflections } = await supabase
        .from('reflections')
        .select('*', { count: 'exact', head: true })
        .gte('created_at', thirtyDaysAgo.toISOString());
      ...
      return {
        ...
        uniqueCheckinUsers,
        recentWeighIns: recentWeighIns || 0,
    ...
    async getNavigationStats() {
      ...
      // Get all user activity to analyze tab/page navigation
    ...
    async getGrowthStats() {
      // User signups by week (last 12 weeks)
      ...
      (subUsers || []).forEach(u => {
        ...
        subscriptionBreakdown[s] = (subscriptionBreakdown[s] || 0) + 1;
      });
