    ...
    window.BalanceLessonReflections = {
        async open(context) {
            ...
            try {
                ...
                if (error || data || window.currentUser?.id !== userId || (content && !resultElement?.isConnected)) return;
                await new Promise(resolve => {
                    ...
                    <p class="lr-help">Optional. Saved with this lesson for you and your coach. Not posted to the Feed.</p>
                    <p class="lr-error" role="status" aria-live="polite"></p>
                    <div class="lr-actions"><button type="button" data-skip>Not now</button><button type="submit">Save reflection</button></div></form>`;
                    ...
                    skip.onclick = () => { if (!saving) close(); };
                    overlay.onkeydown = event => {
                        if (event.key === 'Escape' && !saving) { event.preventDefault(); close(); }
                        if (event.key === 'Tab') {
                            const nodes = [answer, skip, save].filter(el => !el.disabled);
                    ...
                    form.onsubmit = async event => {
                        ...
                        if (!reflection) { status.textContent = 'Add a thought, or choose Not now.'; answer.focus(); return; }
