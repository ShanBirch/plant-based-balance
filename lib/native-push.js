...
function getFirebaseMessaging() {
  if (!_fcmPlugin && window.Capacitor && window.Capacitor.registerPlugin) {
    try {
      _fcmPlugin = window.Capacitor.registerPlugin('FirebaseMessaging');
      console.log('[NativePush] FirebaseMessaging plugin loaded:', !!_fcmPlugin);
    } catch (e) {
      console.log('[NativePush] FirebaseMessaging registerPlugin failed:', e.message);
    }
  }
  return _fcmPlugin;
}
      ...
      if (FirebaseMessaging) {
        try {
          ...
          FirebaseMessaging.addListener('notificationReceived', (event) => {
            // Normalize to the shape handleForegroundNotification expects.
          ...
          console.error('[NativePush] iOS FirebaseMessaging init failed:', iosErr);
          // fall through to legacy path as a last resort
        }
      } else {
        console.warn('[NativePush] FirebaseMessaging plugin missing on iOS — push will not deliver until @capacitor-firebase/messaging is installed and the app is rebuilt');
      }
    }
...
function _eagerIOSPushInit() {
  ...
  setTimeout(function () {
    ...
    if (!FirebaseMessaging) {
      pushDebugBanner('FirebaseMessaging plugin NOT found — rebuild needed', '#cc0000');
