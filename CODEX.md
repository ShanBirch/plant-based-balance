...
- The single scheduled Instagram Chrome operator is the 24-hour browser shift dispatcher. It runs every 30 minutes, starts at the due lane in the durable eight-slot rotation, and rolls forward through later lanes when one finishes early instead of ending the task. It actively works for 22 minutes, reserves eight minutes for reconciliation and clean Chrome release, claims one person at a time, and visits at most one full rotation per scheduled task. Once the task-level action ceiling is reached, it keeps doing read-only inspection, reconciliation, queue preparation, and cursor repair until minute 22. Its replaced dedicated Instagram Chrome jobs remain paused so browser ownership never overlaps.
- `ig_browser_shift_runs` is the dispatcher-level source of truth. Each lane visited in a scheduled task is its own ledger segment: acquire with `start_ig_browser_shift` before acting, resume from the returned same-lane cursor, checkpoint with `heartbeat_ig_browser_shift` after every inspected person or attempted native action, finalize a completed lane before immediately starting the next segment, and finalize the current segment as `partial` with exact `next_resume` at the timebox. `blocked` requires native restriction evidence. A first segment that receives `acquired=false` does no Instagram work.
