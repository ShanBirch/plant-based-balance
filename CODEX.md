...
- Codex task notifications remain the primary review and approval surface for DM-manager and Instagram-dispatcher decisions. The existing Balance admin Your Call surface is the durable secondary notice: eligible pending Instagram/Facebook manager and browser-dispatcher cards show the exact Codex task name to open, while the grouped dispatcher batch card reads the one live awaiting batch and shows its exact Codex reply command. This fallback is informational only, creates no second alert row, sends no client message or Instagram action, grants no approval, and disappears with the source alert or batch lifecycle so it cannot become a duplicate or stale prompt. The grouped dispatcher card must never expose an approve/send control; the retired Balance approval endpoint stays fail-closed. Do not add email fallback unless a verified transactional sender is already configured.
