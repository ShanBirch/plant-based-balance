...
(function (root, factory) {
    ...
    function renderFoundationsOffer(offer) {
        ...
        const copy = weekly ? 'AUD $24.83 every week with a six-week minimum ($148.98 minimum total). After six weeks, cancel any time before your next weekly payment. Access continues through your paid period, with no additional notice period. Same weekly check-in, workout and food review, app and community access.' : 'One ' + formatted + ' payment for the full eight weeks. No subscription or automatic renewal.';
        if (priceCopy) priceCopy.textContent = weekly ? 'AUD $24.83 every week. Six-week minimum ($148.98), then cancel any time.' : copy;
        if (termsCopy) termsCopy.textContent = weekly ? (offer.disclosure || copy) : 'One ' + formatted + ' payment covers the full eight weeks and does not renew automatically.';
    ...
    function readPersonalisedSetup() {
        ...
        const inferredGoal = Array.isArray(goalItems) && goalItems.length
            ...
            : String(profile.primary_goal || profile.goal || profile.goal_type || 'a repeatable first eight weeks');
        ...
        return {
            goal: statedGoal || humanise(inferredGoal) || 'A repeatable first eight weeks',
    ...
    function showExitChoice(stage) {
        ...
        if (body) body.textContent = normalizedStage === 'tour'
            ? 'Restart the guided app tour, or unlock the complete eight-week program now.'
            : 'Continue your setup so Balance can finish your starter week, or unlock the complete eight-week program now.';
    ...
    async function beginCheckout() {
        ...
        try {
            ...
            if (button) { button.disabled = false; button.textContent = 'START MY EIGHT WEEKS'; }
            return false;
        }
    }
