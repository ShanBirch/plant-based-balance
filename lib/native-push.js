...
async function initNativePush() {
  ...
  try {
    var _toast = function(msg) { if (typeof window.showToast === 'function') window.showToast(msg, 'info'); };
    ...
    _toast('🔌 Step 1: Getting plugin...');
    var capInfo = 'Cap:' + !!window.Capacitor +
      ' regPlugin:' + !!(window.Capacitor && window.Capacitor.registerPlugin) +
      ' Plugins:' + !!(window.Capacitor && window.Capacitor.Plugins) +
      ' PushOnPlugins:' + !!(window.Capacitor && window.Capacitor.Plugins && window.Capacitor.Plugins.PushNotifications);
    console.log('[NativePush] Bridge state:', capInfo);
    if (window.Capacitor && window.Capacitor.Plugins) {
      console.log('[NativePush] Available plugins:', Object.keys(window.Capacitor.Plugins).join(', '));
    }
    ...
    if (!PushNotifications) {
      ...
      _toast('❌ Plugin N/A — ' + capInfo);
      return false;
    }
    _toast('🔌 Step 2: Plugin OK, checking perms...');

    // Check current permission status
    const permStatus = await PushNotifications.checkPermissions();
    console.log('[NativePush] Current permission:', permStatus.receive);
    _toast('🔑 Step 3: Permission = ' + permStatus.receive);
    ...
    if (permStatus.receive === 'prompt') {
      ...
      if (result.receive !== 'granted') {
        ...
        _toast('❌ Permission denied by user');
        return false;
      }
    } else if (permStatus.receive !== 'granted') {
      console.log('[NativePush] Permission not granted:', permStatus.receive);
      _toast('❌ Permission not granted: ' + permStatus.receive);
      return false;
    }

    _toast('📡 Step 4: Registering for FCM token...');
    ...
    const tokenPromise = new Promise((resolve) => {
      ...
      PushNotifications.addListener('registration', (token) => {
        ...
        _toast('🎫 Step 5: FCM token received!');
      ...
      PushNotifications.addListener('registrationError', (error) => {
        ...
        _toast('❌ Registration ERROR: ' + (error.error || JSON.stringify(error)).substring(0, 60));
      ...
      setTimeout(() => {
        if (!resolved) {
          ...
          _toast('⏰ TIMEOUT: No FCM token after 15s');
          resolve(null);
        }
      }, 15000);
    });
    ...
    if (!token) {
      ...
      // Show visible confirmation that FCM token was received (temporary debug aid)
      console.log('[NativePush] FCM token obtained, waiting for DB save...');
      // Wait a moment for saveNativePushToken to complete
      setTimeout(function() {
        var status = window._pushTokenStatus || 'pending';
        console.log('[NativePush] Token save status after delay:', status);
        if (typeof window.showToast === 'function') {
          if (status === 'verified' || status === 'saved') {
            window.showToast('✅ Step 6: Token SAVED to DB!', 'success');
          } else {
            window.showToast('⚠️ Step 6: Save status = ' + status, 'warning');
          }
        }
      }, 8000); // Wait 8s for the save + verify to complete
    }
    ...
    if (typeof window.showToast === 'function') {
      window.showToast('❌ NativePush init failed: ' + err.message, 'error');
    }
    return false;
  }
}
...
async function handleForegroundNotification(notification) {
  ...
  // Also show the in-app banner for DMs (handled by the realtime handler,
  // but if the realtime channel is slow this gives immediate feedback)
  if (data.type === 'dm_message' && typeof window.showDMNotificationBanner === 'function') {
