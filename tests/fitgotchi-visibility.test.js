...
assert.match(dashboard, /dashboard-script-5-initialize_stripe_for_inapp_pu\.js\?v=226-profile-photo-persistence/, 'phones should fetch the current Dashboard behavior');
assert.match(dashboard, /dashboard-script-10-points_widget_functions\.js\?v=49/, 'phones should fetch the compact level updates');
assert.match(serviceWorker, /const CACHE_NAME = 'pbb-app-v448-completion-aware-course'/, 'the app shell cache should advance for this feature');
