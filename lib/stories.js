...
window.postCapturedStory = async function() {
    ...
    try {
        ...
        // Always store story media as a durable uploaded URL. Inline base64 payloads can
        // fail mobile/PostgREST writes even after compression.
        const fileSizeMB = selectedStoryFile.size / (1024 * 1024);
        let mediaUrl;

        if (postBtn) {
            postBtn.textContent = `Uploading ${Math.max(1, Math.round(fileSizeMB))}MB...`;
        }
...
async function uploadFeedComposerMedia(file, mediaType, postBtn) {
    ...
    if (postBtn) postBtn.textContent = `Uploading ${Math.max(1, Math.round(fileSizeMB))}MB`;
    ...
    if (mediaType === 'image') {
        const dataUrl = await readFeedComposerFileAsDataUrl(fileToUpload);
        analysisBase64 = getBase64Payload(dataUrl);
    }
    ...
    try {
        ...
        grid.innerHTML = sortedStories.map(story => {
            ...
            const captionHtml = mealCardCaptionText
                ...
                : (story.caption && !isCardType && !isTextPost)
...
window.openFeedPostViewer = async function(storyId) {
    ...
    try {
        ...
        const deleteBtn = isOwn
            ? `<button onclick="deleteFeedPost('${sid}')" style="background:rgba(239,68,68,0.14); border:1px solid rgba(239,68,68,0.28); color:#ff7a7a; -webkit-text-fill-color:#ff7a7a; font-size:0.8rem; font-weight:700; cursor:pointer; padding:8px 14px; border-radius:999px; line-height:1; white-space:nowrap; flex-shrink:0;">Delete</button>`
            : '';
        ...
        viewer.innerHTML = `
            ...
            <div style="position:sticky; top:0; z-index:2; display:flex; align-items:center; padding:calc(15px + env(safe-area-inset-top, 0px)) 16px 14px 16px; gap:12px; flex-shrink:0; background:linear-gradient(180deg, rgba(0,0,0,0.98) 0%, rgba(0,0,0,0.84) 100%); backdrop-filter:blur(14px); -webkit-backdrop-filter:blur(14px); border-bottom:1px solid rgba(255,255,255,0.06);">
                ...
                ${deleteBtn}
                <button onclick="closeFeedPostViewer()" style="background:rgba(255,255,255,0.12); border:1px solid rgba(255,255,255,0.18); color:white; -webkit-text-fill-color:white; font-size:0.8rem; font-weight:700; cursor:pointer; padding:8px 14px; border-radius:999px; line-height:1; white-space:nowrap; flex-shrink:0;">Cancel</button>
            </div>
