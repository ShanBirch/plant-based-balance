...
(function() {
    ...
    const UNITS = {
        ...
        'body-2': { id: 'body-2', moduleId: 'body', title: 'Hormones', description: 'Your chemical signaling system', order: 2 },
    ...
    const LESSONS = {
        ...
        'body-1': [
            {
                ...
                content: {
                    intro: `Muscles do one thing: they contract. They pull, never push.\n\nWhen you "push" a door, your muscles are actually pulling your bones to create that pushing motion. Every movement you make—walking, lifting, even breathing—happens because muscles shorten and pull on bones.\n\nMuscles attach to bones through tough, fibrous cords called tendons. When a muscle contracts, it pulls on the tendon, which pulls on the bone, creating movement. (Ligaments are different—they connect bones to other bones at joints.)\n\nThis simple fact explains why muscles come in opposing pairs.`,
                    keyPoint: "Muscles can only pull by contracting. They attach to bones via tendons, which transmit the pulling force.",
                },
                games: [
        ...
        'body-2': [
            {
                id: 'body-2-1', unitId: 'body-2', order: 1, title: "The Chemical Messenger System",
                content: {
            ...
            {
                id: 'body-2-2', unitId: 'body-2', order: 2, title: "Energy Mobilizers",
                content: {
            ...
            {
                id: 'body-2-3', unitId: 'body-2', order: 3, title: "The Anabolic Messengers",
                content: {
            ...
            {
                id: 'body-2-4', unitId: 'body-2', order: 4, title: "Metabolic Regulators",
                content: {
            ...
            {
                id: 'body-2-5', unitId: 'body-2', order: 5, title: "Sex Hormones in Context",
                content: {
...
There's no perfect program—only the one you'll actually do consistently with effort. Start where you are, apply the principles, and trust the process.`,
        ...
        container.innerHTML = `
            <div style="min-height: 100vh; background: linear-gradient(180deg, ${module.color}15 0%, white 30%); --module-color: ${module.color};">
                ...
                <div style="margin: 0 15px; background: white; border-radius: 24px; padding: 40px 25px; box-shadow: 0 8px 30px rgba(0,0,0,0.12); min-height: 320px; display: flex; flex-direction: column; align-items: center; justify-content: center;">
                    ...
                    <!-- Big Play Button -->
                    <button class="rsvp-play-btn" onclick="window.toggleRsvp()" style="width: 80px; height: 80px; border-radius: 50%; border: none; background: ${module.color}; cursor: pointer; display: flex; align-items: center; justify-content: center; box-shadow: 0 6px 20px ${module.color}50; transition: transform 0.2s ease, box-shadow 0.2s ease;" onmouseover="this.style.transform='scale(1.05)'" onmouseout="this.style.transform='scale(1)'">
    ...
    async function completeLesson() {
            ...
            if (window.supabase && window.currentUser) {
                ...
                if (error) {
                    ...
                    if (dbIsNewLesson && data.score_percentage === 100) {
                        ...
                        // Check if unit is now complete and award bonus XP
                        const unitLessons = getLessonsForUnit(lesson.unitId);
                        const completedInUnit = unitLessons.filter(l => progress.lessons_completed.includes(l.id)).length;
                        if (completedInUnit === unitLessons.length && !progress.units_completed.includes(lesson.unitId)) {
                            try {
                                const { data: unitData, error: unitError } = await window.supabase.rpc('complete_unit', {
                                    p_user_id: window.currentUser.id,
                                    p_unit_id: lesson.unitId,
                                    p_module_id: unit.moduleId
                                });
                                if (!unitError && unitData && unitData.xp_earned > 0) {
                                    dbXpEarned += unitData.xp_earned;
                                    progress.units_completed.push(lesson.unitId);
                                    saveLocalProgress();

                                    // Check if module is now complete and award bonus XP
                                    const moduleUnits = getUnitsForModule(unit.moduleId);
                                    const completedUnits = moduleUnits.filter(u => progress.units_completed.includes(u.id)).length;
                                    if (completedUnits === moduleUnits.length && !progress.modules_completed.includes(unit.moduleId)) {
                                        const { data: moduleData, error: moduleError } = await window.supabase.rpc('complete_module', {
                                            p_user_id: window.currentUser.id,
                                            p_module_id: unit.moduleId
                                        });
                                        if (!moduleError && moduleData && moduleData.xp_earned > 0) {
                    ...
                    // If DB says we earned XP, trigger animations
                    if (dbXpEarned > 0) {
        ...
        const finalXpEarned = rpcSucceeded ? dbXpEarned : xpEarned;
        const result = { xp_earned: finalXpEarned, is_new_lesson: dbIsNewLesson, needs_perfect_score: dbNeedsPerfectScore };
