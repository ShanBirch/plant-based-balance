...
/**
 * Render lesson intro/content screen with interactive reading modes
 */
function renderLessonIntro(lesson) {
  ...
  if (mode === 'rsvp') {
    // RSVP Mode - Flash words one at a time
    contentHtml = `
      <div class="rsvp-container" style="--module-color: ${module.color};">
        ...
        <div class="rsvp-controls">
          <button class="rsvp-btn" onclick="resetRsvp()" title="Reset">
          ...
          <button class="rsvp-btn rsvp-play-btn" onclick="toggleRsvp()" title="Play/Pause">
            <svg viewBox="0 0 24 24" style="width: 24px; height: 24px; fill: ${module.color};"><path d="M8 5v14l11-7z"/></svg>
          </button>
        </div>
        <div class="rsvp-speed">
          <span>Speed:</span>
          <input type="range" min="100" max="500" value="${learningState.rsvpState.speed}" onchange="setRsvpSpeed(this.value)" style="--module-color: ${module.color};">
          <span>${Math.round(60000 / learningState.rsvpState.speed)} wpm</span>
        </div>
      </div>
    `;
  } else if (mode === 'speed') {
    // Bionic Reading Mode - Bold first part of words
    ...
    // Focus Mode - Progressive reveal with animations
    ...
    // Normal Mode - Standard text with interactive terms
  ...
  if (lesson.games?.length > 0) {
    // Pick a relevant question from the games
      ...
      <div class="think-prompt lesson-section" style="--module-color: ${module.color};">
        ...
        <div class="think-prompt-content">
          ...
          <div class="think-prompt-reveal">
            ...
            <button class="think-reveal-btn" onclick="revealThinkAnswer(this)">Reveal Answer</button>
          </div>
        </div>
      </div>
    `;
    ...
    <div style="min-height: 100vh; background: linear-gradient(180deg, ${module.color}15 0%, white 30%); --module-color: ${module.color};">
      ...
      <div style="margin: 10px 15px; background: white; border-radius: 20px; padding: 25px; box-shadow: 0 4px 20px rgba(0,0,0,0.08);">
        ...
        <div class="reading-mode-toggle lesson-section" style="--module-color: ${module.color};">
          <button class="reading-mode-btn ${mode === 'normal' ? 'active' : ''}" onclick="setReadingMode('normal')" style="${mode === 'normal' ? `color: ${module.color};` : ''}">
            Normal
          </button>
          <button class="reading-mode-btn ${mode === 'focus' ? 'active' : ''}" onclick="setReadingMode('focus')" style="${mode === 'focus' ? `color: ${module.color};` : ''}">
            Focus
          </button>
          <button class="reading-mode-btn ${mode === 'speed' ? 'active' : ''}" onclick="setReadingMode('speed')" style="${mode === 'speed' ? `color: ${module.color};` : ''}">
            Speed
          </button>
          <button class="reading-mode-btn ${mode === 'rsvp' ? 'active' : ''}" onclick="setReadingMode('rsvp')" style="${mode === 'rsvp' ? `color: ${module.color};` : ''}">
            Flash
          </button>
        </div>
      ...
      <div class="lesson-section" style="padding: 20px 15px;">
        <button onclick="startLessonGames()" style="width: 100%; background: ${module.color}; color: white; border: none; padding: 16px; border-radius: 14px; font-size: 1.1rem; font-weight: 700; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 10px; box-shadow: 0 4px 12px ${module.color}40; transition: transform 0.2s ease, box-shadow 0.2s ease;" onmouseover="this.style.transform='scale(1.02)'" onmouseout="this.style.transform='scale(1)'">
      ...
      <!-- Tap hint for interactive terms (only show if in normal or focus mode) -->
      ${mode !== 'rsvp' ? `
