...
test('one-time Founders Pass has a complete purchase and activation path', () => {
    ...
    assert.match(page, /Prelude plus six guided weeks/);
    assert.match(page, /31 lessons with practical actions/);
    assert.equal((page.match(/class="feature-number">WEEK [1-6]/g) || []).length, 6);
    assert.match(page, /Why change feels hard/);
    assert.match(page, /Work with your energy/);
    ...
    assert.match(guard, /balance_vegan_founders_pass[\s\S]*?unitAmount: 14900[\s\S]*?mode: "payment"/);
    assert.match(guard, /balance_meta_foundations_pass[\s\S]*?unitAmount: 14900[\s\S]*?balanceProduct: "balance_vegan_founders_pass"[\s\S]*?mode: "payment"/);
