...
function buildPaidMetaZoomHandoff({ currentMessage = '', history = [], flowVariant } = {}) {
    ...
    const frequency = /\b(?:three|3)\s*(?:times|sessions?)?\s*(?:a|per|each)?\s*week|\bzoom pt 3\b/i.test(text) ? 3
        : /\b(?:five|5)\s*(?:times|sessions?)?\s*(?:a|per|each)?\s*week|\bzoom pt 5\b/i.test(text) ? 5
        : /\b(?:one|1|once)\s*(?:times|sessions?)?\s*(?:a|per|each)?\s*week|\bzoom pt 1\b/i.test(text) ? 1 : null;
