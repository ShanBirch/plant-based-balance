...
(function(){
  ...
  function getCurrentISOWeek(){ return getISOWeek(new Date()); }
  ...
  // ============================================================
  //  Promo Reel — marketing slideshow for IG / screen-record use.
  //
  //  Hardcoded feature highlight slides (no user data needed).
  //  Reuses the same slide engine, animations, and synth as the
  //  per-user Wrapped, so everything stays visually consistent.
  //  Music defaults to Synthwave (most hype) but can be overridden
  //  via opts.theme = 'phonk' | 'trap' | etc.
  // ============================================================
  ...
  async function openPromo(opts){
    opts = opts || {};
    ensureStyles();

    // Fresh overlay with a loading screen so the user sees something while
    // the GLB pre-loads (first open ≈ 3–8s; cached opens ≈ instant).
    let overlay = document.getElementById('weekly-wrapped-overlay');
    if (overlay) overlay.remove();
    overlay = document.createElement('div');
    overlay.id = 'weekly-wrapped-overlay';
    ...
    overlay.innerHTML = `
      <div class="ww-slide active ww-bg-intro" style="display:flex;">
        <div class="ww-speed-lines"></div>
        <div class="ww-halftone"></div>
        <div class="ww-kicker" style="animation:none;">Loading promo reel…</div>
        <div style="width:44px; height:44px; border:3px solid rgba(255,255,255,0.3); border-top-color:#fff; border-radius:50%; animation: wwSpeedSpin 0.8s linear infinite; margin-top:18px;"></div>
        <div class="ww-caption" style="margin-top:18px; font-size:0.85rem; opacity:0.75;">Pre-loading FitGotchi model…</div>
        <div id="ww-preload-progress" style="margin-top:14px; width:min(240px, 70vw); height:6px; background:rgba(255,255,255,0.18); border-radius:4px; overflow:hidden;">
          <div id="ww-preload-progress-fill" style="height:100%; width:0%; background:#fff; transition:width 0.2s ease;"></div>
        </div>
        <div id="ww-preload-progress-label" style="margin-top:8px; font-size:0.7rem; font-weight:700; letter-spacing:2px; color:rgba(255,255,255,0.6);">0%</div>
      </div>
      <button class="ww-close" aria-label="Close">&times;</button>
    `;
    document.body.appendChild(overlay);
    ...
    // Ensure model-viewer + meshopt decoder are ready, THEN pre-load the
    // FitGotchi GLB fully and wait for the visibility event before starting
    // the slideshow. Per user: time doesn't matter — they'd rather wait on
    // the load screen than see a missing model on slide 4.
    try {
    ...
    } catch(e) { console.warn('[promo] preload failed, continuing anyway', e); }

    // Bail if the user closed the overlay during preload
    if (!document.getElementById('weekly-wrapped-overlay')) return;

    // Pick a theme — Synthwave by default; pass opts.theme='phonk' etc. to override
    let theme = THEMES[0];
    if (opts.theme) {
      const match = THEMES.find(t => t.id === opts.theme);
    ...
    } catch(e) { console.warn('[ww] promo synth init failed', e); }

    const slides = buildPromoSlides();
    renderSlides(overlay, slides, () => { /* no-op on finish for promo */ });
  }
