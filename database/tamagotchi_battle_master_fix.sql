-- ============================================================
-- Tamagotchi Battle Master Fix
-- Run this once in Supabase SQL Editor to ensure the PvP
-- battle system is fully set up.  All statements are safe
-- to re-run (IF NOT EXISTS / OR REPLACE).
-- ============================================================

-- 1. Ensure tamagotchi_battles table exists
...
DROP POLICY IF EXISTS "Users can view own tamagotchi battles"        ON public.tamagotchi_battles;
DROP POLICY IF EXISTS "Authenticated users can insert tamagotchi battles" ON public.tamagotchi_battles;
DROP POLICY IF EXISTS "Participants can update tamagotchi battles"   ON public.tamagotchi_battles;

CREATE POLICY "Users can view own tamagotchi battles"
    ON public.tamagotchi_battles FOR SELECT
    USING (auth.uid() = challenger_id OR auth.uid() = opponent_id);

CREATE POLICY "Authenticated users can insert tamagotchi battles"
    ON public.tamagotchi_battles FOR INSERT
    WITH CHECK (auth.uid() = challenger_id);

CREATE POLICY "Participants can update tamagotchi battles"
    ON public.tamagotchi_battles FOR UPDATE
    USING (auth.uid() = challenger_id OR auth.uid() = opponent_id);

-- 3. Ensure nudges has the columns needed for battle invites
...
-- 4. create_tamagotchi_battle RPC
...
-- 6. finish_tamagotchi_battle RPC
...
BEGIN
    ...
    IF p_user_id = battle.challenger_id THEN
        UPDATE public.tamagotchi_battles SET challenger_hp_remaining = p_hp_remaining WHERE id = p_battle_id;
        battle.challenger_hp_remaining := p_hp_remaining;
    ELSIF p_user_id = battle.opponent_id THEN
        UPDATE public.tamagotchi_battles SET opponent_hp_remaining = p_hp_remaining WHERE id = p_battle_id;
    ...
    IF both_done THEN
        ...
        ELSE
            winner := battle.challenger_id;
        END IF;
        UPDATE public.tamagotchi_battles SET status = 'completed', winner_id = winner WHERE id = p_battle_id;
    ...
    END IF;
    RETURN jsonb_build_object('finished', false, 'waiting_for_opponent', true);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
