...
test('Foundations community progression contains only the intended in-app Feed action for each week', () => {
  const foundations = journey.slice(journey.indexOf('const ORIGINAL_WEEK_DEFINITIONS'), journey.indexOf("week: 7,"));
  const expected = [
...
test('each Feed action requires specific durable evidence and cannot be satisfied by an unrelated event', () => {
  ...
  assert.ok(journey.includes("foundations_feed_intro: linkedTextPostCount(textActionId('foundations_feed_intro'))"));
  ...
  assert.ok(journey.includes("foundations_feed_reflection: linkedTextPostCount(textActionId('foundations_feed_reflection'))"));
});
...
test('introduction and reflection links persist on the Feed row and are consumed only after a real text post', () => {
  ...
  assert.ok(stories.includes("/^w[1-8]_feed_(intro|reflection)$/.test(foundationsCourseActionId) || !caption"));
