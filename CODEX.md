...
- Follower notification work is two-stage within that dispatcher lane: detection completes the saved notification cursor and queues `follower_operator` / `welcome_follower` actions without sending; response later claims those actions, rechecks the relationship, and welcomes an eligible person once. Discovery following remains separate and never triggers an automatic DM.
