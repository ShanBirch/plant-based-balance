...
(async function() {
  ...
  const isGuestMode = _guestParams.get('guest') === 'true' || sessionStorage.getItem('guestMode') === 'true';
  ...
  if (isGuestMode) {
    ...
    const guestDefaults = {
      ...
      // Prevent onboarding wizard from triggering for female guest
      userCycleData: JSON.stringify({ noPeriodMode: true }),
    };
  ...
  // getUser() makes a network request to validate the JWT; wrap in try/catch
  // so a network error or expired token doesn't leave the page with no currentUser.
  let user;
  try {
    user = await authHelpers.getUser();
  } catch (e) {
  ...
  function syncNativeBalanceSession(activeSession, activeUser) {
    try {
      ...
      if (typeof window.NativePermissions.setBalanceNativeAdminEmail === 'function') {
        // Admin is direct-URL only. Passing a blank value also disables the
        // old dynamic admin shortcut in already-installed Android builds.
        window.NativePermissions.setBalanceNativeAdminEmail('');
      }
  ...
  if (!window.isAdminViewing) {
    ...
    try {
      dbHelpers.activity.log(user.id, 'page_view', {
