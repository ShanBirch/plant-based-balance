 ...
 * Monthly Wrapped
 *
 * Spotify Wrapped-style calendar-month recap. Gathers a completed month plus 4-month trends
 ...
 *
 * Triggered by:
 *   - First-of-month 09:00 AEST push (weekly-wrapped-push.js scheduled fn)
 *   - Home card on the first three days of the month if unseen
 *   - Manual replay from settings
 *
 * Seen state: localStorage `pbb_wrapped_seen_{YYYY-MM}` (client-only dedup)
...
(function(){
  ...
  // the month as seen regardless of which close path fires (X button,
  // swipe-back gesture, Android back button, slideshow completion).
  let activeUserId = null;
  let activePeriodKey = null;

  // ============================================================
  //  Date / calendar-month helpers
  // ============================================================

  function getWrappedMonthStart(date){
    const instant = new Date(date || Date.now());
    const wallClock = new Date(instant.getTime() + BRISBANE_OFFSET_MS);
    return new Date(Date.UTC(wallClock.getUTCFullYear(), wallClock.getUTCMonth() - 1, 1) - BRISBANE_OFFSET_MS);
  }

  function shiftBrisbaneMonthStart(monthStart, monthDelta){
    const wallClock = new Date(monthStart.getTime() + BRISBANE_OFFSET_MS);
    return new Date(Date.UTC(wallClock.getUTCFullYear(), wallClock.getUTCMonth() + monthDelta, 1) - BRISBANE_OFFSET_MS);
  }

  function getMonthKey(date){
    const wallClock = new Date(new Date(date || Date.now()).getTime() + BRISBANE_OFFSET_MS);
    return wallClock.getUTCFullYear() + '-' + String(wallClock.getUTCMonth() + 1).padStart(2, '0');
  }

  function getMonthLabel(date){
    return new Intl.DateTimeFormat('en-AU', { month: 'long', year: 'numeric', timeZone: 'Australia/Brisbane' }).format(date);
  }

  function formatBrisbaneDate(date){
    const wallClock = new Date(date.getTime() + BRISBANE_OFFSET_MS);
    return wallClock.getUTCFullYear() + '-' + String(wallClock.getUTCMonth() + 1).padStart(2, '0') + '-' + String(wallClock.getUTCDate()).padStart(2, '0');
  }
  ...
  async function buildData(userId){
    ...
    const monthStart = getWrappedMonthStart(new Date());
    const monthEnd = shiftBrisbaneMonthStart(monthStart, 1);
    const priorMonthStart = shiftBrisbaneMonthStart(monthStart, -1);
    const fourMonthsStart = shiftBrisbaneMonthStart(monthStart, -3);
    const sinceThis = monthStart.toISOString();
    const untilThis = monthEnd.toISOString();
    const sinceFour = fourMonthsStart.toISOString();
    const sinceThisDate = formatBrisbaneDate(monthStart);
    const untilThisDate = formatBrisbaneDate(monthEnd);
    const sinceFourDate = formatBrisbaneDate(fourMonthsStart);

    const results = await Promise.allSettled([
      supabase.from('workouts')
        .select('workout_date, exercise_name, created_at')
        .eq('user_id', userId).eq('workout_type', 'history')
        .gte('workout_date', sinceFourDate),
      ...
      supabase.from('daily_weigh_ins')
        ...
        .eq('user_id', userId).gte('created_at', sinceFour).lt('created_at', untilThis)
      ...
      supabase.from('weekly_goals')
        .select('id,week_start,week_end,selected_goals,progress_snapshot,completed_count,total_count,completion_rate,points_awarded,points_awarded_amount,points_awarded_at')
        .eq('user_id', userId)
        .gte('week_end', sinceThisDate).lt('week_end', untilThisDate)
        .order('week_start', { ascending: true }),
    ]);

    const [workoutsR, moodR, pbsR, weighsR, storiesR, activityR, battlesR, lessonsR, pointsR, coinsR, mealsR, weeklyGoalsR] = results;
    const pick = result => result.status === 'fulfilled' && result.value && Array.isArray(result.value.data) ? result.value.data : [];
    ...
    const weeklyGoals = buildMonthlyGoalsSummary(pick(weeklyGoalsR));

    let reactionsCount = 0;
    if (stories.length > 0) {
      try {
        const { data: reactions } = await supabase.from('feed_reactions').select('id').in('story_id', stories.map(story => story.id));
        reactionsCount = (reactions || []).length;
      } catch (_) {}
    }

    const inRange = (row, start, end) => {
      const time = new Date(row.created_at || row.logged_at || row.achieved_at || row.completed_at || row.activity_date).getTime();
      return time >= start.getTime() && time < end.getTime();
    };
    const priorMonthStartDate = formatBrisbaneDate(priorMonthStart);
    const workoutsThis = workouts.filter(workout => workout.workout_date >= sinceThisDate && workout.workout_date < untilThisDate);
    const workoutsPrior = workouts.filter(workout => workout.workout_date >= priorMonthStartDate && workout.workout_date < sinceThisDate);
    const workoutDatesThis = [...new Set(workoutsThis.map(workout => workout.workout_date))];
    const workoutDatesPrior = [...new Set(workoutsPrior.map(workout => workout.workout_date))];
    const activityThis = activity.filter(item => item.activity_date >= sinceThisDate && item.activity_date < untilThisDate);
    const activityMin = activityThis.reduce((sum, item) => sum + Number(item.duration_minutes || 0), 0);
    const workoutMin = workoutDatesThis.length * 45;

    const moodThis = moodLogs.filter(row => inRange(row, monthStart, monthEnd));
    const moodPrior = moodLogs.filter(row => inRange(row, priorMonthStart, monthStart));
    const avgMood = moodThis.length ? round1(moodThis.reduce((sum, item) => sum + Number(item.mood_score || 0), 0) / moodThis.length) : null;
    const avgMoodPrior = moodPrior.length ? round1(moodPrior.reduce((sum, item) => sum + Number(item.mood_score || 0), 0) / moodPrior.length) : null;

    const weighsThisMonth = weighs.filter(row => inRange(row, monthStart, monthEnd));
    let weightChange = null;
    let latestWeight = null;
    if (weighsThisMonth.length >= 2) {
      weightChange = round1(weighsThisMonth[weighsThisMonth.length - 1].weight - weighsThisMonth[0].weight);
      latestWeight = weighsThisMonth[weighsThisMonth.length - 1].weight;
    } else if (weighsThisMonth.length === 1) {
      latestWeight = weighsThisMonth[0].weight;
    } else if (weighs.length > 0) {
      latestWeight = weighs[weighs.length - 1].weight;
    }

    const trackedMealDays = new Set(meals.map(meal => meal.meal_date).filter(Boolean)).size;
    const nutritionWindowDays = Math.round((monthEnd - monthStart) / DAY_MS);
    const caloriesAvg = trackedMealDays >= Math.ceil(nutritionWindowDays / 2)
      ? Math.round(meals.reduce((sum, meal) => sum + Number(meal.calories || 0), 0) / trackedMealDays)
      : null;
    const allWorkoutDates = [...new Set(workouts.map(workout => workout.workout_date))].filter(Boolean);
    const streak = computeStreak(allWorkoutDates);
    const battlesComplete = battles.filter(battle => battle.status === 'complete' || battle.status === 'completed');
    const ledgerXp = pointTransactions.reduce((sum, item) => sum + Number(item.points_amount || 0), 0);
    const lessonXp = lessons.reduce((sum, item) => sum + Number(item.xp_earned || 0), 0);

    const workoutsByMonth = binByMonth(workouts, monthStart, 4, 'workout_date').map(group => new Set(group.map(workout => workout.workout_date)).size);
    const moodByMonth = binByMonth(moodLogs, monthStart, 4, 'logged_at').map(group => group.length
      ? round1(group.reduce((sum, item) => sum + Number(item.mood_score || 0), 0) / group.length)
      : null);
    const periodKey = getMonthKey(monthStart);
    ...
    return {
      ...
      workouts: {
        this_month: workoutDatesThis.length,
        prior_month: workoutDatesPrior.length,
        this_week: workoutDatesThis.length,
        last_week: workoutDatesPrior.length,
        delta: workoutDatesThis.length - workoutDatesPrior.length,
      },
      minutes: { total: activityMin + workoutMin, from_activities: activityMin, from_workouts: workoutMin },
      pbs: {
        count: pbs.length,
        items: pbs.slice(0, 4).map(item => ({ name: item.exercise_name, value: item.new_value, type: item.pb_type, improvement: item.improvement })),
      },
      mood: {
        avg: avgMood,
        prior_month_avg: avgMoodPrior,
        last_week_avg: avgMoodPrior,
        logs: moodThis.length,
        daily: buildMonthlyMoodArc(moodThis, monthStart, monthEnd),
      },
      weight: { change: weightChange, latest: latestWeight },
      nutrition: { meals_logged: meals.length, avg_calories: caloriesAvg, tracked_days: trackedMealDays, window_days: nutritionWindowDays },
      social: { posts: stories.length, reactions_received: reactionsCount },
      goals: weeklyGoals,
      streak,
      battles: { played: battlesComplete.length, won: battlesComplete.filter(battle => battle.winner_id === userId).length },
      xp: ledgerXp + lessonXp,
      coins: coins.reduce((sum, item) => sum + Number(item.amount || 0), 0),
      predictions: {
        next_month_workouts: predictNext(workoutsByMonth),
        workout_trend: trendDirection(workoutsByMonth),
        next_month_mood: predictNext(moodByMonth),
        mood_trend: trendDirection(moodByMonth),
        next_month_weight: predictNextWeight(weighs),
        workouts_per_month_history: workoutsByMonth,
      },
    };
  }

  function buildMonthlyGoalsSummary(rows){
    ...
    let earned = 0;
    let max = 0;
    const items = weeks.map((row, index) => {
      const weekTotal = clampWeeklyGoalCount(row.total_count || (Array.isArray(row.selected_goals) ? row.selected_goals.length : 0));
      const weekCompleted = Math.min(weekTotal, clampWeeklyGoalCount(row.completed_count));
      const reward = calculateWeeklyGoalReward(weekCompleted, weekTotal);
      completed += weekCompleted;
      total += weekTotal;
      earned += reward.earned;
      max += reward.max;
      awarded += Math.max(0, Number(row.points_awarded_amount || 0));
      return {
        id: row.id || 'week-' + (index + 1),
        label: 'Week ' + (index + 1),
        category: row.week_start && row.week_end ? row.week_start + ' to ' + row.week_end : 'Weekly goals',
        current: weekCompleted,
        target: weekTotal,
        unit: 'goals',
        complete: weekTotal > 0 && weekCompleted >= weekTotal,
      };
    ...
    return {
      ...
      items,
      arc: null,
      weeks_reviewed: items.length,
      reward: { earned, awarded, just_awarded: 0, max, all_goals_hit: total > 0 && completed >= total, ready: true },
    };
  }

  function buildMonthlyMoodArc(logs, monthStart, monthEnd){
    const bins = [];
    for (let index = 0, start = new Date(monthStart); start < monthEnd; index += 1, start = new Date(start.getTime() + 7 * DAY_MS)) {
      const end = new Date(Math.min(monthEnd.getTime(), start.getTime() + 7 * DAY_MS));
      const weekLogs = logs.filter(item => {
        const time = new Date(item.logged_at).getTime();
        return time >= start.getTime() && time < end.getTime();
      });
      bins.push({
        day: 'W' + (index + 1),
        date: formatBrisbaneDate(start),
        avg: weekLogs.length ? round1(weekLogs.reduce((sum, item) => sum + Number(item.mood_score || 0), 0) / weekLogs.length) : null,
      });
    }
    return bins;
  }
  ...
  function calculateWeeklyGoalReward(completed, total){
    const completedCount = clampWeeklyGoalCount(completed);
    const totalCount = clampWeeklyGoalCount(total);
    const maxPoints = Math.min(
      WEEKLY_GOAL_XP_CAP,
      (totalCount * WEEKLY_GOAL_XP_PER_GOAL) + (totalCount >= 3 ? WEEKLY_GOAL_ALL_HIT_BONUS : 0)
    );
    const earnedPoints = Math.min(
      WEEKLY_GOAL_XP_CAP,
      (completedCount * WEEKLY_GOAL_XP_PER_GOAL) + (completedCount >= 3 && totalCount >= 3 ? WEEKLY_GOAL_ALL_HIT_BONUS : 0)
    );
    return {
  ...
  function binByMonth(rows, reviewedMonthStart, months, field){
    const bins = [];
    for (let index = months - 1; index >= 0; index -= 1) {
      const start = shiftBrisbaneMonthStart(reviewedMonthStart, -index);
      const end = shiftBrisbaneMonthStart(start, 1);
      bins.push(rows.filter(row => {
        const value = row[field];
        const time = field === 'workout_date' ? new Date(value + 'T00:00:00+10:00').getTime() : new Date(value).getTime();
        return time >= start.getTime() && time < end.getTime();
      }));
    }
    return bins;
  ...
  function predictNextWeight(weighs){
    ...
    const monthlyDelta = round1(slopePerDay * 30);
    const projected = round1(recent[recent.length - 1].weight + monthlyDelta);
    return { projected, monthly_delta: monthlyDelta };
  }
  ...
  function hasSeen(periodKey){
    try { return !!localStorage.getItem(SEEN_KEY_PREFIX + periodKey); }
    catch(_) { return false; }
  }
  function markSeen(periodKey){
    try { localStorage.setItem(SEEN_KEY_PREFIX + periodKey, String(Date.now())); }
    catch(_) {}
  }
  // Returns the completed calendar month used by card and push dedup.
  function getCurrentPeriodKey(){ return getMonthKey(getWrappedMonthStart(new Date())); }
  function getCurrentISOWeek(){ return getCurrentPeriodKey(); }

  // ============================================================
  //  Music — 8 procedural themes rotated by month
  //
  ...
  function pickWrappedTheme(periodKey){
    const digits = String(periodKey || '').replace(/\D/g, '');
    const value = parseInt(digits.slice(-6), 10) || 0;
    return THEMES[((value % THEMES.length) + THEMES.length) % THEMES.length];
  }
  ...
  async function markViewed(userId, periodKey){
    const supabase = window.supabaseClient;
    if (!supabase || !userId) return;
    try {
      await supabase.from('weekly_wrapped')
        .update({ viewed_at: new Date().toISOString() })
        .eq('user_id', userId).eq('iso_week', periodKey);
    } catch(_){}
  }
  ...
  const WW_STYLES = `
    ...
    .ww-pb-panel-title {
      ...
      margin-bottom: 9px; letter-spacing: 0.5px;
    }
    .ww-pb-panel-value {
      display: block; font-size: 2.4rem; font-weight: 900; line-height: 1.1;
      color: #dc2626;
    }
    .ww-pb-panel-delta {
      display: block; font-size: 0.85rem; font-weight: 700; line-height: 1.3;
    ...
    /* Month Wrapped: keep dense recap frames clear on short phones. */
    @media (max-height: 700px) {
      .ww-slide-month-pbs,
      .ww-slide-month-goals {
        justify-content: flex-start;
        padding: calc(env(safe-area-inset-top, 0px) + 58px) 14px calc(env(safe-area-inset-bottom, 0px) + 48px);
      }
      .ww-slide-month-pbs > .ww-kicker {
        margin-bottom: 4px;
        font-size: 0.58rem;
        letter-spacing: 2.2px;
      }
      .ww-slide-month-pbs .ww-pb-stack { gap: 2px !important; margin-top: 5px !important; }
      .ww-slide-month-pbs .ww-pb-panel {
      ...
      .ww-slide-month-pbs .ww-pb-panel + .ww-pb-panel { margin-top: 8px; }
      .ww-slide-month-pbs .ww-pb-panel::before {
        top: -11px;
        font-size: 0.55rem;
        padding: 3px 7px;
      }
      .ww-slide-month-pbs .ww-pb-panel-title {
        font-size: 1.08rem;
        line-height: 1.14;
        margin-bottom: 8px;
      }
      .ww-slide-month-pbs .ww-pb-panel-value { font-size: 1.72rem; line-height: 1.08; }
      .ww-slide-month-pbs .ww-pb-panel-delta { font-size: 0.68rem; line-height: 1.2; margin-top: 7px; }
      .ww-slide-month-pbs .ww-pb-more {
        margin-top: 9px !important;
        font-size: 0.78rem;
        line-height: 1.15;
      }
      .ww-slide-month-goals > .ww-kicker { margin-bottom: 5px; font-size: 0.58rem; }
      .ww-slide-month-goals .ww-month-goal-score {
        font-size: 3.05rem !important;
        line-height: 0.9;
      }
      .ww-slide-month-goals .ww-month-goal-caption {
        margin-top: 5px !important;
        margin-bottom: 6px !important;
        font-size: 0.76rem;
        line-height: 1.1;
      }
      .ww-slide-month-goals .ww-month-goal-reward {
        width: min(92vw, 430px) !important;
        margin: 0 auto 5px !important;
        padding: 7px 9px !important;
        border-radius: 12px !important;
      }
      .ww-slide-month-goals .ww-month-goal-reward-main { font-size: 0.78rem !important; line-height: 1.1; }
      .ww-slide-month-goals .ww-month-goal-reward-sub { font-size: 0.52rem !important; line-height: 1.3; }
      .ww-slide-month-goals .ww-month-goal-row {
        width: min(92vw, 420px) !important;
        margin: 3px auto !important;
        padding: 6px 8px !important;
        border-radius: 11px !important;
      }
      .ww-slide-month-goals .ww-month-goal-label { min-width: 0; flex: 1; font-size: 0.72rem !important; }
      .ww-slide-month-goals .ww-month-goal-value { font-size: 0.7rem !important; }
      .ww-slide-month-goals .ww-month-goal-meta {
      ...
      .ww-slide-month-streak .ww-flame { font-size: clamp(5rem, 27vw, 6rem); }
      .ww-slide-month-streak .ww-month-streak-value { margin-top: 34px !important; }
    }
    ...
    .ww-bg-member-proof {
      background:
        radial-gradient(circle at 16% 12%, rgba(245,184,75,0.46) 0%, transparent 28%),
        radial-gradient(circle at 86% 82%, rgba(105,199,121,0.38) 0%, transparent 34%),
        linear-gradient(145deg, #07120d 0%, #11231a 44%, #f8f3e7 44%, #f8f3e7 100%);
    }
    .ww-proof-shell {
      position:relative;
      z-index:2;
    ...
    .ww-proof-chip {
      ...
      text-align:center;
      font-weight:900;
      animation:wwFadeSlideUp 0.42s ease-out both;
      animation-delay:var(--delay, 0s);
    }
    .ww-proof-chip strong {
      font-family:'Impact','Arial Black',sans-serif;
  ...
  async function open(userId, opts){
    ...
    overlay.innerHTML = `
      <div class="ww-slide active ww-bg-intro" style="display:flex;">
        ...
        <div class="ww-kicker" style="animation:none;">Loading your month…</div>
    ...
    if (!data) {
      try { data = await buildData(userId); }
        ...
        overlay.innerHTML = `
          <div class="ww-slide active ww-bg-intro" style="display:flex;">
            <div class="ww-kicker">Couldn't load your month</div>
    ...
    // Register the active month so every close path can mark it seen.
    activeUserId = userId;
    activePeriodKey = data.period_key || data.iso_week;

    // Spin up the synth using the theme picked from this wrapped's ISO week.
    // Done AFTER data fetch so replays of archived months keep the theme they
  ...
  function closeOverlay(){
    ...
    if (activePeriodKey) {
      try { markSeen(activePeriodKey); } catch(_){}
      if (activeUserId) { try { markViewed(activeUserId, activePeriodKey); } catch(_){} }
  ...
  function buildSlides(d){
    ...
    slides.push({
      ...
      html: `
        <div class="ww-speed-lines"></div>
        <div class="ww-halftone"></div>
        <div class="ww-kicker" style="margin-bottom:22px;">${escapeHtml(firstName)}'s Month</div>
        <div class="ww-big-num" style="font-size: clamp(4rem, 16vw, 9rem);">WRAPPED</div>
        <div class="ww-caption">One month. One slideshow. Let's roll.</div>
        <div class="ww-stamp gold" style="margin-top:34px;">${escapeHtml(d.month_label || d.period_key || d.iso_week)}</div>
        <div class="ww-theme-pill">♪ ${escapeHtml(theme.name)}</div>
      `,
    });

    // 2. WORKOUTS
    const wThis = d.workouts.this_month != null ? d.workouts.this_month : d.workouts.this_week;
    const wDelta = d.workouts.delta;
    const deltaHtml = wDelta === 0
      ? `<div class="ww-delta">Same as the month before</div>`
      : wDelta > 0
        ? `<div class="ww-delta up">+${wDelta} vs the month before</div>`
        : `<div class="ww-delta down">${wDelta} vs the month before</div>`;
    let wStamp = '';
    ...
    if (d.pbs.count > 0) {
      ...
      const moreNote = d.pbs.count > 2
        ? `<div class="ww-caption ww-pb-more" style="margin-top:20px;">+${d.pbs.count - 2} more ${d.pbs.count - 2 === 1 ? 'PB' : 'PBs'} smashed</div>`
        : '';
      slides.push({
        bg: 'ww-bg-pbs ww-slide-month-pbs',
        html: `
          <div class="ww-halftone"></div>
          <div class="ww-kicker" style="color:#1a0a00;">${d.pbs.count} Personal Best${d.pbs.count === 1 ? '' : 's'}</div>
          <div class="ww-pb-stack" style="display:flex; flex-direction:column; align-items:center; gap:4px; margin-top:12px;">${panels}</div>
          ${moreNote}
        `,
      });
    }
    ...
    if (d.mood.avg != null || d.mood.daily.some(x => x.avg != null)) {
      ...
      const trendArrow = arrowFor(d.mood.prior_month_avg != null ? d.mood.prior_month_avg : d.mood.last_week_avg, d.mood.avg);
      slides.push({
        bg: 'ww-bg-mood',
        html: `
          <div class="ww-kicker">Mood this month</div>
          <div>
    ...
    if (d.weight.latest != null || d.nutrition.meals_logged > 0) {
      ...
      const mainCaption = hasWeight
        ? (d.weight.change != null
            ? (d.weight.change > 0 ? `+${d.weight.change}kg this month` : `${d.weight.change}kg this month`)
    ...
    if (d.streak > 0) {
      slides.push({
        bg: 'ww-bg-streak ww-slide-month-streak',
        html: `
          <div class="ww-halftone"></div>
          <div class="ww-kicker">Current streak</div>
          <div class="ww-flame">🔥</div>
          <div class="ww-big-num ww-month-streak-value" style="font-size: clamp(3.5rem, 14vw, 7rem); margin-top:34px;">${d.streak}</div>
    ...
    if (d.goals && d.goals.total > 0) {
      const goalRows = d.goals.items.slice(0, 5).map(item => {
        ...
        return `
          <div class="ww-month-goal-row" style="width:min(82vw,420px);background:rgba(255,255,255,0.12);border:1px solid rgba(255,255,255,0.16);border-radius:16px;padding:12px 14px;margin:8px auto;text-align:left;">
            <div style="display:flex;justify-content:space-between;gap:12px;align-items:baseline;">
              <div class="ww-month-goal-label" style="font-size:.95rem;font-weight:850;color:white;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${escapeHtml(item.label)}</div>
              <div class="ww-month-goal-value" style="font-size:.9rem;font-weight:950;color:${item.complete ? '#bbf7d0' : '#e0f2fe'};white-space:nowrap;">${escapeHtml(current)} / ${escapeHtml(target)}</div>
            </div>
            <div class="ww-month-goal-meta" style="font-size:.68rem;color:rgba(255,255,255,.62);font-weight:800;text-transform:uppercase;letter-spacing:.08em;margin-top:3px;">${escapeHtml(item.category || item.unit || '')}</div>
          </div>
        `;
      }).join('');
      const arcLine = d.goals.arc && d.goals.arc.headline ? d.goals.arc.headline : '';
      const goalReward = d.goals.reward || {};
      const rewardLine = goalReward.awarded > 0
        ? goalReward.awarded + ' XP paid across these weeks'
        : goalReward.earned > 0
          ? goalReward.earned + ' XP earned across these weeks'
          : 'No weekly-goal XP this month';
      const rewardSubline = goalReward.max > 0
        ? 'Weekly goal rewards are still paid after each week ends'
        : "Pick weekly goals to build next month's recap";
      slides.push({
        bg: 'ww-bg-social ww-slide-month-goals',
        html: `
          <div class="ww-halftone"></div>
          <div class="ww-kicker">Goals you chose</div>
          <div class="ww-big-num ww-month-goal-score" style="font-size:clamp(4rem,16vw,8rem);">${d.goals.completed}/${d.goals.total}</div>
          <div class="ww-caption ww-month-goal-caption" style="margin-bottom:12px;">weekly goals hit this month</div>
          <div class="ww-month-goal-reward" style="width:min(84vw,430px);margin:0 auto 12px;padding:12px 14px;border-radius:18px;background:rgba(253,230,138,0.16);border:1px solid rgba(253,230,138,0.34);box-shadow:0 14px 34px rgba(15,23,42,0.24);">
            <div class="ww-month-goal-reward-main" style="font-size:1.05rem;font-weight:950;color:#fef3c7;">${escapeHtml(rewardLine)}</div>
            <div class="ww-month-goal-reward-sub" style="font-size:.72rem;font-weight:850;color:rgba(255,255,255,.72);margin-top:3px;text-transform:uppercase;letter-spacing:.06em;">${escapeHtml(rewardSubline)}</div>
          </div>
          ${goalRows}
    ...
    const predParts = [];
    if (p.next_month_workouts != null) {
      predParts.push(`<strong>${p.next_month_workouts}</strong> workouts`);
    }
    if (p.next_month_weight && p.next_month_weight.projected != null) {
      const delta = p.next_month_weight.monthly_delta;
      const sign = delta > 0 ? '+' : '';
      predParts.push(`<strong>${p.next_month_weight.projected}kg</strong> (${sign}${delta}kg)`);
    }
    if (p.next_month_mood != null) {
      predParts.push(`mood around <strong>${p.next_month_mood}/10</strong>`);
    }
    const predBody = predParts.length
      ? `At this pace you're on track for ${predParts.join(' · ')}.`
      : `Keep logging. We need 2+ months of data to forecast your trend.`;
    slides.push({
      bg: 'ww-bg-predict',
      html: `
        <div class="ww-speed-lines"></div>
        <div class="ww-kicker">Next month's forecast</div>
  ...
  function challengeWeekWindow(challenge, weekNumber){
    const now = new Date();
    const week = Math.max(1, Math.floor(Number(weekNumber || 1)) || 1);
    const fallbackStart = addDays(now, -(((week - 1) * 7) + 6));
    const challengeStart = parseDateOnly(challenge && challenge.start_date) || fallbackStart;
    const start = addDays(challengeStart, (week - 1) * 7);
    const fullEndExclusive = addDays(start, 7);
  ...
  function pickChallengeCohort(cohorts){
    ...
    const scored = (cohorts || [])
      ...
      .map(c => {
        ...
        if (started) score += Math.floor(started.getTime() / DAY_MS) / 100000;
  ...
  async function buildChallengeWeekStats(weekNumber){
    const supabase = window.supabaseClient;
    if (!supabase) throw new Error('Supabase client not ready');

    const cohorts = await fetchChallengeCohorts(supabase);
    const challenge = pickChallengeCohort(cohorts);
    ...
    const range = challengeWeekWindow(challenge, weekNumber);
  ...
  function buildChallengeWeekTwoSlides(stats){
    ...
    return [
      { bg: 'ww-bg-challenge-week', hold_ms: 2400, html: `
        ...
        <div class="ww-challenge-pill green">30 day challenge</div>
        <div class="ww-big-num" style="font-size: clamp(2.7rem, 12vw, 5.9rem); line-height:0.96; color:#f8f3e7; text-shadow:5px 5px 0 #101820; margin-top:18px;">WEEK 2<br>REVIEW</div>
        <div class="ww-caption" style="max-width:88%; color:#f8f3e7;">${escapeHtml(title)}<br>${escapeHtml(range)} &middot; ${members} challengers</div>
      `},
      { bg: 'ww-bg-challenge-action', hold_ms: 2600, html: `
        <div class="ww-speed-lines"></div>
        <div class="ww-challenge-pill">two weeks in</div>
        <div class="ww-big-num" style="font-size: clamp(5rem, 24vw, 12rem); color:#f8f3e7; text-shadow:6px 6px 0 #101820;">${workouts}</div>
        <div class="ww-caption" style="color:#fff; font-weight:900;">workouts completed</div>
        <div class="ww-challenge-pill coral" style="margin-top:20px;">approx ${minutes} training minutes</div>
      `},
      { bg: 'ww-bg-challenge-night', hold_ms: 2600, html: `
        ...
        <div class="ww-big-num" style="font-size: clamp(5rem, 24vw, 12rem); color:#f3d36b; text-shadow:6px 6px 0 #101820, 0 0 28px rgba(243,211,107,0.45);">${pbs}</div>
        <div class="ww-big-num" style="font-size: clamp(1.8rem, 8vw, 3.7rem); color:#fff; line-height:1; text-shadow:4px 4px 0 #101820;">PBs HIT</div>
        <div class="ww-caption" style="color:#fff; font-weight:900; margin-top:8px;">in week two</div>
      `},
      { bg: 'ww-bg-challenge-night', hold_ms: 2600, html: `
  ...
  async function buildChallengeWeekTwoReel(){
    try {
      const stats = await buildChallengeWeekStats(2);
  ...
  function buildGoalSettingReel(){
    return [
      ...
      `},
      { bg: 'ww-bg-workouts', hold_ms: 3900, html: `
        <div class="ww-speed-lines"></div>
        <div class="ww-kicker">No more vague weeks</div>
        <div class="ww-big-num" style="font-size: clamp(2.75rem, 11vw, 5.4rem); line-height:1.03;">pick your<br>targets.</div>
        <div class="ww-caption">Training, food, recovery, Health IQ, community, or body progress.</div>
      `},
      { bg: 'ww-bg-xp', hold_ms: 3900, html: `
        <div class="ww-halftone"></div>
        <div class="ww-kicker">How to use it</div>
        <div class="ww-big-num" style="font-size: clamp(4.4rem, 18vw, 8.8rem); line-height:0.92;">1 2 3</div>
        <div class="ww-caption">Open Balance. Find Weekly goals. Tap Set.</div>
      `},
      { bg: 'ww-bg-predict', hold_ms: 4300, html: `
        <div class="ww-speed-lines"></div>
        <div class="ww-kicker">Choose your 3</div>
        <div style="display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:10px; width:min(86vw,420px); margin-top:18px;">
          <div class="ww-store-pill">Body</div>
          <div class="ww-store-pill">Training</div>
          <div class="ww-store-pill">Food</div>
          <div class="ww-store-pill">Recovery</div>
        </div>
        <div class="ww-caption" style="margin-top:18px;">Pick the goals that fit this week, then save.</div>
      `},
      { bg: 'ww-bg-pbs', hold_ms: 4300, html: `
        <div class="ww-halftone"></div>
        <div class="ww-kicker" style="color:#1a0a00;">Balance tracks it</div>
        <div class="ww-big-num" style="font-size: clamp(3.2rem, 12vw, 6rem); color:#1a0a00; line-height:1; text-shadow: 4px 4px 0 #fff;">your week<br>has a score.</div>
        <div class="ww-caption" style="color:#1a0a00; font-weight:800;">Progress rolls into your Month Wrapped.</div>
      `},
  ...
  function buildMemberCheckinStoryReel(){
    return [
      { bg: 'ww-bg-member-proof', hold_ms: 9000, html: `
        <div class="ww-speed-lines"></div>
        <div class="ww-halftone"></div>
        <div class="ww-proof-shell">
          <div class="ww-proof-eyebrow">week 3 check-in</div>
          <div class="ww-proof-card">
            <div class="ww-proof-label">real challenge reply</div>
            <div class="ww-proof-quote">"I feel stronger.<br>I can see<br>myself getting<br>better."</div>
          </div>
          <div class="ww-proof-foot">30 day challenge</div>
        </div>
      `},
      { bg: 'ww-bg-member-proof', hold_ms: 8500, html: `
        <div class="ww-halftone"></div>
        <div class="ww-proof-shell">
          <div class="ww-proof-eyebrow">the shift</div>
          <div class="ww-proof-card alt">
            <div class="ww-proof-label">food awareness</div>
            <div class="ww-proof-body">"I have a lot clearer idea of what I'm eating than I did before."</div>
            <div class="ww-proof-muted">Not perfect. Just more aware, more consistent, and actually learning her body.</div>
          </div>
        </div>
      `},
      { bg: 'ww-bg-member-proof', hold_ms: 8500, html: `
        <div class="ww-speed-lines"></div>
        <div class="ww-proof-eyebrow" style="z-index:2;">habit stack</div>
        <div class="ww-proof-grid" style="margin-top:18px;">
          <div class="ww-proof-chip" style="--delay:0s;"><strong>13-15k</strong><span>steps daily</span></div>
          <div class="ww-proof-chip" style="--delay:0.08s;"><strong>10 min</strong><span>yoga every morning</span></div>
          <div class="ww-proof-chip" style="--delay:0.16s;"><strong>higher</strong><span>protein target</span></div>
          <div class="ww-proof-chip" style="--delay:0.24s;"><strong>still</strong><span>showing up</span></div>
        </div>
      `},
      { bg: 'ww-bg-member-proof', hold_ms: 9500, html: `
        <div class="ww-halftone"></div>
        <div class="ww-proof-shell">
          <div class="ww-proof-eyebrow">this is the win</div>
          <div class="ww-proof-card">
            <div class="ww-proof-quote">not perfection. proof.</div>
            <div class="ww-proof-muted">More awareness with food. More consistency with movement. More trust in her body.</div>
          </div>
          <div class="ww-proof-foot">DM challenge</div>
        </div>
        <button class="ww-cta-btn" data-ww-finish style="margin-top:18px;">Done</button>
      `},
    ];
  ...
  function buildDisciplineFreeEnergyReel(){
    return [
      { bg: 'ww-bg-ask', hold_ms: 3200, html: `
        <div class="ww-halftone"></div>
        <div class="ww-kicker">Discipline works...</div>
        <div class="ww-big-num" style="font-size: clamp(2.7rem, 11vw, 5.4rem); line-height:1.04;">until you<br>get tired.</div>
      `},
      { bg: 'ww-bg-mood', hold_ms: 4200, html: `
        <div class="ww-speed-lines"></div>
        <div class="ww-kicker">Then your brain picks</div>
        <div class="ww-big-num" style="font-size: clamp(2.65rem, 10vw, 5rem); line-height:1.05;">the path it<br>knows best.</div>
      `},
      { bg: 'ww-bg-workouts', hold_ms: 4600, html: `
        <div class="ww-halftone"></div>
        <div class="ww-kicker">That is why night snacks win</div>
        <div class="ww-caption" style="font-size:1.22rem; max-width:88%;">Couch. Phone. Pantry. Your brain has run that pattern a thousand times.</div>
      `},
      { bg: 'ww-bg-predict', hold_ms: 4600, html: `
        <div class="ww-speed-lines"></div>
        <div class="ww-kicker">Free Energy Principle</div>
        <div class="ww-caption" style="font-size:1.22rem; max-width:88%;">Your brain keeps choosing what feels predictable because predictable feels safer.</div>
      `},
      { bg: 'ww-bg-xp', hold_ms: 4500, html: `
        <div class="ww-kicker">So stop yelling at yourself</div>
        <div class="ww-big-num" style="font-size: clamp(2.55rem, 10vw, 5rem); line-height:1.05;">change the<br>setup.</div>
        <div class="ww-caption" style="font-size:1.05rem; max-width:88%;">Put the walk, the meal, the better choice directly in the easy path.</div>
        <div class="ww-stamp blue" style="margin-top:20px;">MAKE IT OBVIOUS</div>
      `},
      ctaSlide(),
    ];
  }
  ...
  const REELS = {
    'member-checkin-story': { name: 'Member Check-in Story', emoji: 'DM', theme: 'lofi', build: buildMemberCheckinStoryReel, needsFitgotchi: false },
    ...
    'discipline-free-energy': { name: "Why Discipline Doesn't Work", emoji: 'FEP', theme: 'lofi', build: buildDisciplineFreeEnergyReel, needsFitgotchi: false },
