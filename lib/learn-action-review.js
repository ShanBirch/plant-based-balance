(function () {
  ...
  function details(row, definition) {
    ...
    return `<p>${esc(definition.prompt)}</p><p><strong>Reflection / plan</strong><br>${esc(row?.reflection_text || 'No reflection saved yet.')}</p>` +
      ...
      (report.feed ? `<p><strong>Saved Feed activity for this week</strong><br>${esc(report.feed.post_count)} posts; comments on ${esc(report.feed.comment_post_count)} other posts.</p>` : '') +
  ...
  function memberPage(week, def, row) {
    ...
    return `<div class="lar-member">
      <div class="lar-week-picker"><label>Course action week<select data-action-week>${Array.from({length:Math.min(state.total_weeks || 6,state.current_week)},(_,i)=>`<option value="${i+1}" ${i+1===week?'selected':''}>Week ${i+1}</option>`).join('')}</select></label><span class="lar-status${done?' is-complete':''}">${done?'✓ ':''}${esc(status(week))}</span></div>
      ...
      ${window.BalanceLearnCurriculum.actionIndex(week,state?.curriculum_version)===3 ? '<details class="lar-panel lar-history"><summary>Your week 1 and 2 records</summary>'+[1,2].map(w=>details(record(w),window.BalanceLearnWeeklyActions.experiment(w))).join('')+'</details>' : ''}
  ...
  async function open(week) {
    try {
      ...
      const row = record(week), def = complete(row) && row.instructions?.fields ? row.instructions : window.BalanceLearnWeeklyActions.experiment(week,state?.curriculum_version);
      ...
      el.querySelector('[data-plan]')?.addEventListener('submit',async e=>{
        ...
        try { await savePlan(week,e.target.elements.reflection.value); el.remove(); window.showToast?.('Reflection saved. Continue this in your weekly check-in.','success'); } catch(error){el.querySelector('[data-error]').textContent=error.message;buttons.forEach(b=>b.disabled=false);}
      });
  ...
  async function savePlan(week,text) {
    const def=window.BalanceLearnWeeklyActions.experiment(week,state?.curriculum_version);
  ...
  function form(weekOverride) {
    ...
    const week = Number(weekOverride || selectedWeek || Math.min(state.total_weeks || 6,state.current_week));
    const def=window.BalanceLearnWeeklyActions.experiment(week,state?.curriculum_version), row=record(week);
    if (!def || week>state.current_week) return '';
    return `<section data-learn-report data-week="${week}" data-revision="${row?.revision || 0}" data-enrollment="${esc(state.enrollment.id)}"><h3>Week ${week}: ${esc(def.title)}</h3><label class="pbb-wci-field">Course action week<select class="pbb-wci-input" data-learn-week>${Array.from({length:Math.min(state.total_weeks || 6,state.current_week)},(_,i)=>`<option value="${i+1}" ${i+1===week?'selected':''}>Week ${i+1}</option>`).join('')}</select></label><p>${esc(def.prompt)}</p><p><strong>${esc(status(week))}</strong></p>${row?.reflection_text ? `<p>Saved reflection: ${esc(row.reflection_text)}</p>` : ''}${row?.review_note ? `<p>Action review: ${esc(row.review_note)}</p>` : ''}${complete(row) ? '<p>This completion is saved. No further report is needed.</p>' : def.fields.map(([key,label])=>`<label class="pbb-wci-field"><span class="pbb-wci-field-label">${esc(label)}</span><textarea class="pbb-wci-input" data-learn-answer="${key}" maxlength="2000">${esc(row?.report?.answers?.[key] || '')}</textarea></label>`).join('') + (def.requiresMeal ? `<label class="pbb-wci-field">Meal saved in your nutrition tracker<select class="pbb-wci-input" data-learn-meal><option value="">Choose a saved meal</option>${(state.meals || []).map(m=>`<option value="${esc(m.id)}" ${row?.report?.meal?.id===m.id?'selected':''}>${esc(m.name)} — P ${esc(m.protein_g)} g / C ${esc(m.carbs_g)} g / F ${esc(m.fat_g)} g</option>`).join('')}</select></label><p>Daily targets: protein ${esc(state.targets?.protein_goal_g ?? 'not set')} g · carbohydrates ${esc(state.targets?.carbs_goal_g ?? 'not set')} g · fat ${esc(state.targets?.fat_goal_g ?? 'not set')} g. Build your meal in the tracker and choose Save for later, then reopen this check-in.</p>` : '') + '<p>Describe what happened in your weekly check-in. Balance checks it alongside your reflection and records the action when the details match.</p>'}</section>`;
  }
