...
(async function() {
  ...
  if (requestedMetaAdTrial && storedSession) {
    sessionStorage.removeItem('guestMode');
    // The explicit account-first onboarding-test link is also used with a real
    // authenticated test account so personalised plans can be generated under
    // that user's RLS scope. Keep the paid onboarding experience active without
    // replacing the signed-in session with the guest preview user.
    const isAuthenticatedOnboardingTest = _guestParams.get('account_first') === '1'
      && _guestParams.get('meta_trial') === 'facebook_5m_foundations_v3'
      && _guestParams.get('utm_campaign') === 'onboarding_test'
      && ['shannonrhysbirch+arunima-onboarding-test@gmail.com', 'shannonrhysbirch+phone-onboarding-test@gmail.com']
        .includes(String(storedSession.user.email || '').toLowerCase());
    window.metaAdTrialMode = window.BalanceMetaAdTrial.restoreAuthenticatedMode(storedSession.user.id, storedSession.user)
      || isAuthenticatedOnboardingTest;
  }
  ...
  // Balance Learn is a fixed six-week programme. Grandfathered lifetime
