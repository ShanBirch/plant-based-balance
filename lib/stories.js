...
const FEED_VIDEO_UPLOAD_TARGET_BYTES = 30 * 1024 * 1024;
const PHONE_VIDEO_PRIMARY_MAX_DIMENSION = 1080;
const PHONE_VIDEO_FALLBACK_MAX_DIMENSION = 900;
const PHONE_VIDEO_FINAL_MAX_DIMENSION = 720;
const PHONE_VIDEO_PRIMARY_BITRATE = 8000000;
const PHONE_VIDEO_FALLBACK_BITRATE = 5000000;
const PHONE_VIDEO_FINAL_BITRATE = 2500000;
...
async function prepareUploadableVideo(file, onProgress, options = {}) {
    ...
    fileToUpload = await compressVideo(fileToUpload, onProgress, {
        ...
        frameRate: 30,
        videoBitsPerSecond: PHONE_VIDEO_PRIMARY_BITRATE,
        minVideoBitsPerSecond: PHONE_VIDEO_FALLBACK_BITRATE,
        label: 'Preparing HD clip...'
    });
    ...
    if (fileToUpload.size > maxBytes) {
        ...
        fileToUpload = await compressVideo(fileToUpload, onProgress, {
            ...
            frameRate: 30,
            videoBitsPerSecond: PHONE_VIDEO_FALLBACK_BITRATE,
            minVideoBitsPerSecond: PHONE_VIDEO_FINAL_BITRATE,
            label: 'Preparing upload clip...'
        });
    }
    ...
    if (fileToUpload.size > maxBytes) {
        ...
        fileToUpload = await compressVideo(fileToUpload, onProgress, {
            ...
            frameRate: 30,
            videoBitsPerSecond: PHONE_VIDEO_FINAL_BITRATE,
            minVideoBitsPerSecond: 1600000,
...
function addFeedPulsePerson(people, userId) {
...
function renderFeedCommunityPulseLoading() {
    ...
    setFeedPulseText('feed-community-pulse-headline', "Loading today's pulse...");
}
...
function renderFeedCommunityPulseError() {
    ...
    setFeedPulseText('feed-community-pulse-headline', 'Pulse is warming up');
}
...
function renderFeedCommunityPulse(pulse) {
    ...
    const people = Math.max(0, Math.floor(Number(pulse && pulse.people) || 0));
    const posts = Math.max(0, Math.floor(Number(pulse && pulse.posts) || 0));
    const cheers = Math.max(0, Math.floor(Number(pulse && pulse.cheers) || 0));
    ...
    const totalActivity = posts + cheers + comments + battles;

    const headline = people > 0
        ? `${people} ${people === 1 ? 'person showed' : 'people showed'} up today`
        : totalActivity > 0
            ? 'The Feed is moving today'
            : 'Be first to get the Feed moving today';

    card.setAttribute('aria-busy', 'false');
    setFeedPulseText('feed-community-pulse-headline', headline);
    setFeedPulseText('feed-pulse-posts', posts);
    setFeedPulseText('feed-pulse-cheers', cheers);
...
window.loadFeedCommunityPulse = async function(options = {}) {
    ...
    try {
        ...
        (Array.isArray(fetchedStories) ? fetchedStories : []).forEach(story => {
            ...
            if (!storyId || !createdAt || Number.isNaN(createdAt.getTime()) || createdAt < todayStart) return;
            if (!todayStoriesById.has(storyId)) todayStoriesById.set(storyId, story);
        });

        const todayStories = Array.from(todayStoriesById.values());
        const storyIds = todayStories.map(getFeedPulseStoryId).filter(Boolean);
        const people = new Set();
        todayStories.forEach(story => addFeedPulsePerson(people, story.user_id));

        const client = getFeedCommunityPulseClient();
        const emptyQuery = Promise.resolve({ data: [], error: null });
        const reactionQuery = client && storyIds.length
            ? client
                .from('feed_reactions')
                ...
                .limit(500)
            : emptyQuery;
        const commentQuery = client && storyIds.length
            ? client
                .from('feed_comments')
                .select('story_id,user_id,created_at')
                .in('story_id', storyIds)
        ...
        const reactions = getFeedPulseQueryRows(reactionResult, 'cheers');
        const comments = getFeedPulseQueryRows(commentResult, 'comments');
        const battles = getFeedPulseQueryRows(battleResult, 'battles');

        reactions.forEach(row => addFeedPulsePerson(people, row.user_id));
        comments.forEach(row => addFeedPulsePerson(people, row.user_id));
        battles.forEach(row => {
            addFeedPulsePerson(people, row.challenger_id);
            addFeedPulsePerson(people, row.opponent_id);
        });
...
window.focusFirstFeedReaction = function() {
    const card = document.querySelector('#friends-photo-feed .feed-post-card');
    if (!card) {
        if (typeof window.focusFeedComposer === 'function') window.focusFeedComposer();
        if (typeof showFeedComposerToast === 'function') {
            showFeedComposerToast('No posts to cheer yet. Post your win first.', 'info');
        }
        return;
    }

    card.scrollIntoView({ behavior: 'smooth', block: 'center' });
    const target = card.querySelector('[id^="feed-reactions-"]') || card;
    const previousBoxShadow = target.style.boxShadow;
    const previousBackground = target.style.background;
    target.style.boxShadow = '0 0 0 3px rgba(13,148,136,0.24)';
    target.style.background = 'rgba(240,253,250,0.92)';
    setTimeout(() => {
        target.style.boxShadow = previousBoxShadow;
        target.style.background = previousBackground;
    }, 1500);

    const firstReaction = target.querySelector('button[data-reaction]');
    setTimeout(() => {
        if (firstReaction && typeof firstReaction.focus === 'function') firstReaction.focus();
    }, 260);
};

window.openFeedPulseBattle = function() {
    const activeGames = document.getElementById('active-games-container');
    if (activeGames && activeGames.style.display !== 'none' && activeGames.innerHTML.trim()) {
        activeGames.scrollIntoView({ behavior: 'smooth', block: 'center' });
        const previousBoxShadow = activeGames.style.boxShadow;
        activeGames.style.boxShadow = '0 0 0 3px rgba(124,58,237,0.18)';
        setTimeout(() => {
            activeGames.style.boxShadow = previousBoxShadow;
        }, 1500);
        return;
    }
    ...
    } else if (typeof switchAppTab === 'function') {
        switchAppTab('learning');
    }

    setTimeout(() => {
        if (typeof window.showQuizBattleInviteModal === 'function') {
            window.showQuizBattleInviteModal();
        } else if (typeof showFeedComposerToast === 'function') {
            showFeedComposerToast('Quiz battles are still loading. Try again in a moment.', 'info');
        }
    }, 260);
};
...
async function initializeStoriesFeature() {
    ...
    if (!storiesFeatureRefreshInterval) {
        storiesFeatureRefreshInterval = setInterval(function() {
            const feedRefresh = loadPhotoFeed('friends-photo-feed', 'friends-feed-empty');
