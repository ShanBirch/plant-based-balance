...
- The single scheduled Instagram Chrome operator is the 24-hour browser shift dispatcher. It runs every 30 minutes, selects one lane from the durable eight-slot rotation, stops claiming new work after 22 minutes, reconciles and releases Chrome before handoff, and claims one person at a time. Its replaced dedicated Instagram Chrome jobs remain paused so browser ownership never overlaps.
- `ig_browser_shift_runs` is the dispatcher-level source of truth. Every shift must acquire the global lease with `start_ig_browser_shift` before claiming Chrome, resume from the returned same-lane cursor, checkpoint with `heartbeat_ig_browser_shift` after every inspected person or attempted native action, and finalize with `finish_ig_browser_shift` before browser release. `completed` means the lane target/boundary was reached; normal timebox handoff is `partial` with an exact `next_resume`; native restriction is `blocked` with evidence. A run that receives `acquired=false` does no Instagram work.
- Follower notification work is one atomic low-volume dispatcher pass: scan from the saved cursor, qualify each new follower/follow-back, claim or create `follower_operator` / `welcome_follower`, and welcome once when eligible with canonical readback. Discovery following remains separate and never triggers an automatic DM.
