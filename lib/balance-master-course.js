...
(function () {
    ...
    const STAGES = [
        ...
        { title: 'Understand compound lifts', outcome: 'Choose movements you can perform with control.',
          lessons: [
    ...
    let state = null, owner = null, context = null, stageIndex = 0, loadPromise = null, templates = [], busy = false;
    const blank = () => ({ answers: {}, reflections: {}, workout: {}, meal: {}, completedStages: {} });
    const esc = value => String(value ?? '').replace(/[&<>"']/g, c => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[c]));
    const uid = () => window.currentUser?.id || null;
    function current() { if (owner !== uid()) { owner = uid(); state = null; loadPromise = null; templates = []; } return state || blank(); }
    ...
    function progress() {
        ...
        return { completed, total: 6, totalTopics: 6, percent: Math.round(completed / 6 * 100), isComplete: completed === 6 };
    }
    function event(name, extra = {}) { context?.track?.(`master_${name}`, { stage: stageIndex + 1, curriculum_version: 'balance_master_v1', ...extra }); }
    async function load(force = false) {
        ...
        loadPromise = (async () => {
            ...
            state = { ...blank(), ...(project.data?.data || {}) }; templates = workouts.data || [];
    ...
    function render(data = current()) {
        ...
        host.innerHTML = `<div id="balance-master" class="master-course">
            <header><span class="master-kicker">Balance Master · Stage ${stageIndex + 1} of 6</span><h2>Build your workout program.<br>Plan your week of food.</h2><p>Six practical stages, at your pace. Leave with two saved projects and a clear reason for your choices.</p><p>${p.completed} of 6 stages complete</p><progress max="6" value="${p.completed}" aria-label="Course progress"></progress></header>
            <nav aria-label="Master stages">${STAGES.map((s, i) => `<button type="button" data-master-action="stage" data-stage="${i}" ${i === stageIndex ? 'aria-current="step"' : ''}>${stageDone(i) ? '✓' : i + 1}. ${esc(s.title)}</button>`).join('')}</nav>
            ...
            <details><summary>Go deeper with your existing lessons</summary><div class="master-references">${s.refs.map(([id, title]) => `<button type="button" data-master-action="lesson" data-lesson="${id}">${esc(title)}</button>`).join('')}</div></details>
            ...
            <div class="master-actions"><button type="button" data-master-action="save">Save draft</button><button type="submit" class="master-primary">Check and complete stage</button></div><p id="master-status" role="status"></p></form>
        ...
        host.querySelector('#master-form').addEventListener('submit', async e => {
            ...
            if (!stageReady(stageIndex, draft)) { status(s.project === 'workout' ? 'Finish your goal, schedule, exercise details, coverage and progression rule. Link at least one saved workout and update its exercise details.' : s.project === 'meal' ? 'Finish all seven days plus dietary needs, shopping, prep and your backup meal.' : 'Add your practical answer before completing this stage.', true); return; }
            draft.completedStages ||= {};
            draft.completedStages[stageIndex] = true;
            if (await save(draft)) { if (!previouslyComplete) event('stage_completed'); if (!coursePreviouslyComplete && progress().isComplete) event('course_completed'); render(); status('Stage complete. Choose the next stage above, or review your work.'); }
        });
    ...
    async function action(button) {
        ...
        if (type === 'stage') { stageIndex = Number(button.dataset.stage); render(); document.getElementById('balance-master')?.scrollIntoView({ block: 'start' }); event('stage_opened'); }
    ...
    async function open(options) {
        ...
        if (!host) return;
        if (!options.unlocked) {
            host.innerHTML = `<div class="master-course"><header><span class="master-kicker">Balance Master</span><h2>Build your workout program and one week of meals</h2><p>Complete Balance Learn to begin. You can preview all six stages here.</p></header><section>${STAGES.map((s, i) => `<h3>${i + 1}. ${esc(s.title)}</h3><p>${esc(s.outcome)}</p>`).join('')}<button type="button" id="master-library">View all courses</button></section></div>`;
    ...
    window.BalanceMaster = { open, progress, load, leave, cancel: () => { context = null; }, stages: STAGES, workoutValid, mealValid, stageDone };
})();
