  ...
  try {
    ...
    if (!token) {
      ...
      if (typeof window.showToast === 'function') {
        window.showToast('⚠️ Push token failed: ' + (window._pushTokenStatus || 'unknown'), 'error');
      }
    } else {
      ...
      setTimeout(function() {
        ...
        if (typeof window.showToast === 'function') {
          if (status === 'verified' || status === 'saved') {
            window.showToast('✅ Native push registered (FCM)', 'success');
          } else {
            window.showToast('⚠️ Push token status: ' + status, 'warning');
          }
        }
