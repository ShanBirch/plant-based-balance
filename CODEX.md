...
- The legacy `hot_lead_feed_nurture` lane name represents one durable full lead list, ordered hot, warm, cold, then unscored-as-cold. Refresh the full eligible list, claim one due person at a time, and use queue status plus `safe_after` as the saved position so later shifts continue down the list instead of restarting at the hottest people. This lane is public-comment-only: never click Like and never use a like-only fallback. Only a verified specific comment receives the seven-day feed cooldown; browser failures get 30 minutes, while no new suitable post, an already-touched post, or no safe specific hook gets 72 hours before reinspection.
