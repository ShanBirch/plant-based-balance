(function () {
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      ...
      .pbb-share-studio__actions{position:absolute;z-index:24;left:10px;right:10px;bottom:calc(9px + env(safe-area-inset-bottom));height:58px;display:grid;grid-template-columns:1fr 1fr;gap:9px}.pbb-share-studio__action{border:1px solid rgba(255,255,255,.3);border-radius:16px;background:rgba(10,10,10,.78);color:#fff;-webkit-text-fill-color:#fff;font-size:.78rem;font-weight:900;line-height:1.1;padding:0 10px;backdrop-filter:blur(14px)}.pbb-share-studio__action--gold{background:var(--gold);border-color:var(--gold);color:var(--ink);-webkit-text-fill-color:var(--ink)}.pbb-share-studio__loading{position:absolute;z-index:30;left:10px;right:10px;bottom:calc(76px + env(safe-area-inset-bottom));min-height:54px;display:flex;align-items:center;justify-content:center;overflow:hidden;border:1px solid rgba(255,255,255,.26);border-radius:15px;background:rgba(10,10,10,.88);color:#fff;-webkit-text-fill-color:#fff;font-size:.78rem;font-weight:900;text-align:center;padding:10px 18px 13px;box-shadow:0 14px 42px rgba(0,0,0,.35);backdrop-filter:blur(16px);-webkit-backdrop-filter:blur(16px)}.pbb-share-studio__loading::after{content:'';position:absolute;left:0;right:0;bottom:0;height:4px;transform-origin:left;background:linear-gradient(90deg,var(--gold),#f5d98a);animation:pbbShareStudioProgress 1.15s ease-in-out infinite}.pbb-share-studio__loading[hidden]{display:none}@keyframes pbbShareStudioProgress{0%{transform:translateX(-100%) scaleX(.3)}50%{transform:translateX(15%) scaleX(.7)}100%{transform:translateX(100%) scaleX(.35)}}
