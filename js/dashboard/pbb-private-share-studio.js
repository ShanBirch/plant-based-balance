(function () {
  ...
  var WORKOUT_LAYOUTS = [
    ...
    { id: 'simple', label: 'Title only' },
    { id: 'full', label: 'Full workout' },
  ...
  var WORKOUT_COLOURS = [
    { id: 'gold', label: 'Gold', value: '#D8B25E' },
    { id: 'cream', label: 'Cream', value: '#F8F5EE' },
    { id: 'white', label: 'White', value: '#FFFFFF' },
    { id: 'black', label: 'Black', value: '#111111' },
    { id: 'soft', label: 'Soft', value: '#F4F0E7' },
    { id: 'gold-light', label: 'Light gold', value: '#F5D98A' }
  ];
  ...
  function svgIcon(name) {
    var icons = {
      ...
      palette: '<path d="M12 3a9 9 0 1 0 0 18h1.2a1.8 1.8 0 0 0 1.3-3l-.2-.2a1.8 1.8 0 0 1 1.3-3H18a3 3 0 0 0 3-3A9 9 0 0 0 12 3Z"/><path d="M7.5 10h.01M9.5 6.8h.01M14 6.5h.01M17 9h.01"/>'
    };
  ...
  function renderWorkoutCompletePage(data) {
    ...
    page.innerHTML = `
      <div class="pwc-shell">
        ...
        <main class="pwc-main">
          <div class="pwc-hero-mark" aria-hidden="true">✓</div>
          <span class="pwc-kicker pwc-celebration">Session complete</span>
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      ...
      .pbb-share-studio__top{position:absolute;z-index:20;top:0;left:0;right:0;min-height:calc(70px + max(32px, env(safe-area-inset-top, 0px)));display:flex;align-items:flex-end;justify-content:space-between;padding:calc(10px + max(32px, env(safe-area-inset-top, 0px))) 14px 10px;background:linear-gradient(rgba(0,0,0,.78),transparent);pointer-events:none}.pbb-share-studio__top-group{display:flex;gap:8px;pointer-events:auto}.pbb-share-studio__top-button{height:44px;min-width:44px;padding:0 12px;border:1px solid rgba(255,255,255,.32);border-radius:999px;background:rgba(8,8,8,.5);color:#fff;-webkit-text-fill-color:#fff;display:flex;align-items:center;justify-content:center;gap:7px;backdrop-filter:blur(14px)}.pbb-share-studio__top-button svg{width:21px;height:21px}.pbb-share-studio__next{background:var(--gold);border-color:var(--gold);color:var(--ink);-webkit-text-fill-color:var(--ink);font-weight:900;padding-inline:18px}
      .pbb-share-studio__rail{position:absolute;z-index:21;right:10px;top:calc(82px + max(32px, env(safe-area-inset-top, 0px)));display:flex;flex-direction:column;gap:8px;max-height:calc(100dvh - 164px - max(32px, env(safe-area-inset-top, 0px)) - max(16px, env(safe-area-inset-bottom, 0px)));overflow-y:auto;overscroll-behavior:contain}.pbb-share-studio__tool{width:66px;min-height:62px;padding:7px 4px;border:1px solid rgba(255,255,255,.3);border-radius:17px;background:rgba(10,10,10,.56);color:#fff;-webkit-text-fill-color:#fff;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:4px;backdrop-filter:blur(14px);font-size:.66rem;font-weight:850}.pbb-share-studio__tool svg{width:22px;height:22px;flex:0 0 auto}.pbb-share-studio__tool span{display:block;line-height:1.05;white-space:nowrap}.pbb-share-studio__tool.is-active{background:var(--gold);border-color:var(--gold);color:var(--ink);-webkit-text-fill-color:var(--ink)}
      ...
      .pbb-share-studio__actions{position:absolute;z-index:24;left:10px;right:10px;bottom:calc(9px + max(16px, env(safe-area-inset-bottom, 0px)));height:58px;display:grid;grid-template-columns:1fr 1fr;gap:9px}.pbb-share-studio__action{border:1px solid rgba(255,255,255,.3);border-radius:16px;background:rgba(10,10,10,.78);color:#fff;-webkit-text-fill-color:#fff;font-size:.78rem;font-weight:900;line-height:1.1;padding:0 10px;backdrop-filter:blur(14px)}.pbb-share-studio__action--gold{background:var(--gold);border-color:var(--gold);color:var(--ink);-webkit-text-fill-color:var(--ink)}.pbb-share-studio__loading{position:absolute;z-index:30;left:10px;right:10px;bottom:calc(76px + max(16px, env(safe-area-inset-bottom, 0px)));min-height:54px;display:flex;align-items:center;justify-content:center;overflow:hidden;border:1px solid rgba(255,255,255,.26);border-radius:15px;background:rgba(10,10,10,.88);color:#fff;-webkit-text-fill-color:#fff;font-size:.78rem;font-weight:900;text-align:center;padding:10px 18px 13px;box-shadow:0 14px 42px rgba(0,0,0,.35);backdrop-filter:blur(16px);-webkit-backdrop-filter:blur(16px)}.pbb-share-studio__loading::after{content:'';position:absolute;left:0;right:0;bottom:0;height:4px;transform-origin:left;background:linear-gradient(90deg,var(--gold),#f5d98a);animation:pbbShareStudioProgress 1.15s ease-in-out infinite}.pbb-share-studio__loading[hidden]{display:none}@keyframes pbbShareStudioProgress{0%{transform:translateX(-100%) scaleX(.3)}50%{transform:translateX(15%) scaleX(.7)}100%{transform:translateX(100%) scaleX(.35)}}
      .pbb-photo-source{position:fixed;inset:0;z-index:100119;display:grid;align-items:end;background:rgba(15,12,7,.55);padding:18px calc(18px + env(safe-area-inset-right)) calc(18px + max(16px, env(safe-area-inset-bottom, 0px))) calc(18px + env(safe-area-inset-left))}.pbb-photo-source__card{width:min(100%,480px);margin:auto;background:var(--cream,#f8f5ee);color:#151515;-webkit-text-fill-color:#151515;border-radius:22px;padding:18px}.pbb-photo-source__title{font-size:1.05rem;font-weight:900}.pbb-photo-source__copy{font-size:.82rem;color:#6f6a61;margin:4px 0 12px}.pbb-photo-source__button{width:100%;min-height:50px;border:0;border-radius:14px;background:#111;color:#fff;-webkit-text-fill-color:#fff;font-weight:900;margin-top:8px}.pbb-photo-source__button--gold{background:#d8b25e;color:#151515;-webkit-text-fill-color:#151515}.pbb-photo-source__button--cancel{background:transparent;color:#151515;-webkit-text-fill-color:#151515;border:1px solid #ded7c9}
      @media(max-height:700px){.pbb-share-studio__rail{top:calc(70px + max(32px, env(safe-area-inset-top, 0px)));gap:5px}.pbb-share-studio__tool{min-height:54px}.pbb-share-studio__drawer{bottom:calc(68px + max(16px, env(safe-area-inset-bottom, 0px)));max-height:34dvh}.pbb-share-studio__actions{height:50px}.pbb-share-studio__workout{top:52%}}
    `;
    document.head.appendChild(style);
  }
  ...
  function ensureElement() {
    ...
    el.innerHTML = `
      ...
      <div class="pbb-share-studio__actions" data-share-actions>
        ...
        <button type="button" class="pbb-share-studio__action pbb-share-studio__action--gold" data-share-instagram>Share to IG Story</button>
      </div>
      <div class="pbb-share-studio__loading" data-share-loading role="status" aria-live="polite" hidden>Preparing your photo...</div>`;
    document.body.appendChild(el);
    bindElement(el);
    return el;
  }
  ...
  function editorState() {
    ...
    return {
      ...
      overlayStyle: active.overlayStyle || 'gold',
      textStyle: active.textStyle || 'bold',
      workoutColour: active.workoutColour || 'gold',
      editorPreset: active.editorPreset || 'gold',
      showPB: active.showPB !== false,
      showStats: active.showStats !== false
    };
  ...
  function workoutLayout(id) {
    return WORKOUT_LAYOUTS.find(function (layout) { return layout.id === id; }) || WORKOUT_LAYOUTS[0];
  }

  function workoutColour(id) {
    return WORKOUT_COLOURS.find(function (colour) { return colour.id === id; }) || WORKOUT_COLOURS[0];
  }

  function workoutColourTokens(id) {
    var colour = workoutColour(id);
    var lightSurface = ['cream', 'white', 'soft'].includes(colour.id);
    return {
      accent: colour.value,
      surface: lightSurface ? colour.value : (colour.id === 'black' ? 'rgba(17,17,17,.92)' : 'rgba(8,8,8,.78)'),
      text: lightSurface ? '#151515' : '#FFFFFF'
    };
  }
  ...
  function renderLayers(el) {
    ...
    var card = el.querySelector('[data-share-workout-layer]');
    card.dataset.layout = active.textStyle;
    var colourTokens = workoutColourTokens(active.workoutColour);
    card.style.setProperty('--workout-accent', colourTokens.accent);
    card.style.setProperty('--workout-surface', colourTokens.surface);
    card.style.setProperty('--workout-text', colourTokens.text);
    card.style.left = Math.round(active.overlayX * 100) + '%';
    card.style.top = Math.round(active.overlayY * 100) + '%';
    card.style.transform = layerTransform(active.overlayX, active.overlayY, active.overlayScale);
    ...
    var layoutName = el.querySelector('[data-share-layout-name]');
    var colourName = el.querySelector('[data-share-colour-name]');
    var colourSwatch = el.querySelector('[data-share-colour-swatch]');
    if (layoutName) layoutName.textContent = workoutLayout(active.textStyle).label;
  ...
  async function runAction(kind, el) {
    ...
    var state = active;
    state.busy = true;
    setActionBusy(el, true, kind === 'feed' ? 'Posting your workout to Feed...' : kind === 'instagram' ? 'Opening Instagram Story...' : 'Preparing your photo...');
    try {
      saveCustomization();
  ...
  function bindElement(el) {
    ...
    el.querySelector('[data-share-cycle-layout]').addEventListener('click', function () {
      ...
      active.textStyle = WORKOUT_LAYOUTS[(index + 1) % WORKOUT_LAYOUTS.length].id;
      renderLayers(el);
    });
    el.querySelector('[data-share-cycle-colour]').addEventListener('click', function () {
      if (!active) return;
      var index = Math.max(0, WORKOUT_COLOURS.findIndex(function (colour) { return colour.id === active.workoutColour; }));
      active.workoutColour = WORKOUT_COLOURS[(index + 1) % WORKOUT_COLOURS.length].id;
      renderLayers(el);
    });
  ...
  function open(options) {
    ...
    return new Promise(function (resolve) {
      active = {
        ...
        textStyle: previous.textStyle || options.textStyle || 'bold',
        workoutColour: previous.workoutColour || 'gold',
        editorPreset: previous.editorPreset || 'gold',
        showPB: previous.showPB !== false,
        showStats: previous.showStats !== false,
