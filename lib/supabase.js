  ...
  friends: {
    ...
    async getFriendsWithFallback(userId) {
      ...
      const { data: sent, error: e1 } = await supabase
        .from('friendships')
        .select('friend_id, status, users!friendships_friend_id_fkey(id, name, email, profile_photo)')
        .eq('user_id', userId)
        .eq('status', 'accepted');
      const { data: received, error: e2 } = await supabase
        .from('friendships')
        .select('user_id, status, users!friendships_user_id_fkey(id, name, email, profile_photo)')
        .eq('friend_id', userId)
        .eq('status', 'accepted');
      ...
      if (sent) {
        sent.forEach(r => {
          const u = r.users;
          if (u) friends.push({ friend_id: u.id, friend_name: u.name || (u.email ? u.email.split('@')[0] : null), friend_email: u.email, friend_photo: u.profile_photo });
        });
      }
      if (received) {
        received.forEach(r => {
