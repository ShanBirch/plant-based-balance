...
const FEED_VIDEO_UPLOAD_TARGET_BYTES = 180 * 1024 * 1024;
...
async function compressVideo(file, onProgress, options = {}) {
    return new Promise((resolve) => {
        ...
        if (!force && fileSizeMB < 3 && (!maxBytes || file.size <= maxBytes)) {
        ...
        const videoBitsPerSecond = Number(options.videoBitsPerSecond || 1200000);
...
async function prepareUploadableVideo(file, onProgress, options = {}) {
    ...
    if (fileToUpload.size > maxBytes) {
        throw new Error('Could not shrink that video enough for upload. Try trimming it to the set you want to share.');
    }
...
async function uploadFeedComposerMedia(file, mediaType, postBtn, options = {}) {
    ...
    const tempStoryId = crypto.randomUUID ? crypto.randomUUID() : Date.now().toString();
    const uploadData = await uploadStoryMediaToBackblaze(fileToUpload, {
        userId: window.currentUser.id,
        storyId: tempStoryId,
        source: uploadSource,
        postBtn
    ...
    if (mediaType === 'image') {
        const analysisDataUrl = await readFeedComposerFileAsDataUrl(fileToUpload);
        analysisBase64 = getBase64Payload(analysisDataUrl);
    }
    ...
    if (config.editKind === 'fitness_diary') {
        bodyParts.push(renderFeedEditorBlock(
            ...
            [
                ...
                '</div>',
                renderFeedEditorTextarea('feed-post-editor-highlight', 'Highlight', cardData.highlight || '', 'rows="3" placeholder="Best thing that happened today"', 'Shown in the top reflection card.', 'flex:1 1 100%;'),
                renderFeedEditorTextarea('feed-post-editor-struggle', 'Struggle', cardData.struggle || '', 'rows="3" placeholder="Hardest part of the day"', 'Shown in the middle reflection card.', 'flex:1 1 100%;'),
