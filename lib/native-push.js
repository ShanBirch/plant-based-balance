...
function getFirebaseMessaging() {
  ...
  // reliably.
  if (_fcmPlugin) return _fcmPlugin;
  // Prefer the already-registered native instance on Capacitor.Plugins —
  // registerPlugin() occasionally returns null when called early even though
  // the native plugin is present (observed on iOS 17). The direct Plugins
  // reference is the actual instance and always works if it's defined.
  if (window.Capacitor && window.Capacitor.Plugins && window.Capacitor.Plugins.FitGotchiPush) {
    _fcmPlugin = window.Capacitor.Plugins.FitGotchiPush;
    console.log('[NativePush] FitGotchiPush plugin loaded via Capacitor.Plugins');
    return _fcmPlugin;
  }
  // Fall back to registerPlugin (this is what works on Android & older iOS).
  if (window.Capacitor && window.Capacitor.registerPlugin) {
    try {
      _fcmPlugin = window.Capacitor.registerPlugin('FitGotchiPush');
      console.log('[NativePush] FitGotchiPush loaded via registerPlugin:', !!_fcmPlugin);
    } catch (e) {
...
function getAppPlugin() {
  if (!_appPlugin && window.Capacitor && window.Capacitor.registerPlugin) {
    _appPlugin = window.Capacitor.registerPlugin('App');
  }
  return _appPlugin;
}
  ...
  try {
    ...
    if (isIOS()) {
      ...
      if (FitGotchiPush && typeof FitGotchiPush.getToken === 'function') {
        try {
          let permStatus = await FitGotchiPush.checkPerm();
          if (permStatus.receive === 'prompt' || permStatus.receive === 'prompt-with-rationale') {
            permStatus = await FitGotchiPush.requestPerm();
          }
          ...
          try { await FitGotchiPush.registerToken(); } catch (_) {}

          // Ask Firebase for the current FCM token. This waits until
          // AppDelegate has handed the APNs device token to Messaging.
          pushDebugBanner('calling getToken()', '#0066cc');
          try {
            const tokenResult = await FitGotchiPush.getToken();
            const token = tokenResult && tokenResult.token;
            if (token) {
              console.log('[NativePush] iOS FCM token obtained');
              pushDebugBanner('FCM token: ' + token.substring(0, 18) + '…', '#009933');
              window._cachedFcmToken = token;
              saveNativePushToken(token);
            } else {
              console.error('[NativePush] iOS getToken returned empty');
              pushDebugBanner('getToken returned empty', '#cc0000');
              window._pushTokenStatus = 'ios_token_empty';
            }
          } catch (tokenErr) {
            pushDebugBanner('getToken ERROR: ' + (tokenErr && tokenErr.message || tokenErr), '#cc0000');
            console.error('[NativePush] getToken threw:', tokenErr);
          }

          await initLocalNotifications();
          registerPushAuthStateListener();
          registerPushAppResumeListener();
    ...
    // Initialize local notifications, then clear the retired meal reminder IDs
    // so existing members do not keep receiving alerts after the Settings row is removed.
    await initLocalNotifications();
    await scheduleMealReminders({ reminders_enabled: false });
...
async function saveNativePushToken(token, _retryCount) {
  ...
  try {
    ...
    // Cache the token so a later resume/auth event can retry even if every
    // current attempt fails. Re-installing the app generates a fresh FCM
    // token, and on a cold first-launch the token can arrive before the user
    // has finished signing in — we want to be able to re-fire later.
    window._cachedFcmToken = token;
    if (!window.currentUser) {
      // Auth may not be ready yet — retry up to 30 times with a steady 2s
      // delay (60s total). On a fresh re-install, the user can spend a long
      // time on the login/onboarding screen before currentUser is populated,
      // and the original 20s window was too tight.
      if (retryCount < 30) {
        var delay = 2000;
        console.log('[NativePush] No user yet, retrying token save in', delay/1000, 's (attempt', retryCount+1, '/30)');
        setTimeout(function() { saveNativePushToken(token, retryCount + 1); }, delay);
        return;
      }
      console.error('[NativePush] No user logged in after 30 retries, giving up token save (will retry on next app resume / auth event)');
    ...
    if (!rpcOk) {
      ...
      try { pushDebugBanner('DB save FAILED: ' + errMsg.substring(0, 80), '#cc0000'); } catch (_) {}
      ...
      try { pushDebugBanner('token saved to Supabase', '#009933'); } catch (_) {}
...
function getFeedPushStoryId(data) {
  const direct = data && (data.storyId || data.story_id || data.feedPostId || data.feed_post_id);
  ...
  try {
    ...
    const match = rawUrl.match(/[?&](?:story_id|feed_post_id)=([^&#]+)/);
    return match ? decodeURIComponent(match[1]) : '';
  }
}

function getFeedPushCommentId(data) {
  const direct = data && (data.commentId || data.comment_id);
  if (direct) return String(direct);
  ...
  try {
    ...
    const match = rawUrl.match(/[?&]comment_id=([^&#]+)/);
    return match ? decodeURIComponent(match[1]) : '';
  }
}

async function openFeedPostFromPushData(data) {
  data = data || {};
  const storyId = getFeedPushStoryId(data);
  const commentId = getFeedPushCommentId(data);
  ...
  if (typeof window.loadPhotoFeed === 'function') {
    try {
      ...
      console.warn('[NativePush] Feed refresh from comment push failed:', err);
    }
  }
  ...
  for (let i = 0; i < 20; i++) {
    if (window.currentUser && (typeof window.openFeedCommentFromPush === 'function' || typeof window.openFeedPostViewer === 'function')) break;
    await new Promise(function(r) { setTimeout(r, 250); });
  }

  if (commentId && typeof window.openFeedCommentFromPush === 'function') {
    const openedComment = await window.openFeedCommentFromPush(storyId, commentId);
    if (openedComment) return true;
  }

  if (typeof window.openFeedPostViewer === 'function') {
    window.openFeedPostViewer(storyId, commentId ? { highlightCommentId: commentId } : undefined);
    return true;
  }
...
window.openFeedPostFromPush = openFeedPostFromPushData;
...
async function reRegisterCachedFcmToken(reason) {
  ...
  let token = window._cachedFcmToken;
  if (!token) {
    // Fresh app process after a reinstall can land here with no cached token —
    // the original `registration` event fired in a previous app launch, or the
    // user backgrounded before init completed. Ask the platform for the
    // current FCM token directly so we can still recover.
    token = await fetchFreshFcmToken(reason);
    if (!token) {
      console.log('[NativePush] No cached FCM token and fresh fetch failed, skipping re-register (' + (reason || '?') + ')');
      return;
    }
    window._cachedFcmToken = token;
  }
  console.log('[NativePush] Re-registering FCM token for current user (' + (reason || '?') + ')');
  await saveNativePushToken(token);
}

/**
 * Ask the platform for the current FCM token without going through the full
 * init dance. Used by the manual "Refresh registration" path and as a fallback
 * when we need to re-bind a token but the in-memory cache is empty (typical
 * after a process restart following a fresh install).
 */
async function fetchFreshFcmToken(reason) {
  try {
    if (isIOS()) {
      const FitGotchiPush = getFirebaseMessaging();
      if (FitGotchiPush && typeof FitGotchiPush.getToken === 'function') {
        try { await FitGotchiPush.registerToken(); } catch (_) {}
        const result = await FitGotchiPush.getToken();
        return result && result.token ? result.token : null;
      }
      return null;
    }
    // Android: re-running register() will re-fire the `registration` listener
    // with the current token. The listener already caches + saves it, so we
    // just need to await register() and read back from the cache.
    const PushNotifications = getPushNotifications();
    if (!PushNotifications) return null;
    try { await PushNotifications.register(); } catch (_) {}
    // Give the registration event a brief moment to populate the cache.
    await new Promise(function (r) { setTimeout(r, 500); });
    return window._cachedFcmToken || null;
  } catch (err) {
    console.warn('[NativePush] fetchFreshFcmToken failed (' + (reason || '?') + '):', err && err.message || err);
    return null;
  }
}
...
function registerPushAuthStateListener() {
  ...
  supabase.auth.onAuthStateChange((event, session) => {
    // SIGNED_IN fires on first login and explicit re-auth. INITIAL_SESSION
    // fires on every cold start when supabase-js v2 restores a cached
    // session — without this, a reinstalled app whose FCM token timed out
    // mid-login would never re-bind on subsequent launches because
    // SIGNED_IN doesn't fire for cached sessions. USER_UPDATED covers
    // email/metadata changes that might coincide with an account switch.
    // Skip SIGNED_OUT — no user to bind the token to.
...
/**
 * Re-attempt token registration whenever the app returns to the foreground
 * if the previous save never reached the verified state. Re-installs are the
 * canonical failure case: a brand-new FCM token arrives before the user has
 * finished signing in, the in-flight retries time out, and without this hook
 * the token would stay unregistered until the next cold launch (which on iOS
 * may never happen if the user just backgrounds the app).
 */
function registerPushAppResumeListener() {
  if (!isNativeApp()) return;
  if (window._pushResumeListenerRegistered) return;
  const App = getAppPlugin();
  if (!App || typeof App.addListener !== 'function') return;
  App.addListener('appStateChange', (state) => {
    if (!state || !state.isActive) return;
    const status = window._pushTokenStatus;
    if (status === 'verified' || status === 'saved') return;
    if (!window._cachedFcmToken) return;
    console.log('[NativePush] App resumed with unverified token status (' + status + '), retrying register');
    reRegisterCachedFcmToken('app_resume');
  });
  window._pushResumeListenerRegistered = true;
  console.log('[NativePush] App resume listener registered — token will retry on foreground');
}
...
async function handleForegroundNotification(notification) {
  ...
  // coach_draft_ready: the native CoachDraftMessagingService already posted
  // the rich inline-reply notification via NotificationManager.notify().
  // When the app is foregrounded Android often suppresses the heads-up
  // banner, so surface an in-app DM banner too — that way Shannon actually
  // sees the incoming client message + can tap to open the conversation.
  // Skip the LocalNotifications re-fire below: it would land on the wrong
  // channel and overwrite the rich notification with "Meal Reminder"
  // fallback text.
  if (data.type === 'coach_draft_ready') {
    try {
      const senderId = data.clientId || '';
      const senderName = data.clientName || 'Client';
      const clientMsg = data.clientMessage || data.body || 'New message';
      if (typeof window.showDMNotificationBanner === 'function') {
        window.showDMNotificationBanner(senderName, '', clientMsg, senderId);
      }
      ...
      console.warn('[NativePush] coach_draft foreground banner failed:', e);
    }
    return;
  }
  ...
  try {
    ...
    const isFeedComment = data.type === 'feed_comment';
    const isCoachAlert = data.type === 'coach_alert';
    const channelId = isCoachAlert ? 'coach-alerts' : (isDM || isFeedComment) ? 'dm-messages' : 'meal-reminders';
    const summaryText = isCoachAlert ? 'Coach Alert' : isFeedComment ? 'Feed Comment' : isDM ? 'Direct Message' : 'Meal Reminder';

    await LocalNotifications.schedule({
      notifications: [{
        id: notifId,
        title: notification.title || (isFeedComment ? 'New feed comment' : isDM ? 'New Message' : 'Meal Reminder'),
        body: notification.body || (isFeedComment ? 'Someone commented on your post' : isDM ? 'You have a new message' : 'Time to log your meal!'),
 ...
 */
async function nativePushCurrentUserIsAdmin() {
  for (var i = 0; i < 20; i++) {
    if (window.currentUser) break;
    await new Promise(function(r) { setTimeout(r, 250); });
  }
  return !!(window.isBalanceAdminEmail && window.isBalanceAdminEmail(window.currentUser && window.currentUser.email));
}

async function approveDispatcherBatchFromPush(data) {
  try {
    const response = await fetch('/.netlify/functions/approve-ig-dispatch-batch', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        batchId: data.batchId || data.batch_id || '',
        batchVersion: data.batchVersion || data.batch_version || '',
        recipientId: data.recipientId || data.recipient_id || '',
        approvalToken: data.approvalToken || data.approval_token || ''
      })
    });
    ...
    if (!response.ok || !result.approved) {
      ...
      const message = stale
        ? 'That reply batch is no longer current. The dispatcher will collect a fresh one.'
        : 'Could not approve that reply batch. Please wait for the next notification.';
      if (typeof window.showToast === 'function') window.showToast(message, stale ? 'info' : 'error');
      console.warn('[NativePush] Dispatcher batch approval rejected:', response.status, result.reason || 'unknown');
      return false;
    }
    if (typeof window.showToast === 'function') {
      window.showToast(result.idempotent ? 'Reply batch was already approved.' : 'Reply batch approved. The dispatcher will re-check it now.', 'success');
    }
    return true;
  } catch (error) {
    console.error('[NativePush] Dispatcher batch approval failed:', error);
    if (typeof window.showToast === 'function') window.showToast('Could not approve that reply batch. Please try the next notification.', 'error');
    return false;
  }
}
...
async function handleNotificationTap(notification) {
  ...
  if (data.type === 'client_checkin_ready') {
    window._pbbPendingCheckinPush = true;
    window.dispatchEvent(new Event('pbbCheckinPush'));
    return;
  }

  if (data.type === 'dispatcher_approval_ready') {
    await approveDispatcherBatchFromPush(data);
    return;
  }

  if (data.type === 'coach_checkins_ready') {
    if (await nativePushCurrentUserIsAdmin()) {
      window.location.href = data.url || '/admin-dashboard.html?tab=checkins';
    } else {
      console.warn('[NativePush] Ignored admin check-in tap for non-admin user');
    }
    return;
  }
  ...
  if (data.type === 'coach_alert') {
    if (!(await nativePushCurrentUserIsAdmin())) {
      console.warn('[NativePush] Ignored coach alert tap for non-admin user');
      return;
    }
  ...
  if (data.type === 'feed_comment') {
    await openFeedPostFromPushData(data);
    return;
  }
  ...
  if (data.type === 'dm_message') {
    ...
    if (data.senderId && typeof window.openDirectMessage === 'function') {
      window.openDirectMessage(data.senderId, data.senderName || 'Message', '');
    } else if (typeof window.openFeedMessagesPanel === 'function') {
      window.openFeedMessagesPanel();
...
async function initLocalNotifications() {
  ...
  try {
    ...
    LocalNotifications.addListener('localNotificationActionPerformed', async (action) => {
      ...
      if (data.type === 'client_checkin_ready') {
        await handleNotificationTap({ data: data });
      } else if (data.type === 'meal_reminder') {
        if (typeof window.openMealInputModal === 'function') {
          window.openMealInputModal('notification');
          if (data.mealType && typeof window.selectMealType === 'function') {
        ...
        // Open the DM conversation directly when possible; fall back to the
        // messages inbox panel so the user can pick the thread themselves.
        if (data.senderId && typeof window.openDirectMessage === 'function') {
          window.openDirectMessage(data.senderId, data.senderName || 'Message', data.senderPhoto || '');
        } else if (typeof window.openFeedMessagesPanel === 'function') {
          window.openFeedMessagesPanel();
        } else if (typeof window.openMessageInbox === 'function') {
          window.openMessageInbox();
        ...
        if (!(await nativePushCurrentUserIsAdmin())) {
          console.warn('[NativePush] Ignored local coach alert tap for non-admin user');
          return;
        }
...
async function scheduleMealReminders(settings) {
  ...
  try {
    ...
    // This feature is retired. Keep this function as the single cleanup path
    // for returning phones, regardless of any stale settings passed by an old UI.
    ...
    // Android notification channels otherwise remain visible in system settings
    // after their feature is removed. Older plugin builds may not expose this.
    if (typeof LocalNotifications.deleteChannel === 'function') {
      try {
        await LocalNotifications.deleteChannel({ id: 'meal-reminders' });
      } catch (channelErr) {
        console.log('[MealReminder] Retired channel cleanup:', channelErr.message);
      }
    }
...
async function requestNativeNotificationPermission() {
  ...
  try {
    if (isIOS()) {
      const FitGotchiPush = getFirebaseMessaging();
      if (FitGotchiPush && typeof FitGotchiPush.checkPerm === 'function') {
        let permStatus = await FitGotchiPush.checkPerm();
        if (permStatus && permStatus.receive === 'granted') return 'granted';
        if (permStatus && permStatus.receive === 'denied') return 'denied';

        if (typeof FitGotchiPush.requestPerm === 'function') {
          const result = await FitGotchiPush.requestPerm();
          return result && result.receive ? result.receive : 'denied';
        }
      }
    }
...
async function checkNativeNotificationPermission() {
  ...
  try {
    if (isIOS()) {
      const FitGotchiPush = getFirebaseMessaging();
      if (FitGotchiPush && typeof FitGotchiPush.checkPerm === 'function') {
        const permStatus = await FitGotchiPush.checkPerm();
        return permStatus && permStatus.receive ? permStatus.receive : 'error';
      }
    }
...
function pushDebugBanner(msg, color) {
  try {
    // Opt-in: append ?pushdebug=1 to the URL OR set localStorage.pushDebug=1
    // to see the banners. Kept in the codebase because they're invaluable
    // when push regresses on iOS and a Mac with Web Inspector isn't handy.
    var url = (window.location && window.location.search) || '';
    var enabled = url.indexOf('pushdebug=1') !== -1 || localStorage.getItem('pushDebug') === '1';
    if (!enabled) return;
...
function _eagerIOSPushInit() {
  ...
  setTimeout(function () {
    ...
    // Dump what Capacitor actually sees so we can tell if FitGotchiPush
    // is registered at all (vs. registerPlugin returning a Proxy stub).
    try {
      var plugins = (window.Capacitor && window.Capacitor.Plugins) || {};
      var names = Object.keys(plugins).sort().join(',') || '(none)';
      pushDebugBanner('Capacitor plugins: ' + names.substring(0, 200), '#555');
    } catch (_) {}

    var FitGotchiPush = getFirebaseMessaging();
    if (!FitGotchiPush) {
      pushDebugBanner('FitGotchiPush NOT registered', '#cc0000');
      try { initNativePush(); } catch (_) {}
      return;
    }
    pushDebugBanner('calling requestPermissions()', '#0066cc');
    FitGotchiPush.requestPerm()
...
window.NativePush = {
  ...
  // Force-fetch a fresh FCM token from the platform and re-save it. Used by the
  // Settings "Refresh registration" button when push is broken after a reinstall.
  forceRefreshAndRegister: async function () {
    if (!isNativeApp()) return { ok: false, reason: 'not_native' };
    try {
      const token = await fetchFreshFcmToken('manual_force_refresh');
      if (!token) {
        return { ok: false, reason: window._pushTokenStatus || 'no_token' };
      }
      window._cachedFcmToken = token;
      // Reset status so saveNativePushToken's outcome is the new source of truth.
      window._pushTokenStatus = 'pending';
      await saveNativePushToken(token);
      return { ok: window._pushTokenStatus === 'verified' || window._pushTokenStatus === 'saved', status: window._pushTokenStatus };
    } catch (err) {
  ...
  getStatus: function () {
    return {
      native: isNativeApp(),
      hasToken: !!window._cachedFcmToken,
      tokenStatus: window._pushTokenStatus || 'unknown',
    };
  },
};
