...
assert.match(qualifierSource, /neuroscience and psychology of lasting change/);
assert.match(qualifierSource, /course is not positioned as plant-based/);
assert.match(clientContextSource, /why change feels hard; work with your energy; build a rhythm that sticks/);
...
assert.match(igDraftSource, /specific life hook -> daily rhythm or preference -> health\/fitness\/food\/energy context -> their goal or blocker -> one relevant Balance Learn idea/);
assert.match(igDraftSource, /normal organic DM conversation, not the paid-ad script/);
assert.match(igDraftSource, /Do not use quick replies, present a menu, or force the paid flow's fixed two-question sequence/);
