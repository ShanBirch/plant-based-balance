(function () {
    ...
    window.BalanceLessonReflections = {
        load, has: id => {
            ...
            const action=Array.from({length:6},(_,i)=>window.BalanceLearnWeeklyActions?.experiment(i+1)).find(a=>a?.lessonId===id && a.week <= (actions?.state?.current_week || 0));
        ...
        async open(context) {
            ...
            try {
                ...
                const action = Array.from({length:6},(_,i)=>window.BalanceLearnWeeklyActions?.experiment(i+1)).find(a=>a?.lessonId===context.lessonId && a.week <= (actions?.state?.current_week || 0));
