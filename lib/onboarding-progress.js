...
(function (root) {
    ...
    function isEstablishedMember(user = root.currentUser) {
        if (!user?.id || root.guestMode || TOUR_TEST_ACCOUNTS.has(String(user.id))) return false;
        return Date.parse(user.created_at || '') < NEW_MEMBER_TOUR_FROM
            || root.__balancePaidTourExemptUserId === user.id;
    }
    function needsFirstRunTour() {
        const user = root.currentUser;
        if (!user?.id || root.guestMode || root.isAdminViewing) return false;
        if (root.__balancePaidTourExemptUserId === user.id) return false;
    ...
    function isTourSuppressed() {
        ...
        if (isEstablishedMember()) return true;
    ...
    function needsCheckoutRecovery() {
        const user = root.currentUser;
        if (!user?.id || root.guestMode || root.isAdminViewing || root.isBalanceAdminEmail?.(user.email)) return false;
        if (!(Date.parse(user.created_at || '') >= NEW_MEMBER_TOUR_FROM)) return false;
        try { return !!(user.user_metadata?.balance_app_tour_completed_at || root.localStorage.getItem('pbb_app_tour_completed:' + user.id)); }
        catch (_) { return !!user.user_metadata?.balance_app_tour_completed_at; }
    }
    root.BalanceOnboardingProgress = { read, save, clear, isEstablishedMember, isTourSuppressed, needsFirstRunTour, completeFirstRunTour, completionDestination, needsCheckoutRecovery };
