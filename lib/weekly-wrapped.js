...
(function(){
    ...
    return `
      <div class="ww-level-ladder">
        ${levels.map((item, i) => {
          ...
          const label = typeof item === 'object' ? item.label : 'Skin';
          return `
  ...
  function levelSagaLogoFinalSlide(){
    return {
      ...
      html: `
        ...
        <div class="ww-big-num" style="font-size: clamp(2.2rem, 9vw, 4.4rem); margin-top:22px; line-height:1;">BALANCE</div>
        <div class="ww-caption" style="font-size:0.95rem; margin-top:6px; opacity:0.92;">Fitness Gamified</div>
        <div class="ww-caption" style="margin-top:16px;">Train your way from first-adventure energy to final-boss roster.</div>
  ...
  function buildLevelCharacterReel(){
    return [
      { bg: 'ww-bg-level-saga', hold_ms: 4200, html: `
        ...
        <div class="ww-big-num" style="font-size: clamp(3rem, 12vw, 6.2rem); line-height:1;">LEVEL<br>SAGA</div>
        <div class="ww-stamp gold" style="margin-top:22px;">CHARACTER VAULT</div>
      `},
      { bg: 'ww-bg-level-saga', hold_ms: 5000, html: `
        ...
        ${powerOrbRing('55')}
        <div class="ww-caption">The roster starts with early-adventure kid hero energy.</div>
      `},
      { bg: 'ww-bg-pbs', hold_ms: 5000, html: `
        <div class="ww-halftone"></div>
        <div class="ww-kicker" style="color:#1a0a00;">Then every 5 levels</div>
        <div class="ww-big-num" style="font-size: clamp(2.8rem, 12vw, 5.8rem); color:#1a0a00; line-height:1.02; text-shadow:4px 4px 0 #fff, 8px 8px 0 rgba(255,255,255,0.45);">another<br>fighter</div>
        ${levelLadder([
          { level: 55, label: 'Kid Hero' },
          { level: 60, label: 'Rival' },
          { level: 65, label: 'Mentor' },
          { level: 70, label: 'Future' },
          { level: 75, label: 'Super' },
        ])}
      `},
      { bg: 'ww-bg-level-saga', hold_ms: 5000, html: `
        <div class="ww-ki-aura"></div>
        ...
        ${levelLadder([
          { level: 80, label: 'Android' },
          { level: 85, label: 'Bio Boss' },
          { level: 90, label: 'Berserker' },
          { level: 95, label: 'Fusion' },
          { level: 99, label: 'Final Boss' },
        ])}
        <div class="ww-caption">From first quest to end-of-arc chaos, your level tells the story.</div>
      `},
      { bg: 'ww-bg-predict', hold_ms: 5000, html: `
        <div class="ww-speed-lines"></div>
        <div class="ww-ki-aura"></div>
        <div class="ww-kicker">When one unlocks</div>
        <div class="ww-unlock-card">
          <div class="ww-unlock-row"><span>Unlock celebration</span><b>BOOM</b></div>
          <div class="ww-unlock-row"><span>3D preview</span><b>SPIN</b></div>
          <div class="ww-unlock-row"><span>Equip Now button</span><b>GO</b></div>
        </div>
        <div class="ww-caption">Same rare-skin flow, now tied to your level grind.</div>
      `},
      ...
      { bg: 'ww-bg-level-saga', hold_ms: 5200, html: `
        ...
        <div class="ww-speed-lines"></div>
        ${powerOrbRing('99')}
        <div class="ww-kicker" style="margin-top:18px;">Build the roster</div>
        <div class="ww-caption">More levels. More skins. More reasons to keep showing up.</div>
  ...
  const REELS = {
    ...
    'level-saga': { name: 'Level 55 Character Unlocks', emoji: '🌟', theme: 'trap', build: buildLevelCharacterReel, needsFitgotchi: false },
