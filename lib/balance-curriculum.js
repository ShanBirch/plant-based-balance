...
function currentLessons() {
 const version=window.socialJourney?.getLearnCurriculum?.() || 'eight_v1';
 const path=window.BalanceLearnCurriculum?.weeks(version);
 if(!path)return lessons;
 const owned=new Map(path.flatMap(w=>w.lessonIds.map(id=>[id,w.number])));
 return lessons.map(row=>({...row,title:window.BalancePredictiveContent?.titles?.[row.id] || row.title})).map(row=>owned.has(row.id)?{...row,course:'learn',week:owned.get(row.id),required:true}:
   row.course==='learn'?{...row,course:'specialist',week:undefined,required:false}:row);
}
window.BalanceCurriculum={get lessons(){return currentLessons();},forCourse:(course,week)=>currentLessons().filter(l=>l.course===course&&(!week||l.week===week)),owner:id=>currentLessons().find(l=>l.id===id)};
})();
