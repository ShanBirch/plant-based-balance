/**
 * ⚠️ NOTE: This file is NOT actively used by the dashboard!
 * ⚠️ The dashboard uses: lib/learning-inline.js
 * ⚠️ If you need to make changes to the learning system, edit learning-inline.js instead.
 *
 * This file (learning.js) is kept for reference/future ES module migration.
 *
...
export async function initLearning() {
  ...
  try {
    ...
    // Helper to ensure value is an array (handles edge case where DB returns {} instead of [])
    const ensureArray = (val) => Array.isArray(val) ? val : [];

    if (data) {
      learningState.userProgress = {
        ...data,
        lessons_completed: ensureArray(data.lessons_completed),
        units_completed: ensureArray(data.units_completed),
        modules_completed: ensureArray(data.modules_completed),
        daily_status: data.daily_status || { can_learn: true, lessons_remaining: 3 }
      };
    } else {
      learningState.userProgress = {
        lessons_completed: [],
...
/**
 * Check order sequence - supports multiple valid orderings via game.acceptableOrders
 */
