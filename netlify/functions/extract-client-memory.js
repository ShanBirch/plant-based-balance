...
async function callOpenAIJSON(prompt) {
    ...
    const { data } = await callOpenAIModelChain({
        ...
        profile: 'default',
        label: 'extract-client-memory-openai',
...
async function callMemoryJSON(prompt) {
    ...
    if (GEMINI_API_KEY && !shouldUseOpenAIPrimary()) {
        try {
            ...
            console.warn(`[extract-memory] Gemini failed, falling back to OpenAI: ${err.message}`);
        }
    }
    if (OPENAI_API_KEY) return callOpenAIJSON(prompt);
    throw geminiError || new Error('No AI key configured for memory extraction');
}
...
exports.handler = async (event) => {
    ...
    if (!GEMINI_API_KEY && !OPENAI_API_KEY) {
        console.warn('[extract-memory] no AI key configured — skipping');
    ...
    try {
        ...
        console.error('[extract-memory] AI extraction failed:', err.message);
        return { statusCode: 200, body: JSON.stringify({ skipped: 'ai_failed' }) };
    }
