  ...
  if (wantToLose) {
    if (isGaining) {
      ...
      suggestion = {
        direction: 'decrease',
        amount: 150,
        reason: `I've noticed your weight has gone up about ${Math.abs(weightChange).toFixed(1)}kg over the past 2 weeks. That's okay - it happens! Sometimes we don't track every little thing perfectly, and that's totally normal.`,
        question: `I'd suggest a small calorie reduction to get things moving in the right direction. But if you think your tracking might have been a bit off, you can also give it another week at your current level and see how it goes.`
      };
    } else if (isStable) {
      ...
      suggestion = {
        direction: 'decrease',
        amount: 100,
        reason: `Your weight has been holding steady over the past 2 weeks. You've been consistent with your tracking which is great! A small calorie reduction could help get things moving.`,
        question: `If you feel like you might not have tracked everything 100% accurately, you could also try another week at your current calories and focus on logging everything.`
      };
    } else if (isLosing) {
      ...
      suggestion = {
        direction: 'none',
        amount: 0,
        reason: `You're doing brilliantly! You've lost ~${Math.abs(weightChange).toFixed(1)}kg over the past 2 weeks. Your current calories are working well - keep doing what you're doing!`,
        question: null
      };
    }
  } else if (wantToGain) {
    if (isLosing) {
      ...
      suggestion = {
        direction: 'increase',
        amount: 150,
        reason: `Your weight has dropped about ${Math.abs(weightChange).toFixed(1)}kg over the past 2 weeks. It can be tough to eat enough sometimes! A small calorie bump could help.`,
        question: `If you think you might have missed logging some meals or snacks, you could also try another week at your current level and make sure everything gets tracked.`
      };
    } else if (isStable) {
      ...
      suggestion = {
        direction: 'increase',
        amount: 100,
        reason: `Your weight has been holding steady over the past 2 weeks. A small calorie increase could help you start gaining.`,
        question: `If you feel like your tracking has been a bit off, you could try another week at your current calories and focus on logging everything accurately.`
      };
    } else if (isGaining) {
      // On track!
      suggestion = {
        direction: 'none',
        amount: 0,
        reason: `You're doing brilliantly! You've gained ~${weightChange.toFixed(1)}kg over the past 2 weeks. Your current calories are working well - keep it up!`,
        question: null
      };
    }
  } else {
    ...
    if (isGaining || isLosing) {
      ...
      suggestion = {
        ...
        amount: 100,
        reason: `Your weight has gone ${changeDir} about ${Math.abs(weightChange).toFixed(1)}kg, but your goal is to maintain. A small adjustment could help keep things steady.`,
        question: `If you think your tracking might have been a bit off lately, you could try another week at your current level first.`
      };
    } else {
      suggestion = {
        direction: 'none',
        amount: 0,
        reason: `Your weight is stable and you're right on track with your maintenance goal! Keep doing what you're doing.`,
        question: null
      };
    }
  }
