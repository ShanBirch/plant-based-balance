...
- The single scheduled Instagram Chrome operator is the 24-hour browser shift dispatcher. Its watchdog wakes every ten minutes, but only the renewable global lease owner works, for at most one 30-minute shift. It starts at the due lane in the durable eight-slot rotation and rolls forward through later lanes when one finishes early instead of ending the task. It actively works for 22 minutes, reserves eight minutes for reconciliation and clean Chrome release, claims one person at a time, and visits at most one full rotation per owned shift. Once the ten-interaction shift limit is reached, it keeps doing read-only inspection, reconciliation, queue preparation, and cursor repair until minute 22. Its replaced dedicated Instagram Chrome jobs remain paused so browser ownership never overlaps.
- The API-based `Balance Lead + Client DM Manager` is a separate 24-hour worker that wakes every ten minutes. It stops claiming new work after minute seven, reconciles and releases its own lock by minute nine, and uses a 15-minute crash-recovery lease. Its local file lock is never shared with the browser dispatcher, so a vanished API pass can suppress at most the immediately following wake instead of blocking replies for 90 minutes.
- Each owned shift may complete up to ten verified native Instagram interactions across all lane segments, with no separate messages/comments subcap. DMs, Story replies, public comments/replies, reactions, follows, and likes each count as one. Profile checks, Story views, skips, queue updates, and cursor work do not count. Startup and pre-action checks deduplicate canonical action IDs for the shift and carry cumulative `verified_native_actions` across segments; a watchdog replacing an abandoned worker inherits the unfinished shift's recent actions instead of resetting the count. Ten is a maximum, never a target to force, and per-person cooldowns, duplicate prevention, context gates, acquisition capacity, and native restrictions still apply.
- `ig_browser_shift_runs` is the dispatcher-level source of truth. Each lane visited in an owned shift is its own ledger segment: acquire with `start_ig_browser_shift(..., 600)` before acting, resume from the returned same-lane cursor, checkpoint with `heartbeat_ig_browser_shift(..., lease_seconds=600)` after every inspected person or attempted native action and at least every five minutes during read-only work, finalize a completed lane before immediately starting the next segment, and finalize the current segment as `partial` with exact `next_resume` at the timebox. If all lanes finish before minute 22, keep the final segment running and heartbeating during read-only work. `blocked` requires native restriction evidence. A first segment that receives `acquired=false` does no Instagram work; an abandoned owner can be replaced only after its renewable lease expires.
...
- Ranked Story nurturing gets one slot per four-hour dispatcher cycle with a base target of three verified replies per shift. Select the next-best eligible relationship from the ranked Story queue first, then visit that person's profile and check for a live Story. Do not use the broad tray to fill the ranked target.
- Story-tray discovery gets the second Story slot and works forward from a durable tray cursor, with a base target of five verified replies per shift. It assesses one owner and one exact paused frame at a time, creates or claims `story_operator` / `story_reply` with `p_supersede=false`, skips active conversations, clients, recent touches, duplicates, weak hooks, unclear or sensitive context, and records `source=story_tray_discovery`, new-versus-known lead, Story context, native/canonical IDs, and the next cursor. Story replies remain rapport-first and never contain a blind pitch or link.
