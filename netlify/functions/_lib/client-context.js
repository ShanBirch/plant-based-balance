...
- Balance Learn path: the branded page is https://plantbased-balance.org/founders. It explains the one AUD $149 payment for the full six weeks and the no-subscription, no-auto-renewal terms, collects legal acceptance, and opens Stripe Checkout. Payment is not complete until Stripe confirms it. After a successful purchase, the buyer is taken to https://plantbased-balance.org/login.html?action=signup to create or access their Balance account.
- Paid-Meta app preview: the ad destination is the Balance Learn course page at https://plantbased-balance.org/founders with the campaign UTMs and Meta identifiers retained. Verified Facebook or Instagram paid-social traffic sees a preview-first action on that page; organic visitors keep the normal purchase action. The legacy https://plantbased-balance.org/meta-app-preview.html route redirects to the Balance Learn page with attribution retained. The person completes onboarding locally, gets their personalised setup, then sees a guided walkthrough before a non-dismissible Balance Learn offer at AUD $149 once for the fixed six-week course, with no automatic renewal. Closing setup or skipping the tour shows only continue/restart or pay; it never exposes an ungated app. The compact gate records legal acceptance, opens Stripe Checkout, and returns the buyer to create or access their Balance account with the preview answers preserved. Existing members keep their normal app path.
- The comeback onboarding asks only for information that changes calculations, safety, training, food, or coaching context: setup type, name, age, height, weight, the outcome they want in their own words, their usual blocker, realistic weekly capacity, routine window, equipment, current activity and energy, movement limitations, food style/restrictions, and optional cycle-aware training. Balance infers the six-week direction, starter duration, split, learning focus, and first three Weekly Goals. Members can edit these later. Their answers are coaching context, so do not ask for the same information again.
...
- Block if the draft says the Founders Pass has no 1:1 or one-to-one coaching. It includes six weeks of one-to-one in-app coaching support; only instant/unlimited access and fully customised weekly plan reviews are excluded.
