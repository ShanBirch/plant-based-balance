-- Morning Pulse — Alert Type Migration
--
-- Adds `morning_pulse` to coach_alerts.alert_type CHECK constraint.
-- Fired by the `morning-pulse-scan` scheduled Netlify function (daily,
-- targeting the coach's clients' morning).
--
-- Uses the DROP+ADD pattern proven in coach_alerts_add_incoming_dm.sql +
-- onboarding_sequence_migration.sql. Idempotent.
