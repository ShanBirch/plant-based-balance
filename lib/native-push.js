...
async function handleForegroundNotification(notification) {
  ...
  // For meal reminders (and other notifications), fire a local notification
  // so the user sees it on screen with sound, vibration, and badge dot
  try {
    ...
    await LocalNotifications.schedule({
      notifications: [{
        id: notifId,
        title: notification.title || 'Meal Reminder',
        body: notification.body || 'Time to log your meal!',
        largeBody: notification.body || 'Time to log your meal!',
...
async function initLocalNotifications() {
  ...
  try {
    ...
    // Create the meal-reminders notification channel on startup so both
    // local and FCM notifications can use it immediately (badge dots and
    // heads-up display require the channel to exist)
    try {
      ...
      console.log('[MealReminder] Notification channel created');
    } catch (channelErr) {
