...
BEGIN
    ...
    -- Bring every already-published comment into the same exact tiny reaction
    -- contract so the live Feed is consistent immediately.
    WITH comments_to_normalize AS (
        SELECT id,
               row_number() OVER (ORDER BY created_at ASC NULLS LAST, id ASC) AS position
          FROM public.feed_comments
         WHERE user_id = v_tahlia_id
           AND lower(trim(comment_text)) NOT IN ('love this', 'amazing work', 'good job')
    )
    UPDATE public.feed_comments comments
       SET comment_text = CASE mod(comments_to_normalize.position - 1, 3)
           WHEN 0 THEN 'love this'
           WHEN 1 THEN 'amazing work'
           ELSE 'good job'
       END
      FROM comments_to_normalize
     WHERE comments.id = comments_to_normalize.id;
