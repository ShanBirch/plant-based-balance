...
assert.ok(dashboard.includes('dashboard-script-10-points_widget_functions.js?v=41'), 'share renderer cache key must be bumped');
assert.ok(serviceWorker.includes("const CACHE_NAME = 'pbb-app-v271'"), 'service worker cache must be bumped');
assert.ok(serviceWorker.includes('dashboard-script-10-points_widget_functions.js?v=41'), 'service worker must precache the new renderer');
