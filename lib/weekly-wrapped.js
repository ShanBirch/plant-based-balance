...
(function(){
  ...
  async function buildData(userId){
    ...
    const trackedMealDays = [...new Set(meals.map(m => m.meal_date).filter(Boolean))].length;
    ...
    return {
      ...
      nutrition: { meals_logged: mealsCount, avg_calories: caloriesAvg, tracked_days: trackedMealDays, window_days: 7 },
