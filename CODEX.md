...
- Default avatar-video workflow: create and approve the Shannon voice track in ElevenLabs first, then use HeyGen Video Translation to lip-sync an approved reusable Shannon source clip to that audio. Do not ask HeyGen to generate the complete Reel from a prompt. Use normal HeyGen Video/avatar generation only when Video Translation cannot produce a usable result, no suitable source clip exists, quality fails, or Shannon explicitly asks for a fresh avatar generation. Build the final Reel separately from the lip-synced presenter, real footage, graphics, captions and music.
