...
assert.match(dashboard, /dashboard-script-6-ai_coach_draft_mode_logic_auth\.js\?v=47-dm-voice-wake-lock/);
...
assert.match(messaging, /navigator\.wakeLock\.request\('screen'\)/);
assert.match(messaging, /startsWith\('audio\/mp4'\)\) dmVoiceRecorder\.start\(\)/);
assert.match(messaging, /if \(!String\(dmVoiceRecorder\.mimeType \|\| ''\).*startsWith\('audio\/mp4'\)\)/);
...
assert.match(upload, /function hasValidAudioHeader\(bytes, contentType\)/);
assert.match(upload, /recording was interrupted\. Please record it again\./);
