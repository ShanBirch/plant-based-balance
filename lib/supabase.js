...
const db = {
  ...
  /**
   * Cardio sessions table operations
   */
  cardioSessions: {
    /**
    ...
    async create(userId, sessionData) {
      const { data, error } = await supabase
        ...
        .insert([{
          ...
          session_time: sessionData.time || new Date().toTimeString().split(' ')[0],
          activity_type: sessionData.activityType,
          activity_name: sessionData.activityName || null,
          duration_minutes: sessionData.duration,
          distance_km: sessionData.distance || null,
          distance_unit: sessionData.distanceUnit || 'km',
          intensity_level: sessionData.intensity || 'moderate',
          perceived_effort: sessionData.perceivedEffort || null,
          calories_burned: sessionData.calories || null,
          avg_heart_rate: sessionData.avgHeartRate || null,
          max_heart_rate: sessionData.maxHeartRate || null,
          activity_data: sessionData.activityData || {},
          location_name: sessionData.location || null,
          is_outdoor: sessionData.isOutdoor || false,
    ...
    /**
     * Get cardio sessions for a specific date
     */
    ...
    /**
     * Get cardio sessions by activity type
     */
    async getByActivityType(userId, activityType, limit = 50) {
      const { data, error } = await supabase
        .from('cardio_sessions')
        .select('*')
        .eq('user_id', userId)
    ...
    async getStats(userId) {
      const { data, error } = await supabase
        .rpc('get_cardio_stats', { p_user_id: userId });
    ...
    /**
     * Get total cardio session count for a user
     */
    ...
    /**
     * Get recent cardio activity summary
     */
    async getRecentSummary(userId, days = 7) {
      ...
      const { data, error } = await supabase
        .from('cardio_sessions')
        .select('activity_type, duration_minutes, distance_km, calories_burned, session_date')
        .eq('user_id', userId)
      ...
      const summary = {
        ...
        totalDuration: data.reduce((sum, s) => sum + (s.duration_minutes || 0), 0),
        totalDistance: data.reduce((sum, s) => sum + (parseFloat(s.distance_km) || 0), 0),
        totalCalories: data.reduce((sum, s) => sum + (s.calories_burned || 0), 0),
        activeDays: [...new Set(data.map(s => s.session_date))].length,
        byActivity: {}
      };

      // Group by activity type
      data.forEach(session => {
        if (!summary.byActivity[session.activity_type]) {
          summary.byActivity[session.activity_type] = {
            count: 0,
            totalDuration: 0,
            totalDistance: 0
          };
        }
        summary.byActivity[session.activity_type].count++;
        summary.byActivity[session.activity_type].totalDuration += session.duration_minutes || 0;
        summary.byActivity[session.activity_type].totalDistance += parseFloat(session.distance_km) || 0;
      });
    ...
    /**
     * Check cardio milestones and award points
     */
    async checkMilestones(userId) {
      const newMilestones = [];
      const sessionCount = await this.getCount(userId);

      // Session count milestones
      const countMilestones = [1, 5, 10, 25, 50, 100, 200, 365];
      for (const threshold of countMilestones) {
        if (sessionCount >= threshold) {
          const exists = await db.milestones.checkExists(userId, 'cardio_count', threshold);
          if (!exists) {
            const milestone = await db.milestones.create(userId, {
              type: 'cardio_count',
              value: threshold,
              data: { total_cardio: sessionCount }
            });
            newMilestones.push({
              ...milestone,
              message: this.getMilestoneMessage(threshold)
    ...
    getMilestoneMessage(count) {
      const messages = {
        1: 'First cardio session logged! Great start! 🎉',
        5: '5 cardio sessions complete! Keep moving! 💪',
        10: 'Double digits! 10 cardio sessions! 🔥',
        25: '25 cardio sessions! You\'re on fire! 🌟',
        50: 'Halfway to 100! 50 cardio sessions! 🏆',
        100: 'CENTURY! 100 cardio sessions complete! 👑',
        200: 'Cardio machine! 200 sessions! 🚀',
        365: 'One full year of cardio! 365 sessions! 🎊'
      };
      return messages[count] || `${count} cardio sessions complete! Amazing! 💪`;
    }
  },
