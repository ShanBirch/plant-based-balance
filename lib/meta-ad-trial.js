/* Balance paid-Meta guided preview.
 *
...
(function (root, factory) {
    ...
    const WEEKLY_PRICE_ID = 'balance_learn_weekly';
    let selectedPriceId = META_FOUNDATIONS_PRICE_ID;
    ...
    function isNativeApp() {
        try {
            return !!(window.Capacitor?.isNativePlatform?.()
                ...
                || /FitGotchi-Native/i.test(window.navigator?.userAgent || ''));
        } catch (_) { return false; }
    }
    function resetCheckoutButton() {
        checkoutBusy = false;
        const button = window.document?.getElementById('meta-ad-trial-checkout-btn');
        if (button) { button.disabled = false; button.textContent = 'START MY SIX WEEKS'; }
    }
    const offers = {};
    function selectedPlan() { return selectedPriceId === WEEKLY_PRICE_ID ? WEEKLY_PRICE_ID : 'balance_foundations_six_week'; }
    function selectPaymentOption(priceId) {
        if (checkoutBusy) return;
        if (![META_FOUNDATIONS_PRICE_ID, WEEKLY_PRICE_ID].includes(priceId)) return;
        selectedPriceId = priceId;
        const terms = window.document?.getElementById('meta-ad-trial-terms');
        if (terms) terms.checked = false;
        window.document?.querySelectorAll('[data-learn-payment]').forEach(button => {
            const active = button.getAttribute('data-learn-payment') === priceId;
            button.setAttribute('aria-pressed', String(active));
            button.style.background = active ? '#171923' : '#fffdf8';
            button.style.color = active ? '#ffffff' : '#171923';
            button.style.webkitTextFillColor = active ? '#ffffff' : '#171923';
        });
        renderFoundationsOffer(offers[priceId] || { plan: selectedPlan(), unitAmount: priceId === WEEKLY_PRICE_ID ? 2483 : (Date.now() < Date.parse('2026-10-21T00:00:00+10:00') ? 14900 : 45000), currency: 'AUD' });
        loadFoundationsOffer();
    }
    ...
    const FOUNDATIONS_COMPLETE_KEY = 'pbb_meta_preview_foundations_complete_v1';
    ...
    const ATTRIBUTION_KEYS = [
        ...
        'placement', 'site_source_name', 'meta_ad_name', 'meta_ref', 'learn_entry'
    ];
    let gateTimer = null;
    let countdownTimer = null;

    function formatOfferAmount(offer) {
        const unitAmount = Number(offer && offer.unitAmount);
        if (!Number.isInteger(unitAmount) || unitAmount < 0) return '';
        const currency = String(offer.currency || 'AUD').toUpperCase();
        const amount = unitAmount / 100;
        return currency + ' $' + amount.toLocaleString('en-AU', {
            minimumFractionDigits: unitAmount % 100 ? 2 : 0,
            maximumFractionDigits: 2
        });
    }

    function renderFoundationsOffer(offer) {
        if (!window.document || !offer || !['balance_foundations_six_week', WEEKLY_PRICE_ID].includes(offer.plan)) return false;
        const formatted = formatOfferAmount(offer);
        if (!formatted) return false;
        if (offer.plan !== 'balance_learn_weekly') {
            const upfrontLabel = window.document.getElementById('learn-upfront-price-label');
            if (upfrontLabel) upfrontLabel.textContent = formatted + ' upfront';
        }
        const priceCopy = window.document.getElementById('meta-ad-trial-price-copy');
        const termsCopy = window.document.getElementById('meta-ad-trial-terms-price');
        const exitButton = window.document.getElementById('meta-ad-trial-exit-pay');
        const weekly = offer.plan === WEEKLY_PRICE_ID;
        const copy = weekly ? 'AUD $24.83 every week with a six-week minimum ($148.98 minimum total). After six weeks, cancel any time before your next weekly payment. Access continues through your paid period, with no additional notice period. Same weekly check-in, workout and food review, app and community access.' : 'One ' + formatted + ' payment for the full six weeks. No subscription or automatic renewal.';
        if (priceCopy) priceCopy.textContent = weekly ? 'AUD $24.83 every week. Six-week minimum ($148.98), then cancel any time.' : copy;
        if (termsCopy) termsCopy.textContent = weekly ? (offer.disclosure || copy) : 'One ' + formatted + ' payment covers the full six weeks and does not renew automatically.';
        if (exitButton) exitButton.textContent = 'CHOOSE MY PAYMENT OPTION';
        return true;
    }

    function loadFoundationsOffer() {
        const priceId = selectedPriceId;
        if (!window.fetch) return Promise.resolve(null);
        return window.fetch('/.netlify/functions/get-checkout-offer?priceId=' + encodeURIComponent(priceId), {
            method: 'GET', headers: { Accept: 'application/json' }
        }).then(function (response) {
            if (!response.ok) throw new Error('offer_unavailable');
            return response.json();
        }).then(function (payload) {
            offers[priceId] = payload.offer;
            if (selectedPriceId === priceId) renderFoundationsOffer(payload.offer);
    ...
    function cleanStatedGoal(value) {
        const goal = String(value || '').replace(/\s+/g, ' ').trim();
        return goal ? goal.charAt(0).toUpperCase() + goal.slice(1) : '';
    }

    function readPersonalisedSetup() {
        const local = storage('localStorage');
        const session = storage('sessionStorage');
        const localProfile = safeParse((local && local.getItem('userProfile')) || '{}', {});
        const sessionProfile = safeParse((session && session.getItem('userProfile')) || '{}', {});
        const profile = Object.assign({}, localProfile, sessionProfile);
        const goalItems = safeParse((local && local.getItem('onboardingGoalIntents')) || '[]', []);
        const statedGoal = cleanStatedGoal(
            profile.why_now
            || (profile.goal_catcher && profile.goal_catcher.why_now)
            || (profile.onboarding_chat_freeform && profile.onboarding_chat_freeform.why_now)
        );
        const inferredGoal = Array.isArray(goalItems) && goalItems.length
            ? String(goalItems[0].label || goalItems[0].id || '')
            : String(profile.primary_goal || profile.goal || profile.goal_type || 'a repeatable first six weeks');

        const calendar = safeParse((local && local.getItem('workoutCalendar')) || 'null', null);
        let trainingDays = 0;
        if (calendar && typeof calendar === 'object') {
            trainingDays = Object.keys(calendar).filter(function (day) {
                const workout = String(calendar[day] || '').toLowerCase();
                return workout
                    && workout !== 'rest'
                    && !workout.startsWith('recovery-')
                    && !workout.startsWith('yoga-');
            }).length;
        }
        if (!trainingDays) trainingDays = Number(profile.training_frequency || profile.workouts_per_week || 0);

        let foodValues = profile.dietary_requirements || profile.dietary_preferences || profile.diet || [];
        if (!Array.isArray(foodValues)) foodValues = String(foodValues || '').split(',');
        foodValues = foodValues.map(humanise).filter(Boolean).slice(0, 2);

        return {
            goal: statedGoal || humanise(inferredGoal) || 'A repeatable first six weeks',
            training: trainingDays > 0 ? trainingDays + ' planned workout' + (trainingDays === 1 ? '' : 's') + ' each week' : 'A realistic weekly workout plan',
            food: foodValues.length ? foodValues.join(' and ') + ' meal plan' : 'A meal plan matched to your preferences'
        };
    }

    function renderPersonalisedSetup() {
        if (!window.document) return false;
        const setup = readPersonalisedSetup();
        const summary = window.document.getElementById('meta-ad-trial-personal-summary');
        if (summary) {
            Object.keys(setup).forEach(function (key) {
                const target = summary.querySelector('[data-summary="' + key + '"]');
                if (target) target.textContent = setup[key];
            });
        }
        const intro = window.document.getElementById('meta-tour-welcome-intro');
        if (intro) intro.textContent = 'Your answers are in. We will now show you the workout, food and weekly actions Balance has set up around ' + setup.goal.toLowerCase() + '.';
        return true;
    }
    ...
    function isVerifiedEntry(input) {
        ...
        if (['website', 'app_signup'].includes(params.get('learn_entry')) && params.get('account_first') === '1') return true;
    ...
    function resetOnboardingForNewTrial() {
        ...
        [
            ...
            'pbb_fitgotchi_needs_character_setup', 'ai_meal_plan',
            'workoutCalendarTimes', 'pbb_onboarding_owner_user_id'
        ].forEach(function (key) { local.removeItem(key); });
        // Paid onboarding uses the clean Home layout. Hide FitGotchi before
        // dashboard markup paints so a stored character cannot flash onscreen.
        local.setItem('pbb_fitgotchi_visibility', 'hidden');
        local.setItem('pbb_fitgotchi_needs_character_setup', 'true');
        ...
        session.removeItem(FOUNDATIONS_COMPLETE_KEY);
    }

    function activate(input, sourceLabel) {
        const params = normalizeParams(input);
        if (!isVerifiedEntry(params)) return false;
        const accountFirst = params.get('account_first') === '1';
        ...
        const state = writeState({
            ...
            claimedAt: null,
            accountFirst: accountFirst || !!(existing && existing.accountFirst),
            attribution: attribution
        });
        preserveAttribution(attribution);
        const session = storage('sessionStorage');
        if (session) {
            if (accountFirst) session.removeItem('guestMode');
            else session.setItem('guestMode', 'true');
        }
    ...
    function activateSignup(userId, input) {
        if (!userId) return false;
        const params = normalizeParams(input);
        const existing = readState();
        // A newly created account owns a new sales journey, even when someone
        // else already used or purchased a preview on this phone.
        if (existing && (existing.claimedAt || (existing.ownerUserId && existing.ownerUserId !== userId))) {
            storage('localStorage')?.removeItem(STATE_KEY);
        }
        if (!activate(params, 'account_signup')) return false;
    ...
    function isActive() {
        ...
        if (state.ownerUserId && window.currentUser?.id && state.ownerUserId !== window.currentUser.id) return false;
        const session = storage('sessionStorage');
        if (state.nativePaymentVerifiedUserId && state.nativePaymentVerifiedUserId === state.ownerUserId) return false;
        return !!(state.accountFirst || window.metaAdTrialMode || (session && session.getItem('guestMode') === 'true'));
    }

    function restoreAuthenticatedMode(userId, user = window.currentUser) {
        const state = readState();
        // Account-first setup belongs to its signed-in account, not to the URL
        // that originally opened it. Do not turn a real member into a guest.
        const established = user?.id === userId && window.BalanceOnboardingProgress?.isEstablishedMember(user);
        // Only activateSignup may bind an unowned preview to a new account.
        // Login must never adopt a visitor's preview, even on the same phone.
        const matches = !!(!established && state && !state.claimedAt && state.accountFirst
            && state.ownerUserId === userId);
        if (established && state && (!state.ownerUserId || state.ownerUserId === userId)) {
            window.BalanceOnboardingProgress?.clear();
            storage('localStorage')?.removeItem(STATE_KEY);
            storage('sessionStorage')?.removeItem(CLAIM_KEY);
            ['meta-ad-trial-gate', 'meta-ad-trial-exit-choice'].forEach(id => {
                const element = window.document?.getElementById(id);
                if (element) element.style.display = 'none';
            });
            window.document?.documentElement.classList.remove('pbb-meta-trial-gated');
        }
        if (matches) { state.ownerUserId = userId; writeState(state); }
        window.metaAdTrialMode = matches;
        return matches;
    }

    function openCheckoutGate() {
        const state = readState();
        if (state && !state.claimedAt && (!state.ownerUserId || state.ownerUserId === window.currentUser?.id)) {
            window.metaAdTrialMode = true;
            return showGate(true, 'manual_unlock');
        }
        // Ordinary guest/member entry also opens the existing payment screen.
        // Never send a temporarily hydrating session to login from this CTA.
        return showGate(true, 'manual_unlock', true);
    }

    function track(eventType, metadata) {
        const progressStep = { trial_gate_shown: 'payment_gate', checkout_started: 'payment_opened', trial_preview_started: 'app_preview', trial_walkthrough_completed: 'walkthrough' }[eventType];
        if (progressStep) window.BalanceOnboardingFunnel?.track('payment', progressStep, eventType === 'trial_walkthrough_completed' ? 'completed' : 'viewed', { mode: 'meta_preview' });
        const state = readState();
    ...
    function onWalkthroughComplete(options) {
        ...
        if (!state || state.claimedAt) return false;
        const session = storage('sessionStorage');
        if (!session || session.getItem(FOUNDATIONS_COMPLETE_KEY) !== 'true') {
            track('trial_walkthrough_blocked', {
                trial_stage: 'walkthrough',
                reason: 'foundations_lesson_incomplete'
            });
            return false;
        }
        const now = Date.now();
        ...
        if (!state.previewStartedAt) {
            ...
            track('trial_preview_started', { duration_seconds: null, completion_triggered: true });
        }
        state.interruptedStage = null;
        writeState(state);
        showGate(true, 'walkthrough_completed');
        return true;
    }
    ...
    function updateBanner() {
        ...
        label.textContent = state && state.onboardingCompletedAt
            ? 'Finish your guided tour to see the Balance Learn option'
            : 'Set up your plan, then take the full guided tour';
    }
    ...
    function showInboxPreview() {
        // The same coach note is used by signed-in first-run onboarding, after
        // the trial has been claimed. It must not depend on an active ad preview.
        if (!window.document || (!isActive() && !window.__balanceGuidedTourActive)) return false;
        const preview = window.document.getElementById('meta-ad-trial-inbox-preview');
        if (!preview) return false;
        preview.style.display = 'flex';
        if (typeof preview.setAttribute === 'function') preview.setAttribute('aria-hidden', 'false');
    ...
    function checkoutCompliance(email) {
        ...
        return {
            ...
            document_versions: {
                terms: '2026-09-09', privacy: '2026-05-19',
                client_agreement: '2026-09-09', refund_policy: '2026-09-09'
            },
            profile: profile,
            ...
            idempotency_key: complianceSessionId + ':checkout_session_created:' + selectedPlan()
        };
    }
    ...
    function resolveKnownCheckoutEmail() {
        const session = storage('sessionStorage');
        const local = storage('localStorage');
        let sessionProfile = {};
        let localProfile = {};
        try { sessionProfile = safeParse((session && session.getItem('userProfile')) || '{}', {}); } catch (_) {}
        try { localProfile = safeParse((local && local.getItem('userProfile')) || '{}', {}); } catch (_) {}
        const candidates = [
            window.currentUser && window.currentUser.email,
            session && session.getItem(CHECKOUT_EMAIL_KEY),
            session && session.getItem('userEmail'),
            local && local.getItem('userEmail'),
            sessionProfile.email,
            localProfile.email
        ];
        return candidates.map(function (value) { return String(value || '').trim().toLowerCase(); })
            .find(function (value) { return value !== 'guest@preview.local' && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value); }) || '';
    }

    let paymentRecoveryBusy = false;
    async function recoverNativePayment() {
        if (!isNativeApp() || paymentRecoveryBusy) return false;
        const state = readState();
        const pending = state && state.nativePayment;
        if (!pending || state.claimedAt || Date.now() - pending.createdAt > 86400000) return false;
        paymentRecoveryBusy = true;
        try {
            const session = window.authHelpers?.getSession ? await window.authHelpers.getSession()
                : (await window.supabaseClient?.auth?.getSession())?.data?.session;
            if (!session?.access_token || !pending.userId || session.user?.id !== pending.userId) return false;
            const endpoint = ['balance_learn_weekly', 'app_community_monthly'].includes(pending.plan)
                ? 'claim-meta-trial-subscription' : 'claim-founders-pass';
            const response = await window.fetch('/.netlify/functions/' + endpoint, {
                method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: 'Bearer ' + session.access_token },
                body: JSON.stringify({ sessionId: pending.sessionId })
            });
            const result = await response.json();
            if (!response.ok || !result.claimed || result.expired || (result.status && !['active', 'trialing'].includes(result.status))) return false;
            const latest = readState();
            const currentSession = window.authHelpers?.getSession ? await window.authHelpers.getSession()
                : (await window.supabaseClient?.auth?.getSession())?.data?.session;
            if (currentSession?.user?.id !== pending.userId || latest?.nativePayment?.sessionId !== pending.sessionId || latest.ownerUserId !== pending.userId) return false;
            latest.nativePaymentVerifiedUserId = pending.userId;
            delete latest.nativePayment;
            writeState(latest);
            const saved = storage('sessionStorage');
            saved?.removeItem(PAYMENT_SESSION_KEY);
            saved?.removeItem(PAYMENT_PLAN_KEY);
            saved?.setItem(CLAIM_KEY, 'true');
            saved?.removeItem('guestMode');
            // Authenticated dashboard startup saves preview work and finishes onboarding.
            window.location.replace('/dashboard.html');
            return true;
        } catch (_) { return false; }
        finally { paymentRecoveryBusy = false; }
    }

    function resumeNativePayment() {
        recoverNativePayment();
        // Stripe may still be settling when the browser first brings the app forward.
        if (isNativeApp() && readState()?.nativePayment) {
            window.setTimeout(recoverNativePayment, 2000);
            window.setTimeout(recoverNativePayment, 6000);
        }
    }

    async function beginCheckout() {
        if (await recoverNativePayment()) return true;
        ...
        try {
            ...
            if (session) session.setItem(PAYMENT_PLAN_KEY, selectedPlan());
            if (result.offer) renderFoundationsOffer(result.offer);
            track('checkout_started', {
                plan: selectedPlan(),
                amount_aud: result.offer && Number.isInteger(Number(result.offer.unitAmount))
                    ? Number(result.offer.unitAmount) / 100
                    : null,
            ...
            if (result.url && window.location) {
                if (isNativeApp()) {
                    ...
                    const paymentState = readState();
                    if (paymentState?.ownerUserId && result.sessionId) {
                        paymentState.nativePayment = { sessionId: result.sessionId, plan: selectedPlan(), userId: paymentState.ownerUserId, createdAt: Date.now() };
                        writeState(paymentState);
                    }
                    // Keep the signed-in app and payment gate in place while Stripe opens.
                    if (typeof window.NativePermissions?.openExternalBrowser === 'function') {
                        window.NativePermissions.openExternalBrowser(result.url);
                        resetCheckoutButton();
                        return true;
                    }
                }
            ...
            checkoutBusy = false;
            track('checkout_error', { plan: selectedPlan(), message: error && error.message || 'request_failed' });
    ...
    function showGate(force, trigger, manualPayment) {
        if (!window.document || (!isActive() && !manualPayment)) return false;
        const state = manualPayment ? null : readState();
        if (!manualPayment && (!state || (!force && (!state.deadlineAt || Date.now() < state.deadlineAt)))) return false;
        ...
        hideExitChoice();
        hideInboxPreview();
        window.pauseFeatureTourForCheckout?.();
        gate.style.display = 'flex';
        if (state) window.BalanceOnboardingProgress?.save('checkout', {});
        const emailInput = window.document.getElementById('meta-ad-trial-email');
        const session = storage('sessionStorage');
        if (emailInput && !emailInput.value) emailInput.value = resolveKnownCheckoutEmail();
        window.document.documentElement.classList.add('pbb-meta-trial-gated');
        const tour = window.document.getElementById('guided-tour-overlay');
        if (tour) tour.classList.remove('active');
        if (state && !state.gateShownAt) {
            state.gateShownAt = Date.now();
            writeState(state);
            track('trial_gate_shown', { elapsed_seconds: null, trigger: trigger || (force ? 'manual_unlock' : 'walkthrough_completed') });
        }
        if (state && (force || state.interruptedStage)) {
    ...
    function showInterruptedGateWhenParsed(state) {
        if (!window.document || !state || state.interruptedStage !== 'checkout') return false;
        let attempts = 0;
        const reveal = function () {
            if (!isActive()) return;
            if (window.document.getElementById('meta-ad-trial-gate')) {
                showGate(true, 'checkout_reload');
                return;
            }
            attempts += 1;
            if (attempts < 120) window.setTimeout(reveal, 25);
        };
        reveal();
        return true;
    }
    ...
    // Shannon requested one replay on the confirmed phone test account.
    // This resets navigation gates only, never setup, learning, points or billing.
    const PHONE_REPLAY_USER_ID = 'e6781875-f657-4f91-91e1-1289c0e345f8';
    const PHONE_REPLAY_KEY = 'pbb_phone_tour_replay:2026-09-07:' + PHONE_REPLAY_USER_ID;
    function hasPendingPhoneReplay(user) {
        // Retired: the verified server onboarding flag now drives test resets.
        // A device-local replay could skip setup or reopen after a completed run.
        return false;
    }

    async function replayPhoneTestTour() {
        const user = window.currentUser;
        if (window.isAdminViewing || !hasPendingPhoneReplay(user) || typeof window.startFeatureTour !== 'function') return false;
        const client = window.supabaseClient;
        if (!client || !client.auth) return false;
        const auth = await client.auth.getUser();
        if (auth.error || !auth.data || auth.data.user?.id !== PHONE_REPLAY_USER_ID) return false;
        const profile = await client.from('users').select('id,is_test_account').eq('id', PHONE_REPLAY_USER_ID).single();
        if (profile.error || !profile.data?.is_test_account || window.currentUser?.id !== PHONE_REPLAY_USER_ID) return false;
        const local = storage('localStorage');
        const session = storage('sessionStorage');
        const state = readState();
        // Never change a paid/claimed preview or an in-flight payment handoff.
        if (state?.claimedAt || session?.getItem(CLAIM_KEY) || session?.getItem(PAYMENT_SESSION_KEY)) return false;
        const nextState = Object.assign({}, state || {
            variant: VARIANT, source: 'phone_test_replay', activatedAt: Date.now(),
            accountFirst: true, attribution: {}
        }, {
            onboardingCompletedAt: state?.onboardingCompletedAt || Date.now(),
            walkthroughCompletedAt: null, previewStartedAt: null,
            deadlineAt: null, gateShownAt: null, interruptedStage: null
        });
        writeState(nextState);
        local.removeItem('featureTourComplete');
        ['meal_plan_intro', 'shopping_list_intro', 'nutrition_tracker_intro', 'workout_week_intro',
            'feed_intro', 'foundations_intro', 'coach_checkin_intro', 'coach_message_intro', 'weekly_goals_intro']
            .forEach(id => local.removeItem('pbb_onboarding_step_seen:' + PHONE_REPLAY_USER_ID + ':' + id));
        if (session) session.removeItem(FOUNDATIONS_COMPLETE_KEY);
        ['meta-ad-trial-gate', 'meta-ad-trial-exit-choice'].forEach(id => {
            const element = window.document && window.document.getElementById(id);
            if (element) element.style.display = 'none';
        });
        window.metaAdTrialMode = true;
        window.startFeatureTour('full', { metaPreview: true, restart: true });
        if (window.__balanceGuidedTourActive !== true) return false;
        local.setItem(PHONE_REPLAY_KEY, '1');
        return true;
    }
    ...
    function init() {
        ...
        const params = window.location ? new URLSearchParams(window.location.search) : new URLSearchParams();
        if (isVerifiedEntry(params) && params.get('fresh_preview') === '1') {
            const local = storage('localStorage');
            if (local) local.removeItem(STATE_KEY);
            resetOnboardingForNewTrial();
            // Treat fresh_preview as a one-shot instruction. Leaving it in the
            // address would erase the test journey again on every app reload.
            try {
                const cleanUrl = new URL(window.location.href);
                cleanUrl.searchParams.delete('fresh_preview');
                window.history.replaceState({}, window.document ? window.document.title : '', cleanUrl.pathname + cleanUrl.search + cleanUrl.hash);
            } catch (_) {}
        }
        ...
        if (state && !state.claimedAt && !isClaiming && session && (state.accountFirst || session.getItem('guestMode') === 'true' || isDashboard)) {
            if (state.accountFirst) session.removeItem('guestMode');
            else session.setItem('guestMode', 'true');
            window.metaAdTrialMode = true;
        }
        // DOMContentLoaded waits for the dashboard's deferred scripts. On a
        // slower phone that briefly exposed the app beneath a completed paid
        // preview. Reveal the already-required checkout as soon as its markup
        // has been parsed instead.
        showInterruptedGateWhenParsed(state);
        if (window.document) {
            window.document.addEventListener('visibilitychange', function () {
                if (window.document.visibilityState === 'visible') {
                    resumeNativePayment();
                    nativeCheckoutOpened = false;
                    resetCheckoutButton();
                }
            });
            window.addEventListener?.('focus', resumeNativePayment);
            window.addEventListener?.('pbbInitComplete', resumeNativePayment);
            window.addEventListener?.('pageshow', function () {
                resumeNativePayment();
                if (nativeCheckoutOpened) {
                    nativeCheckoutOpened = false;
                    resetCheckoutButton();
                }
            });
            const ready = function () {
                ...
                updateBanner();
                scheduleGate();
                // Setup and tour checkpoints resume automatically after auth and
                // dashboard startup. A previous close is not a request to exit.
                if (state && state.interruptedStage === 'checkout') {
                    showGate(true);
                }
            };
    ...
    const api = {
        ...
        FOUNDATIONS_COMPLETE_KEY: FOUNDATIONS_COMPLETE_KEY,
        isVerifiedEntry: isVerifiedEntry,
        extractAttribution: extractAttribution,
        readState: readState,
        hasPendingPhoneReplay: hasPendingPhoneReplay,
        replayPhoneTestTour: replayPhoneTestTour,
        activate: activate,
        activateSignup: activateSignup,
        activateFromNativeQuery: activateFromNativeQuery,
        isActive: isActive,
        restoreAuthenticatedMode: restoreAuthenticatedMode,
        track: track,
        ...
        renderPersonalisedSetup: renderPersonalisedSetup,
        scheduleGate: scheduleGate,
        showGate: showGate,
        showInterruptedGateWhenParsed: showInterruptedGateWhenParsed,
        openCheckoutGate: openCheckoutGate,
        beginCheckout: beginCheckout,
        beginSignup: beginSignup,
        signupUrl: signupUrl,
        markSignupHandoff: markSignupHandoff,
        hasPendingClaim: hasPendingClaim,
        recoverNativePayment: recoverNativePayment,
