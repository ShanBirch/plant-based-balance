...
async function handleNotificationTap(notification) {
  ...
  if (data.type === 'dm_message') {
    ...
    // Track unread sender immediately so the inbox shows the dot (skip for game invites)
    ...
    if (isGameInvite) {
      // Game invites: Try to open the DM with the sender which now has an explicit Accept button
