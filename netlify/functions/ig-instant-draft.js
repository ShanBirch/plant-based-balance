...
function hasRecentPaidMetaProofVideo(history = []) {
    return (Array.isArray(history) ? history : [])
        .filter(item => String(item?.direction || '').toLowerCase() === 'out')
        .slice(-8)
        .some(item => /\b(?:quick video|showing you how it works inside Balance)\b|balance-foundations-(?:app-proof-v(?:5|6)|course-first-v(?:7|8))|balance-learn-dm-(?:149|450)-v(?:9|10|11|13|14|15|16|17)/i
...
Return JSON: {"acknowledgement":"one or two natural sentences, normally 15 to 35 words, at most 360 characters","evidence":["exact short quote from the lead for each personal detail you used"]}. Do not use em dashes or en dashes. Use commas or full stops. Stop once you have explained the practical fit; do not pad to a word target or append a generic sentence about Balance Learn making things easier to repeat.
Keep hypothetical contrasts grounded too. Saying "without restarting every day", "instead of giving up" or "so you stop feeling guilty" still introduces a behaviour or emotion the person may never have reported. Only mention resets, quitting, guilt or all-or-nothing patterns if they supplied that pattern. Otherwise finish with the actual support adjustment.
Choose support by what would actually address the stated obstacle, not just its broad topic. Before returning, check that your suggested change explains WHY it could help this particular concern. A repeatable plan alone does not address boredom with repeating bland meals, and ordinary ingredients alone do not improve flavour. In that case, connect meal preferences and the weekly food review to adjusting lunches they do not enjoy, without guaranteeing endless variety. Budget, taste, cooking facilities and tracking preferences are different needs; do not default every food concern to supermarket ingredients or mention cost/fancy products unless relevant. Apply this cause-and-fit check to every obstacle, not only food.
...
Do not bundle separate obstacles into awkward phrases such as "the kids and chocolate part". Explain the relevant support naturally. If children and chocolate are mentioned together, the fit is a flexible food routine that can include chocolate and be adjusted around family life, not an invented busy evening or a suggestion that simple meals solve parenting. Do not add "busy nights", "rushed mornings", or any other unsupplied circumstance. Before returning, read your sentence as an actual DM: remove jargon, generic filler, and claims that the supplied facts do not support.
...
function collectPaidMetaWriterContractIssues({ draft = {}, currentMessage = '', qualifier = {}, history = [], flowVariant = 'plant_based_control' } = {}) {
    ...
    if ((draft?.personalAcknowledgement || /\b(?:Balance )?Learn\b/i.test(reply)) && /\$\s*(?:149|450)\b/i.test(reply)) {
    ...
    if (asksMealPlanQuestion
        ...
        issues.push(broadFlow
            ? 'Answer the meal-plan question directly before progressing: yes, Balance Learn includes meal-plan support fitted to dietary preferences.'
            : 'Answer the meal-plan question directly before progressing: yes, Balance Learn includes a plant-based meal plan.');
    }
...
function buildPaidMetaGuaranteedContractFallback({ draft = {}, currentMessage = '', issues = [], qualifier = {}, history = [], flowVariant = 'plant_based_control' } = {}) {
    ...
    if (/Unverified lesson captions/i.test(issueText)) {
        ...
        joined = flowVariant === 'broad_pain'
            ? `Yeah, I do. Balance Learn includes meal-plan support fitted to your dietary preferences alongside your workout program and weekly check-in. ${nextQuestion}`
            : `Yeah, I do. Balance Learn includes a plant-based meal plan alongside your workout program and weekly check-in. ${nextQuestion}`;
        ...
        if (metaAdConversationFastLane && !thread.linked_user_id && blockingPaidMetaContractIssues.length === 0) {
            ...
            if (personalised !== draft) {
                ...
                currentAlertData = await persistCocosDraftRepair({
                    alertId,currentAlertData,draft,challengeOfferWarning,
                    repairField:'paid_meta_personal_acknowledgement',
                    repairMeta:{status:'fallback',reason:draft.personalAcknowledgementFailure,candidate:draft.personalAcknowledgementCandidate,attempted_at:new Date().toISOString()},
                });
            }
        }
        if (metaAdConversationFastLane && !thread.linked_user_id && blockingPaidMetaContractIssues.length === 0) {
