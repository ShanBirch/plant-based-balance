...
function getLatestWorkoutSessionRows(rows) {
    const sorted = (rows || [])
        ...
        .sort((a, b) => new Date(b.created_at || b.workout_date || 0) - new Date(a.created_at || a.workout_date || 0));

    if (!sorted.length) return [];
    const anchorTime = new Date(sorted[0].created_at || sorted[0].workout_date || Date.now()).getTime();
    if (!Number.isFinite(anchorTime)) return sorted;

    return sorted.filter(row => {
        const rowTime = new Date(row.created_at || row.workout_date || 0).getTime();
        if (!Number.isFinite(rowTime)) return true;
        return Math.abs(anchorTime - rowTime) <= 45 * 60 * 1000;
    });
}

function buildWorkoutCardPayloadFromRows(rows) {
    const workoutRows = Array.isArray(rows) ? rows : [];
    const first = workoutRows[0] || {};
    const workoutDate = first.workout_date || new Date().toISOString().slice(0, 10);
    ...
    const totalVolumeLabel = totalVolume > 0
        ...
        : '';
    const dateLabel = workoutDate ? new Date(`${workoutDate}T00:00:00`).toLocaleDateString(undefined, {
        weekday: 'short',
        month: 'short',
        day: 'numeric'
    }) : '';
    ...
    return {
        card_type: 'workout',
        workout_name: exercises.length === 1 ? exercises[0].name : 'Workout complete',
...
window.shareLatestWorkoutToFeed = async function(options = {}) {
    ...
    try {
        const rows = await getLatestWorkoutRowsForFeedShare();
        const cardPayload = buildWorkoutCardPayloadFromRows(rows);
...
window.openFeedComposerShareSource = async function(source) {
    ...
    if (shareSource === 'workout') {
        setFeedComposerShareActionLoading(true, 'Workout');
        try {
            await window.shareLatestWorkoutToFeed();
        } finally {
            setFeedComposerShareActionLoading(false);
        }
        return;
...
function renderFeedMentionHandle(handle) {
    ...
    return `<span class="feed-mention-text" style="color:#2563eb; -webkit-text-fill-color:#2563eb; font-weight:800;">${escapeHtml(label)}</span>`;
}
...
function renderFeedPostActionsMenu(storyId, variant = 'card') {
    ...
    const triggerStyles = isViewer
        ? 'background:rgba(255,255,255,0.12); color:white; -webkit-text-fill-color:white; border:1px solid rgba(255,255,255,0.22);'
    ...
    const itemStyles = isViewer
        ? 'background:transparent; color:white; -webkit-text-fill-color:white;'
