...
(function () {
  ...
  function scaleIngredients(ingredients, multiplier) {
    return (ingredients || []).map(ingredient => {
      ...
      return {
        ...ingredient,
        amount: ingredient.scalable === false ? ingredient.amount : scaleAmountString(ingredient.amount, multiplier)
      };
    });
  }
  ...
  function buildPreparedMealPlan(userTargets, foodPreferences) {
    const library = window.BALANCE_PREPARED_MEAL_LIBRARY;
    if (!library) throw new Error('Prepared meal-plan library not loaded');
    if (typeof library.buildPlan === 'function') return library.buildPlan(userTargets || {}, foodPreferences || {});
    const template = library.selectTemplate(foodPreferences || {});
    const styleLabel = template.style.charAt(0).toUpperCase() + template.style.slice(1);
    const restrictionLabels = template.restrictions.map(value => ({
      low_fodmap: 'low-FODMAP', gluten_free: 'gluten-free', dairy_free: 'dairy-free',
      nut_free: 'nut-free', soy_free: 'soy-free'
    })[value] || value);
    const selectedDescription = 'A prepared ' + template.style + ' meal for your Balance week'
      + (restrictionLabels.length ? ', with your selected ' + restrictionLabels.join(', ') + ' preferences' : '') + '.';
    const calorieGoal = Number(userTargets?.calorie_goal) || BASE_DAILY_CALORIES;
    const multiplier = calorieGoal / BASE_DAILY_CALORIES;
    const days = library.expandTemplate(template).map(day => ({
      ...day,
      meals: day.meals.map(placement => ({
        meal_slot: placement.meal_slot,
        meal_time: placement.meal_time,
        recipe_id: placement.recipe_id,
        ...scaleRecipe(library.RECIPES[placement.recipe_id], multiplier),
        description: selectedDescription,
        tags: ['prepared-library', template.style, ...restrictionLabels]
      }))
    }));
    const plan = {
      plan_name: 'Your ' + styleLabel + ' Balance Week',
      plan_description: 'Ready instantly from your saved eating style and restrictions.',
      template_id: template.id,
      template_key: template.key,
      library_version: library.VERSION,
      dietary_requirements: [template.style, ...template.restrictions],
      diet_type: template.style,
      total_meals: 35,
      current_week: 1,
      calorie_goal: calorieGoal,
      protein_goal_g: userTargets?.protein_goal_g || null,
      ...
      weeks: [{
        week_number: 1,
        theme: 'Simple meals worth repeating',
        theme_description: 'One menu Monday to Wednesday, one Thursday to Friday and one across the weekend.',
        days
      }]
    };
    const audit = auditScaledPlan(plan);
    if (!audit.ok) throw new Error(`Prepared meal plan audit failed: ${audit.issues.slice(0, 4).join('; ')}`);
    return plan;
  }

  async function populatePreparedMealPlan(supabase, userId, userTargets, foodPreferences) {
    return persistBuiltMealPlan(supabase, userId, buildPreparedMealPlan(userTargets || {}, foodPreferences || {}));
  }
   ...
   */
  async function persistBuiltMealPlan(supabase, userId, scaled) {
    if (!supabase || !userId) {
      throw new Error('persistBuiltMealPlan: supabase + userId required');
    }
    if (!scaled || !Array.isArray(scaled.weeks) || scaled.weeks.length === 0) {
      throw new Error('persistBuiltMealPlan: a complete meal plan is required');
    }
    ...
    const planInsert = await supabase
      ...
      .insert({
        ...
        // Do not expose a half-written plan if a child insert fails.
        status: 'generating',
        ...
        diet_type: scaled.diet_type || 'vegan',
        total_meals: scaled.weeks.reduce((count, week) => count + (week.days || []).reduce((dayCount, day) => dayCount + (day.meals || []).length, 0), 0),
        current_week: scaled.current_week || 1
      })
      .select('id')
      .single();

    if (planInsert.error) throw planInsert.error;
    planId = planInsert.data.id;
    ...
    const mealsInsert = await supabase
      ...
      .insert(mealRows)
      .select('id,week_number,day_of_week,meal_slot,name');
    if (mealsInsert.error) throw mealsInsert.error;

    const insertedMeals = Array.isArray(mealsInsert.data) ? mealsInsert.data.slice() : [];
    for (const week of scaled.weeks) {
      for (const day of week.days || []) {
        for (const meal of day.meals || []) {
          const matchIndex = insertedMeals.findIndex(row =>
            Number(row.week_number) === Number(week.week_number)
            && Number(row.day_of_week) === Number(day.day_of_week)
            && String(row.meal_slot || '') === String(meal.meal_slot || '')
            && String(row.name || '') === String(meal.name || '')
          );
          if (matchIndex >= 0) {
            meal.id = insertedMeals[matchIndex].id;
            insertedMeals.splice(matchIndex, 1);
          }
        }
      }
    }
    ...
    // A failed build or child insert must never remove the member's current plan.
    // Retire older plans only after all new rows have been saved and activated.
    try {
      const retirement = await supabase.from('ai_generated_meal_plans')
        .update({ status: 'archived' }).eq('user_id', userId).eq('status', 'active').neq('id', planId);
      if (retirement.error) console.warn('[meal-plan] old plan retirement needs retry:', retirement.error);
    } catch (retirementError) {
      console.warn('[meal-plan] keeping the complete new plan after retirement failed:', retirementError);
    }
    ...
    return {
      ...
      if (planId) {
        try {
          const cleanup = await supabase.from('ai_generated_meal_plans').delete().eq('id', planId).eq('user_id', userId);
          if (cleanup.error) console.warn('[meal-plan] incomplete plan cleanup failed:', cleanup.error);
        } catch (cleanupError) {
          console.warn('[meal-plan] incomplete plan cleanup failed:', cleanupError);
        }
      }
      throw error;
    }
  ...
  async function populateVeganChallengeMealPlan(supabase, userId, userTargets) {
    // Cohort auto-enrolment must honour the same saved choices as onboarding.
    const { data, error } = await supabase.from('user_food_preferences')
      .select('*').eq('user_id', userId).maybeSingle();
    if (error) throw error;
    const preferences = { ...(data || {}), diet_type: data?.diet_type || 'vegan' };
    return populatePreparedMealPlan(supabase, userId, userTargets || {}, preferences);
  }
  ...
  window.buildPreparedMealPlan = buildPreparedMealPlan;
  window.persistBuiltMealPlan = persistBuiltMealPlan;
  window.populateVeganChallengeMealPlan = populateVeganChallengeMealPlan;
  window.populatePreparedMealPlan = populatePreparedMealPlan;
