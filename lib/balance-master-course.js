...
(function () {
    ...
    const EXTRA_STAGES = [
    {
        ...
        "lessons": [
            [
                "Start with the job",
                "Choose exercises by the movement and muscles you want to train, then check whether you can perform them comfortably with the equipment you have. The included lessons connect your exercise choices to your training goal."
            ],
            [
                "Build coverage",
    ...
    },
    {
        ...
        "lessons": [
            ...
            [
                ...
                "The included lessons introduce micronutrients and a food-first approach. Detailed supplement lessons remain in the specialist library. Use food labels and reliable sources to check what a product supplies; a supplement is not a substitute for a workable food pattern."
            ],
            [
    ...
    const actionPlan = () => window.BalanceMasterActions;
    const actionProgress = (week, data = current()) => actionPlan().progress(data, week);
    ...
    function workoutValid(workout, available = templates) {
        if (!filled(workout.split) || !filled(workout.splitReason)) return false;
        if (!Array.isArray(workout.dayReasons) || workout.dayReasons.length !== 7 || workout.dayReasons.some(reason => !filled(reason))) return false;
    ...
    function stageReady(i, data = current()) {
        const s = STAGES[i];
        if (!window.BalanceCourseLayout.requirements("master", i + 1).complete) return false;
        ...
        if (!filled(data.quizReflections?.[i])) return false;
        if (!actionProgress(i + 1, data).complete) return false;
        return s.project === 'workout' ? workoutValid(data.workout || {}) : s.project === 'meal' ? mealValid(data.meal || {}) : true;
    }
    ...
    function event(name, extra = {}) { context?.track?.(`master_${name}`, { stage: stageIndex + 1, curriculum_version: 'balance_master_10_weeks_v2', assessment_version: 'master_actions_v4', form_check_requirement: 'master_compound_video_v1', ...extra }); }
    async function load(force = false) {
        ...
        loadPromise = (async () => {
            ...
            if (session.data?.session?.user?.id === userId) {
                const synced = await window.supabaseClient.rpc('sync_balance_master_actions');
                if (synced.error || !synced.data?.data) throw new Error('Could not check your saved actions. Please retry.');
                if (uid() !== userId) throw new Error('Your account changed. Open the course again.');
                state = { ...blank(), ...migrate(synced.data.data) };
            }
    ...
    async function save(data, completedWeek = null) {
        ...
        try {
            ...
            const session = await window.supabaseClient.auth.getSession();
            if (session.data?.session?.user?.id !== userId) throw new Error('Course submissions must be made from the member’s own signed-in account.');
            const result = await window.supabaseClient.rpc('save_balance_master_assessment', { p_data: data, p_week: completedWeek });
            if (result.error || !result.data) throw new Error(result.error?.message || 'Your work was not saved. Please retry.');
            if (uid() !== userId) throw new Error('Your account changed. Open the course again.');
            state = result.data.data; event('project_saved', { complete: stageDone(stageIndex) });
            updateChecklist();
            status('Saved. Completed actions and their dates are recorded for you and your coach.');
            return true;
    ...
    function checklist(data = current()) {
        const week = stageIndex + 1, p = actionProgress(week, data);
        return '<h3>Your required actions</h3><p><strong>' + p.completed + '/' + p.total + ' actions recorded</strong>. Save your work to record completed actions automatically. Quizzes and reflections are required separately.</p><div class="course-topic-list">' + actionPlan().weeks[stageIndex].map((a, i) => {
            const receipt = actionPlan().receipt(data, week, a.key), done = receipt?.isCurrent === true;
            return window.BalanceCourseLayout.row({ title: a.title, description: a.hint, kicker: 'Action ' + (i + 1) + (done ? ' · Saved ' + new Date(receipt.updatedAt).toLocaleString('en-AU') : receipt ? ' · Update required; previous work kept' : ' · Required'), status: done ? 'Done' : 'Start', number: done ? '✓' : i + 1, complete: done, attributes: 'type="button" data-master-jump="' + a.key + '"' });
        }).join('') + '</div>';
    }
    function updateChecklist() {
        const el = document.getElementById('master-action-checklist');
        if (!el) return;
        el.innerHTML = checklist();
        const completion = document.querySelector('.master-complete');
        if (completion) completion.hidden = !progress().isComplete;
        el.querySelectorAll('[data-master-jump]').forEach(button => button.onclick = () => {
            const key = button.dataset.masterJump;
            const targets = { split:'split', workouts:'builder', schedule:'day-0', 'day-reasons':'reason-0', meals:'needs', shopping:'shopping', prep:'prep', backup:'backup' };
            const target = document.querySelector('[name="action-' + key + '"]') || document.querySelector('[data-lift="' + key + '"]') || (targets[key] === 'builder' ? document.querySelector('[data-master-action="builder"]') : document.querySelector('[name="' + targets[key] + '"]'));
            target?.scrollIntoView({block:'center',behavior:'smooth'}); target?.focus({preventScroll:true});
        });
    }
    function writtenActions(data) {
        const rows = actionPlan().weeks[stageIndex].filter(a => a.kind === 'written');
        return rows.map((a, i) => '<fieldset><legend>Action ' + (i + 1) + ': ' + esc(a.title) + '</legend>' + field(a.hint, 'action-' + a.key, data.actionAnswers?.[stageIndex + 1]?.[a.key], 'Write at least 20 characters with enough detail for Shannon to understand your work.') + '<button type="button" data-master-action="save">Save action</button></fieldset>').join('') + (data.reflections?.[stageIndex] ? '<details><summary>Your earlier practical answer</summary><p>' + esc(data.reflections[stageIndex]) + '</p><p>Use this to help answer the separate actions above. Your earlier answer has been kept.</p></details>' : '');
    }
    ...
    function workoutForm(data) {
        ...
        return `<h3>Your workout program</h3><p>Build and save each workout with the existing exercise library, then return here and link it to your week. This course project keeps your plan separate from your current coached schedule.</p>
            ...
            ${field('Your muscle split', 'split', w.split, 'For example: three full-body days, or upper / lower.')}
            ${field('Why this split fits you', 'splitReason', w.splitReason, 'Explain how it supports your goal, available days and recovery.')}
            ...
            ${DAYS.map((day,i)=>field(day + ' explanation', 'reason-'+i, w.dayReasons?.[i], 'For a training day: explain the muscles, exercise choices and order. For a rest day: explain the recovery or other activity.')).join('')}
    ...
    function collect() {
        ...
        if (project === 'workout') {
            ...
            ['split', 'splitReason', 'goal', 'constraints', 'coverage', 'progression'].forEach(key => w[key] = values[key] || '');
            w.dayReasons = DAYS.map((_,i)=>values['reason-'+i] || '');
        ...
        data.actionAnswers ||= {};
        data.actionAnswers[stageIndex + 1] ||= {};
        actionPlan().weeks[stageIndex].filter(a => a.kind === 'written').forEach(a => { data.actionAnswers[stageIndex + 1][a.key] = values['action-' + a.key] || ''; });
        data.quizReflections ||= {};
        data.quizReflections[stageIndex] = values.quizReflection || '';
        return data;
    }
    let showOverview = true;
    function unlocked(i) { return i < window.BalanceCourseWeeks.available(startedAt, 10) || current().completedStages?.[i] === true; }
    function render(data = current()) {
        const host = document.getElementById('learning-content'); if (!host) return;
        if (showOverview || !startedAt || !context?.unlocked) {
            const locked = !context?.unlocked;
            host.innerHTML = window.BalanceCourseLayout.overview({course:'master',title:'Balance Master',subtitle:'Build your workout program and one week of meals',description:'Learn how your body moves, practise your compound lifts and build two plans you can use. One week opens every seven days from the day you start.',weeks:STAGES.map((stage,i)=>({...stage,actions:actionProgress(i+1),done:!locked&&stageDone(i),locked:locked|| (!startedAt?i>0:!unlocked(i)),opens:!locked&&startedAt&&!unlocked(i)?window.BalanceCourseWeeks.date(startedAt,i):''})),progress:locked?{completed:0,percent:0}:progress(),locked,started:!!startedAt,primary:startedAt?'Continue course':'Start my ten weeks',onPrimary:'id="master-start"',number:2});
            const start = async (week=0) => { if (busy) return; busy=true; try { if (!startedAt) { startedAt=await window.BalanceCourseWeeks.start('master'); event('course_started'); } stageIndex=unlocked(week)?week:0; showOverview=false; render(); window.scrollTo({top:0,behavior:"auto"}); context?.onWeek?.(); } catch(e){status(e.message,true);} finally{busy=false;} };
            host.querySelector('#master-start')?.addEventListener('click',()=>start(Math.max(0,STAGES.findIndex((_,i)=>unlocked(i)&&!stageDone(i)))));
            host.querySelectorAll('[data-course-week]').forEach(button=>button.onclick=()=>start(Number(button.dataset.courseWeek)));
            host.querySelector('[data-course-library]').onclick=()=>leave(context?.library);
            return;
        }
        if (!unlocked(stageIndex)) stageIndex=0;
        const s = STAGES[stageIndex], p = progress();
        host.innerHTML = `<div id="course-learning-home" class="course-detail-page course-structured course-week-page">
            ${window.BalanceCourseLayout.weekHeader('Master',stageIndex+1,10,s.title,s.outcome)}
            <section class="course-week-section"><h3>Learn this week</h3>${window.BalanceCourseLayout.introduction(s.lessons)}<div class="course-topic-list">${window.BalanceCourseLayout.lessonRows('master',stageIndex+1)}</div></section>
            <section class="master-course"><h3>Your weekly practice</h3><div id="master-action-checklist"></div><p>Finish this week’s quizzes and reflections, save all required actions, then complete the week. Saved means submitted for review; Shannon can review your reasoning and technique.</p>${data.submittedAt?.[stageIndex] ? `<p class="master-submission-receipt">Last submitted: ${esc(new Date(data.submittedAt[stageIndex]).toLocaleString('en-AU'))}</p>` : ''}
            ...
            ${field('What did you learn in this quiz, and how will you use it?', 'quizReflection', data.quizReflections?.[stageIndex], 'Required for every week, including your workout and meal projects.')}
            ${s.project === 'workout' ? workoutForm(data) : s.project === 'meal' ? mealForm(data) : writtenActions(data)}
            ${stageIndex === 1 ? videoForm() : ''}
            <div class="master-actions"><button type="button" data-master-action="save">Save progress</button><button type="submit" class="master-primary">Check and complete week</button></div><p id="master-status" role="status"></p></form>
            ...
            <button type="button" data-master-action="overview">View course overview</button>
        ...
        host.querySelector('#master-form').addEventListener('submit', async e => {
            ...
            if (!window.BalanceCourseLayout.requirements('master',stageIndex+1).complete) { status('Complete this week’s lesson quizzes and save each reflection before completing the week.',true); return; }
            if (!filled(draft.quizReflections?.[stageIndex])) { status('Write what you learned and how you will use it before completing the week.',true); return; }
            if (!(await save(draft))) return;
            if (stageIndex !== submittedStage || uid() !== submittedOwner || document.getElementById('master-form') !== submittedForm) return;
            draft = current();
            if (!stageReady(stageIndex, draft)) { status('Finish and save every required action in the checklist. For your workout and meal projects, complete the linked planner details.', true); return; }
            if (await save(draft, stageIndex + 1)) { if (!previouslyComplete) event('stage_completed'); if (!coursePreviouslyComplete && progress().isComplete) event('course_completed'); render(); status('Week complete. Your submission is recorded for you and your coach.'); }
        });
        updateChecklist();
    ...
    async function action(button) {
        ...
        if (type === 'videos') { await loadSubmissions(); if (!(await save(current()))) return; render(); status(submissionError || 'Video submissions checked.', !!submissionError); }
        if (type === 'formcheck') {
        ...
        if (type === 'overview') { showOverview=true; render(); window.scrollTo({top:0,behavior:'auto'}); }
        ...
        if (type === 'builder') {
            try {
                if (typeof window.openWorkoutBuilderSafe !== 'function') throw new Error('The workout builder is still loading. Please retry.');
                const returnOptions = {...context,week:stageIndex+1};
                await window.openWorkoutBuilderSafe();
                window.balanceMasterBuilderReturn = async () => {
                    // Keep the current course context; the generic tab initializer resets it to the library.
                    window.hideAllAppViews();
                    ...
                    await open(returnOptions);
                };
                event('workout_builder_opened');
    ...
    async function open(options) {
        ...
        if (!host) return;
        showOverview = !Number.isInteger(options.week);
        if (!showOverview) stageIndex=options.week-1;
        if (!options.unlocked) { render(); return; }
        host.innerHTML = '<div class="master-course"><section><p>Loading your saved course projects...</p></section></div>';
        try { await load(true); if (context !== options) return; render(); window.scrollTo({top:0,behavior:'auto'}); event('opened'); }
    ...
    async function back() { if (!showOverview && context?.unlocked) { if (document.getElementById('master-form') && !(await save(collect()))) return; showOverview=true; render(); window.scrollTo({top:0,behavior:"auto"}); } else await leave(context?.library); }
    async function recordVideoSubmission(userId) {
        if (uid() !== userId) return;
        try {
            const result = await window.supabaseClient.rpc('sync_balance_master_actions');
            if (result.error || !result.data?.data) throw new Error('Your video was sent. Open Master again to refresh its action record.');
            if (uid() !== userId || owner !== userId) return;
            state = { ...blank(), ...migrate(result.data.data) };
            await loadSubmissions();
            if (uid() !== userId) return;
            updateChecklist();
            status('Your video was sent and its required action is recorded.');
        } catch (error) { status('Your video was sent. Open Master again to refresh its action record.', true); }
    }
    window.BalanceMaster = { back, open, progress, load, leave, recordVideoSubmission, cancel: () => { context = null; }, stages: STAGES, actionProgress, workoutValid, mealValid, stageDone, migrate, unlocked };
})();
