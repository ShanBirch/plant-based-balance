...
- The single scheduled Instagram Chrome operator is the 24-hour browser shift dispatcher. Its watchdog wakes every five minutes, but only the renewable global lease owner works, for at most one 30-minute shift. It starts at the due lane in the durable eight-slot rotation and rolls forward through later lanes when one finishes early instead of ending the task. It actively works for 22 minutes, reserves eight minutes for reconciliation and clean Chrome release, claims one person at a time, and visits at most one full rotation per owned shift. Once the rolling action ceiling is reached, it keeps doing read-only inspection, reconciliation, queue preparation, and cursor repair until minute 22. Its replaced dedicated Instagram Chrome jobs remain paused so browser ownership never overlaps.
- The daytime ceiling rolls across the previous 30 minutes: four verified native Instagram actions, with no more than three communication actions (DMs, Story replies, or public comments). A follow or like may use the optional fourth slot. Startup and pre-action checks deduplicate canonical action IDs across recent ledger segments and subtract standardized `verified_native_actions` and `communication_actions`; incomplete evidence uses the higher conservative count, never an assumed zero. Profile checks, Story views, skips, queue updates, and cursor work do not count. From 01:00-04:00 Brisbane the rolling ceiling is one verified native action.
...
- Ranked Story nurturing gets two slots per four-hour dispatcher cycle with a base target of three verified replies per shift. Select the next-best eligible relationship from the ranked Story queue first, then visit that person's profile and check for a live Story; do not start from random Story-tray discovery. A Story reply is rapport-first and never a blind pitch.
