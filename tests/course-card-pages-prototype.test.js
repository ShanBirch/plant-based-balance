...
assert.match(learning, /\$\{action\.type === 'learn_experiment' && canAct \? 'View' : complete \? 'Done' : canAct \? 'Open' : \(action\.availabilityButtonLabel \|\| 'Later'\)\}/);
assert.doesNotMatch(learning, /function renderFoundationsWeekAction[\s\S]{0,1200}'Locked'/);
assert.match(learning, /const actionLabel = action\.type === 'learn_experiment' && canAct \? 'View' : isComplete \? 'Done' : canAct \? 'Open' : \(week\.isCurrent && timingBlocked\)[\s\S]*locked \? 'Later' : 'Not done'/);
