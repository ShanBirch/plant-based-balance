...
- Codex task notifications remain the primary review and approval surface for DM-manager and Instagram-dispatcher decisions. The existing Balance admin Your Call card is the durable secondary notice: eligible pending Instagram/Facebook manager and browser-dispatcher cards show the exact Codex task name to open. This fallback is informational only, creates no second alert row, sends no client message, grants no approval, and disappears with the source card lifecycle so it cannot become a duplicate or stale prompt. Do not add email fallback unless a verified transactional sender is already configured.
