...
(function(){
  ...
  async function buildData(userId){
    ...
    const weeklyGoalRow = (weeklyGoalsR.status === 'fulfilled' && weeklyGoalsR.value && !weeklyGoalsR.value.error)
      ...
      : null;
    const weeklyGoals = buildWeeklyGoalsSummary(weeklyGoalRow);
  ...
  function buildWeeklyGoalsSummary(row, rewardResult){
    if (!row) return { total: 0, completed: 0, completion_rate: 0, items: [], arc: null };
    const selected = Array.isArray(row.selected_goals) ? row.selected_goals : [];
    const snapshot = row.progress_snapshot && typeof row.progress_snapshot === 'object' ? row.progress_snapshot : {};
    ...
    const total = items.length || clampWeeklyGoalCount(row.total_count);
    const completed = Math.min(total, Math.max(items.filter(item => item.complete).length, clampWeeklyGoalCount(row.completed_count)));
    const reward = calculateWeeklyGoalReward(completed, total);
    const awarded = Math.max(
      Number(row.points_awarded_amount || 0),
    ...
    return {
      ...
      items: items,
      arc: row.arc_snapshot && typeof row.arc_snapshot === 'object' ? row.arc_snapshot : null,
      reward: {
  ...
  function buildSlides(d){
    ...
    if (d.weight.latest != null || d.nutrition.meals_logged > 0) {
      ...
      const detailLine = hasWeight
        ? mealLine
        : (d.nutrition.avg_calories ? `avg ${d.nutrition.avg_calories} kcal/day` : '');
      slides.push({
        bg: 'ww-bg-weight',
        html: `
          <div class="ww-halftone"></div>
  ...
  function buildGoalSettingReel(){
    const goalCss = `
      <style>
        ...
        .goal-phone{width:min(86vw,360px);height:min(64vh,600px);max-height:600px;border:7px solid #111827;border-radius:34px;background:#f8fafc;box-shadow:10px 12px 0 #000,0 26px 60px rgba(0,0,0,.32);overflow:hidden;position:relative;margin-top:18px;animation:goalFloatIn .55s ease both;}
        ...
        .goal-card{border-radius:18px;background:radial-gradient(circle at 88% 14%, rgba(251,191,36,.24) 0 38px, transparent 39px),linear-gradient(135deg,#24113f 0%,#3b1b66 48%,#160f2d 100%);border:1px solid rgba(255,255,255,.16);box-shadow:0 16px 36px rgba(32,12,62,.22);padding:15px;color:white;position:relative;overflow:hidden;}
        ...
        .goal-set-btn,.goal-save-btn{border:none;background:white;color:#24113f;border-radius:12px;padding:9px 13px;font-size:.72rem;font-weight:950;box-shadow:0 10px 22px rgba(15,23,42,.22);}
        .goal-save-btn{background:linear-gradient(135deg,#321a55,#4a2575 56%,#d8b25e);color:white;width:100%;animation:goalSaveGlow 1.7s ease-in-out infinite;}
        ...
        .goal-sheet{position:absolute;left:0;right:0;bottom:0;height:88%;background:linear-gradient(180deg,#2a1648 0%,#19102d 100%);border-radius:25px 25px 0 0;box-shadow:0 -20px 46px rgba(18,8,34,.36);overflow:hidden;}
        .goal-sheet-head{padding:16px 18px;background:linear-gradient(135deg,#321a55 0%,#211039 56%,#120b24 100%);color:white;position:relative;}
        .goal-choice-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;padding:14px;}
        .goal-choice{background:white;border:2px solid rgba(15,23,42,.08);border-radius:14px;padding:10px;min-height:72px;text-align:left;box-shadow:0 8px 18px rgba(26,11,50,.08);}
        .goal-choice strong{display:block;font-size:.75rem;line-height:1.15;color:#0f172a;font-weight:950;}
        .goal-choice span{display:inline-block;margin-top:8px;border-radius:999px;padding:4px 7px;font-size:.57rem;font-weight:950;background:#f1f5f9;color:#475569;}
        .goal-choice.on{border-color:#d8b25e;background:#fef3c7;}
        .goal-choice.on span{background:white;color:#92400e;}
        .goal-selected{margin:14px;padding:12px;border-radius:16px;background:white;color:#0f172a;box-shadow:0 12px 24px rgba(18,8,34,.18);}
        .goal-row{display:flex;align-items:center;gap:9px;padding:9px 0;border-top:1px solid #e2e8f0;}
        .goal-row:first-child{border-top:0;}
        .goal-badge{width:30px;height:30px;border-radius:11px;background:linear-gradient(135deg,#321a55,#d8b25e);display:flex;align-items:center;justify-content:center;color:white;font-size:.62rem;font-weight:950;flex-shrink:0;}
        .goal-stepper{margin-left:auto;display:flex;align-items:center;gap:5px;font-size:.72rem;font-weight:950;color:#0f172a;}
        .goal-stepper button{width:27px;height:27px;border:1px solid #d8b25e;background:#fef3c7;color:#92400e;border-radius:9px;font-weight:950;}
        .goal-progress-row{margin-top:9px;padding:9px;border-radius:12px;background:rgba(255,255,255,.1);border:1px solid rgba(255,255,255,.16);font-size:.72rem;color:rgba(255,255,255,.86);font-weight:850;display:flex;justify-content:space-between;gap:10px;}
      </style>`;
    const phone = (inner) => `
      ${goalCss}
    ...
    return [
        ...
        ${phone(`
          <div class="goal-sheet">
            ...
            <div class="goal-choice-grid">
              <div class="goal-choice on"><strong>3 workouts</strong><span>Picked</span></div>
              <div class="goal-choice on"><strong>4 tracked meals</strong><span>Picked</span></div>
              <div class="goal-choice"><strong>2 recovery check-ins</strong><span>Choose</span></div>
              <div class="goal-choice on"><strong>2 Health IQ lessons</strong><span>Picked</span></div>
              <div class="goal-choice"><strong>1 progress photo</strong><span>Choose</span></div>
              <div class="goal-choice"><strong>Group post</strong><span>Choose</span></div>
            </div>
            <div class="goal-tap" style="left:58%;top:255px;"></div>
          </div>
        `)}
        <div class="ww-caption">Choose the targets that match your real week.</div>
      `},
      { bg: 'ww-bg-predict', hold_ms: 5200, html: `
        ...
        ${phone(`
          <div class="goal-sheet">
            <div class="goal-sheet-head">
              <div class="goal-eyebrow">Selected</div>
              <div class="goal-title">Set the amounts</div>
            </div>
            <div class="goal-selected">
              <div class="goal-row">
                <div class="goal-badge">TR</div>
                <div><strong style="font-size:.76rem;">Workouts</strong><div style="font-size:.62rem;color:#64748b;font-weight:850;">Training</div></div>
                <div class="goal-stepper"><button>-</button><span>3</span><button>+</button></div>
              </div>
              <div class="goal-row">
                <div class="goal-badge">FD</div>
                <div><strong style="font-size:.76rem;">Tracked meals</strong><div style="font-size:.62rem;color:#64748b;font-weight:850;">Food</div></div>
                <div class="goal-stepper"><button>-</button><span>4</span><button>+</button></div>
              </div>
              <div class="goal-row">
                <div class="goal-badge">IQ</div>
                <div><strong style="font-size:.76rem;">Health IQ lessons</strong><div style="font-size:.62rem;color:#64748b;font-weight:850;">Learning</div></div>
                <div class="goal-stepper"><button>-</button><span>2</span><button>+</button></div>
              </div>
            </div>
            <div style="position:absolute;left:14px;right:14px;bottom:18px;">
              <button class="goal-save-btn">Save goals</button>
            </div>
            <div class="goal-tap" style="right:38px;bottom:31px;"></div>
          </div>
        `)}
        <div class="ww-caption">Adjust the targets, then save.</div>
      `},
      { bg: 'ww-bg-pbs', hold_ms: 5200, html: `
        <div class="ww-halftone"></div>
        ...
        ${phone(`
          ...
          <div class="goal-card">
            ...
            <div style="display:flex;flex-wrap:wrap;gap:6px;">
              <span class="ww-store-pill" style="font-size:.56rem;padding:6px 8px;">TR 3 workouts</span>
              <span class="ww-store-pill" style="font-size:.56rem;padding:6px 8px;">FD 4 meals</span>
              <span class="ww-store-pill" style="font-size:.56rem;padding:6px 8px;">IQ 2 lessons</span>
            </div>
            <div style="margin-top:10px;padding:8px 10px;border-radius:999px;background:rgba(253,230,138,.13);border:1px solid rgba(253,230,138,.24);color:#fef3c7;font-size:.68rem;font-weight:900;display:flex;justify-content:space-between;">
              <span>Wrapped reward</span><span>Up to 50 XP</span>
            </div>
            <div class="goal-progress-row"><span>Workouts</span><span>0 / 3</span></div>
            <div class="goal-progress-row"><span>Tracked meals</span><span>0 / 4</span></div>
            <div class="goal-progress-row"><span>Health IQ</span><span>0 / 2</span></div>
          </div>
        `)}
