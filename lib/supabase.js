...
const storage = {
  ...
  async uploadExerciseVideo(userId, file, exerciseId) {
    ...
    formData.append('exerciseId', safeExerciseId);
    formData.append('contentType', mimeType);

    let uploadResponse;
    try {
      uploadResponse = await fetch('/api/upload-exercise-video', {
        method: 'POST',
        headers: {
      ...
      logUploadDiagnostic('custom_exercise_upload_network_error', {
        errorName: error?.name || 'Error',
        errorMessage: error?.message || String(error || 'upload request failed')
      });
      throw error;
    }

    if (!uploadResponse.ok) {
      let errorMessage = 'Exercise video upload failed. Try again with reception, or trim the clip if it keeps failing.';
      try {
        const errorData = await uploadResponse.json();
        errorMessage = errorData.error || errorMessage;
      } catch (_) {}
      logUploadDiagnostic('custom_exercise_upload_response', {
    ...
    logUploadDiagnostic('custom_exercise_upload_response', {
      httpStatus: uploadResponse.status,
      responseOk: true,
      uploadedBytes: Number(uploadData.size || file.size || 0),
      hasPublicUrl: !!(uploadData.publicUrl || uploadData.url)
    });
    return {
      publicUrl: uploadData.publicUrl || uploadData.url,
      storagePath: uploadData.storagePath || uploadData.fileName,
      upload: uploadData,
      b2FileId: uploadData.fileId || '',
      contentType: uploadData.contentType || mimeType,
