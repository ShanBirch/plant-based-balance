...
const db = {
  ...
  /**
   * Nudges system operations (friend reminders)
   */
  nudges: {
    /**
    ...
    /**
     * Check if user can nudge a friend (1 per 24 hours limit)
     */
    async canNudge(senderId, receiverId) {
      const { data, error } = await supabase
        .rpc('can_nudge_friend', {
          sender: senderId,
          receiver: receiverId
        });
    ...
    async getUnread(userId) {
      const { data, error } = await supabase
        .rpc('get_unread_nudges', { user_uuid: userId });
  ...
  /**
   * Friends activity operations (for friends bar)
   */
  friendsActivity: {
    /**
     * Get friends with their activity status (workout today, streak, etc.)
     */
    async getFriendsWithStatus(userId) {
      const { data, error } = await supabase
        .rpc('get_friends_with_status', { user_uuid: userId });
    ...
    /**
     * Get a single friend's activity status
     */
    async getFriendStatus(friendId) {
      const { data, error } = await supabase
        .rpc('get_friend_activity_status', { friend_uuid: friendId });
    ...
    /**
     * Get user's own activity status (for comparison)
     */
    async getMyStatus(userId) {
      ...
      const { data: points } = await supabase
        .from('user_points')
        .select('current_streak, weekly_points:current_points')
        .eq('user_id', userId)
        .single();

      return {
        has_workout_today: workouts && workouts.length > 0,
        has_meal_today: meals && meals.length > 0,
  ...
  /**
   * Private messages operations (1-on-1 chat)
   */
  privateMessages: {
    /**
    ...
    /**
     * Get all conversations list with last message preview
     */
    async getConversationsList(userId) {
      const { data, error } = await supabase
        .rpc('get_conversations_list', { user_uuid: userId });
