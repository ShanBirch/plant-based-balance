(function () {
    ...
    let owner = null, data = {}, context = null, index = 0, busy = false, showOverview = true;
    const current = () => { if (owner !== window.currentUser?.id) { owner = window.currentUser?.id; data = {}; } return data; };
    const done = (i, draft = current()) => window.BalanceCourseLayout.requirements('lead',i+1).complete && draft.weeks?.[i]?.complete === true && draft.weeks[i].answer === weeks[i].answer && String(draft.weeks[i].reflection || '').trim().length >= 20;
    ...
    async function save(complete = false) {
        ...
        const row = { answer: values.has('answer') ? Number(values.get('answer')) : null, reflection: String(values.get('reflection') || '').trim(), complete: complete || current().weeks?.[week]?.complete === true };
        ...
        if (complete && !window.BalanceCourseLayout.requirements('lead',week+1).complete) { status('Complete this week’s lesson quizzes before completing the week.'); return false; }
        busy = true;
        try {
    ...
    function render() {
        ...
        if (showOverview || !context?.unlocked) {
            const locked=!context?.unlocked;
            host.innerHTML=window.BalanceCourseLayout.overview({course:'lead',title:'Balance Lead',subtitle:'Help someone else take the next step',description:'Practise encouragement, invite someone into a small challenge and learn how supporting others strengthens the identity you are building. Six practical weeks to work through at your pace.',weeks:weeks.map((w,i)=>({...w,outcome:w.task,done:!locked&&done(i),locked})),progress:locked?{completed:0,percent:0}:p,locked,primary:'Continue course',onPrimary:'id="lead-start"',number:4});
            const openWeek=i=>{index=i;showOverview=false;render();window.scrollTo({top:0,behavior:"auto"});context?.onWeek?.();};
            host.querySelector('#lead-start')?.addEventListener('click',()=>openWeek(Math.max(0,weeks.findIndex((_,i)=>!done(i)))));
            host.querySelectorAll('[data-course-week]').forEach(button=>button.onclick=()=>openWeek(Number(button.dataset.courseWeek)));
            host.querySelector('[data-course-library]').onclick=()=>leave(context?.library);return;
        }

        host.innerHTML = `<div id="course-learning-home" class="course-detail-page course-structured course-week-page">${window.BalanceCourseLayout.weekHeader('Lead',index+1,6,week.title,'Learn the idea, complete the lesson quizzes and put it into practice.')}<section class="course-week-section"><h3>Learn this week</h3>${window.BalanceCourseLayout.introduction([[week.title,week.idea]])}<div class="course-topic-list">${window.BalanceCourseLayout.lessonRows('lead',index+1)}</div></section><section class="master-course"><h3>Your weekly practice</h3><form id="lead-form"><fieldset><legend>${esc(week.question)}</legend>${week.choices.map((a,i)=>`<label class="master-answer"><input type="radio" name="answer" value="${i}" ${row.answer===i?'checked':''}><span>${esc(a)}</span></label>`).join('')}</fieldset><label class="master-field"><span>${esc(week.task)}</span><textarea name="reflection" rows="6" maxlength="3000">${esc(row.reflection)}</textarea></label><p>This is your own reflection. It does not send a message or verify another person’s activity.</p><div class="master-actions"><button type="button" id="lead-save">Save draft</button><button type="submit">Complete this week</button></div><p id="lead-status" role="status"></p></form>${p.isComplete?'<h3>Lead complete</h3><p>You have reflected on a practical pattern of support. Keep your next commitment small and useful.</p>':''}<button id="lead-overview">View course overview</button></section></div>`;
        ...
        host.querySelector('#lead-overview').onclick = back;
    }
    async function open(options) {
        context = options; current();
        const host = document.getElementById('learning-content'); if (!host) return;
        showOverview=!Number.isInteger(options.week); if(!showOverview) index=options.week-1;
        if (!options.unlocked) { render(); return; }
        host.innerHTML = '<div class="master-course"><p>Loading your Lead work...</p></div>';
        try { await load(); if (context === options) { render(); window.scrollTo({top:0,behavior:"auto"}); } }
    ...
    async function back() { if (!showOverview && context?.unlocked) { if (!(await save())) return; showOverview=true;render();window.scrollTo({top:0,behavior:"auto"}); } else await leave(context?.library); }
    window.BalanceLead = { back, open, load, progress, leave, weeks, done, cancel: () => { context=null; } };
})();
