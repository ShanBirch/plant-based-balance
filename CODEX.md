...
Shannon confirmed the public journey order: Learn → Master → Become → Lead. The website describes each stage and identifies the eight-week Learn offer as part one, rather than implying it includes all four courses. This website presentation does not alter existing app progression, saved progress or entitlements.
...
- Pricing:
  - Balance Learn: one AUD $149 payment for the full eight weeks. It is a fixed eight-week course with eight weeks of app/community access, one weekly check-in, and workout and food review/adjustments from Shannon. It does not auto-renew. Do not describe the public offer as `one-off` or merely `$149 once`; say `one AUD $149 payment for the full eight weeks`. Instant daily replies, unlimited access and live calls are not included. Existing members who purchased the earlier lifetime Founders Pass keep the lifetime entitlement they were promised. The server-side `BALANCE_FOUNDATIONS_SIX_WEEK_OFFER` in `netlify/edge-functions/lib/checkout-guard.js` is the executable source of truth for its checkout amount, currency, duration and renewal type; customer-facing payment gates must read its public offer response rather than maintain a separate campaign price.
  ...
  - Balance Online Coaching: the same ongoing individual coaching at three weekly-billed commitment tiers. Six months is AUD $29.99/week with a 26-week initial minimum, three months is AUD $49.99/week with a 13-week initial minimum, and month-to-month is AUD $74.99/week with a four-week initial minimum. Each includes app/community access, one weekly check-in, and workout and food review/adjustments from Shannon. After the initial minimum, the selected weekly rate continues until cancelled.
- Coaching + Calls was removed from the public offer on 9 September 2026 at Shannon's request. Do not advertise or recommend it to new clients. Existing subscription records and entitlements remain unchanged.
...
- Route by fit instead of forcing the Founders Pass: structured eight-week starting course -> Founders Pass; self-directed ongoing app/community -> App + Community; ongoing personalised weekly progression and review -> Online Coaching; live supervised workouts -> 1:1 Zoom PT. The Founders Pass and Online Coaching both include one weekly review during the first eight weeks, but Founders follows the fixed Balance Learn curriculum and ends after eight weeks while Online Coaching continues at the selected tier until cancelled. Use `https://plantbased-balance.org/coaching.html` for package comparison. Zoom PT requires an availability and health-fit check before payment.
...
4. When earned, offer the eight-week Balance Learn course as one AUD $149 payment for the full eight weeks and complete the sale through DMs.
...
7. At the end of Balance Learn, route by fit: App + Community at $19.99/month for self-directed continuation, or Online Coaching at $29.99/week for six months, $49.99/week for three months, or $74.99/week month-to-month for ongoing individual progression and weekly review. Online Coaching may also be chosen directly from day one.

Critical: Balance no longer uses a free 30-day challenge as its acquisition or conversion path. DMs are the primary sales surface, and the Balance Learn Founders Pass is the primary six-week entry offer. App + Community and Online Coaching are the natural continuation options, not automatic renewals.
...
Canonical Balance Learn Founders Pass signup path:

1. Explain the fit as one AUD $149 payment for the full eight weeks inside the DM. Be clear that it is an eight-week course with one weekly check-in and plan review, eight weeks of app/community access, and no automatic renewal. Ongoing access after week six is separate.
...
Current paid-acquisition routing:
...
- Every verified `paid_meta` thread uses one neutral general-audience route, stored as `broad_pain` for compatibility. `plant_based_control` is retired as a live paid-ad route. Legacy variant fields, referral wording and ad IDs remain attribution evidence only and are normalised to `broad_pain` before any paid reply, preview or checkout handoff.
- Never infer paid acquisition for an organic follower, outreach lead or general inbound merely because they mention work, kids, busyness, consistency, follow-through or starting again. Organic Instagram conversations remain a separate statement-led relationship lane. Position Balance Learn around the neuroscience and psychology of lasting change; plant-based or vegan identity is optional personal/dietary context only when the person raises it, never the default offer frame or a qualification gate.
- Paid Meta leads knowingly entered a commercial conversation, so answer offer, price, inclusions, fit and starting questions directly from the first reply and preserve that contract throughout the thread. Organic conversations begin from the exact human context, use statement-led elicitation, and bridge only from the lead's own goal, blocker, help signal or explicit request. Do not import the paid quick replies or fixed two-question progression into a normal DM. When useful, connect one relevant Learn-series idea to the person's own pattern; give the full eight-week curriculum only when they ask what they will learn.
- Paid Meta uses zero artificial queue delay after a clean review. Treat the whole exchange as live chat from the first inbound: show typing almost immediately, begin the opening response after roughly half a second to 1.5 seconds, and target a verified public reply within 5 to 12 seconds whenever generation and transport are healthy. Continuing replies use the same immediate typing and dispatch as soon as the clean draft/review finishes. Before claiming the turn, briefly settle and reload the complete unanswered inbound batch so rapid double or triple messages are not dropped. Answer every distinct message or direct question. Use one concise bubble when it covers the turn naturally, or two to three brief back-to-back bubbles in the same synchronous delivery when separate questions or thought breaks make that clearer.
- Keep any paid-Meta multi-bubble plus native-media send inside one synchronous sender window. Cap between-item pacing instead of allowing human-long gaps to outlive the function. A native Graph echo confirms only that exact item; while `send_claim_id` is active it must never mark the whole alert sent, clear the remaining items, or trigger edit learning. Only the owning sender may mark the alert complete after every planned item is delivered and canonically logged.
- Common paid-Meta funnel turns use one guided progression contract: opening quick reply -> desired eight-week change -> matched client transformation photo when outcome, identity and safety fit are reliable -> real-life blocker or support need -> neutral tailored offer plus the approved native Balance app video -> personalised app-preview handoff -> explicit checkout handoff. The offer is positioned around the neuroscience and psychology of lasting change, translated into practical weekly action rather than brain-hack or rewiring claims. Ask no more than two discovery questions in the complete episode and skip facts already supplied. Every ordinary reply starts from one exact detail and is statement-led, with at most one decision-changing question. Once goal and blocker are known, stop interviewing. Explain the eight-week Balance Learn setup, workout program around their week, meal-plan support fitted to dietary preferences, one weekly training/food review and adjustment, and eight weeks of app/community access. The verified fixed curriculum is week 1, Why change feels hard; week 2, Experience shapes reality; week 3, The prediction-action loop; week 4, Work with your energy; week 5, What actually is learning; week 6, Make change repeatable; week 7, Take the fight out of food; week 8, Build your sustainable way forward. It uses lessons, practical actions and Weekly Goals alongside the person's workout and nutrition setup. Give the full outline only when they ask about the course, lessons or week-by-week curriculum; otherwise use the one or two themes relevant to their goal or blocker. Keep the fixed curriculum separate from the personalised workout, nutrition setup and weekly review. State one AUD $149 payment for the full eight weeks with no subscription or auto-renewal, then offer the personalised app preview before payment. Send the signed preview immediately when requested or accepted without reconfirming. A generic `I'm ready` stays on the promised preview path; only an explicit request to join, pay, sign up or receive checkout may bypass it. Paid Meta replies are text-only apart from approved native proof media; never proactively generate or send a synthetic Shannon voice note in this funnel. For unlinked leads, the only content-based safety stopper is explicit suicide or self-harm language. Pregnancy, postpartum, injury, pain, hospital, medical, eating/body-image, and difficult-emotion language stays in the careful non-diagnostic AI-coach lane. Direct checkout handoffs, real opt-outs, authenticity questions, and genuine media/context uncertainty remain separate exceptions. `I stop and start again` is a blocker, not an opt-out.
- The one paid route uses `https://future-balance.netlify.app/fitness`; its signed preview uses the same neutral public host. Meta campaign, ad-set, ad and creative attribution stays on the canonical Instagram thread and handoff receipt instead of being pasted into the public DM URL. The existing plant-based site/domain and plant-specific product facts may remain for non-ad uses, but they do not select paid-ad DM behavior.

Revenue operating loop:

- Francesca (`cavazzanafrancesca`) is no longer an active Balance coaching client as of 2026-09-02. Her former permanent Needs You/manual-only instruction is retired. Keep her exact live Instagram thread opted into manager-reviewed reactive replies with `custom_data.client_manager_auto_reply_enabled=true` and `custom_data.client_manager_browser_dispatch_enabled=true`, including duplicate canonical threads. This authorizes ordinary understood inbound Instagram text replies after the full live-thread, safety, stale-message, and no-double-send checks; it does not authorize proactive outreach, inactivity check-ins, sales pressure, or sending around media, support, authenticity, personal-boundary, or safety holds. Treat her relationship state as `former_client_auto_reply`, not an active client or cold acquisition lead.
...
- Treat an objection as decision information, not something to overcome. Answer or reflect the exact concern first, ask at most one clarifier only when it changes fit, give one truthful relevant fact when useful, then preserve choice. A clear no or request for thinking time ends the sales move for that turn. Never minimise financial pressure, argue that someone has time, promise success, bypass a partner, manufacture a deadline, or send a link while online/in-person fit is unresolved.
...
- The single scheduled Instagram Chrome operator is the 24-hour browser shift dispatcher. Its watchdog wakes every 10 minutes, but only the renewable dispatcher lease owner works, for at most one 30-minute shift. A dispatcher wake that finds a healthy active lease immediately exits without opening Chrome, closing the active run, or altering its ledger. Each owned shift works one persistent lane task instead of restarting the full checklist or rolling through all eight lanes. Technical interruptions, uncertain actions, and browser failures resume the same lane first. A normal feed-comment work slice instead ends at 30 verified comments or minute 25, whichever comes first, saves its exact feed cursor, and advances rotation; feed resumes from that cursor when its normal slot returns. The worker reserves five minutes for reconciliation and clean Chrome release and claims one person at a time. Its replaced dedicated Instagram Chrome jobs remain paused so scheduled dispatcher ownership never overlaps. This lease coordinates dispatcher wakes only; it is not a global lock on browser use by other Codex chats.
...
- Codex task notifications remain the primary review and approval surface for DM-manager and Instagram-dispatcher decisions. Codex local-task notification routing is product-owned and may not deep-link reliably on mobile; the supported recovery is ChatGPT mobile **Remote** → the clearly named/pinned Codex task. The existing Balance admin Your Call surface is the durable secondary notice: eligible pending Instagram/Facebook manager and browser-dispatcher cards show the exact Codex task name to open, while the grouped dispatcher batch card reads the one live awaiting batch, shows its exact Codex reply command, and offers a copy-only affordance for that command. This fallback is informational only, creates no second alert row, sends no client message or Instagram action, grants no approval, and disappears with the source alert or batch lifecycle so it cannot become a duplicate or stale prompt. Copying or opening never approves; only posting the exact batch/version reply in the matching Codex task does. The grouped dispatcher card must never expose an approve/send control; the retired Balance approval endpoint stays fail-closed. Do not add email fallback unless a verified transactional sender is already configured.
...
- Failed delivery is recoverable ownership, never a completed reply. At the start of each ten-minute cloud manager pass, `reconcile_unanswered_dm_delivery_failures` compares the exact latest canonical `ig_messages` row with its source alert and controller. A safe unlinked-lead reply with a send error or overdue schedule is reopened for the API manager while its inbound remains latest. A known Graph/window failure, or any eligible reply still unanswered after 60 minutes, is atomically promoted to `browser_dispatcher` with `failed_delivery_rescue=true`, the exact source alert/inbound IDs, and approved reply text. The dispatcher pre-empts weighted work, re-reads the complete native thread, repairs the wording for that person if necessary, performs one native send, and requires visible plus canonical readback. A newer outbound cancels the rescue; visible Sent with delayed attribution is never repeated. Linked clients and every manual, Needs You, safety, authenticity, identity, media, support, app-problem, commercial-decision, personal-boundary, opt-out, Story-native, active-claim, or native-restriction hold remain excluded from automatic rescue and must stay visibly routed rather than silently dropped.
- Each owned shift may complete up to 30 verified native Instagram interactions in its current lane task. General, relationship, and viewer Story lanes normally target five canonical replies; plant-based discovery-follow slots target three verified follows; ranked, follower, inbound, and client lanes are inventory-bound. Boundary-driven lanes clear only their eligible live boundary without manufacturing volume. DMs, Story replies, public comments/replies, reactions, follows, and likes each count as one. Profile checks, Story views, skips, queue updates, and cursor work do not count. Startup and pre-action checks deduplicate canonical action IDs for the shift. Thirty is a hard ceiling, never permission to force a weak action, and per-person cooldowns, duplicate prevention, context gates, acquisition capacity, and native restrictions still apply. The grouped Codex phone approval close condition remains 60 minutes for the oldest retained item; changing the interaction ceiling does not change that usual hourly notification cadence. Codex presents one notification-backed task result for the exact durable batch/version, and only Shannon's exact `APPROVE IG DISPATCH <batch_id> VERSION <version>` reply in that task grants action-time approval. Opening the notification, a generic approval, or the retired Balance app push/tap path does not approve Instagram actions.
...
- The legacy `hot_lead_feed_nurture` lane name represents one durable full lead list, ordered hot, warm, cold, then unscored-as-cold, with an exact target and hard ceiling of 30 verified public comments per owned shift. Load at most 25 people at a time through `get_ig_feed_nurture_batch`, using tier rank, warmth score, and thread id as the durable cursor. Work every person in order and fetch the next batch while time remains. The current feed work slice ends at 30 comments or minute 25, saves its exact cursor, and rotates; the next feed slot resumes there. Normal no-post/no-hook skips advance immediately with no timed cooldown and cannot be revisited until the next full cycle; browser failure keeps the cursor on that person for retry. Only a zero-row batch after the saved cursor proves `full_cycle_complete=true`. This lane is public-comment-only: never click Like and never use a like-only fallback. Only a verified specific comment receives the rolling seven-day duplicate gate.
...
- The current general-audience paid Meta campaign promotes one public offer: **Balance Learn**, one AUD $149 payment for the complete eight-week curriculum, eight weeks inside the app/community, and one weekly check-in with training and food review/adjustments. Do not rename it Online Coaching in this campaign or divert an old ad prompt into a weekly package.
- Current Meta messaging-template quick replies are:
  - "I keep starting over"
  - "I struggle to stay consistent"
  - "How does Balance work?"
- Older Meta prompt questions that may still appear in conversation history include:
  - "What's included in the eight-week Balance Learn program?"
  - "How does the weekly check-in work?"
  - "Do I need to already be plant-based?"
- Older ads may still send "Do you offer personalized coaching plans?". Answer yes by explaining the Balance Learn eight-week curriculum and weekly review, then ask about the lead's goal. Do not mention Online Coaching in that paid-ad branch.
...
Learn now has eight weeks. Use docs/learn-eight-weeks.md and lib/learn-curriculum.js for the current sequence. New starters cover experience, free energy and model updating before repeatable habits. Existing Learn members keep their original first eight weeks, then experience/prediction and learning. Existing Become calendars are retained. Upfront access is 56 days at the existing price; weekly billing minimums are unchanged. Teach the free-energy principle as a framework, explain confidence-weighted learning from surprise, retain agency and distinguish inference from parameter learning. Use a concrete example across the explanation.
