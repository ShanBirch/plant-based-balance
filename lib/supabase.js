  ...
  admin: {
    ...
    async getUserWeekData(userId, days = 7) {
      ...
      const [
        ...
        supabase.from('user_facts').select('*').eq('user_id', userId).maybeSingle(),
        supabase.from('quiz_results').select('*').eq('user_id', userId).order('created_at', { ascending: false }).limit(1).maybeSingle(),
        supabase.from('workouts').select('*').eq('user_id', userId).eq('workout_type', 'history').gte('workout_date', sinceDate).order('workout_date', { ascending: true }),
        supabase.from('meal_logs').select('*').eq('user_id', userId).gte('meal_date', sinceDate).order('meal_date', { ascending: true }),
        supabase.from('daily_nutrition').select('*').eq('user_id', userId).gte('date', sinceDate).order('date', { ascending: true }),
        supabase.from('daily_checkins').select('*').eq('user_id', userId).gte('checkin_date', sinceDate).order('checkin_date', { ascending: true }),
        supabase.from('conversations').select('*').eq('user_id', userId).gte('timestamp', sinceISO).order('timestamp', { ascending: true }),
        supabase.from('fitbit_daily_activity').select('*').eq('user_id', userId).gte('date', sinceDate).order('date', { ascending: true }).catch(() => ({ data: [] })),
        supabase.from('fitbit_sleep').select('*').eq('user_id', userId).gte('date', sinceDate).order('date', { ascending: true }).catch(() => ({ data: [] })),
        supabase.from('whoop_recovery').select('*').eq('user_id', userId).gte('created_at', sinceISO).order('created_at', { ascending: true }).catch(() => ({ data: [] })),
        supabase.from('whoop_sleep').select('*').eq('user_id', userId).gte('created_at', sinceISO).order('created_at', { ascending: true }).catch(() => ({ data: [] })),
        supabase.from('oura_readiness').select('*').eq('user_id', userId).gte('created_at', sinceISO).order('created_at', { ascending: true }).catch(() => ({ data: [] })),
        supabase.from('oura_sleep').select('*').eq('user_id', userId).gte('created_at', sinceISO).order('created_at', { ascending: true }).catch(() => ({ data: [] })),
        supabase.from('strava_activities').select('*').eq('user_id', userId).gte('start_date', sinceISO).order('start_date', { ascending: true }).catch(() => ({ data: [] })),
        supabase.from('personal_bests').select('*').eq('user_id', userId).order('achieved_date', { ascending: false }).limit(10).catch(() => ({ data: [] })),
      ]);
      ...
      return {
        ...
        wearables: {
          fitbitActivity: fitbitActivity.data || fitbitActivity || [],
          fitbitSleep: fitbitSleep.data || fitbitSleep || [],
          whoopRecovery: whoopRecovery.data || whoopRecovery || [],
          whoopSleep: whoopSleep.data || whoopSleep || [],
          ouraReadiness: ouraReadiness.data || ouraReadiness || [],
          ouraSleep: ouraSleep.data || ouraSleep || [],
          stravaActivities: stravaActivities.data || stravaActivities || [],
        },
  ...
  workoutRatings: {
    ...
    async create(userId, ratingData) {
      const { data, error } = await supabase
        ...
        .insert([{
          user_id: userId,
          workout_date: ratingData.workout_date || new Date().toISOString().split('T')[0],
