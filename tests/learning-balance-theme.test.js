...
test('reward overlays remain scrollable and safe with zero or nonzero phone insets',()=>{
 ...
 assert.equal((dashboard.match(/pbb-quiz-theme.css\?v=4-page-photos/g)||[]).length,1);
 assert.equal((dashboard.match(/learning-inline.js\?v=page-photos-20260918/g)||[]).length,2);
 assert.match(read('sw.js'),/pbb-quiz-theme.css\?v=4-page-photos/);
});
