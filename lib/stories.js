...
window.loadPhotoFeed = async function(targetGridId = 'photo-feed-grid', emptyStateId = 'photo-feed-empty') {
    ...
    try {
        ...
        grid.innerHTML = sortedStories.map(story => {
            ...
            const isCardType = isWorkoutCard || isNutritionCard || isLevelUpCard;
            ...
            return `
                <div class="feed-post-card" data-story-id="${storyId}" style="background:white; border-bottom:8px solid #f1f5f9;">
                    ...
                    ${isCardType ? (() => {
                        ...
                        const renderFn = isNutritionCard ? renderNutritionCard
...
window.openFeedPostViewer = async function(storyId) {
    ...
    try {
        ...
        if (isCardType) {
            ...
            if (cardData && story.media_url && story.media_url.length > 10 && isWorkoutCard) {
                ...
                mediaHtml = `<div style="width:100%; max-height:70vh; overflow:auto;">${cardRenderFn(cardData)}</div>`;
            } else {
...
async function loadMessagesPanel() {
    ...
    try {
        const { data: allFriends, error } = await window.supabaseClient
        ...
        const friends = allFriends ? allFriends.filter(f => !coachId || f.friend_id !== coachId) : null;
