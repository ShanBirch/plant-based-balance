...
async function saveNativePushToken(token, _retryCount) {
  ...
  try {
    if (!window.currentUser) {
      // Auth may not be ready yet — retry up to 5 times with increasing delay
      if (retryCount < 5) {
        var delay = (retryCount + 1) * 2000; // 2s, 4s, 6s, 8s, 10s
        console.log('[NativePush] No user yet, retrying token save in', delay/1000, 's (attempt', retryCount+1, '/5)');
        setTimeout(function() { saveNativePushToken(token, retryCount + 1); }, delay);
        return;
      }
      console.error('[NativePush] No user logged in after 5 retries, giving up token save');
