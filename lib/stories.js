...
window.renderMealCard = function(cardData) {
    ...
    return `
        <div style="width:100%; aspect-ratio:4/5; background:linear-gradient(160deg, #052e2b 0%, #0f766e 44%, #2563eb 100%); display:flex; flex-direction:column; padding:28px 24px; box-sizing:border-box; position:relative; overflow:hidden; color:#fff; text-shadow:0 2px 8px rgba(0,0,0,0.34);">
            ...
            <div style="z-index:1; flex:1; display:flex; flex-direction:column;">
                <div style="font-size:0.7rem; font-weight:800; letter-spacing:3px; text-transform:uppercase; color:rgba(255,255,255,0.9); margin-bottom:10px;">Meal shared</div>
                <div style="font-family:'Playfair Display',serif; font-size:2rem; line-height:1.05; font-weight:800; color:white; margin-bottom:12px;">${escapeHtml(mealType)}</div>
                ...
                <div style="background:rgba(255,255,255,0.12); border:1px solid rgba(255,255,255,0.14); border-radius:14px; padding:15px 16px; margin-bottom:16px;">
                    <div style="font-size:0.75rem; color:rgba(255,255,255,0.84); text-transform:uppercase; letter-spacing:1px; margin-bottom:6px;">Food</div>
                    <div style="font-size:1rem; color:white; font-weight:750; line-height:1.35;">${escapeHtml(foods)}</div>
                </div>

                <div style="display:flex; align-items:baseline; gap:7px; margin-bottom:14px;">
                    <div style="font-size:2.4rem; line-height:1; color:white; font-weight:900;">${calories}</div>
                    <div style="font-size:0.82rem; color:rgba(255,255,255,0.84); font-weight:800;">kcal</div>
                </div>
                ...
                <div style="display:grid; grid-template-columns:repeat(3,1fr); gap:9px; margin-top:auto;">
                    <div style="background:rgba(255,255,255,0.1); border-radius:12px; padding:11px 8px; text-align:center;">
                        <div style="font-size:1.15rem; color:white; font-weight:900;">${protein}g</div>
                        <div style="font-size:0.65rem; color:rgba(255,255,255,0.84); font-weight:800;">Protein</div>
                    </div>
                    <div style="background:rgba(255,255,255,0.1); border-radius:12px; padding:11px 8px; text-align:center;">
                        <div style="font-size:1.15rem; color:white; font-weight:900;">${carbs}g</div>
                        <div style="font-size:0.65rem; color:rgba(255,255,255,0.84); font-weight:800;">Carbs</div>
                    </div>
                    <div style="background:rgba(255,255,255,0.1); border-radius:12px; padding:11px 8px; text-align:center;">
                        <div style="font-size:1.15rem; color:white; font-weight:900;">${fat}g</div>
                        <div style="font-size:0.65rem; color:rgba(255,255,255,0.84); font-weight:800;">Fat</div>
                    </div>
                </div>
            </div>
...
// Render weekly progress photo set card for feed display.
window.renderProgressPhotoCard = function(cardData) {
    if (!cardData) return '<div style="padding:40px; text-align:center; color:#999;">Card data unavailable</div>';

    const rawShots = Array.isArray(cardData.shots) ? cardData.shots : [];
    const normalizedShots = rawShots
        .map(function(shot, index) {
            ...
            return {
                ...
                photo_url: shot.photo_url || shot.url || shot.media_url || '',
                storage_path: shot.storage_path || shot.fileName || ''
            };
        })
        .filter(function(shot) { return shot && shot.photo_url; });

    const weekLabel = cardData.photo_week
        ? new Date(cardData.photo_week + 'T00:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric' })
        : '';
    const headline = cardData.title || 'Weekly progress photos';
    const summary = cardData.summary || cardData.subtitle || 'Front, side, back check-in';
    const gridColumns = Math.min(3, Math.max(1, normalizedShots.length || 1));

    return `
        <div style="width:100%; aspect-ratio:4/5; background:linear-gradient(160deg, #5f1236 0%, #db2777 35%, #f43f5e 68%, #fb7185 100%); display:flex; flex-direction:column; padding:24px; box-sizing:border-box; position:relative; overflow:hidden; color:#fff; text-shadow:0 2px 8px rgba(0,0,0,0.34);">
            <div style="position:absolute; top:-36px; right:-36px; font-size:8.5rem; opacity:0.08; transform:rotate(12deg);">📸</div>
            <div style="position:absolute; bottom:-24px; left:-24px; font-size:6rem; opacity:0.06; transform:rotate(-8deg);">✨</div>

            <div style="z-index:1; flex:1; display:flex; flex-direction:column;">
                <div style="display:flex; justify-content:space-between; align-items:flex-start; gap:12px; margin-bottom:12px;">
                    <div style="min-width:0;">
                        <div style="font-size:0.7rem; font-weight:800; letter-spacing:3px; text-transform:uppercase; color:rgba(255,255,255,0.9); margin-bottom:8px;">Weekly progress photos</div>
                        <div style="font-family:'Playfair Display',serif; font-size:1.5rem; line-height:1.15; font-weight:700; color:white;">${escapeHtml(headline)}</div>
                    </div>
                    ${weekLabel ? `<div style="background:rgba(255,255,255,0.14); backdrop-filter:blur(8px); border-radius:16px; padding:8px 10px; color:white; font-size:0.75rem; font-weight:700; white-space:nowrap;">${escapeHtml(weekLabel)}</div>` : ''}
                </div>

                <div style="font-size:0.82rem; color:rgba(255,255,255,0.9); font-weight:600; line-height:1.4; margin-bottom:14px;">${escapeHtml(summary)}</div>

                <div style="display:grid; grid-template-columns:repeat(${gridColumns}, minmax(0, 1fr)); gap:8px; margin-bottom:12px;">
                    ${normalizedShots.slice(0, 3).map(function(shot) {
                        return `
                            <div style="position:relative; border-radius:14px; overflow:hidden; background:rgba(255,255,255,0.12); border:1px solid rgba(255,255,255,0.12); aspect-ratio:3/4;">
                                <img src="${escapeHtml(shot.photo_url)}" alt="${escapeHtml(shot.title)}" style="width:100%; height:100%; object-fit:cover; display:block;">
                                <div style="position:absolute; inset:auto 0 0 0; background:linear-gradient(transparent, rgba(0,0,0,0.72)); padding:20px 10px 8px; color:white;">
                                    <div style="font-size:0.68rem; font-weight:800; letter-spacing:1px; text-transform:uppercase;">${escapeHtml(shot.title)}</div>
                                </div>
                            </div>
                        `;
                    }).join('')}
                ...
                <div style="display:flex; gap:8px; flex-wrap:wrap; margin-top:auto;">
                    ${normalizedShots.slice(0, 3).map(function(shot) {
                        return `<div style="background:rgba(255,255,255,0.14); padding:6px 10px; border-radius:999px; color:white; font-size:0.72rem; font-weight:700;">${escapeHtml(shot.title)}</div>`;
                    }).join('')}
                </div>
            </div>
...
window.clearFeedComposerMedia = function(resetInput = true) {
    ...
    const video = document.getElementById('feed-composer-preview-video');
    const fileName = document.getElementById('feed-composer-file-name');
    const input = document.getElementById('feed-composer-file-input');
...
async function uploadFeedComposerMedia(file, mediaType, postBtn) {
    let fileToUpload = file;
    const originalSize = (fileToUpload.size / (1024 * 1024)).toFixed(1);

    if (mediaType === 'video') {
        if (postBtn) postBtn.textContent = `Compressing ${originalSize}MB`;
        fileToUpload = await compressVideo(fileToUpload, (status) => {
            if (postBtn) postBtn.textContent = status;
        });
    ...
    if (fileSizeMB > 5) {
        if (postBtn) postBtn.textContent = `Uploading ${Math.round(fileSizeMB)}MB`;
        ...
        mediaUrl = await readFeedComposerFileAsDataUrl(fileToUpload);
        if (mediaType === 'image') {
            analysisBase64 = getBase64Payload(mediaUrl);
        }
    }
...
window.submitFeedComposerPost = async function() {
    ...
    try {
        ...
        resetFeedComposer();
        showFeedComposerToast('Posted to Feed!', 'success');
...
window.loadPhotoFeed = async function(targetGridId = 'photo-feed-grid', emptyStateId = 'photo-feed-empty') {
    ...
    try {
        ...
        grid.innerHTML = sortedStories.map(story => {
            ...
            const isProgressPhotoCard = story.media_type === 'progress_photo_card';
            const isCardType = isWorkoutCard || isNutritionCard || isMealCard || isLevelUpCard || isProgressPhotoCard;
            const isTextPost = story.media_type === 'text' || (!isCardType && !isVideo && !story.media_url && !story.thumbnail_url);
            ...
            // Caption (skip for card types since caption contains JSON data)
            const captionHtml = (story.caption && !isCardType && !isTextPost)
            ...
            return `
                <div class="feed-post-card" data-story-id="${storyId}" style="background:white; border-bottom:8px solid #f1f5f9;">
                    ...
                    })() : isTextPost ? textPostHtml : `<div onclick="openFeedPostViewer('${storyId}')" style="cursor:pointer; position:relative; background:#000; width:100%;">
...
window.openFeedPostViewer = async function(storyId) {
    ...
    try {
        ...
        if (isCardType) {
            ...
            try { cardData = JSON.parse(story.caption); } catch(e) {}
            const isMealPayload = cardData && cardData.card_type === 'meal';
            const renderCard = isMealPayload ? renderMealCard : cardRenderFn;

            if (cardData && story.media_url && story.media_url.length > 10 && (isWorkoutCard || isMealCard || isMealPayload)) {
        ...
        const captionHtml = (story.caption && !isCardType && !isTextPost)
            ...
            : '';
        const mediaStageBackground = (isWorkoutCard || isTextPost) ? 'background:white;' : 'background:black;';
