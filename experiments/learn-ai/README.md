...
## Link handoff and two-hour check-in

After a booking card, ask them to let me know once booked so I can check it came through. After a website preview card, ask them to let me know if they join so I can check onboarding. These are separate text actions after the card.

`followup.cjs` queues one reminder two hours after confirmed card delivery, anchored to the completed handoff. The existing per-minute scheduled worker claims it and dispatches `learn-link-followup-background`, authenticated with the existing server service key. A second atomic claim prevents background retries from sending again. The reminder uses the same test-account identity, canonical receipts, typing/pacing and delivery ownership as normal replies. It cancels for changed selection, any newer conversation message, opt-out, linked app/paying status, recorded signup/purchase/onboarding, or a booking associated with the signed link. Rechecks happen before typing and immediately before sending. No send occurs outside the standard messaging window, and lookup/dispatch failures do not trigger blind retries. No retrospective reminder is queued for old links.

Booking cards retain a signed `meta_ref`; the booking form submits it and the server verifies it before recording `metadata.ig_thread_id`. Matching uses that verified identity, never a guessed name or email. Existing preview/payment-gate reminders are suppressed for the selected alternative to avoid overlapping nudges. Other threads keep their existing behaviour. Run `node --test experiments/learn-ai/followup.test.cjs` for scheduling, cancellation, conversion checks and concurrent execution coverage.
