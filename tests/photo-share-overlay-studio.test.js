...
assert.ok(dashboard.includes('dashboard-script-10-points_widget_functions.js?v=44'), 'phones must load the new share composer');
assert.ok(serviceWorker.includes("const CACHE_NAME = 'pbb-app-v309'"), 'the app shell cache must be refreshed');
assert.ok(serviceWorker.includes('dashboard-script-10-points_widget_functions.js?v=44'), 'the new share composer must be precached');
