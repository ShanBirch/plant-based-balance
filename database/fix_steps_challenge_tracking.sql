-- ============================================================
-- FIX: Steps challenge tracking
--
-- Two problems fixed:
--
-- 1. Missing upsert_native_daily_steps RPC.
--    The client calls this to write native HealthKit / Health Connect
--    steps into oura_daily_activity so the challenge scoring picks
--    them up.  Uses GREATEST so a real Oura sync never gets
--    overwritten by a lower native value.
--
-- 2. Steps challenge scoring only read from oura_daily_activity.
--    Fitbit users always showed 0 steps.  Now uses the same
--    multi-source approach as the sleep challenge:
--    pick the highest step count per day across Oura and Fitbit,
--    then sum across the challenge window.
--
-- Run this in Supabase SQL Editor.
-- ============================================================

-- ============================================================
-- 1. Create upsert_native_daily_steps
-- ============================================================
...
-- ============================================================
-- 2. Update challenge scoring: steps reads from ALL wearable sources
--    (Oura/native via oura_daily_activity, Fitbit via fitbit_daily_activity)
--    taking the highest value per day, matching the sleep challenge pattern.
-- ============================================================
...
BEGIN
    ...
    LOOP
        ...
        WHEN 'steps' THEN
            -- Steps: total steps from ALL wearable sources, picking the highest
            -- value per day so any connected device (Oura, native HealthKit /
            -- Health Connect, or Fitbit) counts.
