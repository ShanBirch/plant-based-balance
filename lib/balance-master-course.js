...
(function () {
    ...
    function event(name, extra = {}) { context?.track?.(`master_${name}`, { stage: stageIndex + 1, curriculum_version: 'balance_master_10_weeks_v2', assessment_version: 'master_assessments_v3', form_check_requirement: 'master_compound_video_v1', ...extra }); }
    ...
    function render(data = current()) {
        ...
        host.innerHTML = `<div id="course-learning-home" class="course-detail-page course-structured course-week-page">
            ...
            <section class="master-course"><h3>Your weekly practice</h3><p>Your required action: finish the quizzes and reflections, complete the practical task, then submit this week. Your submission and time are recorded for you and your coach. Saving a draft does not complete the week.</p>${data.submittedAt?.[stageIndex] ? `<p class="master-submission-receipt">Last submitted: ${esc(new Date(data.submittedAt[stageIndex]).toLocaleString('en-AU'))}</p>` : ''}
        ...
        host.querySelector('#master-form').addEventListener('submit', async e => {
            ...
            if (!stageReady(stageIndex, draft)) { status(stageIndex === 1 ? 'Add your practical answer and successfully send all four videos. Use Check submitted videos to see what is still needed.' : s.project === 'workout' ? 'Finish your muscle split, its reasoning, all seven day explanations, goal, schedule, exercise details, coverage and progression rule. Link at least one saved workout and update its exercise details.' : s.project === 'meal' ? 'Finish all seven days plus dietary needs, shopping, prep and your backup meal.' : 'Add your practical answer before completing this stage.', true); return; }
    ...
    window.BalanceMaster = { back, open, progress, load, leave, cancel: () => { context = null; }, stages: STAGES, workoutValid, mealValid, stageDone, migrate, unlocked };
})();
