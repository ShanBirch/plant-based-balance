    ...
    WITH tx AS (
        SELECT
            ...
            CASE
                ...
                WHEN LOWER(COALESCE(pt.transaction_type, '')) LIKE '%checkin%'
                  OR LOWER(COALESCE(pt.transaction_type, '')) LIKE '%check_in%'
                  OR LOWER(COALESCE(pt.transaction_type, '')) LIKE '%check-in%'
                  OR LOWER(COALESCE(pt.description, '')) LIKE '%check-in%'
                  OR LOWER(COALESCE(pt.description, '')) LIKE '%check in%'
                  OR LOWER(COALESCE(pt.description, '')) LIKE '%checkin%' THEN 'check_ins'
    ...
    SELECT COALESCE(jsonb_agg(
        jsonb_build_object(
            ...
            'label', CASE
                ...
                WHEN LOWER(COALESCE(transaction_type, '')) LIKE '%checkin%'
                  OR LOWER(COALESCE(transaction_type, '')) LIKE '%check_in%'
                  OR LOWER(COALESCE(transaction_type, '')) LIKE '%check-in%' THEN 'Daily check-in'
