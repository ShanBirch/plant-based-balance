...
- Shannon's spoken Reel copy should sound like his approved voice notes rather than polished presenter writing. Use contractions and never write the stiff spoken phrase `is not`; use `isn't` or rewrite it. Across a normal 45–90 second Reel, use one to three imperfect thinking beats total, require at least one genuine `um` or `ah`, add punctuation-led pauses and occasional fragments, and never stack fillers or put them in every sentence.
