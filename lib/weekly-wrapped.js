...
(function(){
  ...
  // ============================================================
  //  Music — a procedural 4-bar anime-synthwave loop
  //
  //  No audio file needed. Synthesized live with Web Audio API:
  //  kick on every beat, hi-hat on 8th off-beats, pulsing saw bass
  //  through a lowpass, square-wave arp on the last beat of each
  //  bar, progression Am → F → C → G at 126 BPM. The scheduler
  //  re-arms every ~60ms with a 0.3s lookahead so it stays tight
  //  under GC pauses.
  //
  //  Autoplay: iOS Safari blocks AudioContext before a user
  //  gesture. We create the context in open(), try to resume, and
  //  if it stays suspended we show a 🔇 unmute button that
  //  resumes it on the user's first tap inside the overlay.
  // ============================================================

  const MUTED_KEY = 'pbb_wrapped_muted';

  function createWrappedSynth(){
    ...
    function arp(t, freq, dur){
      ...
      osc.frequency.value = freq;
      g.gain.setValueAtTime(0.001, t);
      g.gain.exponentialRampToValueAtTime(0.08, t + 0.002);
      g.gain.exponentialRampToValueAtTime(0.001, t + dur);
      osc.connect(g).connect(master);
      osc.start(t);
      osc.stop(t + dur + 0.04);
    ...
    function scheduleLoop(startAt){
      for (let b = 0; b < bars; b++){
        const barStart = startAt + b * bar;
        const root = bassFreqs[b];
        const fifth = root * 1.498;
        const minorThird = root * 1.189;
        // Kick four-on-the-floor
        for (let i = 0; i < 4; i++) kick(barStart + i * beat);
        // Hat on 8th off-beats
        for (let i = 0; i < 8; i++) hat(barStart + (i + 0.5) * (beat / 2));
        // Bass 8th-note pulse (R-R-5-R-R-R-5-R)
        const pattern = [root, root, fifth, root, root, root, fifth, root];
        for (let i = 0; i < 8; i++) bass(barStart + i * (beat / 2), pattern[i], 0.17);
        // Arp flourish on beat 4 (16ths: R-b3-5-8ve)
        const notes = [root * 2, minorThird * 2, fifth * 2, root * 4];
        for (let i = 0; i < 4; i++) arp(barStart + 3 * beat + i * (beat / 4), notes[i], 0.11);
      }
    }
    ...
    function tick(){
      if (!running) return;
      if (ctx.currentTime + 0.3 >= nextLoopAt){
  ...
  async function open(userId, opts){
    ...
    // Spin up the synthwave loop. If the user had muted last time, honour that.
    // If the AudioContext starts suspended (autoplay blocked on iOS), the music
    // button will pulse; tapping it will resume the context.
    try {
      if (currentSynth) { currentSynth.stop(); currentSynth = null; }
      currentSynth = createWrappedSynth();
      if (currentSynth) {
