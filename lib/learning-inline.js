...
(function() {
    ...
    async function completeLesson() {
        ...
        // Also try to award XP to user's main account
        if (isNewLesson && xpEarned > 0 && window.awardXP) {
            try {
                await window.awardXP(xpEarned, 'learning_lesson');
            } catch (e) {
                console.log('XP award pending sync:', e.message);
            }
        }
