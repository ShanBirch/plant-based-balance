...
async function shareMealRecordToFeed(meal, btn, options = {}) {
    if (!meal || !meal.id) {
    ...
    try {
        ...
        if (!options.skipPrivateStudio && window.BalancePrivateShareStudio?.isEnabled?.()) {
            if (btn) {
                btn.disabled = false;
                btn.textContent = btn.dataset.originalText || getMealFeedShareButtonText();
                btn.style.opacity = '1';
            }
            await openPrivateMealShareStudio(mealForShare);
            return null;
        }
...
async function shareMealRecordToInstagram(meal, btn, target = 'story', options = {}) {
    if (!meal || !meal.id) {
        showToast('Meal is not ready to share yet', 'info');
        return false;
    }
...
function showMealFeedSharePrompt(meal) {
    ...
    const instagramEarnsXp = !isMealInstagramShareUsedToday();
...
function openMealCameraDirect(source) {
    ...
    // iOS Meal Builder — use the in-WebView unified camera for both
    // the Photo and Barcode builder buttons. The native file-input
    // flow used by the tracker can't scan barcodes, so keeping both
    // builder buttons on a single camera (the unified one) is the
    // only way to give users a consistent experience where either
    // button opens the same camera. We fall through to
    // openUnifiedCamera() below for this case.
...
// Share the currently-open meal detail as a Feed card.
async function openPrivateMealShareStudio(meal) {
    if (!window.BalancePrivateShareStudio?.isEnabled?.() || !meal) return false;
    const freshMeal = await getFreshMealRecordForFeedShare(meal);
    const cardPayload = buildMealFeedCardPayload(freshMeal);
    if (!cardPayload.photo_url) return false;
    const photoDataUrl = await getMealInstagramPhotoDataUrl(cardPayload);
...
async function shareCurrentMealToFeed() {
    ...
    if (await openPrivateMealShareStudio(meal)) return;
...
async function openUnifiedCamera(callback) {
    ...
    // button. Both the Photo and Barcode builder buttons funnel into
    // this single camera flow — the builder handles naming separately,
    // so hide the "What is this? (optional)" description input, keep
    // the manual-barcode icon visible in case the user wants to toggle
...
function closeUnifiedCamera() {
    ...
    // If the meal builder dropped itself down to make room for this
    // camera, slide it back up now that the camera is gone. This
    // covers the cancel case — successful capture / scan paths call
    // it earlier, but it's idempotent so calling twice is fine.
    if (typeof window._showBuilderAfterCamera === 'function') {
        try { window._showBuilderAfterCamera(); } catch (e) {}
    }
...
async function startUnifiedCamera() {
    ...
    try {
        ...
        //
        // Exception: Meal Builder mode. Both the Photo and Barcode
        // builder buttons share this single camera, so auto-start the
