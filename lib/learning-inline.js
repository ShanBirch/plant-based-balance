...
There's no perfect program—only the one you'll actually do consistently with effort. Start where you are, apply the principles, and trust the process.`,
    ...
    function renderLearningHome() {
        ...
        // Daily Quiz Card
        const dailyQuizDone = isDailyQuizCompletedToday();
        const nextLesson = !dailyQuizDone ? getNextAvailableLesson(progress) : null;
        let dailyQuizCardHtml = '';
        if (nextLesson && !dailyQuizDone) {
            const { lesson, unit, module } = nextLesson;
            dailyQuizCardHtml = `
            <div id="daily-quiz-card" onclick="window.startDailyQuiz('${lesson.id}')" style="margin: 15px; background: linear-gradient(135deg, #f59e0b 0%, #f97316 100%); border-radius: 16px; padding: 20px; color: white; position: relative; overflow: hidden; cursor: pointer;">
                ...
                <div style="position: relative; z-index: 1;">
                    <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 12px;">
                        <div>
                            ...
                            <p style="margin: 0; font-size: 0.85rem; opacity: 0.9;">Complete a lesson, earn bonus XP!</p>
                        </div>
                    ...
                    <div style="background: rgba(255,255,255,0.15); border-radius: 12px; padding: 14px 16px; display: flex; align-items: center; gap: 12px;">
                        <div style="width: 44px; height: 44px; background: rgba(255,255,255,0.2); border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 1.4rem;">
                            ${module.id === 'body' ? '💪' : module.id === 'fuel' ? '🥗' : module.id === 'mind' ? '🧠' : module.id === 'longevity' ? '⏳' : module.id === 'workouts' ? '🏋️' : module.id === 'hormones' ? '🧪' : '📚'}
                        </div>
                        <div style="flex: 1;">
                            <div style="font-weight: 600; font-size: 0.95rem;">${lesson.title}</div>
                            <div style="font-size: 0.8rem; opacity: 0.85; margin-top: 2px;">${module.title} &bull; ${unit.title}</div>
                        </div>
            ...
            `;
        } else if (dailyQuizDone) {
            dailyQuizCardHtml = `
            <div style="margin: 15px; background: linear-gradient(135deg, #059669 0%, #10b981 100%); border-radius: 16px; padding: 16px 20px; color: white; display: flex; align-items: center; gap: 15px;">
