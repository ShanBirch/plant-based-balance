...
const storage = {
  ...
  async uploadExerciseVideo(userId, file, exerciseId) {
    ...
    const startResponse = await fetch('/api/create-exercise-video-upload', {
      method: 'POST',
      headers: {
    ...
    if (!startResponse.ok) {
      let errorMessage = 'Could not start exercise video upload.';
      try {
    ...
    const uploadResponse = await fetch(startData.uploadUrl, {
      ...
      headers: {
        ...
        'X-Bz-Info-Author': `user-${String(userId).replace(/[^a-zA-Z0-9_-]/g, '')}`,
        'X-Bz-Info-upload-type': 'custom-exercise-video',
        'X-Bz-Info-exercise-id': safeExerciseId
      },
      body: file
    });
    ...
    if (!uploadResponse.ok) {
      ...
      console.warn('Exercise video B2 upload failed:', errorText);
      throw new Error('Exercise video upload failed. Try again with reception, or trim the clip if it keeps failing.');
    }
