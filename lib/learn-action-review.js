(function () {
  ...
  function memberPage(week, def, row) {
    ...
    return `<div class="lar-member">
      ...
      <div class="lar-completion"><span class="lar-completion-icon" aria-hidden="true">${done?'✓':'○'}</span><div><h4>${done?'Completed and saved':'How your course tick appears'}</h4><p>${done?(row?.status==='legacy_completed'?'Your earlier course completion is preserved. You can return to this record at any time.':'Your coach has confirmed this action. You can return to these notes at any time.'):'Save your notes, report what happened, then your coach confirms completion. A reflection alone does not tick off the action.'}</p>${row?.review_note?`<p class="lar-review-note">Coach: ${esc(row.review_note)}</p>`:''}${row?.reviewed_at?`<p class="lar-caption">Reviewed ${esc(new Date(row.reviewed_at).toLocaleString())}</p>`:''}</div></div>
    </div>`;
  }
