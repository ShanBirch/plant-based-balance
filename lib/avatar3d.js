...
class Avatar3D {
  ...
  setupEnvironment() {
    const environments = {
      ...
      mountain: {
        skyColor: 0xb8cce0,
        groundColor: 0x6b7b8c,
        fogColor: 0xb8cce0,
        ambientIntensity: 0.6
      }
    };

    const env = environments[this.state.roomBackground] || environments.gym;

    // Scene background
    this.scene.background = new THREE.Color(env.skyColor);

    // Fog for atmosphere
    this.scene.fog = new THREE.Fog(env.fogColor, 5, 20);
