...
function _eagerIOSPushInit() {
  ...
  setTimeout(function () {
    ...
    // Check if the plugin is actually registered natively by testing the
    // real Plugins registry (not the Proxy from registerPlugin, which has
    // every method be truthy even for missing plugins).
    var nativeRegistered = !!(window.Capacitor && window.Capacitor.Plugins && window.Capacitor.Plugins.FitGotchiPush);
    if (!FitGotchiPush || !nativeRegistered) {
      pushDebugBanner('FitGotchiPush NOT registered (nativeReg=' + nativeRegistered + ')', '#cc0000');
