...
(function (root, factory) {
    ...
    function isVerifiedEntry(input) {
        ...
        if (params.get('meta_trial') !== VARIANT) return false;
    ...
    function readState() {
        ...
        return state && state.variant === VARIANT ? state : null;
    }
    ...
    function showGate() {
        ...
        const state = readState();
        if (!state || !state.deadlineAt || Date.now() < state.deadlineAt) return false;
        ...
        if (!state.gateShownAt) {
            ...
            writeState(state);
            track('trial_gate_shown', { elapsed_seconds: DURATION_MS / 1000 });
        }
        return true;
    }
    ...
    function markSignupHandoff() {
        ...
        if (params.get('source') !== 'meta_ad_trial') return false;
        const session = storage('sessionStorage');
        if (session) {
            session.setItem(CLAIM_KEY, 'true');
            session.removeItem('guestMode');
