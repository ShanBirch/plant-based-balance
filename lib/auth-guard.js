...
(async function() {
  ...
  const isGuestMode = requestedGuestMode || (storedGuestMode && !storedSession);
  ...
  const session = storedSession || await authHelpers.getSession();
  ...
  if (!window.isAdminViewing) {
    ...
    try {
      dbHelpers.activity.log(user.id, 'page_view', collectPageViewDiagnostics()).catch(error => {
