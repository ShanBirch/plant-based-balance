 ...
 * `won`, `lost`, `paused`. `pitched` means Shannon offered the Vegan Fitness Founders Pass.
...
const STAGES = [
    ...
    },
    {
        ...
        index: 4,
        label: 'Commitment',
        what_to_learn: 'ready-to-start signal for the Balance Vegan Fitness Founders Pass + the real-life situation it can naturally support',
...
function buildEvaluationPrompt({ leadName, channel, currentQualifier, history, currentMessage, draftText, customData, meaningfulLeadReplyCount }) {
    ...
    return `You are scoring a lead's progress through a 4-stage qualifier funnel for Shannon, who is currently offering the Balance Vegan Fitness Founders Pass. It is AUD $99 once, does not need a phone call, and includes a guided six-week kickstart plus lifetime access to the core Balance app and vegan fitness community. It does not include ongoing individual weekly coaching. Starter Coaching at AUD $29.99/week is an optional upgrade after joining or when someone explicitly wants Shannon personally adjusting their plan each week. Close the Founders Pass through DMs by default. Balance no longer uses a free challenge as its acquisition or conversion path.
...
YOUR JOB: read the conversation, update the qualifier state, and decide whether THIS turn should keep chatting, gently bridge toward health/fitness, or move toward the paid Vegan Fitness Founders Pass because they have admitted they need help or asked how to start.
