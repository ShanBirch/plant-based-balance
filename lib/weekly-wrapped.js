...
(function(){
  ...
  function challengeWeekWindow(challenge){
    const now = new Date();
    const start = parseDateOnly(challenge && challenge.start_date) || addDays(now, -6);
    const fullEndExclusive = addDays(start, 7);
    const isPartial = now.getTime() < fullEndExclusive.getTime();
    const endForIso = isPartial ? now : fullEndExclusive;
    const dateEndExclusive = isPartial ? addDays(now, 1) : fullEndExclusive;
    const labelEnd = isPartial ? now : addDays(fullEndExclusive, -1);
    return {
      start,
      endForIso,
  ...
  async function buildChallengeWeekOneStats(){
    ...
    const range = challengeWeekWindow(challenge);
  ...
  function buildChallengeWeekOneSlides(stats){
    ...
    const weightLostKg = Number(stats.weightLostKg || 0);
    const hasWeightLoss = weightLostKg > 0;
    const weightHeadline = hasWeightLoss ? formatWeightKg(weightLostKg) : compactNumber(stats.weighIns || 0);
    const weightCaption = hasWeightLoss
      ? 'lost across week one weigh-ins'
      : ((stats.weighIns || 0) > 0 ? 'weigh-ins logged so far' : 'weight-loss data needs weigh-ins');
    const weightPill = hasWeightLoss
      ? compactNumber(stats.weightLossUsers || 0) + ' members trending down'
      : compactNumber(stats.weightTrackedUsers || 0) + ' members logged weight';
    ...
    return [
      ...
      { bg: 'ww-bg-challenge-night', hold_ms: 2600, html: `
        ...
        <div class="ww-big-num" style="font-size: clamp(1.8rem, 8vw, 3.7rem); color:#fff; line-height:1; text-shadow:4px 4px 0 #101820;">${pbs} PBs HIT</div>
      ...
      { bg: 'ww-bg-challenge-night', hold_ms: 2600, html: `
        ...
        <div class="ww-caption" style="max-width:86%; color:#fff; font-weight:900;">${meals} meals tracked</div>
      ...
      { bg: 'ww-bg-challenge-score', hold_ms: 2500, html: `
        <div class="ww-halftone"></div>
        <div class="ww-challenge-pill">team weight check</div>
        <div class="ww-big-num" style="font-size: clamp(4.6rem, 20vw, 10rem); color:#101820; line-height:0.95; text-shadow:5px 5px 0 #f8f3e7;">${weightHeadline}</div>
        <div class="ww-caption" style="color:#101820; font-weight:900;">${escapeHtml(weightCaption)}</div>
        <div class="ww-challenge-pill green" style="margin-top:18px;">${escapeHtml(weightPill)}</div>
      `},
