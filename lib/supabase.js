  ...
  admin: {
    ...
    async getFeatureUsageStats() {
      ...
      const [meals, workouts, lessons, battles, quizBattles] = await Promise.all([
        ...
        supabase.from('workouts').select('user_id', { count: 'exact', head: false }).gte('created_at', since),
    ...
    async getFeatureUsageTrend(days = 7) {
      ...
      for (let i = days - 1; i >= 0; i--) {
        ...
        const [meals, workouts, lessons] = await Promise.all([
          ...
          supabase.from('workouts').select('*', { count: 'exact', head: true }).gte('created_at', start).lte('created_at', end),
