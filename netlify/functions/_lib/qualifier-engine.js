...
function buildEvaluationPrompt({ leadName, channel, currentQualifier, history, currentMessage, draftText, customData, meaningfulLeadReplyCount }) {
    ...
    return `You are scoring a lead's progress through a 4-stage qualifier funnel for Shannon, a personal coach who is currently offering Balance Starter Coaching. It is AUD $29.99/week, does not need a phone call, and includes Balance app access, tailored workout structure, food direction, progress tracking, and one weekly check-in with Shannon. Close through DMs by default. Balance no longer uses a free challenge as its acquisition or conversion path.
