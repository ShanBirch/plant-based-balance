(function() {
    ...
    _onReady(async function() {
        ...
        // Without this, the previous user's character, stats, and streak remain visible
        // until the new user's data loads — causing an overlap/flicker.
        if (window.currentUser) {
            ...
            if (lastUserId && lastUserId !== window.currentUser.id) {
                ...
                // Clear stale DOM values that were already painted
                var el;
                el = document.getElementById('tamagotchi-level'); if (el) el.textContent = '';
                el = document.getElementById('tamagotchi-rank'); if (el) el.textContent = '';
                el = document.getElementById('tamagotchi-xp-text'); if (el) el.textContent = '';
                el = document.getElementById('tamagotchi-xp-bar'); if (el) el.style.width = '0%';
                el = document.getElementById('tamagotchi-streak'); if (el) el.textContent = '0';
                el = document.getElementById('tamagotchi-model'); if (el) el.removeAttribute('src');
                el = document.getElementById('profile-age-display'); if (el) el.textContent = '';
                el = document.getElementById('profile-weight-display'); if (el) el.textContent = '';
                el = document.getElementById('profile-goal-display'); if (el) el.textContent = '';

                // Show the loading overlay so the user doesn't see stale data
