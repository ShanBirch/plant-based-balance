...
(function (root, factory) {
    ...
    function resolve(meal, template) {
        ...
        const explicitUrls = [meal?.photo_url, meal?.image_url, meal?.image].map(safeImageUrl);
        const remoteExplicit = explicitUrls.find(url => /^https:\/\//i.test(url)) || '';
        if (remoteExplicit) return { url: remoteExplicit, source: 'explicit', recipeName: null };

        // Per-client directories contain reviewed photos assigned to exact meals.
        // Do not accept root-level library paths here: legacy plans often stored an
        // approximate library image that depicted a different recipe.
        const clientPlanExplicit = explicitUrls.find(url => /^images\/meals\/[^/]+\/[^"'<>]+$/i.test(url)) || '';
        if (clientPlanExplicit) return { url: clientPlanExplicit, source: 'explicit', recipeName: null };
