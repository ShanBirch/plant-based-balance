...
- Shannon's spoken Reel copy should sound like his approved voice notes rather than polished presenter writing. Natural contractions are mandatory wherever one exists: write `I'll`, `you're`, `we're`, `it's`, `that's`, `there's`, `don't`, `doesn't`, `isn't`, `can't`, `won't`, `haven't`, and similar forms instead of their expanded equivalents. Exact quotations are the only normal exception. Across a normal 45–90 second Reel, use four to six imperfect thinking beats, including at least three genuine `um` or `ah` sounds, plus punctuation-led pauses, fragments, and an occasional restart or self-correction. Spread them through the script rather than stacking them together.
