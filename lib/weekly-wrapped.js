...
(function(){
  ...
  const WW_STYLES = `
    ...
    .tw-label {
      ...
      animation: twLabelIn 0.45s ease-out 0.08s both;
      font-family: 'Inter', system-ui, sans-serif;
    }
    .tw-label.gpt { color: rgba(34, 211, 238, 0.78); }
    .tw-label.user { color: rgba(253, 224, 71, 0.78); }
    .tw-label-dot {
  ...
  function buildAskMotivationReel(){
    ...
    return [
      ...
      twLine('gpt',  'Good question. Let me show you.',                                     { speed: NORM, pause: 700 }),

      // Three very different Sundays
      twLine('user', 'Why does one man spend Sunday fixing his car?',                       { speed: NORM, pause: 600 }),
      twLine('user', 'Why does another sit in a church pew?',                               { speed: NORM, pause: 600 }),
      twLine('user', 'Why does another feed his paycheck into the pokies?',                 { speed: NORM, pause: 900 }),

      // Setup — punchy
      twLine('gpt', 'Same brain. Different behaviour.',                                     { speed: SLOW, pause: 800 }),
      twLine('gpt', "Here's why.",                                                          { speed: SLOW, pause: 700 }),

      // One job — emphasis
      twLine('gpt', 'Every human brain has <strong>one job</strong>: keep you alive.',       { speed: SLOW, pause: 1100 }),

      // Watches + learns
      twLine('gpt', 'But it doesn\'t know what "alive" looks like for <em>you</em>.',       { speed: NORM, pause: 700 }),
      twLine('gpt', 'So it watches. And it learns.',                                        { speed: SLOW, pause: 900 }),

      // The HOW — the mechanism (4 beats, each their own line)
      twLine('gpt', "Here's how it works.",                                                 { speed: SLOW, pause: 700 }),
      twLine('gpt', '1. You do something.',                                                 { speed: NORM, pause: 600 }),
      twLine('gpt', '2. Something happens next — a reward, a feeling, relief.',             { speed: NORM, pause: 700 }),
      twLine('gpt', '3. Your brain files it: <em>"do this → get that."</em>',               { speed: NORM, pause: 800 }),
      twLine('gpt', '4. Next time, it <strong>nudges you</strong> toward what paid off.',   { speed: NORM, pause: 1000 }),

      // The nudge IS motivation — slow punch
      twLine('gpt', 'That nudge… is <strong>motivation</strong>.',                          { speed: SLOW, pause: 1200 }),

      // Three guys → three learned loops
      twLine('gpt', 'Car guy learned: fix the thing → feel competent → safe.',              { speed: NORM, pause: 700 }),
      twLine('gpt', 'Churchgoer learned: show up → belong → safe.',                         { speed: NORM, pause: 700 }),
      twLine('gpt', 'Pokies player learned: near-win → dopamine → safe.',                   { speed: NORM, pause: 900 }),

      // Same code, different environments
      twLine('gpt', 'Three different men. Three different Sundays.',                         { speed: NORM, pause: 700 }),
      twLine('gpt', '<strong>Same survival code. Different environments.</strong>',           { speed: SLOW, pause: 1200 }),

      // Shan follow-up
      twLine('user', 'So motivation isn\'t willpower?',                                     { speed: FAST, pause: 600 }),
      twLine('gpt',  'No. It\'s your brain predicting what <strong>worked last time</strong>.', { speed: NORM, pause: 1000 }),

      // The environment lever — staccato
      twLine('gpt', 'Change your <strong>environment</strong> →',                           { speed: NORM, pause: 500 }),
      twLine('gpt', 'change the <strong>prediction</strong> →',                             { speed: NORM, pause: 500 }),
      twLine('gpt', 'change what you do.',                                                  { speed: SLOW, pause: 1400 }),
  ...
  function buildAskConditioningReel(){
    return [
      // Opening
      ...askLines([
        { who: 'user', text: 'Hey ChatGPT…<br>what\'s Pavlovian conditioning?',        hold: 3500 },
        { who: 'gpt',  text: 'Oldest trick in the brain.',                              hold: 3000 },
        { who: 'gpt',  text: 'Let me show you <strong>three versions</strong> of it.', hold: 3500 },
      ], 'Episode 2'),

      // The classic — Pavlov's dog
      ...askLines([
        { who: 'gpt', text: 'Pavlov had a dog.<br>And a bell.',                         hold: 3200 },
        { who: 'gpt', text: 'Every time he rang the bell…<br>he served the dog steak.', hold: 4200 },
      ], 'The classic'),

      // Loop visual
      ...askLines([
        { who: 'gpt', text: '<span style="font-size:1.8rem; letter-spacing:6px;">🔔 → 🥩 → 🤤</span>',                                 hold: 3500 },
        { who: 'gpt', text: 'Do it enough times, and…',                                                                                 hold: 3000 },
        { who: 'gpt', text: '<span style="font-size:1.8rem; letter-spacing:6px;">🔔 → 🤤</span><br><em>(no steak needed.)</em>',         hold: 4500 },
      ]),

      // The rule
      ...askLines([
        { who: 'gpt', text: 'The brain doesn\'t just learn <em>"do this → get that."</em>',                      hold: 4500 },
        { who: 'gpt', text: 'It learns:<br><strong>"when THIS happens, THAT is coming."</strong>',                hold: 4800 },
        { who: 'gpt', text: 'Then it fires the reward response <strong>before</strong> the reward even arrives.', hold: 4800 },
      ], 'The rule'),

      // Version 1 — the dog
      ...askLines([
        { who: 'user', text: 'Okay, so that\'s how we teach a dog to sit?',                                                                   hold: 3500 },
        { who: 'gpt',  text: 'Exactly.<br>Say <em>"sit"</em> → push the bum down → treat.',                                                    hold: 4200 },
        { who: 'gpt',  text: 'Pretty soon the word <strong>"sit"</strong> alone triggers it.<br>The dog\'s brain learned: <em>"sit" = treat is coming.</em>', hold: 5000 },
      ], 'Version 1 — the dog'),

      // Version 2 — casino setup
      ...askLines([
        { who: 'user', text: 'What\'s the scary version?',                                  hold: 2800 },
        { who: 'gpt',  text: 'Casinos.<br>They\'ve studied this for a hundred years.',      hold: 4000 },
      ], 'Version 2 — the casino'),

      // Casino loop
      ...askLines([
        { who: 'gpt', text: '<span style="font-size:1.25rem;">🎰 lights → 🔔 jingle → 🪙 coins clatter → <strong>occasional win.</strong></span>', hold: 5000 },
        { who: 'gpt', text: 'Do it enough times, and…',                                                                                              hold: 2800 },
        { who: 'gpt', text: 'The <strong>lights and sounds alone</strong> give you a dopamine hit.',                                                 hold: 4200 },
        { who: 'gpt', text: 'You\'re not chasing the money.<br>You\'re chasing <strong>the bell.</strong>',                                           hold: 4500 },
      ]),

      // Near-misses
      ...askLines([
        { who: 'gpt', text: 'Here\'s the evil bit.',                                                                              hold: 2800 },
        { who: 'gpt', text: 'Even a <strong>near-miss</strong> — two cherries and a lemon — lights up the same brain region as a win.', hold: 5200 },
        { who: 'gpt', text: 'Your brain says: <em>"almost! keep going!"</em>',                                                     hold: 3800 },
        { who: 'gpt', text: 'That\'s not luck.<br>That\'s <strong>engineering</strong>.',                                            hold: 3800 },
      ], 'The near-miss trap'),

      // Version 3 — YOU
      ...askLines([
        { who: 'user', text: 'So… is this happening to me too?',           hold: 3200 },
        { who: 'gpt',  text: 'Every day.<br>You just never called it that.', hold: 3800 },
      ], 'Version 3 — you'),

      // Your bells
      ...askLines([
        { who: 'gpt', text: '📱 Phone buzz → dopamine.<br><em>Before you\'ve even read the message.</em>',                          hold: 4500 },
        { who: 'gpt', text: '🧊 Fridge door at 9pm → craving.<br><em>Before you\'re even hungry.</em>',                               hold: 4500 },
        { who: 'gpt', text: '🏋️ Gym car park → dread, or pride.<br><em>Depending on which one your brain paired it with.</em>',      hold: 5200 },
      ], 'Your bells'),

      // Realisation
      ...askLines([
        { who: 'gpt', text: 'Your life is full of <strong>bells</strong>.',                             hold: 3500 },
        { who: 'gpt', text: 'Every one of them is either training you…<br>or being trained <em>by</em> you.', hold: 4500 },
      ]),

      // The fix
      ...askLines([
        { who: 'user', text: 'So how do I un-train the bad ones?',                                                   hold: 3200 },
        { who: 'gpt',  text: '<strong>Break the pair.</strong>',                                                      hold: 2800 },
        { who: 'gpt',  text: '🔔 Ring the bell → <em>don\'t serve the steak.</em>',                                    hold: 4000 },
        { who: 'gpt',  text: '📱 Phone buzzes → leave it face-down.<br>🧊 Fridge opens at 9pm → water, not biscuits.',  hold: 5200 },
        { who: 'gpt',  text: 'Do it enough times, and the bell <strong>stops meaning anything.</strong>',              hold: 4500 },
      ], 'The fix'),
  ...
  function buildAskFreewillReel(){
    // Inline timeline visual showing the Libet gap. Kept as an inline
    // flex row so it stays readable inside a single chat bubble.
    const LIBET_TIMELINE = `
      <div style="display:flex; align-items:center; justify-content:space-between; gap:6px; margin:10px 0 6px; font-size:0.78rem; font-weight:700; line-height:1.2;">
        <span style="background:#0ea5e9; color:#fff; padding:6px 8px; border-radius:8px; text-align:center; flex:1;">🧠<br><span style="font-size:0.62rem; letter-spacing:1px;">brain fires</span></span>
        <span style="color:#fde047; font-weight:800; font-size:0.7rem; white-space:nowrap;">── 350ms ──</span>
        <span style="background:#6366f1; color:#fff; padding:6px 8px; border-radius:8px; text-align:center; flex:1;">👁<br><span style="font-size:0.62rem; letter-spacing:1px;">you feel it</span></span>
        <span style="color:#fde047; font-weight:800; font-size:0.7rem; white-space:nowrap;">── 200ms ──</span>
        <span style="background:#22d3ee; color:#042f3a; padding:6px 8px; border-radius:8px; text-align:center; flex:1;">✋<br><span style="font-size:0.62rem; letter-spacing:1px;">hand moves</span></span>
      </div>`;

    return [
      // Opening
      ...askLines([
        { who: 'user', text: 'Hey ChatGPT…<br>do I actually have free will?',                              hold: 3000 },
        { who: 'gpt',  text: 'Honest answer from 30 years of brain scans?<br><strong>Probably not.</strong>', hold: 3800 },
        { who: 'gpt',  text: 'But stick with me — this is <em>good news</em>.<br>Three experiments.',       hold: 3800 },
      ], 'Episode 3'),

      // The claim
      ...askLines([
        { who: 'gpt', text: 'Everyone feels like they\'re the one making decisions. <em>"I chose this."</em>',                           hold: 4000 },
        { who: 'gpt', text: 'But when scientists watch the actual moment of choice on a brain scanner, something weird happens:',         hold: 4500 },
        { who: 'gpt', text: 'The brain decides <strong>first</strong>. The <em>feeling</em> of deciding arrives <strong>later</strong>.',  hold: 4200 },
      ], 'The claim'),

      // Libet setup
      ...askLines([
        { who: 'gpt', text: 'Experiment 1.<br>Benjamin Libet hooks you up to an <strong>EEG</strong> — a cap that reads your brain activity in real time.', hold: 5000 },
        { who: 'gpt', text: 'Task: flick your wrist whenever you feel like it. Watch this clock — the instant you <em>feel the urge</em>, remember where the dot is.', hold: 6000 },
      ], 'Study 1 · Libet · 1983'),

      // Libet finding
      ...askLines([
        { who: 'gpt', text: 'Now he has three timestamps: when your <strong>brain</strong> started preparing, when <strong>you</strong> felt yourself decide, and when your <strong>hand</strong> moved.', hold: 5500 },
        { who: 'gpt', text: `Here\'s what he found:${LIBET_TIMELINE}`,                                                                                  hold: 5200 },
        { who: 'gpt', text: 'Your brain committed <strong>a third of a second before</strong> you consciously chose.<br>Your awareness was the <em>last to know.</em>', hold: 5200 },
      ], 'Study 1 · Libet · 1983'),

      // Rebuttal → Soon intro
      ...askLines([
        { who: 'user', text: 'Okay but that\'s just a wrist flick. Maybe that\'s reflex.', hold: 3500 },
        { who: 'gpt',  text: 'That\'s what everyone said for 25 years.<br>Then 2008 happened.', hold: 3800 },
      ]),

      // Soon — 10 seconds
      ...askLines([
        { who: 'gpt',  text: 'Berlin. A proper <strong>fMRI scanner</strong> — reads deep brain activity, not just the scalp.',                    hold: 4200 },
        { who: 'gpt',  text: 'You pick <strong>left button</strong> or <strong>right button</strong>, whenever you want.',                          hold: 3500 },
        { who: 'gpt',  text: 'A computer reads your brain patterns and predicts your choice <strong>up to 10 seconds before</strong> you feel yourself choose.', hold: 5200 },
        { who: 'user', text: 'Ten <em>seconds?!</em>',                                                                                              hold: 2000 },
      ], 'Study 2 · Soon et al. · 2008'),

      // Desmurget setup
      ...askLines([
        { who: 'gpt', text: 'Final experiment — the kicker.',                                                              hold: 2500 },
        { who: 'gpt', text: 'French surgeon Michel Desmurget, operating on brain tumours.<br>Patients are <strong>awake</strong> — the brain has no pain receptors, and staying conscious lets the surgeon test each region before cutting.', hold: 6500 },
        { who: 'gpt', text: 'With the skull open, he gently stimulates different spots with an electrode and asks what the patient feels.', hold: 5000 },
      ], 'Study 3 · Desmurget · 2009'),

      // Parietal zap
      ...askLines([
        { who: 'gpt', text: 'First spot: the <strong>parietal cortex</strong>, near the top of your head.',                                                                          hold: 3800 },
        { who: 'gpt', text: 'He zaps it. The patient instantly says:<br><em>"I wanted to move my right hand."</em>',                                                                  hold: 4500 },
        { who: 'gpt', text: 'But here\'s the thing — <strong>their hand never moved.</strong> The muscles never fired.',                                                              hold: 4200 },
        { who: 'gpt', text: 'The <em>feeling of willing it</em> was manufactured by the electrode. No decision was made. They just <strong>felt like they made one.</strong>',          hold: 5500 },
      ], 'Study 3 · Desmurget · 2009'),

      // Premotor zap + "two rooms"
      ...askLines([
        { who: 'gpt', text: 'Now he moves the electrode a few centimetres over, to the <strong>premotor cortex</strong>, and zaps that instead.',                                     hold: 4800 },
        { who: 'gpt', text: 'The patient\'s hand <strong>actually moves.</strong> Clearly, visibly.',                                                                                  hold: 3500 },
        { who: 'gpt', text: 'He asks: <em>"Did you just move your hand?"</em><br>Patient: <strong>"No. I didn\'t do anything."</strong>',                                                hold: 4500 },
        { who: 'gpt', text: 'Feeling-of-choosing and act-of-choosing live in <strong>different rooms</strong> of your brain — your sense of <em>"I chose this"</em> is a <strong>separate signal</strong>, produced alongside the action to make it feel like yours.', hold: 7500 },
      ], 'Study 3 · Desmurget · 2009'),

      // Gazzaniga — the interpreter
      ...askLines([
        { who: 'gpt', text: 'One more twist. What happens when you do something your conscious brain didn\'t order?',                                                                  hold: 4500 },
        { who: 'gpt', text: 'Michael Gazzaniga studied <strong>split-brain</strong> patients — left and right hemispheres surgically separated, unable to talk to each other.',          hold: 5800 },
        { who: 'gpt', text: 'He\'d flash an instruction to the silent right hemisphere: <em>"stand up."</em><br>The patient stood up. Then he\'d ask: <em>"Why?"</em>',                   hold: 5000 },
        { who: 'gpt', text: 'The verbal left hemisphere never saw the instruction — but it <strong>never says "I don\'t know."</strong> It invents a reason: <em>"I felt like getting a drink."</em>', hold: 6500 },
        { who: 'gpt', text: 'Your brain would rather <strong>make up a story</strong> than admit it isn\'t in charge.', hold: 4200 },
      ], 'The interpreter'),

      // The lesson
      ...askLines([
        { who: 'user', text: 'So… who\'s actually running me?',                                                                                                     hold: 2800 },
        { who: 'gpt',  text: '<strong>You</strong> are what your brain shows you — a third of a second after it\'s already decided.<br>Not the captain. The <em>narrator</em>.', hold: 5500 },
        { who: 'gpt',  text: 'Here\'s why that\'s good news: every time you\'ve called yourself <strong>lazy, weak, no willpower</strong> — you were blaming the <em>narrator</em> for what the brain had already committed to.', hold: 6800 },
        { who: 'gpt',  text: 'You can\'t out-willpower a brain that decided before you felt it.<br>But you <strong>can</strong> change what it learns to decide.',    hold: 5200 },
        { who: 'gpt',  text: '🔔 Change what you <strong>pair</strong> <em>(Ep 2)</em>.<br>🌱 Change your <strong>environment</strong> <em>(Ep 1)</em>.<br>That\'s the lever.', hold: 5500 },
        { who: 'gpt',  text: '<em>"We are not captains of our ship.<br>Our ships never had captains."</em><br>— Sapolsky',                                           hold: 5500 },
      ], 'The lesson'),
  ...
  function buildAskIdentityReel(){
    return [
      ...
      ...askLines([
        { who: 'user', text: 'Hey ChatGPT, quick question—',                                            hold: 2800 },
        { who: 'gpt',  text: 'Before you ask.<br>Look straight at the screen.',                           hold: 3500 },
        { who: 'gpt',  text: '…now <strong>think about your nose.</strong>',                              hold: 3800 },
        { who: 'user', text: '…wait. I can see it now.<br>I couldn\'t a second ago.',                     hold: 3800 },
        { who: 'gpt',  text: 'Exactly.<br>Buckle up.',                                                    hold: 2800 },
      ], 'Episode 4'),

      // Why you don't see your nose
      ...askLines([
        { who: 'gpt', text: 'Your nose is in your visual field <strong>every waking second of your life.</strong>',     hold: 4500 },
        { who: 'gpt', text: 'Your eyes have been sending that image to your brain the entire time.',                      hold: 4200 },
        { who: 'gpt', text: 'But your brain decided a long time ago: <em>"this is useless information."</em>',             hold: 4500 },
        { who: 'gpt', text: 'So it <strong>edits it out.</strong><br>You stop seeing what\'s actually there.',              hold: 4000 },
      ], 'Your brain edits reality'),

      // The blind spot
      ...askLines([
        { who: 'gpt', text: 'Want something weirder?',                                                                                          hold: 2500 },
        { who: 'gpt', text: 'Every eye has a literal <strong>hole</strong> in the retina — the spot where the optic nerve exits. No photoreceptors. No image.', hold: 5500 },
        { who: 'gpt', text: 'So why don\'t you see two black dots floating in front of you right now?',                                         hold: 4500 },
        { who: 'gpt', text: 'Because your brain <strong>paints over the hole</strong> with its best guess of what should be there.<br>You never notice.', hold: 5000 },
      ], 'The blind spot'),

      // The rule
      ...askLines([
        { who: 'gpt', text: 'Here\'s the rule you were never taught:',                                                                                 hold: 3200 },
        { who: 'gpt', text: 'You don\'t see reality.<br>You see your brain\'s <strong>best guess</strong> of reality — edited for usefulness.',          hold: 5500 },
        { who: 'gpt', text: 'When the guess is close enough, you never question it.<br>When it isn\'t, you call it an <em>illusion</em>.',               hold: 5500 },
      ], 'The rule'),

      // It's not just vision
      ...askLines([
        { who: 'gpt', text: 'And it\'s not just eyes.<br>Every sense you have is a <strong>controlled hallucination.</strong>',   hold: 4500 },
        { who: 'gpt', text: '🎨 <strong>Colour</strong> is a label your brain invents.<br>A tomato isn\'t "red" — it\'s reflecting a wavelength.', hold: 5000 },
        { who: 'gpt', text: '👅 <strong>Flavour</strong> is 80% smell.<br>Block your nose and strawberry tastes like sugar water.', hold: 4800 },
        { who: 'gpt', text: '🩹 <strong>Pain</strong> is constructed.<br>Amputees feel pain in limbs that aren\'t there anymore.',  hold: 4800 },
      ], 'All of it'),

      // Shan's pushback
      ...askLines([
        { who: 'user', text: 'Okay, but that\'s just my senses.<br>I\'m still <em>me</em>.', hold: 3500 },
        { who: 'gpt',  text: 'Ah.<br>About that.',                                            hold: 2500 },
      ]),

      // The twist — and so are you
      ...askLines([
        { who: 'gpt', text: 'You feel like there\'s a <strong>"you"</strong> watching through your eyes — a single, continuous person, the voice in your head, the one making decisions.', hold: 6500 },
        { who: 'gpt', text: 'Neuroscientists have spent 50 years looking for that person.',                                   hold: 3800 },
        { who: 'gpt', text: 'They can\'t find one.<br>There is no single spot in your brain that\'s <em>"you."</em>',          hold: 4800 },
      ], 'And so are you'),

      // Memory is reconstructed
      ...askLines([
        { who: 'gpt', text: 'You think you <em>remember</em> your past.',                                                                                hold: 3500 },
        { who: 'gpt', text: 'You don\'t. Every time you recall a memory, your brain <strong>rebuilds it from scratch</strong> — and the new version overwrites the old one.', hold: 6500 },
        { who: 'gpt', text: 'Your childhood is a painting your brain repaints every time you look at it.',                                                hold: 4500 },
      ]),

      // The rubber hand illusion
      ...askLines([
        { who: 'gpt', text: 'Classic experiment. Hide someone\'s real hand. Put a <strong>rubber hand</strong> in front of them, and stroke both with a brush at the same time.', hold: 6000 },
        { who: 'gpt', text: 'In about <strong>30 seconds</strong>, their brain decides the rubber hand is theirs.',                    hold: 4500 },
        { who: 'gpt', text: 'Smash it with a hammer — they flinch. Real panic.',                                                      hold: 4000 },
        { who: 'gpt', text: 'Your brain doesn\'t even know what counts as <strong>your own body.</strong><br>It\'s guessing.',          hold: 4500 },
      ], 'The rubber hand'),

      // Identity as a story
      ...askLines([
        { who: 'gpt', text: 'So if there\'s no "you" in the brain, what <em>are</em> you?',                                                               hold: 3800 },
        { who: 'gpt', text: 'You\'re a <strong>story</strong>.',                                                                                            hold: 2200 },
        { who: 'gpt', text: 'A narrative your brain keeps writing, second by second — from memories (reconstructed), senses (hallucinated), and decisions (already made. remember Ep 3?).', hold: 7500 },
        { who: 'gpt', text: 'Identity isn\'t a thing you <em>have</em>.<br>It\'s a thing your brain is <strong>making right now.</strong>',                hold: 4800 },
      ]),

      // The lesson — the freeing version
      ...askLines([
        { who: 'user', text: 'That\'s terrifying.<br>And also… kinda freeing?',                                                                                              hold: 3500 },
        { who: 'gpt',  text: 'Exactly.',                                                                                                                                      hold: 2000 },
        { who: 'gpt',  text: 'If the "you" is a construction, you don\'t have to <em>discover</em> yourself.<br>You get to <strong>build</strong> yourself.',                 hold: 5500 },
        { who: 'gpt',  text: '🔔 Every action is a brush stroke in the painting <em>(Ep 2)</em>.<br>🌱 Every environment feeds it new colours <em>(Ep 1)</em>.<br>🧭 Every choice is a vote for who you\'re becoming <em>(Ep 3)</em>.', hold: 7500 },
        { who: 'gpt',  text: '<em>"You\'re not discovering who you are.<br>You\'re deciding who you become."</em>',                                                            hold: 5500 },
      ], 'The lesson'),
