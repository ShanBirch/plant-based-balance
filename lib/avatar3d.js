...
class Avatar3D {
  constructor(containerId, options = {}) {
    ...
    this.options = {
      ...
      cacheBustModel: options.cacheBustModel !== false,
  ...
  async loadCharacter() {
    ...
    if (this.options.modelUrl) {
      try {
        const modelUrl = this.options.cacheBustModel === false
          ? this.options.modelUrl
          : this.options.modelUrl + (this.options.modelUrl.includes('?') ? '&' : '?') + 'cb=' + Date.now();
        await this.loadGLTFModel(modelUrl);
    ...
    // Add cache-busting for compressed models
    const cacheBust = `?v=${Date.now()}`;

    for (const path of defaultModelPaths) {
      try {
        await this.loadGLTFModel(path + cacheBust);
        this.modelLoaded = true;
   ...
   */
  async loadGLTFModel(url, maxRetries = 2) {
    let lastError;
    
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        await this._loadGLTFWithTimeout(url, 30000); // 30 second timeout
        return;
      } catch (err) {
        lastError = err;
        console.warn(`Model load attempt ${attempt + 1} failed:`, err.message);
   ...
   */
  _loadGLTFWithTimeout(url, timeout) {
    return new Promise((resolve, reject) => {
      ...
      // Add MeshoptDecoder for meshopt-compressed models if available
      ...
      loader.load(
        ...
        (progress) => {
          ...
          const percent = hasTotal
            ? Math.min(100, Math.max(0, progress.loaded / progress.total * 100))
            : null;

          if (typeof this.options.onModelProgress === 'function') {
  ...
  updateAnimationForMood() {
    ...
    // Map moods to actual GLB animation names
    const moodAnimations = {
      champion: ['arms_up_still', 'clap', 'dance', 'laugh', 'heart_pose'],
      energetic: ['dance', 'dance_1', 'strut', 'walk', 'cartwheel'],
      happy: ['laugh', 'greet', 'agree', 'idle'],
      neutral: ['idle', 'stand', 'stand_hands_on_hips'],
      tired: ['sad_stand', 'sit', 'fold_arms', 'idle'],
      sad: ['sad_stand', 'depressed_walk', 'lose', 'idle']
    };
  ...
  animate() {
    ...
    if (document.hidden || this.isRenderPaused) {
      return;
    }
  ...
  async loadModel(url) {
    try {
      await this.loadGLTFModel(url + (url.includes('?') ? '&' : '?') + 'cb=' + Date.now());
      this.modelLoaded = true;
