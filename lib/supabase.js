...
function getLocalDateString(date) { return window.getLocalDateString(date); }
  ...
  admin: {
    ...
    async getUserWeekData(userId, days = 7) {
      ...
      const [
        ...
        safeQuery(() => supabase.from('daily_nutrition').select('*').eq('user_id', userId).gte('date', sinceDate).order('date', { ascending: true })),
