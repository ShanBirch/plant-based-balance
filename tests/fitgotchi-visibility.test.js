...
assert.match(dashboard, /dashboard-script-5-initialize_stripe_for_inapp_pu\.js\?v=225-calendar-home-coins/, 'phones should fetch the current Calendar, Home coin, and meal-history behavior');
assert.match(dashboard, /dashboard-script-10-points_widget_functions\.js\?v=49/, 'phones should fetch the compact level updates');
assert.match(serviceWorker, /const CACHE_NAME = 'pbb-app-v433-insights-history-toggle'/, 'the app shell cache should advance for this feature');
