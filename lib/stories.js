...
async function prepareUploadableVideo(file, onProgress, options = {}) {
    ...
    const fallbackMinBitrate = isSmallUploadTarget ? 1200000 : PHONE_VIDEO_FINAL_BITRATE;
    const finalMaxDimension = isSmallUploadTarget ? 540 : PHONE_VIDEO_FINAL_MAX_DIMENSION;
    const finalMinBitrate = isSmallUploadTarget ? 700000 : 6000000;
...
window.renderMealCard = function(cardData) {
    ...
    const backgroundPhotoHtml = hasBackgroundPhoto
        ...
        : '';
    const overlayHtml = hasBackgroundPhoto
        ? '<div style="position:absolute; inset:0; z-index:0; background:linear-gradient(180deg, rgba(0,0,0,0.52) 0%, rgba(0,0,0,0.68) 48%, rgba(0,0,0,0.82) 100%);"></div>'
        : '';
...
window.createWorkoutFeedSharePost = async function(options = {}) {
    ...
    try {
        ...
        logFeedUploadDiagnostic('share_set_post_success', {
            ...
            storyId: story?.id || '',
            pointsAwarded,
            uploadMode: upload.uploadData?.directUpload ? 'direct_b2' : 'netlify'
        });
        return {
            story,
            pointsAwarded,
    ...
    try {
        ...
        const renderedFeedHtml = sortedStories.map(story => {
            ...
            const quickReactions = FEED_REACTIONS.map(r => {
                ...
                return `<button type="button" class="feed-reaction-btn feed-reaction-inline-btn" data-reaction="${r.key}" data-feed-reaction-variant="inline" data-active="${isActive ? 'true' : 'false'}" aria-pressed="${isActive ? 'true' : 'false'}" aria-label="${ariaLabel}" onclick="event.stopPropagation(); toggleFeedReaction('${storyId}', '${r.key}', this)" style="background:none; border:none; padding:4px; cursor:pointer; font-size:1.2rem; line-height:1; opacity:${isActive ? '1' : '0.5'}; transform:${isActive ? 'scale(1.15)' : 'scale(1)'}; --feed-reaction-rest-transform:${isActive ? 'scale(1.15)' : 'scale(1)'}; transition:opacity 0.18s ease, transform 0.22s cubic-bezier(0.2, 1.28, 0.34, 1); will-change:transform;">${r.emoji}</button>`;
            }).join('');
                ...
                <div class="feed-post-card" data-story-id="${storyId}" style="background:white; border-bottom:8px solid #f1f5f9;">
                    ...
                    ${isCardType ? (() => {
                        ...
                        if (cardData && hasPhoto && (isMealCard || isMealPayload)) {
                            return '<div onclick="openFeedPostViewer(\'' + storyId + '\')" style="cursor:pointer; width:100%;">' + renderFn(withFeedCardPhoto(cardData, cardPhotoSrc)) + '</div>';
                        }
