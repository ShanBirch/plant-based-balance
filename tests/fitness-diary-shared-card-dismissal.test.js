...
const nextSteps = fs.readFileSync('js/dashboard/pbb-next-obvious-steps.js', 'utf8');
...
test('sharing the diary also removes its stale To do next suggestion', () => {
  assert.match(diary, /hideFitnessDiaryShareCards\(dateKey\);[\s\S]*?window\.pbbNextSteps\.refresh\(\)/);
  assert.match(nextSteps, /pbbFitnessDiarySharedToFeed_/);
});
