...
- The Balance AI coach pipeline creates the first draft for every unlinked lead. The API manager owns final review and delivery for normal unlinked-lead replies, with full-thread readback and no-double-send reconciliation. Only the first inbound carrying a verified current Meta `ADS` referral may be auto-scheduled by the AI-coach pipeline for four minutes later; the manager reconciles it but never sends a parallel reply. Shannon should not receive lead handoffs for ordinary draft/style judgment calls; explicit safety, authenticity, transport, or unrecoverable-media holds still route to Needs You. Linked IG/FB clients remain approval-only.
...
- A first inbound carrying a verified Meta `ADS` referral starts the paid-ad fast lane: persist its ad/referral attribution on the thread, run the normal media/context/safety/draft review, and schedule a clean passing draft for four minutes later. Never send it instantly. Keep follow-up replies fast while the ad referral is no more than 24 hours old and the thread has been active within the last two hours. A historic ad origin alone never grants a permanent fast lane, and only the actual first ad inbound uses the deterministic ad-opening reply; later messages use the full conversation-aware draft generator. Separately, any current unlinked-lead inbound, or an exchange active within the last two hours, that clearly discusses exercise or training may use the same four-minute lane after a clean pass. Any reviewer warning or hard hold falls back to the DM manager; linked clients always remain draft-only.
