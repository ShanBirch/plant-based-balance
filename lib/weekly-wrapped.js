...
(function(){
  ...
  async function buildData(userId){
    ...
    const untilThisDate = formatDate(thisWeekEnd);
    ...
    const caloriesAvg = trackedMealDays
  ...
  async function maybeAwardWeeklyGoalsInWrappedData(data){
    const supabase = window.supabaseClient;
    if (!supabase || !data || !data.goals || !data.goals.total || !data.week_start) return null;
    const weekStartDate = formatDate(new Date(data.week_start));
    const reward = data.goals.reward || calculateWeeklyGoalReward(data.goals.completed, data.goals.total);
    const alreadyAwarded = Math.max(0, Math.min(WEEKLY_GOAL_XP_CAP, Number(reward.awarded || 0)));

    if (Number(reward.earned || 0) <= alreadyAwarded) return null;
    ...
    try {
      ...
      if (error) {
        ...
        return {
          success: false,
          pointsAwarded: 0,
          totalWeeklyGoalPoints: alreadyAwarded,
          maxWeeklyGoalPoints: reward.max || 0,
          completedCount: data.goals.completed,
          totalCount: data.goals.total,
          allGoalsHit: data.goals.total > 0 && data.goals.completed >= data.goals.total
        };
      }

      const result = normalizeRpcJson(rpcData);
      const pointsAwarded = Number(result.pointsAwarded || result.points_awarded || 0);
      const totalAwarded = Math.max(
        alreadyAwarded + pointsAwarded,
        Number(result.totalWeeklyGoalPoints || result.total_weekly_goal_points || 0)
      );
      const maxPoints = Number(result.maxWeeklyGoalPoints || result.max_weekly_goal_points || reward.max || WEEKLY_GOAL_XP_CAP);

      data.goals.reward = Object.assign({}, data.goals.reward || {}, {
        awarded: Math.min(WEEKLY_GOAL_XP_CAP, totalAwarded),
        just_awarded: Math.max(0, Math.min(WEEKLY_GOAL_XP_CAP, pointsAwarded)),
        max: maxPoints,
        all_goals_hit: Boolean(result.allGoalsHit || result.all_goals_hit),
  ...
  function buildWeeklyGoalsSummary(row, userId, weekStartDate, rewardResult){
    ...
    const liveRow = liveMatchesReviewWeek && liveState && liveState.row && typeof liveState.row === 'object'
      ? liveState.row
      : null;
    // Review the week being wrapped, not whichever week the home card has
    // already rolled to. Prefer the matching live state, then the persisted
    // snapshot the home card saves for that exact week.
    ...
    const items = selected.map(goal => {
      const progress = snapshotItems.find(item => item && item.id === goal.id) || {};
      return {
        id: goal.id,
        ...
        target: progress.target != null ? progress.target : goal.target,
        unit: goal.unit || progress.unit || '',
        complete: !!progress.complete
      };
    });
    // Prefer the live snapshot count when we have one. The stored row can lag
    // behind or carry a stale completed_count from an older render.
  ...
  function buildGoalSettingReel(){
    return [
      ...
      { bg: 'ww-bg-pbs', hold_ms: 4300, html: `
        ...
        <div class="ww-caption" style="color:#1a0a00; font-weight:800;">Progress rolls into your Weekly Wrapped.</div>
      `},
