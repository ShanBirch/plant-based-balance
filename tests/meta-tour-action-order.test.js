...
test('paid tour explains guided actions before performing them', () => {
  for (const button of ['Open workout', 'Open meal plan', 'Open first lesson', 'Open message', 'Choose my goals']) {
  ...
  assert.match(dashboard, /if \(pendingPromptedAction && pendingPromptedAction\.index === idx\)[\s\S]*await prompted\.step\.action\(\)/);
});
