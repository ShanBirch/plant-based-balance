...
const {OWNER_ID,classifierPrompt,parseDecision,replyFor}=require('./_lib/lcp-dm-knowledge');
...
exports.handler=async event=>{
 ...
 try{
  ...
  if(input.action==='preview'){
   ...
   return {statusCode:200,body:JSON.stringify({decision,...replyFor(decision,await offerNow()),sent:false})};
  }
  ...
  const access=secrets.find(x=>x.key==='lcp_ig_access_token')?.value;
  ...
  return {statusCode:200,body:JSON.stringify({version:'lcp-dm-v1',settings,identity,signatureConfigured:secrets.some(x=>x.key==='lcp_ig_app_secret'&&x.value),shop:await offerNow()})};
