...
window.loadPhotoFeed = async function(targetGridId = 'photo-feed-grid', emptyStateId = 'photo-feed-empty', options = {}) {
    ...
    try {
        ...
        const renderedFeedHtml = sortedStories.map(story => {
            ...
            return `
                <div class="feed-post-card" data-story-id="${storyId}" data-feed-created-at="${escapeHtml(story.created_at || '')}" data-feed-created-ms="${getFeedStoryCreatedMs(story)}" style="background:white; border-bottom:8px solid #f1f5f9;">
