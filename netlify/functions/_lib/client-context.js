...
- Paid-Facebook app preview: the ad destination is https://plantbased-balance.org/meta-app-preview.html with the campaign UTMs and Meta identifiers retained. Only that attributed route can open the read-only preview. The person completes onboarding locally, then gets five minutes to explore before a non-dismissible Balance Foundations offer at AU$89.99 once for the fixed six weeks, with no automatic renewal. The compact gate records legal acceptance, opens Stripe Checkout, and returns the buyer to create or access their Balance account with the preview answers preserved. Organic visitors and existing members keep their normal app path.
