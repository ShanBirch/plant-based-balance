...
assert.ok(dashboard.includes('dashboard-script-10-points_widget_functions.js?v=58-reliable-share-actions'), 'phones must load the new share composer');
assert.ok(dashboard.includes('dashboard-script-11-calorie_tracker_functions.js?v=39-private-share-studio'), 'phones must load the current meal tracker controls');
...
assert.ok(serviceWorker.includes("const CACHE_NAME = 'pbb-app-v464-reliable-share-actions'"), 'the app shell cache must be refreshed');
assert.ok(serviceWorker.includes('dashboard-script-10-points_widget_functions.js?v=58-reliable-share-actions'), 'the new share composer must be precached');
