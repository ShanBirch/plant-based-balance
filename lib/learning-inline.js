...
There's no perfect program—only the one you'll actually do consistently with effort. Start where you are, apply the principles, and trust the process.`,
    ...
    async function awardQuizChallengePoint() {
        try {
            ...
            // Trigger server-side recalculation of challenge points from lesson_completions
            // (the SQL function counts 100% completions within the challenge date range)
        ...
        try {
            if (window.supabaseClient && window.currentUser) {
                ...
                if (error) {
                    ...
                    // Refresh quiz challenge scores (server recalculates from lesson_completions)
