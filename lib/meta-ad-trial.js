...
(function (root, factory) {
    ...
    function renderFoundationsOffer(offer) {
        if (!window.document || !offer || offer.plan !== 'balance_foundations_six_week') return false;
        ...
        if (priceCopy) priceCopy.textContent = 'One ' + formatted + ' payment for the full six weeks. No subscription or automatic renewal.';
        if (termsCopy) termsCopy.textContent = 'One ' + formatted + ' payment covers the full six weeks and does not renew automatically.';
        if (exitButton) exitButton.textContent = 'PAY ' + formatted + ' NOW';
        return true;
    }
    ...
    function loadFoundationsOffer() {
        ...
        offerRequest = window.fetch('/.netlify/functions/get-checkout-offer?priceId=' + encodeURIComponent(META_FOUNDATIONS_PRICE_ID), {
            ...
            return renderFoundationsOffer(offer) ? offer : null;
        }).catch(function () {
            offerRequest = null;
            return null;
        });
    ...
    function checkoutCompliance(email) {
        ...
        return {
            ...
            email: email,
            name: profile.name || '',
            plan_key: 'balance_foundations_six_week',
            ...
            idempotency_key: complianceSessionId + ':checkout_session_created:balance_foundations_six_week'
        };
    }
    ...
    async function beginCheckout() {
        ...
        try {
            ...
            track('checkout_started', {
                plan: 'balance_foundations_six_week',
