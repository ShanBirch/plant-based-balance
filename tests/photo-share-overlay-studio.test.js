...
assert.ok(dashboard.includes('dashboard-script-10-points_widget_functions.js?v=63-preview-matched-activity'), 'phones must load the new share composer');
assert.ok(dashboard.includes('dashboard-script-11-calorie_tracker_functions.js?v=41-share-done-flow'), 'phones must load the current meal tracker controls');
...
assert.ok(serviceWorker.includes("const CACHE_NAME = 'pbb-app-v491-preview-matched-activity'"), 'the app shell cache must be refreshed');
assert.ok(serviceWorker.includes('dashboard-script-10-points_widget_functions.js?v=63-preview-matched-activity'), 'the new share composer must be precached');
