(function () {
    ...
    function requirements(course, week, completed = window.getCourseLessonCompletions?.() || []) {
        ...
        const count = rows.filter(row => completed.includes(row.id) && (course !== 'master' || window.BalanceLessonReflections?.has(row.id))).length;
