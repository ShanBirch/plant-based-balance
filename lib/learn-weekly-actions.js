(function(root, factory) {
  ...
  const experiments = [
    ...
    { title:'Take part in the Balance Feed', lessonId:'mind-6-5', requiresFeed:true,
      prompt:'Across the week, share three Balance Feed posts about a meal, workout or walk. Leave a meaningful comment on three other people\'s posts, responding to what they shared. A PB post can be one of your three; no PB is required. In your weekly check-in, describe what you shared, your interactions and whether participating changed your motivation or what you actually did. No change is a valid outcome.',
      reflection:'What meals, workouts or walks could you share this week? Plan three posts across the week and comments on three other people\'s posts. Notice whether participating changes your motivation or actions.',
      fields:[['posts','What I shared in my three meal, workout or walk posts'],['comments','What I said on three other people\'s posts and how the interactions felt'],['outcome','Whether participating changed my motivation or what I did, including no change']],
      criteria:'Saved Feed evidence shows at least three member posts and comments on three distinct posts by other people during this course week. The check-in describes three meal/workout/walk posts, meaningful interactions and the effect or lack of effect on motivation or actions. A PB and a positive outcome are not required.' },
  ...
  function experiment(week, version = currentVersion()) {
    ...
    return item ? Object.assign({ week: Number(week), key:'learn_w' + Number(week) + '_v3', version:index===5?5:4, curriculum_version:version, requiresMeal:index===6 },item) : null;
  }
  ...
  function feedEvidence(posts, comments, user, start, end) {
    const within = row => row.user_id === user && Date.parse(row.created_at) >= Date.parse(start) && Date.parse(row.created_at) < Date.parse(end);
    const unique = (rows,key) => [...new Map(rows.filter(r=>r[key]).map(r=>[r[key],r])).values()];
    const own = unique(posts.filter(within),'id');
    const others = unique(comments.filter(r=>within(r) && r.stories?.user_id && r.stories.user_id!==user && String(r.comment_text||'').trim()),'story_id');
    return {posts:own,comments:others,post_count:own.length,comment_post_count:others.length,complete:own.length>=3 && others.length>=3};
  }
  return { experiment, meals, workouts, effectiveWeek, reportsForWeek, movementDays, reportComplete, feedEvidence, curriculum, version: 5 };
});
