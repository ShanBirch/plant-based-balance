...
assert.match(igDraftSource, /specific life hook -> daily rhythm or preference -> health\/fitness\/food\/energy context -> their goal or blocker -> optional quick call/);
assert.match(igDraftSource, /quick call is a late, optional bridge, not an opener/);
assert.match(igDraftSource, /explicitly accept Shannon's just-offered quick chat after the earned lead window/);
assert.match(igDraftSource, /approved booking link in the next DM/);
...
assert.match(clientContextSource, /quick-call question is a valid bridge/);
