...
- Common paid-Meta funnel turns use one deterministic progression contract before the general relationship-style reviewer: clicked prompt -> goal -> blocker -> support fit -> matched offer -> explicit link handoff. Every non-terminal progression reply asks exactly one purposeful next question; style-only question-fatigue or sales-heaviness feedback cannot strip it or push it into the ten-minute manager queue. That question must advance one stage and must never repeat the semantic question the lead just answered. Treat blocker evidence stored as relationship context or stressors, including `work and kids get in the way`, as a real blocker so the next turn moves to support/accountability instead of asking what gets in the way again. Direct checkout handoffs, real opt-outs, safety/medical or authenticity questions, and genuine media/context uncertainty remain exceptions. `I stop and start again` is a blocker, not an opt-out. On the plant-based route, send the native proof video after the goal is stated. A voice note is earned after at least two meaningful lead replies when both the goal and real blocker are known, especially for accountability or a personalised-program explanation. Use a proactive voice note at most once per day, always allow an immediate voice reply when the lead sends a new voice note, and answer in text whenever they say they cannot or do not want to listen.
...
- Write loose spoken sentences, not Reel copy. The approved scripts each had three to four imperfect thinking beats, such as `umm`, `ahh`, `okay`, `yeah`, `honestly`, `anyway`, `alright`, `like`, `you know`, or a small repeated thought. Require at least one genuine `umm` or `ahh`. Use commas, sentence breaks, or an occasional ellipsis as audible breathing space, and vary placement so notes do not share a template. Follow the context, reductions and measured pause bands in `content-lab/SHANNON-SPOKEN-VOICE-BIBLE.md`.
