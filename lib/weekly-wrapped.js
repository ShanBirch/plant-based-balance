...
(function(){
  ...
  function buildPromoSlides(){
    ...
    // Level-5-ish FitGotchi (level_10_real is the first evolved form off the
    // starter egg — closest we have to "lvl 5"). Model-viewer is lazy-loaded
    // in openPromo() so the element is defined by the time this slide shows.
    const FITGOTCHI_MODEL = `
      <model-viewer
        src="https://f005.backblazeb2.com/file/shannonsvideos/level_10_real_final.glb"
        auto-rotate
        disable-zoom
        disable-pan
        ...
        style="width:clamp(240px, 64vw, 340px); height:clamp(240px, 64vw, 340px); background:transparent; --poster-color: transparent;"
      ></model-viewer>`;
    ...
    slides.push({
      ...
      html: `
        ...
        <div class="ww-halftone"></div>
        ${LOGO}
        <div class="ww-big-num" style="font-size: clamp(2.6rem, 11vw, 5.5rem); margin-top:24px;">BALANCE</div>
        <div class="ww-caption" style="letter-spacing:2px; margin-top:10px;">plantbased-balance.org</div>
  ...
  async function openPromo(opts){
    ...
    // Kick model-viewer loading asap so the FitGotchi slide has it ready by
    // the time we get to slide 4 (roughly 13s in). Fire-and-forget.
    ensureModelViewer();
