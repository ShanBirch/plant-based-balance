-- Fresh perfect quizzes in the Mind module award 20 XP.
-- Other modules remain at 10 XP and completed lesson replays remain non-repeatable.
...
RETURNS JSON AS $$
DECLARE
  xp_per_lesson INTEGER := CASE
    WHEN p_module_id = 'mind'
      AND p_unit_id LIKE 'mind-%'
      AND p_lesson_id LIKE 'mind-%'
      THEN 20
    ELSE 10
  END;
...
BEGIN
  ...
  IF xp_earned > 0 THEN
    ...
    INSERT INTO public.point_transactions (
      ...
      CASE WHEN p_module_id = 'mind' THEN 'Perfect Mind quiz' ELSE 'Perfect Health IQ quiz' END
    );
  END IF;
