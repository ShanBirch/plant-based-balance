...
const db = {
  ...
  /**
   * Fitness Integrations table operations
   */
  fitnessIntegrations: {
    /**
     * Create a new fitness integration
     */
    async connect(userId, provider, providerUserId, accessToken, refreshToken, expiresAt, scopes) {
      const { data, error } = await supabase
    ...
    /**
     * Get user's integration for a provider
     */
     ...
     * Update access token
     */
    async updateToken(integrationId, accessToken, refreshToken, expiresAt) {
      const { data, error } = await supabase
        .from('fitness_integrations')
        .update({
          access_token: accessToken,
     ...
     */
    async updateLastSync(integrationId, errorMessage = null) {
      const { data, error } = await supabase
        .from('fitness_integrations')
        .update({
    ...
    /**
     * Disconnect/deactivate integration
     */
  ...
  /**
   * Fitness Sync History table operations
   */
  fitnessSyncHistory: {
    /**
     * Create a new sync record
     */
    async create(integrationId, userId, syncType, dataTypes) {
      const { data, error } = await supabase
     ...
     * Update sync status
     */
    async complete(syncId, status, recordsSynced, errorMessage = null, startDate = null, endDate = null) {
      const { data, error } = await supabase
        .from('fitness_sync_history')
        .update({
          status: status,
  ...
  /**
   * Fitness Metrics table operations
   */
  fitnessMetrics: {
    /**
     * Store fitness metrics
     */
    async upsert(integrationId, userId, metricDate, metricType, values, unit = null) {
      const { data, error } = await supabase
        ...
        .upsert({
          ...
          value_integer: values.integer || null,
          value_decimal: values.decimal || null,
          value_text: values.text || null,
          value_json: values.json || null,
          unit: unit
        }, {
     ...
     */
    async getByDateRange(userId, startDate, endDate, metricTypes = null) {
      let query = supabase
        .from('fitness_metrics')
        .select('*')
        .eq('user_id', userId)
     ...
     */
    async getLatest(userId, metricType, limit = 30) {
      const { data, error } = await supabase
        .from('fitness_metrics')
        .select('*')
        .eq('user_id', userId)
    ...
    /**
     * Get summary stats for a metric type
     */
    async getSummary(userId, metricType, days = 30) {
