...
let feedComposerVideoAttemptId = '';
let feedComposerVideoAttemptStartedAt = 0;
...
const FEED_UPLOAD_DIAGNOSTIC_SCHEMA_VERSION = 2;
...
function getFeedUploadSupportCode(attemptId) {
...
function getFeedUploadClientScriptVersion() {
    try {
        const script = Array.from(document.scripts || []).find(function (item) {
            return String(item && item.src || '').includes('/lib/stories.js');
        });
...
function buildFeedUploadDiagnosticItem(event, data) {
    ...
    return {
        ...
        diagnosticSchemaVersion: FEED_UPLOAD_DIAGNOSTIC_SCHEMA_VERSION,
        ...
        clientScriptVersion: clampFeedUploadDiagnosticText(getFeedUploadClientScriptVersion(), 80),
        pagePath: clampFeedUploadDiagnosticText(window.location?.pathname || '', 120),
...
window.getBalanceVideoUploadSupportCode = getFeedUploadSupportCode;
...
async function renderStory() {
    ...
    if (avatar) {
        ...
        avatar.setAttribute('data-feed-avatar-user-id', String(story.user_id || ''));
        avatar.innerHTML = renderFeedAvatarContent(story, { fontSize: '0.85rem' });
    }
...
function renderBalanceEarnedCelebration(kind, options = {}) {
    const safeKind = ['workout', 'activity', 'pb', 'milestone'].includes(kind) ? kind : 'workout';
    const major = options.major === true || safeKind === 'pb' || safeKind === 'milestone';
    const particles = [
        ['12%', '18%', '-18px', '-34px', '0ms'], ['25%', '10%', '-8px', '-42px', '55ms'],
        ['42%', '15%', '4px', '-36px', '110ms'], ['60%', '9%', '14px', '-44px', '30ms'],
        ['78%', '17%', '24px', '-34px', '145ms'], ['90%', '28%', '30px', '-18px', '80ms'],
        ['84%', '47%', '28px', '2px', '175ms'], ['70%', '62%', '20px', '18px', '120ms'],
        ['50%', '69%', '0px', '26px', '65ms'], ['31%', '61%', '-18px', '18px', '195ms'],
        ['16%', '48%', '-28px', '2px', '95ms'], ['8%', '32%', '-30px', '-16px', '155ms']
    ];
    const colours = major
        ? ['#fbbf24', '#ffffff', '#fde68a', '#34d399']
        : ['#f5c45c', '#ffffff', '#5eead4'];
    return `
        <div class="balance-earned-celebration balance-earned-celebration--${safeKind}${major ? ' is-major' : ''}" aria-hidden="true">
            <span class="balance-earned-ring"></span>
            <span class="balance-earned-glimmer"></span>
            ${particles.map((particle, index) => `<span class="balance-earned-particle" style="--x:${particle[0]};--y:${particle[1]};--dx:${particle[2]};--dy:${particle[3]};--delay:${particle[4]};--celebration-colour:${colours[index % colours.length]};--twist:${index % 2 ? '52deg' : '-38deg'};"></span>`).join('')}
        </div>
    `;
}
...
window.renderWorkoutCard = function(cardData) {
    ...
    if (cardData.card_type === 'milestone') {
        ...
        return `
            <div class="balance-share-celebration-host" style="width:100%; aspect-ratio:4/5; background:linear-gradient(160deg, #422006 0%, #92400e 34%, #d97706 68%, #facc15 100%); display:flex; flex-direction:column; justify-content:center; align-items:center; padding:30px 26px; box-sizing:border-box; position:relative; overflow:hidden; color:#fff; text-shadow:0 2px 8px rgba(0,0,0,0.34);">
                ${renderBalanceEarnedCelebration('milestone', { major: true })}
    ...
    if (cardData.card_type === 'activity') {
        ...
        return `
            <div class="balance-share-celebration-host" style="width:100%; aspect-ratio:4/5; background:linear-gradient(160deg, #0c4a6e 0%, #0284c7 35%, #0ea5e9 70%, #38bdf8 100%); display:flex; flex-direction:column; justify-content:center; align-items:center; padding:30px; box-sizing:border-box; position:relative; overflow:hidden; color:#fff; text-shadow:0 2px 8px rgba(0,0,0,0.36);">
                ${renderBalanceEarnedCelebration('activity')}
    ...
    if (cardData.card_type === 'pb') {
        ...
        return `
            <div class="balance-share-celebration-host" style="width:100%; aspect-ratio:4/5; background:linear-gradient(160deg, #92400e 0%, #d97706 30%, #f59e0b 60%, #fbbf24 100%); display:flex; flex-direction:column; justify-content:center; align-items:center; padding:30px; box-sizing:border-box; position:relative; overflow:hidden;">
                ${renderBalanceEarnedCelebration('pb', { major: true })}
    ...
    return `
        <div class="balance-share-celebration-host" style="width:100%; aspect-ratio:4/5; background:linear-gradient(160deg, #022c22 0%, #046A38 35%, #059669 70%, #10b981 100%); display:flex; flex-direction:column; padding:28px 24px; box-sizing:border-box; position:relative; overflow:hidden; color:#fff; text-shadow:0 2px 8px rgba(0,0,0,0.34);">
            ${renderBalanceEarnedCelebration('workout', { major: !!pbsHtml })}
...
function getFeedStoryProfilePhoto(story) {
    if (!story) return '';
    const storyUserId = String(story.user_id || story.author_id || '');
    const currentUserId = String(window.currentUser?.id || '');
    if (storyUserId && currentUserId && storyUserId === currentUserId) {
        return getActiveProfilePhoto() || String(story.profile_photo || '').trim();
    }
    return String(story.profile_photo || story.user_photo || '').trim();
}

function renderFeedAvatarContent(story, options = {}) {
    const name = String(story?.user_name || story?.name || 'Unknown');
    const initial = name.charAt(0).toUpperCase() || '?';
    const photo = getFeedStoryProfilePhoto(story);
    const fontSize = options.fontSize || '0.7rem';
    const fallback = `<span class="feed-avatar-fallback" style="color:var(--feed-avatar-ink,#ffffff); -webkit-text-fill-color:var(--feed-avatar-ink,#ffffff); font-size:${fontSize}; font-weight:800;">${escapeHtml(initial)}</span>`;
    if (!photo) return fallback;
    return `${fallback}<img class="feed-avatar-image" src="${escapeHtml(photo)}" alt="" onerror="this.remove()" style="position:absolute; inset:0; z-index:1; width:100%; height:100%; border-radius:50%; object-fit:cover;">`;
}

window.addEventListener('pbbProfilePhotoUpdated', event => {
    const photo = String(event?.detail?.profilePhoto || '').trim();
    const userId = String(window.currentUser?.id || '');
    if (!photo || !userId) return;

    document.querySelectorAll('[data-feed-avatar-user-id]').forEach(avatar => {
        if (String(avatar.getAttribute('data-feed-avatar-user-id') || '') !== userId) return;
        const oldImage = avatar.querySelector('.feed-avatar-image');
        if (oldImage) oldImage.remove();
        const image = document.createElement('img');
        image.className = 'feed-avatar-image';
        image.alt = '';
        image.src = photo;
        image.style.cssText = 'position:absolute;inset:0;z-index:1;width:100%;height:100%;border-radius:50%;object-fit:cover;';
...
async function uploadStoryMediaToBackblaze(file, options = {}) {
    ...
    const diagnosticBase = {
        ...
        supportCode: getFeedUploadSupportCode(options.attemptId),
...
function shouldUseMultipartFeedUpload(file, source) {
    ...
    const isVideo = String(file && file.type || '').toLowerCase().startsWith('video/');
    const isReliableVideoSource = normalizedSource.includes('workout_share')
        || normalizedSource.includes('share_set')
        || normalizedSource.includes('form_check')
        || normalizedSource.includes('feed_composer');
    return isIosNativeFeedUpload()
        && isVideo
        && isReliableVideoSource
        && size > 5 * 1024 * 1024
        && size <= 1024 * 1024 * 1024;
}
...
// Shared by form checks and other coaching video surfaces so large clips go
// directly from the phone to B2 instead of passing through a Netlify request.
window.uploadBalanceVideoToBackblaze = function(file, options = {}) {
    return uploadStoryMediaToBackblaze(file, {
        ...options,
        preferDirectUpload: true,
        logUploadDiagnostics: true
    });
};
...
window.updateFeedComposerAvatar = function() {
    ...
    const profile = getStoredUserProfile();
    const photo = getActiveProfilePhoto();
...
window.insertFeedComposerMention = function() {
    const input = document.getElementById('feed-composer-text');
    if (!input) return;
    ...
    input.value = before + spacer + '@' + after;
...
function resetFeedComposerMediaInputs() {
    ['feed-composer-camera-photo-input', 'feed-composer-camera-video-input', 'feed-composer-video-input', 'feed-composer-camera-input', 'feed-composer-file-input'].forEach((id) => {
...
function beginFeedComposerVideoAttempt(trigger, file) {
    feedComposerVideoAttemptId = getFeedUploadDiagnosticId();
    feedComposerVideoAttemptStartedAt = Date.now();
    logFeedUploadDiagnostic('video_upload_attempt_start', {
        source: 'feed_composer',
        attemptId: feedComposerVideoAttemptId,
        supportCode: getFeedUploadSupportCode(feedComposerVideoAttemptId),
        trigger: trigger || 'unknown',
        clientFileName: file?.name || '',
        contentType: file?.type || '',
        fileSizeBytes: Number(file?.size || 0)
    });
    return feedComposerVideoAttemptId;
}
...
window.openFeedComposerMediaSource = function(source) {
    ...
    const normalizedSource = String(source || '').toLowerCase();
    if (normalizedSource === 'camera-video' && typeof window.openWorkoutFeedShareCameraForFile === 'function') {
        window.openWorkoutFeedShareCameraForFile({ target: 'feed-composer' });
        return;
    }
    if (normalizedSource === 'library-video' && typeof window.openWorkoutFeedShareGalleryForFile === 'function') {
        window.openWorkoutFeedShareGalleryForFile({ target: 'feed-composer' });
        return;
    }
    const inputId = normalizedSource === 'camera-photo' || normalizedSource === 'camera'
        ...
        : normalizedSource === 'camera-video'
            ...
            : normalizedSource === 'library-video'
                ? 'feed-composer-video-input'
...
function applyFeedComposerSelectedFiles(selectedFiles, options = {}) {
    ...
    if (hasVideo) {
        if (!feedComposerVideoAttemptId) beginFeedComposerVideoAttempt(options.trigger || 'file_picker', selectedFiles[0]);
        logFeedUploadDiagnostic('feed_composer_file_selected', {
            source: 'feed_composer',
            attemptId: feedComposerVideoAttemptId,
            supportCode: getFeedUploadSupportCode(feedComposerVideoAttemptId),
            trigger: options.trigger || 'file_picker',
            clientFileName: selectedFiles[0]?.name || '',
            contentType: selectedFiles[0]?.type || '',
            fileSizeBytes: Number(selectedFiles[0]?.size || 0)
        });
    } else {
        feedComposerVideoAttemptId = '';
        feedComposerVideoAttemptStartedAt = 0;
    }
...
window.handleFeedComposerFileSelect = async function(event) {
    const input = event?.target || null;
    const selectedFiles = Array.from(input?.files || []);
    const options = {
        input,
        trigger: input?.id === 'feed-composer-camera-video-input' ? 'camera_input' : 'file_picker'
    };
    const isSinglePhoto = selectedFiles.length === 1 && getFeedComposerMediaType(selectedFiles[0]) === 'image';
    if (isSinglePhoto && window.BalancePrivateShareStudio?.isEnabled?.()) {
        const reader = new FileReader();
        ...
        const studioResult = await window.BalancePrivateShareStudio.open({
            context: 'feed',
            photoDataUrl,
            rawPhoto: true,
            onDone: async result => applyFeedComposerSelectedFiles([result.file || selectedFiles[0]], options)
        });
        if (studioResult?.action === 'closed' && input) input.value = '';
        return;
    }
    applyFeedComposerSelectedFiles(selectedFiles, options);
};

window.handleFeedComposerNativeVideoFile = function(file) {
    if (!file) return;
    const sharedContext = typeof window.getBalanceVideoUploadAttemptContext === 'function'
        ? window.getBalanceVideoUploadAttemptContext()
        : {};
    if (sharedContext.attemptId) {
        feedComposerVideoAttemptId = sharedContext.attemptId;
        feedComposerVideoAttemptStartedAt = Date.now();
    }
    if (!feedComposerVideoAttemptId) beginFeedComposerVideoAttempt('native_picker', file);
    applyFeedComposerSelectedFiles([file], { trigger: 'native_picker' });
};
...
async function uploadFeedComposerMedia(file, mediaType, postBtn, options = {}) {
    ...
    const isNativeVideo = mediaType === 'video' && !!(
        window.Capacitor?.isNativePlatform && window.Capacitor.isNativePlatform()
    );
    const preserveOriginalVideo = options.skipVideoPreparation === true || isNativeVideo;

    if (mediaType === 'video' && !preserveOriginalVideo) {
        if (shouldCaptureFeedUploadDiagnostic(uploadSource, options)) {
            logFeedUploadDiagnostic('feed_video_prepare_start', {
                source: uploadSource,
                attemptId,
                supportCode: getFeedUploadSupportCode(attemptId),
                clientFileName: fileToUpload.name || '',
                contentType: fileToUpload.type || '',
                fileSizeBytes: Number(fileToUpload.size || 0)
            });
        }
        if (postBtn) postBtn.textContent = `Compressing ${originalSize}MB`;
        ...
        if (shouldCaptureFeedUploadDiagnostic(uploadSource, options)) {
            logFeedUploadDiagnostic('feed_video_prepare_success', {
                source: uploadSource,
                attemptId,
                supportCode: getFeedUploadSupportCode(attemptId),
                clientFileName: fileToUpload.name || '',
    ...
    const skipIosQuickTimeThumbnail = mediaType === 'video' && isIosNativeFeedUpload();
        ...
        for (let attempt = 1; attempt <= maxUploadAttempts; attempt += 1) {
            try {
                uploadData = await uploadStoryMediaToBackblaze(fileToUpload, {
                    ...
                    preferDirectUpload: options.preferDirectUpload === true || isNativeVideo
                });
...
window.submitFeedComposerPost = async function() {
    ...
    const videoAttemptId = feedComposerVideoAttemptId;
    const videoAttemptStartedAt = feedComposerVideoAttemptStartedAt;
    const hasComposerVideo = composerFiles.some(file => getFeedComposerMediaType(file) === 'video');
    if (hasComposerVideo) {
        logFeedUploadDiagnostic('feed_composer_submit_start', {
            source: 'feed_composer',
            attemptId: videoAttemptId,
            supportCode: getFeedUploadSupportCode(videoAttemptId),
            elapsedMs: videoAttemptStartedAt ? Date.now() - videoAttemptStartedAt : 0,
            clientFileName: composerFiles[0]?.name || '',
            contentType: composerFiles[0]?.type || '',
            fileSizeBytes: Number(composerFiles[0]?.size || 0)
        });
    }
    ...
    try {
        ...
        for (let index = 0; index < composerFiles.length; index += 1) {
            ...
            const upload = await uploadFeedComposerMedia(file, itemType, postBtn, {
                source: 'feed_composer',
                attemptId: itemType === 'video' ? videoAttemptId : '',
                preferDirectUpload: isPhotoCarousel,
                maxUploadAttempts: isPhotoCarousel ? 3 : itemType === 'video' ? 2 : 1,
                retryLabel: itemType === 'video'
                    ? 'Reconnecting video upload...'
                    : `Reconnecting photo ${index + 1} of ${composerFiles.length}...`
            });
        ...
        let foundationsCourseActionId = '';
        try {
            foundationsCourseActionId = String(sessionStorage.getItem('pbb_foundations_feed_action') || '');
        } catch (_) {}
        if (!['w1_feed_intro', 'w6_feed_reflection'].includes(foundationsCourseActionId) || !caption) {
            foundationsCourseActionId = '';
        }
        ...
        const story = await dbHelpers.stories.create(window.currentUser.id, {
            ...
            expires_at: new Date(Date.now() + FEED_COMPOSER_POST_TTL_MS).toISOString(),
            course_action_id: foundationsCourseActionId || null
        });

        if (foundationsCourseActionId) {
            try { sessionStorage.removeItem('pbb_foundations_feed_action'); } catch (_) {}
        }
        ...
        const pointsAwarded = Number(story?.points_awarded || 0);
        if (hasComposerVideo) {
            logFeedUploadDiagnostic('feed_composer_submit_success', {
                source: 'feed_composer',
                attemptId: videoAttemptId,
                supportCode: getFeedUploadSupportCode(videoAttemptId),
                elapsedMs: videoAttemptStartedAt ? Date.now() - videoAttemptStartedAt : 0,
                storyId: story?.id || '',
                uploadMode: uploadData?.multipartUpload
                    ? 'multipart_b2'
                    : uploadData?.directUpload
                        ? 'direct_b2'
                        : 'netlify'
            });
        }
        ...
        try {
            window.dispatchEvent(new CustomEvent('pbbFeedPostCreated', {
                detail: { storyId: story?.id || null, caption: caption || '' }
            }));
        } catch (_) {}
        ...
        if (window.socialJourney && typeof window.socialJourney.refresh === 'function') {
            window.socialJourney.refresh().catch(e => console.warn('Foundations post progress refresh failed:', e));
        }
        if (typeof loadStories === 'function') {
            loadStories();
        }
        ...
        const supportCode = getFeedUploadSupportCode(videoAttemptId);
        if (hasComposerVideo) {
            logFeedUploadDiagnostic('feed_composer_submit_failed', {
                source: 'feed_composer',
                attemptId: videoAttemptId,
                supportCode,
                elapsedMs: videoAttemptStartedAt ? Date.now() - videoAttemptStartedAt : 0,
        ...
        const message = hasComposerVideo
            ? `${error?.message || 'Video upload did not finish.'}${supportCode ? ` Code ${supportCode}.` : ''}`
            : 'Failed to post. Please try again.';
        showFeedComposerToast(message, 'error');
    } finally {
        if (postBtn) {
...
function renderFeedPostUnavailable(story, storyId) {
    ...
    const avatarHtml = renderFeedAvatarContent(story);
    ...
    return `
        <div class="feed-post-card feed-post-card-unavailable" data-story-id="${escapeHtml(storyId || '')}" data-feed-created-at="${escapeHtml((story && story.created_at) || '')}" data-feed-created-ms="${getFeedStoryCreatedMs(story)}" style="background:white; border-bottom:8px solid #f1f5f9;">
            <div style="display:flex; align-items:center; padding:10px 14px; gap:10px;">
                <div class="feed-avatar" data-feed-avatar-user-id="${escapeHtml((story && story.user_id) || '')}" style="width:34px; height:34px; border-radius:50%; display:flex; align-items:center; justify-content:center; overflow:hidden; flex-shrink:0;">
                    ${avatarHtml}
                </div>
...
window.loadPhotoFeed = async function(targetGridId = 'photo-feed-grid', emptyStateId = 'photo-feed-empty', options = {}) {
    ...
    try {
        ...
        const stories = (Array.isArray(fetchedStories) ? fetchedStories : []).filter(story => {
            return typeof window.shouldShowMealFeedStory !== 'function' || window.shouldShowMealFeedStory(story);
        });
        ...
        rememberFeedMentionProfiles(sortedStories.map(story => ({
            ...
            profile_photo: getFeedStoryProfilePhoto(story)
        })));
        ...
        const renderedFeedHtml = sortedStories.map(story => {
            ...
            const resolvedProfilePhoto = getFeedStoryProfilePhoto(story);
            const profilePhotoArg = escapeJsString(resolvedProfilePhoto);
            ...
            return `
                <div class="feed-post-card${isPendingTahliaApproval ? ' feed-post-card-pending-tahlia' : ''}" data-story-id="${storyId}" data-feed-created-at="${escapeHtml(story.created_at || '')}" data-feed-created-ms="${getFeedStoryCreatedMs(story)}" style="background:white; border-bottom:8px solid #f1f5f9;">
                    ...
                    <div onclick="if(typeof viewUserProfile==='function') viewUserProfile('${story.user_id}', '${profileNameArg}', '${profilePhotoArg}')" style="display:flex; align-items:center; padding:10px 14px; gap:10px; cursor:pointer;">
                        <div class="feed-avatar" data-feed-avatar-user-id="${escapeHtml(story.user_id || '')}" style="width:34px; height:34px; border-radius:50%; display:flex; align-items:center; justify-content:center; overflow:hidden; flex-shrink:0;">
                            ${avatarHtml}
                        </div>
                        <div style="flex:1; min-width:0;">
                            <div style="font-weight:600; font-size:0.85rem; color:var(--text-main); overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">${safeUserName}</div>
                    ...
                    ${isCardType ? (() => {
                        ...
                        if (isPhotoOverlayCard) {
                            const photoCelebrationKind = cardData.card_type === 'pb' ? 'pb' : (cardData.card_type === 'activity' ? 'activity' : 'workout');
                            const photoCelebration = renderBalanceEarnedCelebration(photoCelebrationKind, {
                                major: photoCelebrationKind === 'pb' || !!(cardData.pbs && cardData.pbs.length)
                            });
                            return '<div class="balance-share-celebration-host balance-share-photo-celebration" ' + cardTapAttrs + ' style="cursor:pointer; width:100%; background:#020617; position:relative; overflow:hidden;">' +
...
window.openFeedPostViewer = async function(storyId, options = {}) {
    ...
    try {
        ...
        // User avatar
        const avatarHtml = renderFeedAvatarContent(story, { fontSize: '0.9rem' });
        ...
        if (isCardType) {
            ...
            if (cardData && cardPhotoSrc && isPhotoOverlayCard) {
                ...
                mediaHtml = `<div class="balance-share-celebration-host balance-share-photo-celebration" style="width:100%; max-height:70vh; overflow:auto; background:#020617; position:relative;"><img src="${escapeHtml(cardPhotoSrc)}" alt="Workout share" style="width:100%; max-height:70vh; object-fit:contain; display:block;" referrerpolicy="no-referrer">${photoCelebration}</div>`;
        ...
        viewer.innerHTML = `
            ...
            <div style="position:sticky; top:0; z-index:2; display:flex; align-items:center; padding:calc(15px + env(safe-area-inset-top, 0px)) 16px 14px 16px; gap:12px; flex-shrink:0; background:linear-gradient(180deg, rgba(0,0,0,0.98) 0%, rgba(0,0,0,0.84) 100%); backdrop-filter:blur(14px); -webkit-backdrop-filter:blur(14px); border-bottom:1px solid rgba(255,255,255,0.06); color:white; -webkit-text-fill-color:white;">
                <div class="feed-avatar" data-feed-avatar-user-id="${escapeHtml(story.user_id || '')}" onclick="closeFeedPostViewer(); if(typeof viewUserProfile==='function') viewUserProfile('${story.user_id}', '${profileNameArg}', '${profilePhotoArg}')" style="width:40px; height:40px; border-radius:50%; display:flex; align-items:center; justify-content:center; overflow:hidden; flex-shrink:0; cursor:pointer;">
                    ${avatarHtml}
                </div>
...
window.toggleFeedReaction = async function(storyId, reactionKey, buttonEl) {
    ...
    try {
        ...
        if (typeof window.refreshWeeklyGoalsCard === 'function') {
            Promise.resolve(window.refreshWeeklyGoalsCard()).catch(error => console.warn('Weekly Feed goal refresh failed:', error));
        }
...
window.postFeedComment = async function(storyId, inputEl) {
    ...
    try {
        ...
        if (window.socialJourney && typeof window.socialJourney.refresh === 'function') {
            window.socialJourney.refresh().catch(e => console.warn('Foundations comment progress refresh failed:', e));
        }
    } catch (error) {
...
function renderFeedMentionHandle(handle) {
    ...
    if (profile && profile.id) {
        return `<button type="button" class="feed-mention-link" onclick="event.stopPropagation(); openFeedMentionProfile('${escapeJsString(profile.id)}', '${escapeJsString(profile.name)}', '${escapeJsString(profile.photo)}')" style="display:inline; appearance:none; -webkit-appearance:none; border:0 !important; background:transparent !important; background-color:transparent !important; box-shadow:none !important; padding:0; margin:0; color:var(--feed-mention-color,var(--text-main,#2563eb)) !important; -webkit-text-fill-color:var(--feed-mention-color,var(--text-main,#2563eb)) !important; font:inherit; font-weight:800; text-decoration:underline; text-decoration-color:var(--feed-mention-color,var(--secondary,#2563eb)); text-underline-offset:2px; cursor:pointer;">${escapeHtml(label)}</button>`;
    }
